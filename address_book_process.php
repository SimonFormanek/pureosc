...
if (isset($HTTP_GET_VARS['action']) && ($HTTP_GET_VARS['action'] == 'deleteconfirm')
    && isset($HTTP_GET_VARS['delete']) && is_numeric($HTTP_GET_VARS['delete']) && isset($HTTP_GET_VARS['formid'])
    && ($HTTP_GET_VARS['formid'] == md5($sessiontoken))) {
        ...
        if ($HTTP_POST_VARS['action'] == 'update') {
            ...
            if (tep_db_num_rows($check_query) == 1) {
                tep_db_perform(TABLE_ADDRESS_BOOK, $sql_data_array, 'update',
                    "address_book_id = '".(int) $HTTP_GET_VARS['edit']."' and customers_id ='".(int) $customer_id."'");

// reregister session variables
                if ((isset($HTTP_POST_VARS['primary']) && ($HTTP_POST_VARS['primary']
                    == 'on')) || ($HTTP_GET_VARS['edit'] == $customer_default_address_id)) {
...
if (isset($HTTP_GET_VARS['edit']) && is_numeric($HTTP_GET_VARS['edit'])) {
    ...
    if (!tep_db_num_rows($entry_query)) {
        $messageStack->add_session('addressbook',
            ERROR_NONEXISTING_ADDRESS_BOOK_ENTRY);
...
if (isset($HTTP_GET_VARS['edit']) && is_numeric($HTTP_GET_VARS['edit'])) {
    $breadcrumb->add(NAVBAR_TITLE_MODIFY_ENTRY,
        tep_href_link(FILENAME_ADDRESS_BOOK_PROCESS,
            'edit='.$HTTP_GET_VARS['edit'], 'SSL'));
} elseif (isset($HTTP_GET_VARS['delete']) && is_numeric($HTTP_GET_VARS['delete'])) {
    $breadcrumb->add(NAVBAR_TITLE_DELETE_ENTRY,
        tep_href_link(FILENAME_ADDRESS_BOOK_PROCESS,
            'delete='.$HTTP_GET_VARS['delete'], 'SSL'));
} else {
...
?>
    ...
    <div class="contentContainer">
        ...
        <?php echo tep_address_label($customer_id,
            (int) $HTTP_GET_VARS['delete'], true, ' ', '<br />'); ?>
                    </div>
                </div>
            </div>
        </div>
        ...
        'fa fa-trash',
        tep_href_link(FILENAME_ADDRESS_BOOK_PROCESS,
            'delete='.$HTTP_GET_VARS['delete'].'&action=deleteconfirm&formid='.md5($sessiontoken),
    ...
    <?php echo tep_draw_form('addressbook',
        tep_href_link(FILENAME_ADDRESS_BOOK_PROCESS,
            (isset($HTTP_GET_VARS['edit']) ? 'edit='.$HTTP_GET_VARS['edit'] : ''),
    ...
    if (isset($HTTP_GET_VARS['edit']) && is_numeric($HTTP_GET_VARS['edit'])) {
        ?>
            ...
            'update').tep_draw_hidden_field('edit', $HTTP_GET_VARS['edit']).tep_draw_button(IMAGE_BUTTON_UPDATE,
