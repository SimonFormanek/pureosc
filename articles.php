...
<div class="contentContainer">
  ...
  if ($topic_depth == 'nested') {
    if ( tep_not_null($topic['topics_description']) ) { ?>
       ...
       if (tep_db_num_rows($topics_query) > 0) {
           while ($topics = tep_db_fetch_array($topics_query)) {
             ...
             echo '<div class="main"><a href="' . tep_href_link(FILENAME_ARTICLES, 'tPath='.$topics['topics_id']) . '"><b>' . $topics['topics_name'] . '</b></a> ( ' . $articles['ttl'] . ' )</div>';
    ...
    if (ARTICLE_LIST_FILTER) {
      ...
      if (tep_db_num_rows($filterlist_query) > 1) {
        echo '<div align="right" class="main">' . tep_draw_form('filter', FILENAME_ARTICLES, 'get') . TEXT_SHOW . '&nbsp;';
        if (isset($_GET['authors_id'])) {
   ...
   ?>
              ...
              <?php if (tep_not_null($authors_description)) { ?>
                ...
                <div class="main" colspan="2" valign="top"><?php echo sprintf(TEXT_MORE_INFORMATION, $authors_url); ?></div>
              <?php } ?>
  ...
  ?>
     <div class="main"><?php echo '<b>' . (($showBlogArticles == 'true') ? TEXT_CURRENT_ARTICLES : TEXT_CURRENT_BLOG_ARTICLES) . '</b>'; ?></div>
     <?php
     ...
     ?> 
       <div class="row">
         ...
         <div class="col-sm-6">
           <div class="pull-right pagenav"><ul class="pagination"><?php echo $articles_all_split->display_links(MAX_DISPLAY_PAGE_LINKS, tep_get_all_get_params(array('page', 'info', 'x', 'y'))); ?></ul></div>
