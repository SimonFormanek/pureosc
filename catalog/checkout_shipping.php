...
if (!tep_session_is_registered('sendto')) {
  ...
  $default_address_query = tep_db_query("select customers_default_address_id  from customers_real WHERE customers_id =" . (int) $customer_id);
  $default_address = tep_db_fetch_array($default_address_query);
  $sendto = $default_address['customers_default_address_id'];
} else {
...
<?php
echo tep_draw_form('checkout_address', tep_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL'), 'post', 'class="form-horizontal"', true) . tep_draw_hidden_field('action', 'process');
?>
...
<?php
echo tep_draw_button(IMAGE_BUTTON_CHANGE_ADDRESS, 'fa fa-home', tep_href_link(FILENAME_CHECKOUT_SHIPPING_ADDRESS, '', 'SSL'));
?>
                </div>
                <div class="clearfix"></div>
            </div>
