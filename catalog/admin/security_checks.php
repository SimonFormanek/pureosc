    ...
    while ($file = $secdir->read()) {
        if (!is_dir(DIR_FS_ADMIN.'includes/modules/security_check/'.$file)) {
            if (substr($file, strrpos($file, '.')) == '.php') {
                ...
                $modules[] = array('title' => isset($$class->title) ? $$class->title
...
if ($extdir = @dir(DIR_FS_ADMIN.'includes/modules/security_check/extended/')) {
    while ($file = $extdir->read()) {
        if (!is_dir(DIR_FS_ADMIN.'includes/modules/security_check/extended/'.$file)) {
            if (substr($file, strrpos($file, '.')) == '.php') {
                $class = 'securityCheckExtended_'.substr($file, 0,
                        strrpos($file, '.'));
...
<table border="0" width="100%" cellspacing="0" cellpadding="2">
    ...
    foreach ($modules as $module) {
        ...
        '    <td class="dataTableContent" align="center" valign="top">'.tep_image(DIR_WS_IMAGES.'ms_'.$secCheck->type.'.png',
            '', 16, 16).'</td>'."\n".
        ...
        '    <td class="dataTableContent" align="center" valign="top">'.((isset($secCheck->has_doc)
        && $secCheck->has_doc) ? '<a href="http://library.oscommerce.com/Wiki&oscom_2_3&security_checks&'.$module['code'].'" target="_blank">'.tep_image(DIR_WS_IMAGES.'icons/preview.gif').'</a>'
                : '').'</td>'."\n".
        '  </tr>'."\n";
    }
    ?>
