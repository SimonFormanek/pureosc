...
// set the values that the user didn't
    ...
    if ($vals['type'] == 0) {
        ...
        $html_graph_string .= double_horizontal_graph($names, $values, $bars,
            $vals, $dvalues, $dbars);
    } elseif ($vals['type'] == 3) {
        $html_graph_string .= double_vertical_graph($names, $values, $bars,
            $vals, $dvalues, $dbars);
    }
...
// decide if the value in bar is a color code or image.
        if (preg_match('/^#/', $bars[$i])) {
            $horizontal_graph_string .= '<table cellpadding="0" cellspacing="0" bgcolor="'.$bars[$i].'" width="'.($values[$i]
                * $vals['scale']).'">'."\n".
                '  <tr>'."\n".
                '    <td>&nbsp;</td>'."\n".
                '  </tr>'."\n".
            ...
            $horizontal_graph_string .= '<img src="'.$bars[$i].'" height="10" width="'.($values[$i]
                * $vals['scale']).'">';
        }
...
// if a background was choosen don't print cell BGCOLOR
        ...
        $double_horizontal_graph_string .= '><table align="left" cellpadding="0" cellspacing="0" width="'.($dvalues[$i]
            * $vals['scale']).'">'."\n".
            '      <tr>'."\n".
            '        <td';
...
// decide if the value in bar is a color code or image.
        if (preg_match('/^#/', $bars[$i])) {
            $double_horizontal_graph_string .= '<table align="left" cellpadding="0" cellspacing="0" bgcolor="'.$bars[$i].'" width="'.($values[$i]
                * $vals['scale']).'">'."\n".
                '  <tr>'."\n".
                '    <td>&nbsp;</td>'."\n".
                '  </tr>'."\n".
            ...
            $double_horizontal_graph_string .= '<img src="'.$bars[$i].'" height="10" width="'.($values[$i]
                * $vals['scale']).'">';
        }
...
function tep_banner_graph_yearly($banner_id)
{
    ...
    $graph_vals = @array('vlabel' => TEXT_BANNERS_DATA,
        'hlabel' => sprintf(TEXT_BANNERS_YEARLY_STATISTICS,
...
function tep_banner_graph_monthly($banner_id)
{
    ...
    while ($banner_stats       = tep_db_fetch_array($banner_stats_query)) {
        $names[($banner_stats['banner_month'] - 1)]   = strftime('%b',
            mktime(0, 0, 0, $banner_stats['banner_month']));
        $values[($banner_stats['banner_month'] - 1)]  = (($banner_stats['value'])
                ? $banner_stats['value'] : '0');
        $dvalues[($banner_stats['banner_month'] - 1)] = (($banner_stats['dvalue'])
    ...
    $graph_vals = @array('vlabel' => TEXT_BANNERS_DATA,
        'hlabel' => sprintf(TEXT_BANNERS_MONTHLY_STATISTICS,
            $banner['banners_title'], date('Y')),
        'type' => '3',
        'cellpadding' => '',
        'cellspacing' => '1',
        'border' => '',
...
function tep_banner_graph_daily($banner_id)
{
    ...
    while ($banner_stats       = tep_db_fetch_array($banner_stats_query)) {
        ...
        $values[($banner_stats['banner_day'] - 1)]  = (($banner_stats['value']) ? $banner_stats['value']
                : '0');
        $dvalues[($banner_stats['banner_day'] - 1)] = (($banner_stats['dvalue'])
                ? $banner_stats['dvalue'] : '0');
    }
    ...
    $graph_vals = @array('vlabel' => TEXT_BANNERS_DATA,
        'hlabel' => sprintf(TEXT_BANNERS_DAILY_STATISTICS,
            $banner['banners_title'], strftime('%B', mktime(0, 0, 0, $month)),
            $year),
        'type' => '3',
        'cellpadding' => '',
        'cellspacing' => '1',
