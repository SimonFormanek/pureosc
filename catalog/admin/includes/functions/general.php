...
function tep_get_path($current_category_id = '')
{
    ...
    if ($current_category_id == '') {
        ...
        if (sizeof($cPath_array) == 0) {
            ...
            $cPath_new           = '';
            $last_category_query = tep_db_query("select parent_id from ".TABLE_CATEGORIES." where categories_id = '".(int) $cPath_array[(sizeof($cPath_array)
                - 1)]."'");
...
function tep_date_long($raw_date)
{
    ...
    return strftime(DATE_FORMAT_LONG,
...
function tep_date_short($raw_date)
{
    ...
    if (@date('Y', mktime($hour, $minute, $second, $month, $day, $year)) == $year) {
        ...
        return preg_replace('/2037$/', $year,
            date(DATE_FORMAT,
                mktime($hour, $minute, $second, $month, $day, 2037)));
    }
}
...
function tep_not_null($value)
{
    if (is_array($value)) {
        ...
        if ((is_string($value) || is_int($value)) && ($value != '') && ($value != 'NULL')
            && (strlen(trim($value)) > 0)) {
            return true;
        } else {
            return false;
...
// :Vat Address Field mod
    ...
    if (isset($address['country_id']) && tep_not_null($address['country_id'])) {
        ...
        if (isset($address['zone_id']) && tep_not_null($address['zone_id'])) {
            $state = tep_get_zone_code($address['country_id'],
                $address['zone_id'], $state);
        }
...
function tep_products_in_category_count($categories_id,
                                        $include_deactivated = false)
{
    ...
    if (tep_db_num_rows($childs_query)) {
        while ($childs = tep_db_fetch_array($childs_query)) {
            $products_count += tep_products_in_category_count($childs['categories_id'],
                $include_deactivated);
        }
    }
...
{
    return tep_draw_pull_down_menu('configuration_value', tep_get_countries(),
        $country_id);
}

function tep_cfg_pull_down_zone_list($zone_id)
{
    return tep_draw_pull_down_menu('configuration_value',
        tep_get_country_zones(STORE_COUNTRY), $zone_id);
}
...
function tep_generate_category_path($id, $from = 'category',
                                    $categories_array = '', $index = 0)
{
    global $languages_id;
    ...
    if ($from == 'product') {
        ...
        while ($categories       = tep_db_fetch_array($categories_query)) {
            if ($categories['categories_id'] == '0') {
                ...
                $category                   = tep_db_fetch_array($category_query);
                $categories_array[$index][] = array('id' => $categories['categories_id'],
                    'text' => $category['categories_name']);
                if ((tep_not_null($category['parent_id'])) && ($category['parent_id']
                    != '0'))
                        $categories_array           = tep_generate_category_path($category['parent_id'],
...
function tep_output_generated_category_path($id, $from = 'category')
{
    ...
    for ($i = 0, $n = sizeof($calculated_category_path); $i < $n; $i++) {
        ...
        $calculated_category_path_string = substr($calculated_category_path_string,
                0, -16).'<br />';
    }
    $calculated_category_path_string = substr($calculated_category_path_string,
        0, -6);
...
function tep_remove_category($category_id)
{
    ...
    if (USE_CACHE == 'true') {
        ...
        if (defined('SEO_FRIENDLY_URLS_STATUS') && SEO_FRIENDLY_URLS_STATUS == 'True') {
            ...
            if (file_exists(DIR_FS_CACHE.'seo_friendly_urls.cache'))
                    @unlink(DIR_FS_CACHE.'seo_friendly_urls.cache');
            if (extension_loaded('apc'))
                    apc_delete('seo_friendly_urls_cache_aliases');
        }
        //reset SEO FRIENDLY URLS cache
    }
}
        ...
        if ($cache_blocks[$i]['code'] == $cache_block) {
            if ($cache_blocks[$i]['multiple']) {
                if ($dir = @opendir(DIR_FS_CACHE)) {
                    while ($cache_file = readdir($dir)) {
                        ...
                        for ($j = 0, $k = sizeof($languages); $j < $k; $j++) {
                            ...
                            if (preg_match('/^'.$cached_file_unlink.'/',
                                    $cache_file)) {
                ...
                for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
                    $cached_file = preg_replace('/-language/',
                        '-'.$languages[$i]['directory'], $cached_file);
        ...
        while ($file = $dir->read()) {
            if (($file != '.') && ($file != '..')) {
                if (tep_is_writable($source.'/'.$file)) {
                    ...
                    $messageStack->add(sprintf(ERROR_FILE_NOT_REMOVEABLE,
                            $source.'/'.$file), 'error');
                  ...
                  $from_email_name, $from_email_address)
{
    ...
    $message->send($to_name, $to_email_address, $from_email_name,
        $from_email_address, $email_subject);
}
...
function tep_get_ip_address()
{
    ...
    if (isset($HTTP_SERVER_VARS['HTTP_X_FORWARDED_FOR']) && !empty($HTTP_SERVER_VARS['HTTP_X_FORWARDED_FOR'])) {
        foreach (array_reverse(explode(',',
                $HTTP_SERVER_VARS['HTTP_X_FORWARDED_FOR'])) as $x_ip) {
            $x_ip = trim($x_ip);
...
function tep_get_products_name_review($product_id, $language_id = 0)
{
    ...
    if (!is_array($product))
            $product = array('products_name' => ADMIN_TESTIMONIAL);
...
function create_coupon_code($salt = "somemumbojumbo",
                            $length = CCGV_SECURITY_CODE_LENGTH)
{
...
//mail it
function tep_mm_sendmail($mail, $email_address, $sender_name, $sender,
                         ...
                         $output_content_txt)
{
    ...
    $mimemessage->send($mail, $email_address, $sender_name, $sender,
        $output_subject, $output_content_html, $output_content_txt);
}
...
function tep_reset_cache_data_seo_urls($action)
{
    ...
    # Its NOT returining a false to indicate failure!!
    return 'false';
}
