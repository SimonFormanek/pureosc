...
function tep_get_topic_tree($parent_id = '0', $spacing = '', $exclude = '',
                            $topic_tree_array = '', $include_itself = false)
{
    global $languages_id;
    ...
    while ($topics       = tep_db_fetch_array($topics_query)) {
        ...
        $topic_tree_array   = tep_get_topic_tree($topics['topics_id'],
            $spacing.'&nbsp;&nbsp;&nbsp;', $exclude, $topic_tree_array);
    }

    return $topic_tree_array;
}

function tep_generate_topic_path($id, $from = 'topic', $topics_array = '',
                                 $index = 0)
{
    global $languages_id;
        ...
        while ($topics       = tep_db_fetch_array($topics_query)) {
            if ($topics['topics_id'] == '0') {
                ...
                if ((tep_not_null($topic['parent_id'])) && ($topic['parent_id'] != '0'))
                        $topics_array           = tep_generate_topic_path($topic['parent_id'],
                        'topic', $topics_array, $index);
...
function tep_output_generated_topic_path($id, $from = 'topic')
{
    ...
    for ($i = 0, $n = sizeof($calculated_topic_path); $i < $n; $i++) {
        ...
        $calculated_topic_path_string = substr($calculated_topic_path_string, 0,
                -16).'<br>';
    }
...
{
    $authors_query = tep_db_query("select authors_image from ".TABLE_AUTHORS_INFO." where authors_id = '".(int) $authors_id."' and languages_id = '".(int) $language_id."'");
...
{
    if ($status == '1') {
        return tep_db_query("update ".TABLE_ARTICLES." set articles_is_blog = '1', articles_last_modified = now() where articles_id = '".(int) $articles_id."'");
    } elseif ($status == '0') {
        return tep_db_query("update ".TABLE_ARTICLES." set articles_is_blog = '0', articles_last_modified = now() where articles_id = '".(int) $articles_id."'");
    } else {
        return -1;
    }
}
...
{
    $article_query = tep_db_query("select articles_image from ".TABLE_ARTICLES_DESCRIPTION." where articles_id = '".(int) $article_id."' and language_id = '".(int) $language_id."'");
    $article       = tep_db_fetch_array($article_query);
...
function tep_articles_in_topic_count($topics_id, $include_deactivated = false)
{
    ...
    if (tep_db_num_rows($childs_query)) {
        while ($childs = tep_db_fetch_array($childs_query)) {
            $articles_count += tep_articles_in_topic_count($childs['topics_id'],
                $include_deactivated);
        }
    }
