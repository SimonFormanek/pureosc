...
function AnnounceVersion($contribPath, $currentVersion, $contribName)
{
    ...
    if (tep_not_null($lines)) {
        ...
        switch ($result) {
            case 0: return sprintf(TEXT_VERSION_LATEST, $currentVersion);
                break;
            case 'ERROR_NO_MATCH_FOUND': return TEXT_VERSION_NO_MATCH_FOUND;
                break;
            default: if ($result > 0) {
                    $plural = ($result > 1) ? sprintf(TEXT_ARE, $result) : sprintf(TEXT_IS,
                            $result);
                    return '<a style="font-weight: bold; color: red;" target="_blank" href="'.$contribPath.'">'.sprintf(TEXT_VERSION_FOUND_UPDATE,
                            $plural).'</a>';
                    break;
                }
        }
...
function CheckContributionVersion($lines, $currentVersion, $contribName)
{
    ...
    foreach ($lines as $line) {
        ...
        if ($skip && strpos($line, 'Expand All') === FALSE) continue;

        /*         * ********** Found it so stop skipping ************ */
        if (strpos($line, 'Expand All') !== FALSE) $skip = false;

        /*         * ********** Only look at version strings ************ */
        $line = trim(strip_tags($line));
        if (strpos($line, $contribName) !== FALSE) {
            if ($line === $currentVersion) return $versionFound;
            else $versionFound++;
        }
    }
