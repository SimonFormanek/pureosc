...
{
    ($dir          = opendir(DIR_FS_CATALOG)) || die(sprintf(ERROR_FAILED_DIR_OPEN,
                DIR_FS_CATALOG));
    $newfiles     = array();
    $fileList     = array();
    $existingInDB = array();
            ...
            if (!is_dir($file) && substr($file, -4, 4) === ".php") { //only look at php files
                if (($result = FileNotUsingHeaderTags($file)) === 'FALSE' || $isTemplate) { //header tags title code is in file
                    ...
                    if (($presentInDB && $notDuplicatePage) || (!$presentInDB && !$excludedFile)) {
                        if (($addResult = AddedToHeaderTagsIncludesFile($file,
                            $fp, $languages_id, $notDuplicatePage))) { //the new file was added to includes/header_tags.php
...
function AddedToHeaderTagsIncludesFile($file, &$fp, $languages_id,
                                       $notDuplicatePage)
{ //add a new entry to the includes/header_tags.php file if not present
    if (!$fp) {
    ...
    if ($notDuplicatePage) {
        ...
        for ($idx = 0; $idx < count($fp); ++$idx) { //find where to insert the new page
            ...
            } else if (strpos($fp[$idx], "// ALL OTHER PAGES NOT DEFINED ABOVE")
                !== FALSE) {
                $insertPoint = $idx;
                break;
            }
        ...
        foreach ($lines as $name) {
            ...
            if (isset($definedFileName[count($definedFileName) - 2])) { //save some loops
                if ($definedFileName[count($definedFileName) - 2] === $file) { //actual filenames match
                    $fileUpper = substr(strtoupper($definedFileName[1]),
                        strlen('FILENAME_')); //so save the defined name
                    $fileMatch = true;
                    break;
                }
            }
        ...
        if (!$fileMatch) {                     //couldn't find a match
            global $messageStack;
            $fix = '<a href="'.tep_href_link('headertags_seo_exclude.php',
                    'exclude='.$file).'"><span style="color:#00dd00; font-size:10ps;">'.ERROR_INVALID_FILENAME_EXCLUDE.'</span></a>&nbsp;&nbsp;&nbsp;&nbsp;';
            $fix .= '<a href="'.tep_href_link('headertags_seo_exclude.php',
                    'delete='.$file).'"><span style="color:#00dd00; font-size:10ps;">'.ERROR_INVALID_FILENAME_DELETE.'</span></a>';
            $messageStack->add(sprintf(ERROR_INVALID_FILENAME,
                    $file.' '.$fix), 'error');
            return false;
        }
...
function AddedToHeaderTagsIncludesFilePseudo($file, &$fp, $languages_id)
{ //add a new entry to the includes/header_tags.php file if not present
    if (!$fp) {
        echo sprintf(ERROR_FAILED_FILE_OPEN, $file);
        return false;
    ...
    if (NotDuplicatePage($fp, $parts[0])) {
        ...
        for ($idx = 0; $idx < $cnt; ++$idx) {
            if (!$firstLine) {
                ...
                } else if (strpos($fp[$idx], "page =") !== FALSE && ($idx - $beginSection
                    < 4)) {
...
function BuildGenericString($set, &$addComma, $field, $name, $option, $text,
                            $allowNull = false)
{
    ...
    if (tep_db_num_rows($products_query) > 0) {
        if ($fullList) {
            ...
            while ($products   = tep_db_fetch_array($products_query)) {
                ...
                $missingStr .= '<td><a class="htc_Link" href="'.tep_href_link(FILENAME_CATEGORIES,
                        'action=new_product&pID='.$products['products_id']).'">'.(tep_not_null($products['products_name'])
                        ? $products['products_name'] : ERROR_NO_NAME).'</a></td>';
                $missingStr .= '</tr>';
            }
        } else {
            $missingStr          .= sprintf(ERROR_MISSING_TAGS_TYPE, 'Products',
    ...
    if (tep_db_num_rows($categories_query) > 0) {
        if ($fullList) {
            ...
            while ($categories = tep_db_fetch_array($categories_query)) {
                $missingStr .= '<tr class=smallText">';
                $missingStr .= '<td><a class="htc_Link" href="'.tep_href_link(FILENAME_CATEGORIES,
                        'cPath=&cID='.$categories['categories_id']).'">'.(tep_not_null($categories['categories_name'])
                        ? $categories['categories_name'] : ERROR_NO_NAME).'</a></td>';
                $missingStr .= '</tr>';
            }
        } else {
            $missingStr      .= sprintf(ERROR_MISSING_TAGS_TYPE, 'Categories',
    ...
    if (tep_db_num_rows($manufacturers_query) > 0) {
        if ($fullList) {
            ...
            while ($manufacturers = tep_db_fetch_array($manufacturers_query)) {
                $missingStr .= '<tr class=smallText">';
                $missingStr .= '<td><a class="htc_Link" href="'.tep_href_link(FILENAME_MANUFACTURERS,
                        'page=1&mID='.$manufacturers['manufacturers_id']).'">'.(tep_not_null($manufacturers['manufacturers_name'])
                        ? $manufacturers['manufacturers_name'] : ERROR_NO_NAME).'</a></td>';
                $missingStr .= '</tr>';
            }
        } else {
            $missingStr      .= sprintf(ERROR_MISSING_TAGS_TYPE,
                'Manufacturers', tep_db_num_rows($manufacturers_query));
...
function DeletePage($page_to_delete)
{
    ...
    for ($idx = 0; $idx < count($lines); ++$idx) {
        if (!$found && (strpos($lines[$idx], $page_to_delete) !== FALSE || strpos($lines[$idx],
                strtoupper($page_to_delete))) !== FALSE) {
            $delStart = $idx; // + 1;  //adjust for 0 start
            $found    = true;
        } else if ($found && ( strpos($lines[$idx],
                "ALL OTHER PAGES NOT DEFINED ABOVE") === FALSE && strpos($lines[$idx],
                "//") === FALSE && strpos($lines[$idx], ".php") === FALSE)) {
            $delStop++;
        } else if ($found && (strpos($lines[$idx],
                "ALL OTHER PAGES NOT DEFINED ABOVE") !== FALSE || (strpos($lines[$idx],
                "//") !== FALSE && strpos($lines[$idx], ".php") !== FALSE))) {
...
function FileNotUsingHeaderTags($file)
{
    ...
    if (($fp = @file($file))) {
        ...
        return 'TRUE';
    } else return sprintf(ERROR_FAILED_FILE_OPEN, $file);
...
function RemoveSymbols($string)
{
    $symbols = array('/', '\\', '"', ',', '.', '<', '>', '?', ';', ':', '[', ']',
        '{', '}', '|', '=', '+', '-', '_', ')', '(', '*', '&', '^', '%', '$', '#',
        '@', '!', '~', '`', 'nbsp'); //this will remove punctuation
...
function in_array_nocase($search, &$array)
{
    ...
    foreach ($array as $item)
        if (strtolower($item) == $search) return TRUE;
    return FALSE;
}
...
function GetKeywordsFromSite($pageName)
{ //attempts to figure out the best keywords to use from the page on the site based on keyword density
    ...
    if (count($lines) > 1) {
        ...
        if (count($ignoreWords) > 1) {
            ...
            $startCnt    = false;
            $wordCnt     = 0;
            list($kd_limitBot, $kd_limitTop) = explode(",",
                HEADER_TAGS_KEYWORD_DENSITY_RANGE);
            ...
            for ($i = 0; $i < count($lines); ++$i) {
                ...
                if (!tep_not_null($lines[$i])) continue;
                ...
                for ($w = 0; $w < count($words); ++$w) { //go through the words and add to the list
                    ...
                    if (in_array_nocase($words[$w], $ignoreWords) || is_numeric($words[$w])
                        || $words[$w] === '') continue;

                    $singleWords[$words[$w]] = isset($singleWords[$words[$w]]) ? $singleWords[$words[$w]]
                        + 1 : 1;
                    $wordCnt++;
                }

                if (!$startCnt && strpos($lines[$i], "<!-- body_text_eof //-->")
                    === FALSE) break;
            }
            ...
            foreach ($singleWords as $key => $data) {
                ...
                if ($kd > $kd_limitBot && $kd < $kd_limitTop) { //KD for word is between that set in admin
                    if (tep_not_null($keywordList)) $keywordList .= ',';
                    $keywordList .= $key;
                }
            }
            return (tep_not_null($keywordList) ? $keywordList : sprintf(ERROR_FAILED_NO_KEYWORDS_FOUND,
                    $pageName));
        } else
                return (sprintf(ERROR_FAILED_WORDS,
                    'includes/header_tags_seo_words.txt'));
    } else return (sprintf(ERROR_FAILED_PAGE_LOAD, $path));
}
...
function GetFilesWithHeaderTagsInclude($dir)
{ //returns all files in the root dir that calls includes/header_tags.php
    $newfiles = array();
    if ($handle   = @opendir($dir)) {
        while (false !== ($file = readdir($handle))) {
            ...
            if (($result = FileNotUsingHeaderTags($file)) === 'FALSE') {
                $newfiles[] = $file;
            } else if ($result !== 'TRUE') echo $result;
        }
        closedir($handle);
    }
    return $newfiles;
...
function GetKey($fileList, $file)
{
    for ($i = 0; $i < count($fileList); ++$i) {
        if ($fileList[$i]['text'] === $file) {
            if ($i == 0) return SELECT_A_FILE;
            else if ($i == 1) return SHOW_ALL_FILES;
            else if ($i == 2) return ADD_MISSING_PAGES;
            else return $i;
        }
    }
}
...
function GetQuickStats($missingArray)
{
    ...
    $statsArray['missing_p'] = ($missingArray['products'] > 0 ? $missingArray['products']
            : 0);  //already ran above so don't redo      
    $statsArray['missing_c'] = ($missingArray['cats'] > 0 ? $missingArray['cats']
            : 0);
    $statsArray['missing_m'] = ($missingArray['manu'] > 0 ? $missingArray['manu']
            : 0);
    ...
    if (substr_count($pageName, '?') > 1) { // this is a pseudo page
        ...
        foreach ((array) $lines as $line_num => $line) {
            if (preg_match('/<title>(.*)<\/title>/i', $line, $matches))
                    $metaInfo['title']       = $matches[1];
            if (preg_match('/<meta name=\"Description\" content=\"(.*)\"/i',
                    $line, $matches)) $metaInfo['description'] = $matches[1];
            if (preg_match('/<meta name=\"Keywords\" content=\"(.*)\"/i', $line,
                    $matches)) $metaInfo['keywords']    = $matches[1];

            if (tep_not_null($metaInfo['keywords'])) break;
        }
    }
...
function IsTemplate()
{ //return if BTS or STS is enabled, or not
...
function NotDuplicatePage($fp, $pagename)
{
    for ($idx = 0; $idx < count($fp); ++$idx) {
        ...
        if (tep_not_null($section)) {
            if (strcasecmp($section, $pagename) === 0) return false;
        }
    }
    return true;
}
...
function ResetCache_HeaderTags($name, $id, $deleteAll = false)
{
    ...
    while ($cache = tep_db_fetch_array($cache_query)) {
        ...
        for ($j = 0, $k = sizeof($languages); $j < $k; $j++) {
            ...
            if ($deleteAll) {
                $cacheExt = ( $gzip == 1 ? @gzinflate(base64_decode($cache['title']))
                        : stripslashes($cache['title']) );
                $cacheExt = unserialize($cacheExt);

                if (strpos($cacheExt, $cacheFileName) !== FALSE && (tep_not_null($id)
                            ? strpos($cacheExt, $id) !== FALSE : true)) {
...
function SetBaseFilesToDefaultLanguage($languages_id = 1)
{ //switch the headertags language settings in the original
...
function SMMultiSelectMenu($name, $values, $selected_vals, $params = '',
                           $required = false)
{
