...
class upload
{
    ...
    {
        $this->upload($file, $destination, $permissions, $extensions);
    }

    function upload($file = '', $destination = '', $permissions = '777',
                    $extensions = '')
    {
        $this->set_file($file);
    ...
    function parse()
    {
        ...
        if (tep_not_null($file['tmp_name']) && ($file['tmp_name'] != 'none') && is_uploaded_file($file['tmp_name'])) {
            if (sizeof($this->extensions) > 0) {
                if (!in_array(strtolower(substr($file['name'],
                    ...
                    if ($this->message_location == 'direct') {
                        ...
                        $messageStack->add_session(ERROR_FILETYPE_NOT_ALLOWED,
                            'error');
                    }
            ...
            $this->set_filename(preg_replace("/[\s_]/", "-",
                    preg_replace("/[\s-]+/", " ",
                        preg_replace("/[^a-z0-9_\s-\.]/", "",
                            strtolower(iconv("utf-8", "ascii//TRANSLIT",
                                    $file['name']))))));
    ...
    function save()
    {
        ...
        if (move_uploaded_file($this->file['tmp_name'],
            ...
            if ($this->message_location == 'direct') {
                ...
                $messageStack->add_session(SUCCESS_FILE_SAVED_SUCCESSFULLY,
                    'success');
            }
        ...
        if (!tep_is_writable($this->destination)) {
            if (is_dir($this->destination)) {
                if ($this->message_location == 'direct') {
                    ...
                    $messageStack->add_session(sprintf(ERROR_DESTINATION_NOT_WRITEABLE,
                            $this->destination), 'error');
                }
            } else {
                if ($this->message_location == 'direct') {
                    $messageStack->add(sprintf(ERROR_DESTINATION_DOES_NOT_EXIST,
                            $this->destination), 'error');
                } else {
                    $messageStack->add_session(sprintf(ERROR_DESTINATION_DOES_NOT_EXIST,
                            $this->destination), 'error');
                }
            }
