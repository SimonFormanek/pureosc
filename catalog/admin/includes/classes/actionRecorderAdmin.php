        ...
        if (defined('MODULE_ACTION_RECORDER_INSTALLED') && tep_not_null(MODULE_ACTION_RECORDER_INSTALLED)) {
            if (tep_not_null($module) && in_array($module.'.'.substr($_SERVER['PHP_SELF'],
                ...
                if (!class_exists($module)) {
                    if (file_exists(DIR_FS_CATALOG.'includes/modules/action_recorder/'.$module.'.'.substr($_SERVER['PHP_SELF'],
                                (strrpos($_SERVER['PHP_SELF'], '.') + 1)))) {
                        include(DIR_FS_CATALOG.'includes/languages/'.$language.'/modules/action_recorder/'.$module.'.'.substr($_SERVER['PHP_SELF'],
                                (strrpos($_SERVER['PHP_SELF'], '.') + 1)));
                        include(DIR_FS_CATALOG.'includes/modules/action_recorder/'.$module.'.'.substr($_SERVER['PHP_SELF'],
                                (strrpos($_SERVER['PHP_SELF'], '.') + 1)));
                    } else {
                        return false;
                    }
