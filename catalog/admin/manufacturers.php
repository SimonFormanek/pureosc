    ...
    switch ($action) {
        ...
        case 'save':
            ...
            if ($action == 'insert') {
                ...
                tep_db_perform(TABLE_MANUFACTURERS, $sql_data_array, 'update',
            ...
            for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
                ...
                if ($action == 'insert') {
                    ...
                    tep_db_perform(TABLE_MANUFACTURERS_INFO, $sql_data_array,
                        'update',
                        "manufacturers_id = '".(int) $manufacturers_id."' and languages_id = '".(int) $language_id."'");
                }
            }
...
while ($manufacturers           = tep_db_fetch_array($manufacturers_query)) {
    if ((!isset($_GET['mID']) || (isset($_GET['mID']) && ($_GET['mID'] == $manufacturers['manufacturers_id'])))
    ...
    ?>
                ...
                <tr>
                    <td colspan="3"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                            <tr>
                                ...
                                echo $manufacturers_split->display_count($manufacturers_query_numrows,
                                    MAX_DISPLAY_SEARCH_RESULTS, $_GET['page'],
                                    TEXT_DISPLAY_NUMBER_OF_MANUFACTURERS);
                                ?></td>
                                <td class="smallText" align="right"><?php
                                echo $manufacturers_split->display_links($manufacturers_query_numrows,
                ...
                if (empty($action)) {
                    ?>
                    <tr>
                        <td align="right" colspan="3" class="smallText"><?php
                            echo tep_draw_button(IMAGE_INSERT, 'plus',
                                tep_href_link(FILENAME_MANUFACTURERS,
                                    'page='.$_GET['page'].'&mID='.$mInfo->manufacturers_id.'&action=new'));
                            ?></td>
                    </tr>
                    <?php
                }
        ...
        switch ($action) {
            case 'new':
                ...
                $contents   = array('form' => tep_draw_form('manufacturers',
                        FILENAME_MANUFACTURERS, 'action=insert', 'post',
                ...
                $contents[] = array('text' => '<br />'.TEXT_MANUFACTURERS_SEO_TITLE.'<br />'.tep_draw_input_field('manufacturers_seo_title',
                        NULL, 'style="width: 300px;"'));
                ...
                for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
                    $manufacturer_seo_description_string .= '<br />'.tep_image(DIR_WS_CATALOG_LANGUAGES.$languages[$i]['directory'].'/images/'.$languages[$i]['image'],
                            $languages[$i]['name'], '', '',
                            'style="vertical-align: top;"').'&nbsp;'.tep_draw_textarea_field('manufacturers_seo_description['.$languages[$i]['id'].']',
                            'soft', '80', '10');
                }
                for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
                    $manufacturer_seo_keywords_string .= '<br />'.tep_image(DIR_WS_CATALOG_LANGUAGES.$languages[$i]['directory'].'/images/'.$languages[$i]['image'],
                            $languages[$i]['name']).'&nbsp;'.tep_draw_input_field('manufacturers_seo_keywords['.$languages[$i]['id'].']',
                            NULL,
                ...
                $languages                       = tep_get_languages();
                for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
                    $manufacturer_inputs_string      .= '<br />'.tep_image(tep_catalog_href_link(DIR_WS_LANGUAGES.$languages[$i]['directory'].'/images/'.$languages[$i]['image'],
                                '', 'SSL'), $languages[$i]['name']).'&nbsp;'.tep_draw_input_field('manufacturers_url['.$languages[$i]['id'].']');
                    $manufacturer_description_string .= '<br />'.tep_image(tep_catalog_href_link(DIR_WS_LANGUAGES.$languages[$i]['directory'].'/images/'.$languages[$i]['image'],
                                '', 'SSL'), $languages[$i]['name'], '', '',
                            'style="vertical-align: top;"').'&nbsp;'.tep_draw_textarea_field('manufacturers_description['.$languages[$i]['id'].']',
                            'soft', '80', '10');
                }
            ...
            case 'edit':
                ...
                $contents[] = array('text' => '<br />'.TEXT_MANUFACTURERS_NAME.'<br />'.tep_draw_input_field('manufacturers_name',
                        $mInfo->manufacturers_name, 'style="width: 300px;"'));
                $contents[] = array('text' => '<br />'.TEXT_MANUFACTURERS_SEO_TITLE.'<br />'.tep_draw_input_field('manufacturers_seo_title',
                        $mInfo->manufacturers_seo_title, 'style="width: 300px;"'));
                ...
                for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
                    $manufacturer_seo_description_string .= '<br />'.tep_image(DIR_WS_CATALOG_LANGUAGES.$languages[$i]['directory'].'/images/'.$languages[$i]['image'],
                            ...
                            'style="vertical-align: top;"').'&nbsp;'.tep_draw_textarea_field('manufacturers_seo_description['.$languages[$i]['id'].']',
                            'soft', '80', '10',
                            tep_get_manufacturer_seo_description($mInfo->manufacturers_id,
                                $languages[$i]['id']));
                }
                for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
                    $manufacturer_seo_keywords_string .= '<br />'.tep_image(DIR_WS_CATALOG_LANGUAGES.$languages[$i]['directory'].'/images/'.$languages[$i]['image'],
                            $languages[$i]['name']).'&nbsp;'.tep_draw_input_field('manufacturers_seo_keywords['.$languages[$i]['id'].']',
                            tep_get_manufacturer_seo_keywords($mInfo->manufacturers_id,
                                $languages[$i]['id']),
                            'style="width: 300px;" placeholder="'.PLACEHOLDER_COMMA_SEPARATION.'"');
                }
                ...
                for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
                    $manufacturer_inputs_string      .= '<br />'.tep_image(tep_catalog_href_link(DIR_WS_LANGUAGES.$languages[$i]['directory'].'/images/'.$languages[$i]['image'],
                                '', 'SSL'), $languages[$i]['name']).'&nbsp;'.tep_draw_input_field('manufacturers_url['.$languages[$i]['id'].']',
                            tep_get_manufacturer_url($mInfo->manufacturers_id,
                                $languages[$i]['id']));
                    $manufacturer_description_string .= '<br />'.tep_image(tep_catalog_href_link(DIR_WS_LANGUAGES.$languages[$i]['directory'].'/images/'.$languages[$i]['image'],
                                '', 'SSL'), $languages[$i]['name'], '', '',
                            'style="vertical-align: top;"').'&nbsp;'.tep_draw_textarea_field('manufacturers_description['.$languages[$i]['id'].']',
                            'soft', '80', '10',
                            tep_get_manufacturer_description($mInfo->manufacturers_id,
                                $languages[$i]['id']));
                }
