    ...
    <head>
        ...
        <?php if ($modules_count > 0) {
            foreach ($modules as $key => $module) {
                ?>
                <?php
                if (tep_not_null($module['css']['style'])) {
                    foreach ($module['css']['style'] as $ext_css) {
                        if (tep_not_null($ext_css)) {
                            ?><link href="ext/aas_modules/<?php echo $key; ?>/css/<?php echo $ext_css; ?>" rel="stylesheet" /><?php
                        }
                    }
                }
                ?>
                ...
                <?php
                if (tep_not_null($module['css']['print'])) {
                    foreach ($module['css']['print'] as $ext_css) {
                        if (tep_not_null($ext_css)) {
                            ?><link href="ext/aas_modules/<?php echo $key; ?>/css/<?php echo $ext_css; ?>" rel="stylesheet" media="print" /><?php
                        }
                    }
                }
                ?>
    ...
    <body>
        ...
        <div id="panel" style="<?php echo isset($search) && $search != '' ? '' : 'display:none;'; ?>">
            <div style="overflow:hidden;text-align:center">
                    <?php
                    echo tep_draw_form('search', FILENAME_AAS, '', 'get');
                    ...
                    ?>
                    <?php if (isset($_GET['search'])) { ?><input class="search_clear_button lfor" onclick="clearSearchQuery('<?php
                        echo isset($_SESSION['admin']['AAS']['preSearchUrl']) ? $_SESSION['admin']['AAS']['preSearchUrl']
                                : tep_href_link(FILENAME_AAS, '');
                        ?>')" type="button" value="x" /><?php } ?>
<?php
echo tep_draw_pull_down_menu('searchOnField', $searchOnFieldArray, '',
...
<?php
foreach ($ascDescArray as $key => $value)
    echo '<option '.(isset($_GET['ascDesc']) && $_GET['ascDesc'] == $key ? 'selected="selected"'
...
<?php
if (!isset($aasAac['default']['print'][$_SESSION['admin']['id']])) {
    if ($categories_entries > 0 || $entries > 0) {
        ?>
                    ...
                    <ul class="dropdown">
                    <?php if ($categories_entries > 0 && $entries
                        > 0) { ?><li><a onclick="printHtmlElement(1, this)" href="#"><?php echo AAS_TEXT_PRINT_CATEGORIES_AND_PRODUCTS_TABLE; ?></a><?php if ($entries
                > 0) {
                ?><label><input type="checkbox" name="printHtmlElement1" ><?php echo AAS_TEXT_PRINT_INCLUDE_PAGINATION; ?></label><?php } ?></li><?php } ?>
...
<?php if ($defaults['displayLogoffButton']) { ?><div id="logoffbutton" class="koumpakia buttonakia" data-title="<?php echo AAS_HEADING_TITLE_LOGOFF; ?>" ><a href="<?php echo tep_href_link(FILENAME_LOGIN,
        'action=logoff');
    ?>" ><img src="ext/aas/images/glyphicons_026_road.png" alt="" style="height:23px;"></a></div><?php } ?>
            ...
            'cPath='.$parent_id);
                    ?>"><img src="ext/aas/images/glyphicons_221_unshare.png" alt=""></a></div>
            <?php
            }
        }
        ?>
        ...
        <section id="tbl-wrapper">
                ...
                <div id="pagination">
                        ...
                        <?php if ($defaults['inTableSearchPosition']['visible']
                            && $defaults['inTableSearchPosition']['y'] == 'top') {
                            ?><div class="inTableSearch-wrapper inTableSearch-wrapper-<?php echo isset($defaults['inTableSearchPosition']['x'])
                                ? $defaults['inTableSearchPosition']['x'] : 'right';
                            ?>"><input type="text" id="search" placeholder="<?php echo AAS_TEXT_IN_TABLE_SEARCH; ?>" class="inTableSearch inTableSearch-top" /></div><?php } ?>
                        ...
                        <tbody class="categories_tbody">
                                ...
                                <tr class="folder gradeX" id="cat_<?php echo $categories['categories_id']; ?>">
                                    ...
                                    <td class="nojedit centerAlign previewPage" style="padding:0px;"><?php
                                         echo '<a href="'.tep_href_link(FILENAME_AAS,
                                             'cPath='.$categories['categories_id']).'">'.tep_image(DIR_WS_ICONS.'folder.gif',
                                             ICON_FOLDER).'</a>';
                                         ?>

                                        <a target="_blank" class="view_category_class" title="<?php echo AAS_VIEW_CATEGORYS_PAGE; ?>" href="<?php
                                         echo tep_aas_link('front', 'index.php',
                                             'cPath='.$categories['categories_id'].'&&language='.$languages_selected['code']);
                                         ?>"><img src="ext/aas/images/glyphicons_152_new_window_op.png" alt=""></a>
                                    </td>
                                 ...
                                 || $bool_count_subcategories ? 'nojedit' : 'cursor-pointer';
                                             ?> centerAlign"><?php echo $categories['categories_name'].$anwkatwteleia.$bcp.$comma.$bcs; ?></td>
                                             <?php
                                         }
...
//store for attributes cloning select list
                                $products_names_id_array[$products['products_id']]
                                    = $fieldsArray['products_model']['visible'] ? $products['products_name'].' [ '.$products['products_model'].' ]'
                                        : $products['products_name'];

                                if ($fieldsArray['products_status']['visible'] && $products['products_status']
                                    == '0')
                                        $status_unavailable_class = ' unavailable';
                            ...
                            <tr class="product <?php
                                echo ($counter & 1) && $defaults['colorEachTableRowDifferently']
                                        ? 'even' : 'odd';
                                echo $status_unavailable_class.' '.$isLinked;
                                ?>" id="pid_<?php echo (int) $products['products_id']; ?>" data-category="<?php echo $products['categories_id']; ?>">
                                ...
                                <?php
                                if ($ascDesc === 'DESC')
                                        echo $totalRows - ( ($counter++) + (($currentPage
                                    - 1) * $entriesPerPage));
                                else
                                        echo ( ++$counter) + (($currentPage - 1)
                                    * $entriesPerPage);
                                ?>    
                                    <a target="_blank" class="view_product_class" title="<?php echo AAS_VIEW_PRODUCTS_PAGE; ?>" href="<?php
                            echo tep_aas_link('front', 'product_info.php',
                                'products_id='.$products['products_id'].'&&language='.$languages_selected['code']);
                                ?>"><img src="ext/aas/images/glyphicons_152_new_window_op.png" alt=""></a>
                                </td>
                             <?php
                                 ...
                                 if ($value['visible']) {
                                     ...
                                     switch ($key) {
                                         ...
                                         case 'products_description':
                                             ...
                                             ?>
                                        <img data-productname="<?php echo $products['products_name']; ?>" id="trigger_<?php echo (int) $products['products_id']; ?>" title="<?php echo AAS_VIEW_EDIT_DESCRIPTION; ?>" class="<?php
                                             echo isset($aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']])
                                             && $aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']]
                                                     ? '' : 'descriptionbuttonImg';
                                             ?>" src="<?php echo DIR_WS_CATALOG_LANGUAGES.$languages_selected['directory'].'/images/'.$languages_selected['image']; ?>" alt="<?php echo $languages_selected['name']; ?>" >&nbsp;
                                             <?php
                                             if ($hasDescription)
                        ...
                        == '0' ? 'selected="selected"' : '';
                                            ?>><?php echo AAS_NONE; ?></option><?php
                                            foreach ($tax_class_array as $tca)
                                                echo '<option value="'.$tca['id'].'_'.$tca['tax_rate'].'" '.($tca['id']
                                                == $products['products_tax_class_id']
                                                        ? 'selected="selected"' : '').' >'.$tca['title'].' '.$tca['tax_rate'].'%</option>';
                                            ?>
                                                </select></td>
                                            <?php
                                        }
                        ...
                        == '0' ? 'selected="selected"' : '';
                                            ?>><?php echo AAS_NONE; ?></option><?php
                                            foreach ($manufacturers_array as $man)
                                                echo '<option value="'.$man['id'].'" '.($man['id']
                                ...
                                0, -9);
                                    ...
                                    ? '' : 'visibility:hidden;';
                                                    ?>" href="#"><img style="opacity:0.3;height:20px" src="ext/aas/images/remove_white_no_round_1.png" alt="never expire"></a>
                                ...
                                ? '' : 'nojedit';
                                                ?>" >
                                            <?php
                                            $caid = tep_not_null($categoryId) ? (int) $categoryId
                                                    : 0;
                                            ...
                                            if (!isset($ptc_array[$products['products_id']])) {
                                                ...
                                                if (count($protoca_array[$products['products_id']])
                                                    ...
                                                    foreach ($protoca_array[$products['products_id']] as $kpropo => $propro) {
                                                        ...
                                                        echo count($tjohn) > 0 ? implode('&nbsp;&raquo;&nbsp;',
                                                                $tjohn) : AAS_TEXT_TOP;
                                    ...
                                    case 'sort_order':
                                        ...
                                        if (isset($aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']])
                                            ...
                                            echo'<td class="nojedit products_sort_order '.$_table_td_rules[$key]['class'].'" data-column="products_'.$key.'" >'.(isset($products['products_sort_order'])
                                                    ? $products['products_sort_order']
                                                    : '---').'</td>';
                                        } else {

                                            echo'<td style="'.(isset($products['products_sort_order'])
                                            && (!isset($search) || $search == "" )
                                                    ? $_table_td_rules[$key]['style']
                                                    : '').'" class="'.($value['cellEditable']
                                            && isset($products['products_sort_order'])
                                    ...
                                    case 'date_added':
                                        ...
                                        if (isset($products['products_'.$key])) {
                                            ...
                                            $dtdateAdded = substr($products['products_'.$key],
                                                0, -9);
...
<?php } ?>
                        ...
                        <?php if ($defaults['inTableSearchPosition']['visible']
                            && $defaults['inTableSearchPosition']['y'] == 'bottom') {
                            ?><div class="inTableSearch-wrapper inTableSearch-wrapper-<?php echo isset($defaults['inTableSearchPosition']['x'])
                                ? $defaults['inTableSearchPosition']['x'] : 'right';
                            ?>"><input type="text" id="search" placeholder="In Table Search" class="inTableSearch inTableSearch-top" /></div><?php } ?>
    ...
    <?php echo $pagination->draw(); ?>
        ...
        <?php } ?>
        <?php if ($defaults['displayCountProducts'] && $categories_entries
            > 0) { ?>
...
<?php if ($defaults['displayBottomInformation']) { ?>
    ...
    echo AAS_VERSION;
    ?><br><br> <?php echo AAS_TEXT_CREATED_BY; ?><a target="_blank" href="http://www.johnbarounis.com/">John Barounis</a></div>
<?php } ?>
...
<?php if ($defaults['enableClocks']) include 'ext/aas/plugins/clocks/index.php'; ?>
                ...
                <?php if ($defaults['enableToolBox'] && count($products_names_id_array)
                    > 0) { ?>
            <section id="toolBox">
    ...
    <?php if (isset($aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']])
        ...
        <?php if (sizeof($languages_array) > 1 && $defaults['displayLanguageSelection']) { ?>
            ...
            <?php
            echo tep_draw_form('adminlanguage1', FILENAME_AAS,
...
<?php if ($defaults['displayByStatusSelection']) { ?>
    ...
    ?>>
            ...
            ? 'selected="selected"' : '';
    ?> ><?php echo AAS_ACTIVE_PRODUCTS; ?></option>
                            <option value="0" <?php echo $show_products_by_status == 0
                ? 'selected="selected"' : '';
        ?>><?php echo AAS_INACTIVE_PRODUCTS; ?></option>
                            <option value="2" <?php echo $show_products_by_status == 2
                ? 'selected="selected"' : '';
        ?> ><?php echo AAS_ALL_PRODUCTS; ?></option>
                        </select>
                    </form>
                </div>
<?php } ?>
...
<?php if ($defaults['displayCategoriesSelection']) { ?>
    ...
    ?>>
            ...
            : '';
    ?> ><?php echo $displayCategories == 1 ? AAS_CATEGORIES_VISIBLE : AAS_DISPLAY_CATEGORIES;
    ?></option>
