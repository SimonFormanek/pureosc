...
$modules_installed = (tep_not_null(MODULE_CONTENT_INSTALLED) ? explode(';',
...
if ($maindir = @dir(DIR_FS_CATALOG_MODULES.'content/')) {
    ...
    {
        return strnatcmp($a['group'].'-'.(int) $a['sort_order'].'-'.$a['title'],
            $b['group'].'-'.(int) $b['sort_order'].'-'.$b['title']);
    }

    function _sortContentModuleFiles($a, $b)
    {
        return strnatcmp($a['group'].'-'.$a['title'],
            $b['group'].'-'.$b['title']);
    }
        ...
        case 'install':
            ...
            foreach ($modules['new'] as $m) {
                if ($m['code'] == $class) {
                    ...
                    tep_db_query("update ".TABLE_CONFIGURATION." set configuration_value = '".implode(';',
                            $modules_installed)."' where configuration_key = 'MODULE_CONTENT_INSTALLED'");

                    tep_redirect(tep_href_link('modules_content.php',
                            'module='.$class.'&action=edit'));
                }
            }

            tep_redirect(tep_href_link('modules_content.php',
                    'action=list_new&module='.$class));
            ...
            foreach ($modules['installed'] as $m) {
                if ($m['code'] == $class) {
                    ...
                    if (in_array($m['group'].'/'.$m['code'], $modules_installed)) {
                        unset($modules_installed[array_search($m['group'].'/'.$m['code'],
                                $modules_installed)]);
                    }
                            ...
                            <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                ...
                                foreach ($modules['new'] as $m) {
                                    ...
                                    if (isset($mInfo) && is_object($mInfo) && ($module->code
                                        ...
                                        echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\''.tep_href_link('modules_content.php',
                                            'action=list_new&module='.$module->code).'\'">'."\n";
                                    }
                                    ?>
        ...
        if (isset($mInfo)) {
            ...
            if ($action == 'list_new') {
                $contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_MODULE_INSTALL,
                        'plus',
                        tep_href_link('modules_content.php',
                            'module='.$mInfo->code.'&action=install')));
...
//            $keys .= '<strong>' . $value['title'] . '</strong><br />';
                ...
                $contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_EDIT,
                        'document',
                        tep_href_link('modules_content.php',
                            'module='.$mInfo->code.'&action=edit')).tep_draw_button(IMAGE_MODULE_REMOVE,
                        'minus',
                        tep_href_link('modules_content.php',
                            'module='.$mInfo->code.'&action=remove')));
