...
<table border="0" width="100%" cellspacing="0" cellpadding="2">
    ...
    if (($action == 'edit') && ($order_exists === true)) {
        ...
        <tr>
            <td width="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0">
                    <tr>
                        <?php /*                         * * Altered for Manual Order Maker ** PHP TAG CHANGED BELOW
                    ...
                    <tr>
                        <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="2">
                                <tr>
                                    ...
                                    <td class="main"><?php
                                        echo tep_address_format($order->customer['format_id'],
                                                $order->customer, 1, '', '<br />');
                                        ?></td>
                                </tr>
                                <tr>
    ...
    ?>
            ...
            <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
                    ...
                    for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
                        ...
                        '            <td class="dataTableContent" align="right" valign="top"><strong>' . $currencies->format(tep_add_tax($order->products[$i]['final_price'],
                                ...
                                true, $order->info['currency'],
                                $order->info['currency_value']) . '</strong></td>' . "\n" .
                        '            <td class="dataTableContent" align="right" valign="top"><strong>' . $currencies->format($order->products[$i]['final_price'] * $order->products[$i]['qty'], true,
                                $order->info['currency'],
                                $order->info['currency_value']) . '</strong></td>' . "\n" .
                        '            <td class="dataTableContent" align="right" valign="top"><strong>' . $currencies->format(tep_add_tax($order->products[$i]['final_price'],
        ...
        <tr><?php
                echo tep_draw_form('status', FILENAME_ORDERS,
                        tep_get_all_get_params(array('action')) . 'action=update_order');
                ?>
            <td class="main"><?php
                echo tep_draw_textarea_field('comments', 'soft',
                        '60', '5');
                ?></td>
        </tr>
        <tr>
        ...
        <tr>
            <td><table border="0" cellspacing="0" cellpadding="2">
                    <tr>
                        <td><table border="0" cellspacing="0" cellpadding="2">
                                <tr>
                                    <td class="main"><strong><?php echo ENTRY_STATUS; ?></strong> <?php
                ...
                ?></td>
                                ...
                                <tr>
                                    <td class="main"><strong><?php echo ENTRY_NOTIFY_CUSTOMER; ?></strong> <?php
                                        echo tep_draw_checkbox_field('notify',
                                                '', true);
                                        ?></td>
                                    <td class="main"><strong><?php echo ENTRY_NOTIFY_COMMENTS; ?></strong> <?php
                                        echo tep_draw_checkbox_field('notify_comments',
                                                '', true);
                                        ?></td>
                                </tr>
                            </table></td>
                        <td class="smallText" valign="top"><?php
                        echo tep_draw_button(IMAGE_UPDATE,
                                'disk', null, 'primary');
                                        ?></td>
                    </tr>
                </table></td>
...
//old      $orders_query_raw = "select o.orders_id, o.customers_name,  " . ( defined('MODULE_CONTENT_PWA_LOGIN_STATUS') ? "o.customers_guest, " : '' ) . " o.payment_method, o.date_purchased, o.last_modified, o.currency, o.currency_value, o.customer_service_id, s.orders_status_name, ot.text as order_total from " . TABLE_ORDERS . " o left join " . TABLE_ORDERS_TOTAL . " ot on (o.orders_id = ot.orders_id), " . TABLE_ORDERS_STATUS . " s where o.orders_status = s.orders_status_id and s.language_id = '" . (int)$languages_id . "' and ot.class = 'ot_total' order by o.orders_id DESC";
    ...
    ?>
                    ...
                    <!-- BOF Order Maker -->
                    <tr>
                        <td align="right" colspan="5" class="smallText"><?php
                                    echo tep_draw_button(IMAGE_CREATE_ORDER,
            ...
            switch ($action) {
                ...
                default:
                    if (isset($oInfo) && is_object($oInfo)) {
                        ...
                        $contents[] = array('text' => '<br />' . cfg('TEXT_INFO_CUSTOMER_SERVICE_ID') . ' ' . $oInfo->customer_service_id);
