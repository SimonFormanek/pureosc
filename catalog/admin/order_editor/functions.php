...
function oe_draw_selection_field($name, $type, $value = '', $checked = false,
                                 $compare = '', $parameters = '')
{
...
function oe_draw_checkbox_field($name, $value = '', $checked = false,
                                $compare = '', $parameters = '')
{
    return oe_draw_selection_field($name, 'checkbox', $value, $checked,
        $compare, $parameters);
}

////
// Output a form radio field
function oe_draw_oe_radio_field($name, $value = '', $checked = false,
                                $compare = '', $parameters = '')
{
    return or_draw_selection_field($name, 'radio', $value, $checked, $compare,
        $parameters);
}
...
function oe_get_subcategories(&$subcategories_array, $parent_id = 0)
{
    ...
    while ($subcategories       = tep_db_fetch_array($subcategories_query)) {
        ...
        if ($subcategories['categories_id'] != $parent_id) {
            oe_get_subcategories($subcategories_array,
                $subcategories['categories_id']);
        }
    }
}

function oe_select_ot_options($key_value,
                              $key = 'ORDER_EDITOR_ALLOWED_OT_MODULES')
{
    ...
    while (list(, $value) = each($installed_ot_modules)) {
        ...
        if (in_array($ot_class, $key_values))
...
function oe_generate_search_SQL($keywords_array, $fields_array,
                                $search_type = 'OR')
{
    $search_string = '';
    $end_trim      = 0;
