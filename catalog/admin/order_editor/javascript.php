...
?>
    ...
    <script language="javascript" type="text/javascript"><!--
        ...
        function rewriteDiv(field, stack) {
            ...
            if (stack == 'orders') {
                document.getElementById("ordersMessageStack").innerHTML = '<table><tr class="messageStackSuccess"><td class="messageStackSuccess"><?php echo tep_image(DIR_WS_ICONS.'success.gif',
            ICON_SUCCESS).'&nbsp;'.sprintf(AJAX_MESSAGE_STACK_SUCCESS, 'field'); ?></td></tr></table>';

                document.getElementById("productsMessageStack").innerHTML = '<?php echo tep_draw_separator('pixel_trans.gif',
            '1', '10'); ?>';

                document.getElementById("historyMessageStack").innerHTML = '<?php echo tep_draw_separator('pixel_trans.gif',
            '1', '10'); ?>';
            ...
            if (stack == 'products') {
                document.getElementById("productsMessageStack").innerHTML = '<table><tr class="messageStackSuccess"><td class="messageStackSuccess"><?php echo tep_image(DIR_WS_ICONS.'success.gif',
            ICON_SUCCESS).'&nbsp;'.sprintf(AJAX_MESSAGE_STACK_SUCCESS, 'field'); ?></td></tr></table>';

                document.getElementById("historyMessageStack").innerHTML = '<?php echo tep_draw_separator('pixel_trans.gif',
            '1', '10'); ?>';

                document.getElementById("ordersMessageStack").innerHTML = '<?php echo tep_draw_separator('pixel_trans.gif',
            '1', '10'); ?>';
            ...
            if (stack == 'history') {
                document.getElementById("historyMessageStack").innerHTML = '<table><tr class="messageStackSuccess"><td class="messageStackSuccess"><?php echo tep_image(DIR_WS_ICONS.'success.gif',
            ICON_SUCCESS).'&nbsp;'.sprintf(AJAX_MESSAGE_STACK_SUCCESS, 'field'); ?></td></tr></table>';
                    ...
                    xmlHttp.onreadystatechange =
                            function () {
                                if (xmlHttp.readyState != 4)
                                    return;
                                if (xmlHttp.status == 200) {
                                    rewriteDiv(xmlHttp.responseText, 'products');
                                    obtainTotals();
                                }
                            };
                } else {//action == 'update'
                    ...
                    xmlHttp.onreadystatechange =
                            function () {
                                if (xmlHttp.readyState != 4)
                                    return;
                                if (xmlHttp.status == 200) {
                                    rewriteDiv(xmlHttp.responseText, 'history');
                                    deleteRow(info, 'commentsTable')
                                }
                            };
                ...
                xmlHttp.onreadystatechange =
                        function () {
                            if (xmlHttp.readyState != 4)
                                return;
                            if (xmlHttp.status == 200) {
                                reloadDiv('commentsBlock', xmlHttp.responseText);
                                clearComments();
                            }
                        };
                                ...
                                xmlHttp.onreadystatechange =
                                        function () {
                                            if (xmlHttp.readyState != 4)
                                                return;
                                            if (xmlHttp.status == 200) {
                                                reloadDiv('ordersMessageStack', xmlHttp.responseText)
                                            }
                                        };
...
///begin standard JavaScript for edit_orders.php file
        ...
        <?php echo oe_js_zone_list('SelectedCountry', 'theForm', 'zoneID',
            'inputID', 'menuID'); ?>
                        }
