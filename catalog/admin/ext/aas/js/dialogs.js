    ...
    $("#dialog-attributes").dialog({
        buttons: [
                ...
                click: function () {
                    ...
                    $('#attributes-table tbody tr').each(function (i, e) {
                        ...
                        if (jthis.find('.hidden_products_attributes_id').length > 0) {
                            ...
                            switch (pref_new) {
                                ...
                                case'+':
                                    temp_arr_new.push('plus');
                                    break;
                                case'-':
                                    temp_arr_new.push('minus');
                                    break;
                                default:
                                    temp_arr_new.push('none');
                    ...
                    aas.ajax.do({data: 'value=' + attr_obj.join('_@_') + '&value1=' + attr_obj_new.join('_@_') + '&id=' + dt.pid + '&column=products_attributes', url: config.url.ajax}, function (msg) {
                        ...
                        if (msg == 'ok_reload') {
                            ...
                            //update select menu
                            var lsopt = [];
                            for (var i in opt_names_list)
                                lsopt.push('<option name="' + opt_names_list[i] + '" value="' + i + '" >' + opt_names_list[i] + '</option>');
...

            {
                ...
                click: function () {
                    $(this).dialog("close");
                    aas.localStorage.remove('AAS:checkedAttributes');
                }
            }
        ...
        close: function () {
            $('#dialog-attributes-options').html('');
        }
    });
        ...
        open: function () {
            ...
            aas.ajax.do({data: {action: 'getProductsList', pid: spid}, url: config.url.plugins + 'specials/aas.php'}, function (msg) {
                ...
                if (msg == '0') {
                    ...
                    $('#specials_add_select_products-list').html(msg);
        ...
        buttons: [
            {
                ...
                click: function () {
                    ...
                    if (checked.length > 0) {
                        ...
                        var data = $(this).data('data');
                        if (data.t)
                            data.t.next().val(checked.val());
                        $(this).dialog("close");

                    } else
                        aas.dialog.open('dialog-error', translate.AAS_DIALOG_TITLE_DOWNLOADABLE_ATTRIBUTES_CONTENT_MANAGER_YOU_DID_NOT_SELECT_A_FILE);
    ...
    $("#dialog-aac").dialog({
                    ...
                    if (type == 'default-dcd') {
                        ...
                        aas.ajax.do({data: {item: 'saveDcd', dcd: dcd, type: type}, dataType: 'json', url: config.url.actions}, function (msg) {
                            ...
                            if (msg == '0') {
                                ...
                                aas.dialog.open('dialog-success', translate.AAS_AAC_TEXT_ADMIN_OPTIONS_SAVED);
                        ...
                        for (var i = 0, n = ams.length; i < n; i++)
                            dcd[ams[i]] = {};
                        ...
                        aas.ajax.do({data: {item: 'saveAac', aac: aac, type: type}, url: config.url.actions}, function (msg) {
                            ...
                            if (msg == '0') {
                                ...
                                aas.dialog.open('dialog-success', translate.AAS_DIALOG_AAC_TEXT_UPDATED);
        ...
        open: function () {
            ...
            if (dt['data']) {
                ...
                if (dt['data'].tab == 'modules')
                    $("#dialog-aac #modulesTabLink").addClass("active").click();
                else
