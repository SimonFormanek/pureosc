...
function tep_aas_draw_categories_tree($root_id = 0, $spacing = '&nbsp;',
                                      $selected = 0, $selectedClass = '',
                                      $drawSelectTag = true, $selectParams = '',
                                      $selectClass = '')
{
    global $languages_id;
    ...
    if ($citems <= 0) return '';
    elseif ($citems == 1)
            $children[]                     = $items; //in case we have one category item without subcategories, rare but possible
    ...
    if ($drawSelectTag)
            $html[] = '<select class="'.$selectClass.'" '.$selectParams.'>';

    if ($root_id == 0)
            $html[] = '<option value="0" '.($selected == 0 ? 'selected="selected" class="'.$selectedClass.'"'
                : '').'>'.TEXT_TOP.'</option>';
    else {

        if (isset($items[$root_id]))
                $html[] = '<option value="'.$items[$root_id]['id'].'" '.($items[$root_id]['id']
                == $selected ? 'selected="selected" class="'.$selectedClass.'"' : '').'>'.stripslashes($items[$root_id]['name']).'</option>';
    }
    ...
    while ($loop && ( ( $option = each($children[$parent]) ) || ( $parent > $root_id ) )) {
        ...
        if ($option === false) {
            ...
            $html[] = '<option value="'.$option['value']['id'].'" '.($option['value']['id']
                == $selected ? 'selected="selected" class="'.$selectedClass.'"' : '').'>'.str_repeat($spacing,
                    ( count($parent_stack) + 1 ) * 2 - 1).stripslashes($option['value']['name']).'</option>';
...
function tep_aas_get_category_ids($root_id = '0')
{
    ...
    $items                               = array();
    while ($categories                          = tep_db_fetch_array($categories_query))
        $items[$categories['categories_id']] = array('parent_id' => $categories['parent_id'],
...
function tep_get_category_ids($parent_id = '0', $exclude = '',
                              $category_tree_array = '')
{
    global $languages_id;

    if (!is_array($category_tree_array)) $category_tree_array   = array();
    if ((sizeof($category_tree_array) < 1) && ($exclude != $parent_id))
            $category_tree_array[] = (int) $parent_id; //array('id' => $parent_id);
    ...
    while ($categories = tep_db_fetch_array($categories_query)) {
        if ($exclude != $categories['categories_id'])
                $category_tree_array[] = (int) $categories['categories_id']; //array('id' => (int)$categories['categories_id']);
        $category_tree_array   = tep_get_category_ids($categories['categories_id'],
...
function tep_get_products_parent_categories($products_id = 0)
{
    ...
    if ((int) $products_id != 0) {
        ...
        while ($categories = tep_db_fetch_array($categories_query))
            $parents[]  = tep_get_category_parent_by_category($categories['categories_id']);

        return $parents;
    }
}

function tep_get_category_parent_by_category($categories_id = 0,
                                             $category_tree_array = '')
{
    ...
    while ($categories = tep_db_fetch_array($categories_query)) {
        ...
        if ($categories['parent_id'] != '0')
                $category_tree_array = tep_get_category_parent_by_category($categories['parent_id'],
                $category_tree_array);
    }
...
function tep_get_category_parents($categories_id = 0, $category_tree_array = '')
{
    ...
    while ($categories = tep_db_fetch_array($categories_query)) {
        ...
        if ($categories['parent_id'] != '0')
                $category_tree_array = tep_get_category_parents($categories['parent_id'],
                $category_tree_array);
    }
...
function tep_get_price_with_tax($products_price, $products_tax,
                                $default_currency,
                                $calculate_currency_value = true)
{

    $price = tep_round(tep_add_tax($products_price, $products_tax, true),
        $default_currency['decimal_places']);

    if ($calculate_currency_value) {
        $format_string = number_format($price * $default_currency['value'],
            $default_currency['decimal_places'],
            $default_currency['decimal_point'],
            $default_currency['thousands_point']);
    } else {
        $format_string = number_format($price,
            $default_currency['decimal_places'],
            $default_currency['decimal_point'],
            $default_currency['thousands_point']);
    }
...
function tep_aas_link($section = 'admin', $page = '', $parameters = '',
    ...
    foreach ($ffs as $ff) {
        if ($ff != '.' && $ff != '..' && is_dir($dir.'/'.$ff)) {
            ...
            if (!tep_is_writable($dir.'/'.$ff))
                    echo '<li><label style="color:lightGray"><input disabled="disabled" type="radio" name="image_folder" value="'.$aas_cpath.'">'.substr($aas_cpath,
                    strlen(DIR_FS_CATALOG)).'&nbsp;<span class="no-writable">'.AAS_DIALOG_TEXT_NO_WRITABLE.'</span></label>';
            else {
                echo '<li><label><input type="radio" name="image_folder" value="'.$aas_cpath.'">'.substr($aas_cpath,
                    strlen(DIR_FS_CATALOG)).'</label>';
            }
...
function listFolders($dir, $name = 'image_folder', $class = '', $echo = true,
                     $defaultPath = '.')
{
    global $aas_cpath;
    if (!$echo) global $html;
    ...
    if ($echo) {
        ...
        foreach ($ffs as $ff) {
            if ($ff != '.' && $ff != '..' && is_dir($dir.'/'.$ff)) {
                $aas_cpath         = $dir.'/'.$ff;
                $aas_cpath_sub_img = substr($aas_cpath,
                    strlen(DIR_FS_CATALOG_IMAGES));
                ...
                if (!tep_is_writable($dir.'/'.$ff))
                        ...
                        echo '<li><label><input class="'.$class.'" '.($defaultPath
                    == $aas_cpath_sub_img ? 'checked="checked"' : '').' type="radio" name="'.$name.'" value="'.$aas_cpath_sub_img.'">'.$aas_cpath_sub_cat.'</label>';
        ...
        foreach ($ffs as $ff) {
            if ($ff != '.' && $ff != '..' && is_dir($dir.'/'.$ff)) {
                ...
                if (!tep_is_writable($dir.'/'.$ff))
                        ...
                        $html              .= '<li><label><input class="'.$class.'" '.($defaultPath
                        == $aas_cpath_sub_img ? 'checked="checked"' : '').' type="radio" name="'.$name.'" value="'.$aas_cpath_sub_img.'">'.$aas_cpath_sub_cat.'</label>';
...
function tep_formatSeconds($secs,
                           $display = array('year' => 1, 'month' => 1, 'week' => 1,
        'day' => 1, 'hour' => 1, 'min' => 1, 'sec' => 1))
{

    if (!$secs = (int) $secs) return '0 secs';

    $units = array('year' => 31536000, 'month' => 2628000, 'week' => 604800, 'day' => 86400,
