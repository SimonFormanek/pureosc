...
for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
    ...
    if ($languages[$i]['directory'] == $language)
            $languages_selected = $languages[$i];
}

//create session cache array so to store various data
if (!isset($_SESSION['admin']['AAS']['cache']))
        $_SESSION['admin']['AAS']['cache'] = array();

//CHECK TO SEE IF table aas_settings IS INSTALLED IN DB
//unset($_SESSION['admin']['AAS']['table_columns_exist']['aas_settings']);
if ((isset($_SESSION['admin']['AAS']['table_columns_exist']['aas_settings']) && !$_SESSION['admin']['AAS']['table_columns_exist']['aas_settings'] )
    || !isset($_SESSION['admin']['AAS']['table_columns_exist']['aas_settings'])) {

    if (tep_db_num_rows(tep_db_query("SHOW TABLES LIKE 'aas_settings'")) == 1)
            $_SESSION['admin']['AAS']['columns_exist']['aas_settings'] = true;
    else {
...
if (tep_db_num_rows($aasDcd_query) > 0) {
    ...
    while ($aasDcd_row                  = tep_db_fetch_array($aasDcd_query))
        $aasDcd[$aasDcd_row['skey']] = unserialize($aasDcd_row['value']);

    foreach ($fieldsArray as $kf => $f) {

        $fieldsArray[$kf]['visible']        = $aasDcd['default_columns_display'][$kf]['v']
            == '1' ? true : false;
        $fieldsArray[$kf]['lockVisibility'] = $aasDcd['default_columns_display'][$kf]['l']
            == '1' ? true : false;

        //overide default columns per admin

        if (isset($aasDcd['default_columns_display_per_admin'][$_SESSION['admin']['id']][$kf]['o'])
            && $aasDcd['default_columns_display_per_admin'][$_SESSION['admin']['id']][$kf]['o']
            == '1') {

            $fieldsArray[$kf]['visible']        = $aasDcd['default_columns_display_per_admin'][$_SESSION['admin']['id']][$kf]['v']
                == '1' ? true : false;
            $fieldsArray[$kf]['lockVisibility'] = $aasDcd['default_columns_display_per_admin'][$_SESSION['admin']['id']][$kf]['l']
                == '1' ? true : false;
        } else {

            if (isset($aasDcd['default_columns_display_per_admin'][$_SESSION['admin']['id']][$kf]['v'])
                && $aasDcd['default_columns_display_per_admin'][$_SESSION['admin']['id']][$kf]['v']
                == '1') $fieldsArray[$kf]['visible']        = true;
            if (isset($aasDcd['default_columns_display_per_admin'][$_SESSION['admin']['id']][$kf]['l'])
                && $aasDcd['default_columns_display_per_admin'][$_SESSION['admin']['id']][$kf]['l']
                == '1') $fieldsArray[$kf]['lockVisibility'] = true;
        }
    }
}
    ...
    while ($aasAac_row   = tep_db_fetch_array($aasAac_query)) {
        ...
        if ($aasAac_row['skey'] == 'fields_disable_action') {
            ...
            foreach ($aac_temp as $aac_temp_value) {
                ...
                if ($aac_temp_value != '')
                        $aasAac[$aasAac_row['type']][$aasAac_row['skey']][$aac_temp_value]
                        = true;
...
if (isset($aasAac['default'])) {
    foreach ($aasAac['default'] as $aasAacDefaultKey => $aasAacDefaultValue) {
        ...
        if (isset($aasAacDefaultValue[$_SESSION['admin']['id']]))
                $defaults[$aasAacDefaultKey] = false;
    }
}
...
if (isset($_POST['radio'])) {
    if ($_POST['radio'] == '1' || $_POST['radio'] == '0' || $_POST['radio'] == '2') {
        ...
        $_SESSION['admin']['AAS']['show_products_by_status'] = $show_products_by_status;
            ...
            $show_products_by_status = (isset($_SESSION['admin']['AAS']['show_products_by_status'])
                ? $_SESSION['admin']['AAS']['show_products_by_status'] : $defaults['displayByStatus']);
} else
        $show_products_by_status = (isset($_SESSION['admin']['AAS']['show_products_by_status'])
            ? $_SESSION['admin']['AAS']['show_products_by_status'] : $defaults['displayByStatus']);
...
if (isset($_POST['entriesPerPage'])) {
    if (in_array($_POST['entriesPerPage'], $perPageArray)) {
        ...
        $_SESSION['admin']['AAS']['entriesPerPage'] = $entriesPerPage;
            ...
            $entriesPerPage = (isset($_SESSION['admin']['AAS']['entriesPerPage'])
                ? $_SESSION['admin']['AAS']['entriesPerPage'] : $defaults['entriesPerPage']);
} else
        $entriesPerPage = (isset($_SESSION['admin']['AAS']['entriesPerPage']) ? $_SESSION['admin']['AAS']['entriesPerPage']
            : $defaults['entriesPerPage']);

$show_success_alert_messages = (isset($_SESSION['admin']['AAS']['show_success_alert_messages'])
        ? $_SESSION['admin']['AAS']['show_success_alert_messages'] : $defaults['displaySuccessAlertMessages']);
$show_error_alert_messages   = (isset($_SESSION['admin']['AAS']['show_error_alert_messages'])
        ? $_SESSION['admin']['AAS']['show_error_alert_messages'] : $defaults['displayErrorAlertMessages']);

if (isset($_POST['orderBy'])) {
    $orderBy                             = $_POST['orderBy'];
    $_SESSION['admin']['AAS']['orderBy'] = $orderBy;
} else
        $orderBy = (isset($_SESSION['admin']['AAS']['orderBy']) && $_SESSION['admin']['AAS']['orderBy']
        != 'p.products_sort_order' ? $_SESSION['admin']['AAS']['orderBy'] : $defaults['orderBy']);

if (isset($_POST['ascDesc'])) {
    $ascDesc                             = $_POST['ascDesc'];
    $_SESSION['admin']['AAS']['ascDesc'] = $ascDesc;
} else
        $ascDesc = (isset($_SESSION['admin']['AAS']['ascDesc']) ? $_SESSION['admin']['AAS']['ascDesc']
            : $defaults['ascDesc']);
...
if (isset($_POST['displayCategories'])) {
    ...
    $_SESSION['admin']['AAS']['displayCategories'] = $displayCategories;
        ...
        $displayCategories = (isset($_SESSION['admin']['AAS']['displayCategories'])
            ? (int) $_SESSION['admin']['AAS']['displayCategories'] : $defaults['displayCategories']);
...
if (isset($_POST['check'])) {
    ...
    if (isset($_POST['columncheck']) && isset($_POST['columncheckAll'])) {
        ...
        foreach ($fieldsArray as $kf => $fa) {
            ...
            if (isset($_POST['columncheck'][$kf])) {
                ...
                $columns_bool[$kf]           = (isset($fieldsArray[$kf]['visible']) )
                        ? true : false;
            } else {
    ...
    if (isset($_SESSION['admin']['AAS']['tbl_fields'])) {
        ...
        foreach ($_SESSION['admin']['AAS']['tbl_fields'] as $key => $val)
            $fieldsArray[$key]['visible'] = $val;
    }
}
...
if ($fieldsArray['sort_order']['visible']) {
    ...
    if (isset($_SESSION['admin']['AAS']['orderByProductsSortOrder']) && $_SESSION['admin']['AAS']['orderByProductsSortOrder']) {
        ...
        if (tep_db_num_rows($result_sort_order)) {
            ...
            if (isset($_SESSION['admin']['AAS']['orderByProductsSortOrder']))
                    unset($_SESSION['admin']['AAS']['orderByProductsSortOrder']);
        }
    }
} else {
...
if (isset($_POST['ccheck'])) {
    if (isset($_POST['countcheck'])) {
        ...
        foreach ($counts_array as $counts) {
            ...
            if (in_array($counts, $_POST['countcheck'], true)) {
                ...
                switch ($counts) {
                    case 'count_products': $bool_count_products      = true;
                        break;
                    case 'count_subcategories': $bool_count_subcategories = true;
                        break;
                }
            } else {

                switch ($counts) {
                    case 'count_products': $bool_count_products      = false;
                        break;
                    case 'count_subcategories': $bool_count_subcategories = false;
                        break;
                }
            }
        }
    ...
    $bool_count_products      = isset($_SESSION['admin']['AAS']['count_products'])
            ? $_SESSION['admin']['AAS']['count_products'] : $defaults['countProducts'];
    $bool_count_subcategories = isset($_SESSION['admin']['AAS']['count_subcategories'])
            ? $_SESSION['admin']['AAS']['count_subcategories'] : $defaults['countSubcategories'];
}
...
if (isset($_SESSION['admin']['AAS']['sorted_fields_array'])) {
    ...
    $jexp            = explode(',',
        $_SESSION['admin']['AAS']['sorted_fields_array']);
    $tempFieldsArray = array();
    foreach ($jexp as $sfa) {
        $tempFieldsArray[$sfa] = $fieldsArray[$sfa];
    }
    $fieldsArray = $tempFieldsArray;
} elseif (isset($aasAac['columns_sorted'][$_SESSION['admin']['id']])) {

    $jexp            = explode(',',
        $aasAac['columns_sorted'][$_SESSION['admin']['id']]);
    $tempFieldsArray = array();
    foreach ($jexp as $sfa) {
        $tempFieldsArray[$sfa] = $fieldsArray[$sfa];
    }
...
if ($displayCategories) {
    ...
    $catsOrderBy = isset($fieldsArray['sort_order']) && $fieldsArray['sort_order']['visible']
            ? 'c.sort_order' : 'c.sort_order, cd.categories_name';
    ...
    if (!isset($_SESSION['admin']['AAS']['cache_manufacturers_array'])) {
        ...
        if (tep_db_num_rows($manufacturers_query) > 0) {
            ...
            while ($manufacturers_row                                     = tep_db_fetch_array($manufacturers_query))
                $manufacturers_array[]                                 = array('id' => $manufacturers_row['manufacturers_id'],
                    'name' => $manufacturers_row['manufacturers_name']);
    ...
    if (!isset($_SESSION['admin']['AAS']['cache_tax_class_array'])) {
        ...
        if (tep_db_num_rows($tax_class_query) > 0) {
            ...
            while ($tax_class_row                                     = tep_db_fetch_array($tax_class_query))
                $tax_class_array[]                                 = array('id' => $tax_class_row['tax_class_id'],
                    'title' => $tax_class_row['tax_class_title'], 'tax_rate' => tep_display_tax_value($tax_class_row['tax_rate']));
...
$cPath = isset($_GET['cPath']) && is_numeric($_GET['cPath']) ? (int) $_GET['cPath']
        : 0;
...
if (isset($_GET['search']) && tep_not_null($_GET['search'])) {
    ...
    if (isset($_GET['searchOnField']))
            $cPathString .= '&searchOnField='.$_GET['searchOnField'];
...
if (intval($currentPage) < 1) {
    header('Location: aas.php?'.$cPathString.'&page=1');
    die;
}
...
if ($defaults['enableTempProductsList']) {
    ...
    if (isset($_SESSION['admin']['AAS']['parkedProducts']) && is_array($_SESSION['admin']['AAS']['parkedProducts'])
        && count($_SESSION['admin']['AAS']['parkedProducts']) > 0) {

        $parked_products_query = tep_db_query('SELECT p2c.categories_id, p.*, pd.* '.implode(' ',
                $selectQuery).' 
		    FROM  '.implode(' ', $fromQuery).' '.TABLE_PRODUCTS.' p, '.TABLE_PRODUCTS_DESCRIPTION.' pd'.$tblCategories.' 
		    WHERE '.implode(' ', $whereQuery).' '.($show_products_by_status == 2 ? ''
                : "p.products_status='".(int) $show_products_by_status."' AND").' p.products_id = pd.products_id AND p.products_id = p2c.products_id and pd.language_id = '.(int) $languages_id.' '.$searchSql.' AND p.products_id IN ('.implode(',',
                array_keys($_SESSION['admin']['AAS']['parkedProducts'])).')
...
//GET PRODUCTS QUERY
$products_query = tep_db_query('SELECT SQL_CALC_FOUND_ROWS p2c.categories_id, p.*, pd.* '.implode(' ',
        $selectQuery).' 
    FROM  '.implode(' ', $fromQuery).' '.TABLE_PRODUCTS.' p, '.TABLE_PRODUCTS_DESCRIPTION.' pd'.$tblCategories.' 
...
if ($currentPage > $totalPages && $totalPages != 0) {
    header('Location: '.FILENAME_AAS.'?'.$cPathString.'&page='.( $totalPages - 1
        <= 0 ? 1 : $totalPages - 1 ));
    die;
}

if (isset($_GET['search']) && !tep_not_null($_GET['search'])) {
    header('Location: '.(isset($_SESSION['admin']['AAS']['preSearchUrl']) ? $_SESSION['admin']['AAS']['preSearchUrl']
                : 'aas.php?'.$cPathString ));
    die;
}
...
if (isset($fieldsArray['special']) && $fieldsArray['special']['visible']) {
    ...
    if (tep_db_num_rows($specials_query) > 0) {
        ...
        while ($specials = tep_db_fetch_array($specials_query)) {
            ...
            $specials_products[$specials['products_id']] = array('specials_id' => $specials['specials_id'],
                'status' => $specials['status'], 'old_price' => $specials['products_price'],
                'new_price' => $specials['specials_new_products_price'], 'expires_date' => $specials['expires_date']);
        }
    }
}
...
if ($fieldsArray['products_order_status']['visible']) {
    ...
    while ($osr                                           = tep_db_fetch_array($os_query))
        $orders_status_array[$osr['orders_status_id']] = $osr['orders_status_name'];
    ...
    while ($pqr                                                                     = tep_db_fetch_array($pq_query))
        $products_quantity_by_orders[$pqr['products_id']][$pqr['orders_status']]
            = $pqr['SUM(op.products_quantity)'];
}

//store fetched products data for later use
$products_array   = array();
$productsIds      = array();
while ($products_fetched = tep_db_fetch_array($products_query)) {
    $products_array[] = $products_fetched;
    $productsIds[]    = $products_fetched['products_id'];
}
...
if (isset($fieldsArray['attributes']) && $fieldsArray['attributes']['visible']) {
    ...
    if (count($productsIds) > 0) {
        ...
        $products_attributes_query                                    = tep_db_query("SELECT products_id, COUNT(products_attributes_id) FROM ".TABLE_PRODUCTS_ATTRIBUTES." WHERE products_id IN (".implode(',',
                $productsIds).") GROUP BY products_id ");
        $products_attributes                                          = array();
        while ($products_attributes_row                                      = tep_db_fetch_array($products_attributes_query))
            $products_attributes[$products_attributes_row['products_id']] = $products_attributes_row['COUNT(products_attributes_id)'];
...
if ($ptc_columns > 0) {
    ...
    foreach ($products_array as $products)
        $products_fetched_ids[] = $products['products_id'];
    ...
    if ($fieldsArray['products_linked']['visible']) {
	    ...
	    WHERE ptc.products_id IN (".implode(',', $products_fetched_ids).") AND ptc.categories_id=cd.categories_id AND cd.language_id='".(int) $languages_id."' ");
            ...
            $protoca_array                            = array();
            while ($protoca                                  = tep_db_fetch_array($protoca_query))
                $protoca_array[$protoca['products_id']][] = array('cid' => $protoca['categories_id'],
                    'cname' => $protoca['categories_name'], 'linked' => $protoca['linked']);
        }
    }
    ...
    if (count($products_fetched_ids) > 0) {
	  ...
	  WHERE ptc.products_id IN (".implode(',', $products_fetched_ids).") AND ptc.categories_id='".$categoryId."' AND ptc.linked='1' ");
        $ptc_array                        = array();
        while ($ptc                              = tep_db_fetch_array($ptc_query))
            $ptc_array[$ptc['products_id']][] = array('cid' => $ptc['categories_id']);
    }
} else {
...
if (isset($_POST['select-export-type']) && tep_not_null($_POST['select-export-type'])
    && isset($_POST['select-export-delimeter']) && tep_not_null($_POST['select-export-delimeter']))
        include 'ext/aas/plugins/export/index.php';

//disable sorting if we displaying one or less entries
if ($entries > 1 || $categories_entries > 1)
        $enableColumnSorting = (isset($_SESSION['admin']['AAS']['enable_column_sorting'])
            ? $_SESSION['admin']['AAS']['enable_column_sorting'] : $defaults['tableSorting']);
...
if (isset($_SESSION['admin']['AAS']['modules'])) {
    ...
    foreach ($modules as $km => $vm) {
        ...
        if ($vm['aac']) {
            ...
            if (file_exists('ext/aas_modules/'.$km.'/languages/'.$language.'.php'))
                    require 'ext/aas_modules/'.$km.'/languages/'.$language.'.php';
            else require 'ext/aas_modules/'.$km.'/languages/english.php';
        }
    }
                ...
                if (file_exists($dir->path.'/'.$file.'/loader.php')) {
                    ...
                    if ($extTmp['enable']) {
                        ...
                        if (isset($aasAac['modules'][$file][$_SESSION['admin']['id']])) {
                            ...
                            //load language file
                            if (file_exists('ext/aas_modules/'.$file.'/languages/'.$language.'.php'))
                                    require 'ext/aas_modules/'.$file.'/languages/'.$language.'.php';
                            else
                                    require 'ext/aas_modules/'.$file.'/languages/english.php';
