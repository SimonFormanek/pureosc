...
while ($modules       = tep_db_fetch_array($modules_query)) {
    ...
    $modules_list_array[] = array('id' => $modules['module'],
        'text' => (is_object(${$modules['module']}) ? ${$modules['module']}->title
            : $modules['module']));
}
...
if (tep_not_null($action)) {
    switch ($action) {
        case 'expire':
            ...
            $messageStack->add_session(sprintf(SUCCESS_EXPIRED_ENTRIES,
                    $expired_entries), 'success');
                ...
                <tr>
                    ...
                    <td align="right"><table border="0" width="100%" cellspacing="0" cellpadding="0">
                            ...
                            <tr>
                                <td class="smallText" align="right">
                                    <?php
                                    echo tep_draw_form('filter',
                                        FILENAME_ACTION_RECORDER, '', 'get');
                                    echo tep_draw_pull_down_menu('module',
                                        $modules_list_array, null,
                    ...
                    <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                            ...
                            while ($actions           = tep_db_fetch_array($actions_query)) {
                                ...
                                if ((!isset($_GET['aID']) || (isset($_GET['aID'])
                                    ...
                                    $aInfo_array = array_merge($actions,
                                        $actions_extra,
                                        array('module' => $module_title));
                                ...
                                if ((isset($aInfo) && is_object($aInfo)) && ($actions['id']
                                    ...
                                    echo '                  <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\''.tep_href_link(FILENAME_ACTION_RECORDER,
                                        tep_get_all_get_params(array('aID')).'aID='.$actions['id']).'\'">'."\n";
                                }
                                ?>
