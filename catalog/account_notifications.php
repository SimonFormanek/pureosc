...
if (isset($_POST['action']) && ($_POST['action'] == 'process') && isset($_POST['formid'])
    ...
    if ($product_global != $global['global_product_notifications']) {
        $product_global = (($global['global_product_notifications'] == '1') ? '0'
                : '1');
        ...
        if (sizeof($products_parsed) > 0) {
            $check_query = tep_db_query("select count(*) as total from ".TABLE_PRODUCTS_NOTIFICATIONS." where customers_id = '".(int) $customer_id."' and products_id not in (".implode(',',
                    $products_parsed).")");
            $check       = tep_db_fetch_array($check_query);

            if ($check['total'] > 0) {
                tep_db_query("delete from ".TABLE_PRODUCTS_NOTIFICATIONS." where customers_id = '".(int) $customer_id."' and products_id not in (".implode(',',
                        $products_parsed).")");
            }
        }
    } else {
    ...
    $messageStack->add_session('account', SUCCESS_NOTIFICATIONS_UPDATED,
        'success');
        ...
        <div class="form-group">
            ...
            <div class="col-sm-8">
                <div class="checkbox">
                    <label>
                        <?php
                        echo tep_draw_checkbox_field('product_global', '1',
                        ...
                        <div class="col-sm-8">
                            ...
                            while ($products       = tep_db_fetch_array($products_query)) {
                                ...
                                <div class="checkbox">
                                    <label>
                                        <?php
                                        echo tep_draw_checkbox_field('products['.$counter.']',
