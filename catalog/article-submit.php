...
if (isset($_POST['action']) && $_POST['action'] == 'process') {
    ...
    $article['text']      = preg_replace('/$/', '</p>',
        preg_replace('/^/', '<p>',
            str_replace('NEWLINE', '</p><p>',
                tep_db_prepare_input(str_replace("\n", "NEWLINE",
                        $_POST['article_long_desc'])))));
//orig:      $article['text'] = tep_db_prepare_input($_POST['article_long_desc']);
    ...
    if (isset($_FILES['uploadedfile']) && tep_not_null($_FILES['uploadedfile']['tmp_name'])) {
        ...
        $remove_these = array(' ', '`', '"', '\'', '\\', '/');
        $imgName      = str_replace($remove_these, '',
    ...
    if (!$error && isset($_FILES['uploadedfile']) && tep_not_null($_FILES['uploadedfile']['tmp_name'])) {
        ...
        //Sanitize the filename (See note below)
        $remove_these = array(' ', '`', '"', '\'', '\\', '/');
        $imgAuthor    = str_replace($remove_these, '',
            $_FILES['authors_image']['name']);
        ...
        $imgTmpName   = str_replace("'", '',
            $_FILES['authors_image']['tmp_name']); //"
    ...
    if (!$error) {
        ...
        $body            = sprintf(ARTICLES_EMAIL_TEXT_BODY,
            $customer_first_name, $article['name'], $topicName['topics_name']);
        tep_mail('', STORE_OWNER_EMAIL_ADDRESS, $subject, $body, STORE_OWNER,
