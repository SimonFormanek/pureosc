...
//moved to begin:        $customer_id = tep_db_insert_id();

    $userLog->setCustomerID($customer_id);
    $userLog->logMySQLEvent(TABLE_CUSTOMERS, 'customers_password', $customer_id, 'created');
    $userLog->logMySQLEvent(TABLE_CUSTOMERS, 'customers_firstname', $customer_id, 'created');
    $userLog->logMySQLEvent(TABLE_CUSTOMERS, 'customers_lastname', $customer_id, 'created');
    $userLog->logMySQLEvent(TABLE_CUSTOMERS, 'customers_telephone', $customer_id, 'created');
    $userLog->logMySQLEvent(TABLE_CUSTOMERS, 'customers_newsletter', $customer_id, $newsletter);
...
//            'stat' => $country,
      ...
      if ($kontakter->lastResponseCode == 201) {
        $userLog->logFlexiBeeEvent($kontakter, ['jmeno', 'prijmeni', 'email']);
      }
    }
...
// build the message content
    ...
    if ($newsletter !== false) {
      $gdprNewsLetterConsentReq = new \PureOSC\NewsletterConsentMailer($customer_id, $email_address, $name);
...
<?php
echo tep_draw_form('create_account', tep_href_link(FILENAME_CREATE_ACCOUNT, '', 'SSL'), 'post', 'class="form-horizontal"', true) . tep_draw_hidden_field('action', 'process');
?>
...
<div class="contentContainer">
          ...
          <div class="form-group has-feedback">
              ...
              <div class="col-sm-9">
                  <label class="radio-inline">
                      <?php
                      echo tep_draw_radio_field('gender', 'm', NULL, 'required aria-required="true" aria-describedby="atGender"') . ' ' . MALE;
                      ?>
                  </label>
                  <label class="radio-inline">
                      <?php
    ...
    <div class="buttonSet">
        <div class="text-right"><?php
            echo tep_draw_button(IMAGE_BUTTON_CONTINUE, 'fa fa-user', null, 'primary', null, 'btn-success');
            ?></div>
    </div>
