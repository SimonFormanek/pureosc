...
if (isset($_POST['action']) && ($_POST['action'] == 'process') && isset($_POST['formid']) && ($_POST['formid'] == $sessiontoken)) {
    ...
    if (cfg('ACCOUNT_GENDER') == 'true') {
        if (($gender != 'm') && ($gender != 'f')) {
            ...
            $messageStack->add('create_account', cfg('ENTRY_GENDER_ERROR'));
        }
    }

    if (strlen($firstname) < cfg('ENTRY_FIRST_NAME_MIN_LENGTH')) {
        $error = true;

        $messageStack->add('create_account', cfg('ENTRY_FIRST_NAME_ERROR'));
    }

    if (strlen($lastname) < cfg('ENTRY_LAST_NAME_MIN_LENGTH')) {
        $error = true;

        $messageStack->add('create_account', cfg('ENTRY_LAST_NAME_ERROR'));
    }
    ...
    if (strlen($email_address) < cfg('ENTRY_EMAIL_ADDRESS_MIN_LENGTH')) {
        $error = true;

        $messageStack->add('create_account', __('ENTRY_EMAIL_ADDRESS_ERROR', sprintf(_('Your E-Mail Address must contain a minimum of %s characters.'), cfg('ENTRY_EMAIL_ADDRESS_MIN_LENGTH'))));
    ...
    if (strlen($street_address) < cfg('ENTRY_STREET_ADDRESS_MIN_LENGTH')) {
        ...
        $messageStack->add('create_account', cfg('ENTRY_STREET_ADDRESS_ERROR'));
    }

    if (strlen($postcode) < cfg('ENTRY_POSTCODE_MIN_LENGTH')) {
        $error = true;
    ...
    if (cfg('ACCOUNT_STATE') == 'true') {
        ...
        if ($entry_state_has_zones === true) {
            ...
            if (strlen($state) < cfg('ENTRY_STATE_MIN_LENGTH')) {
                ...
                $messageStack->add('create_account', cfg('ENTRY_STATE_ERROR'), sprintf(_('Your State must contain a minimum of %d characters'), cfg('ENTRY_STATE_MIN_LENGTH')));
            }
        }
    }
    ...
    if (strlen($password) < cfg('ENTRY_PASSWORD_MIN_LENGTH')) {
        ...
        $messageStack->add('create_account', cfg('ENTRY_PASSWORD_ERROR', sprintf(_('Your Password must contain a minimum of %d characters.'), cfg('ENTRY_PASSWORD_MIN_LENGTH'))));
    } elseif ($password != $confirmation) {
        $error = true;

        $messageStack->add('create_account', cfg('ENTRY_PASSWORD_ERROR_NOT_MATCHING'), _('The Password Confirmation must match your Password.'));
    }
