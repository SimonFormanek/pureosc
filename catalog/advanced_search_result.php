...
if ((isset($_GET['keywords']) && empty($_GET['keywords'])) &&
    ...
    if (($date_check_error === false) && tep_not_null($dfrom) && tep_not_null($dto)) {
        if (mktime(0, 0, 0, $dfrom_array[1], $dfrom_array[2], $dfrom_array[0]) > mktime(0,
            ...
            $messageStack->add_session('search',
    ...
    if (($price_check_error === false) && is_float($pfrom) && is_float($pto)) {
        if ($pfrom >= $pto) {
            ...
            $messageStack->add_session('search',
                ERROR_PRICE_TO_LESS_THAN_PRICE_FROM);
        }
    }
...
$breadcrumb->add(NAVBAR_TITLE_2,
    tep_href_link(FILENAME_ADVANCED_SEARCH_RESULT, tep_get_all_get_params(),
        'NONSSL', true, false));
...
// create column list
    ...
    if ((DISPLAY_PRICE_WITH_TAX == 'true') && (tep_not_null($pfrom) || tep_not_null($pto))) {
        ...
        $from_str .= " left join ".TABLE_TAX_RATES." tr on p.products_tax_class_id = tr.tax_class_id left join ".constant('TABLE_ZONES_TO_GEO_ZONES')." gz on tr.tax_zone_id = gz.geo_zone_id and (gz.zone_country_id is null or gz.zone_country_id = '0' or gz.zone_country_id = '".(int) $customer_country_id."') and (gz.zone_id is null or gz.zone_id = '0' or gz.zone_id = '".(int) $customer_zone_id."')";
    }
        ...
        for ($i = 0, $n = sizeof($search_keywords); $i < $n; $i++) {
            switch ($search_keywords[$i]) {
                ...
                default:
                     ...
                     */
                    $where_str .= "(";
                    if ((defined('MODULE_HEADER_TAGS_PRODUCT_META_KEYWORDS_STATUS'))
                        && (MODULE_HEADER_TAGS_PRODUCT_META_KEYWORDS_STATUS != 'Meta')) {
                        $where_str .= "pd.products_seo_keywords LIKE '%".tep_db_input($keyword)."%' OR ";
                    }
                    $where_str .= "pd.products_name like '%".tep_db_input($keyword)."%' or p.products_model like '%".tep_db_input($keyword)."%' or m.manufacturers_name like '%".tep_db_input($keyword)."%'";
                    /*                     * * EOF alteration for SEO Header Tags RELOADED ** */
                    if (isset($_GET['search_in_description']) && ($_GET['search_in_description']
                        == '1'))
    ...
    if ((!isset($_GET['sort'])) || (!preg_match('/^[1-8][ad]$/', $_GET['sort']))
        ...
        switch ($column_list[$sort_col - 1]) {
            case 'PRODUCT_LIST_MODEL':
                $order_str = " order by p.products_model ".($sort_order == 'd' ? 'desc'
                        : '').", pd.products_name";
                break;
            case 'PRODUCT_LIST_NAME':
                ...
                break;
            case 'PRODUCT_LIST_MANUFACTURER':
                $order_str = " order by m.manufacturers_name ".($sort_order == 'd'
                        ? 'desc' : '').", pd.products_name";
                break;
            case 'PRODUCT_LIST_QUANTITY':
                $order_str = " order by p.products_quantity ".($sort_order == 'd'
                        ? 'desc' : '').", pd.products_name";
                break;
            case 'PRODUCT_LIST_IMAGE':
                $order_str = " order by pd.products_name";
                break;
            case 'PRODUCT_LIST_WEIGHT':
                $order_str = " order by p.products_weight ".($sort_order == 'd' ? 'desc'
                        : '').", pd.products_name";
                break;
            case 'PRODUCT_LIST_PRICE':
                $order_str = " order by final_price ".($sort_order == 'd' ? 'desc'
                        : '').", pd.products_name";
                break;
        }
    }
