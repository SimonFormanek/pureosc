...
if (isset($_POST['action']) && ($_POST['action'] == 'process') && isset($_POST['formid']) && ($_POST['formid'] == $sessiontoken)) {
  ...
  if ($error == false) {
    $sql_data_array = array('customers_firstname' => encrypt($firstname, $customer_id),
    ...
    $userLog->logMySQLEvent(TABLE_CUSTOMERS, 'customers_password', $customer_id, 'updated');
    $userLog->logMySQLEvent(TABLE_CUSTOMERS, 'customers_firstname', $customer_id, 'updated');
    $userLog->logMySQLEvent(TABLE_CUSTOMERS, 'customers_lastname', $customer_id, 'updated');
    $userLog->logMySQLEvent(TABLE_CUSTOMERS, 'customers_telephone', $customer_id, 'updated');
    $userLog->logMySQLEvent(TABLE_CUSTOMERS, 'customers_fax', $customer_id, 'updated');
...
//            'stat' => $country,
      ...
      if ($adresar->lastResponseCode == 201) {
        $userLog->logFlexiBeeEvent($adresar, ['nazev', 'email', 'ic', 'dic']);
      }
    }
...
<?php
echo tep_draw_form('account_edit', tep_href_link(FILENAME_ACCOUNT_EDIT, '', 'SSL'), 'post', 'class="form-horizontal"', true) . tep_draw_hidden_field('action', 'process');
?>
...
<div class="contentContainer">
    ...
    <div class="form-group has-feedback">
        ...
        <div class="col-sm-9">
            <?php
            echo tep_draw_input_field('firstname', decrypt($account['customers_firstname'], $customer_password, $customer_id), 'required aria-required="true" id="inputFirstName" placeholder="' . ENTRY_FIRST_NAME . '"');
            ?>
            <?php echo _('Required'); ?>
        </div>
    </div>
    <div class="form-group has-feedback">
        ...
        <div class="col-sm-9">
            <?php
            echo tep_draw_input_field('lastname', decrypt($account['customers_lastname'], $customer_password, $customer_id), 'required aria-required="true" id="inputLastName" placeholder="' . ENTRY_LAST_NAME . '"');
            ?>
            <?php echo _('Required'); ?>
        </div>
    </div>
    ...
    if (ACCOUNT_DOB == 'true') {
      ...
      <div class="form-group has-feedback">
          ...
          <div class="col-sm-9">
              <?php
              echo tep_draw_input_field('dob', tep_date_short($account['customers_dob']), 'required aria-required="true" aria-describedby="atDob" id="dob" placeholder="' . ENTRY_DATE_OF_BIRTH_TEXT . '"');
              ?>
    ...
    <div class="form-group has-feedback">
        ...
        <div class="col-sm-9">
            <?php
            echo tep_draw_input_field('email_address', $account['customers_email_address'], 'required aria-required="true" id="inputEmail" placeholder="' . ENTRY_EMAIL_ADDRESS . '"', 'email');
            ?>
            <?php echo _('Required'); ?>
        </div>
    </div>
    <div class="form-group has-feedback">
        ...
        <div class="col-sm-9">
            <?php
            echo tep_draw_input_field('telephone', decrypt($account['customers_telephone'], $customer_password, $customer_id), 'required aria-required="true" id="inputTelephone" placeholder="' . ENTRY_TELEPHONE_NUMBER . '"', 'tel');
            ?>
            <?php echo _('Required'); ?>
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-sm-9">
            <?php
            echo tep_draw_input_field('fax', decrypt($account['customers_fax'], $customer_password, $customer_id), 'id="inputFax" placeholder="' . ENTRY_FAX_NUMBER . '"');
            ?>
        </div>
    </div>
