...
class ht_gpublisher
{
    ...
    function execute()
    {
        ...
        $oscTemplate->addBlock('<link rel="publisher" href="'.tep_output_string(MODULE_HEADER_TAGS_GPUBLISHER_ID).'" />'."\n",
            $this->group);
    }
    ...
    function install()
    {
        ...
        tep_db_query("insert into ".TABLE_CONFIGURATION." (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('G+ Publisher Address', 'MODULE_HEADER_TAGS_GPUBLISHER_ID', '', 'Your G+ URL.', '6', '0', now())");
        tep_db_query("insert into ".TABLE_CONFIGURATION." (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_HEADER_TAGS_GPUBLISHER_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
    }
    ...
    function keys()
    {
        return array('MODULE_HEADER_TAGS_GPUBLISHER_STATUS', 'MODULE_HEADER_TAGS_GPUBLISHER_ID',
