...
class ht_google_analytics
{
  ...
  _gaq.push([\'_trackPageview\']);'."\n";
            ...
            if ((MODULE_HEADER_TAGS_GOOGLE_ANALYTICS_EC_TRACKING == 'True') && (basename($_SERVER['PHP_SELF'])
                == FILENAME_CHECKOUT_SUCCESS) && tep_session_is_registered('customer_id')) {
    ...
    \''.(isset($totals['ot_total']) ? $this->format_raw($totals['ot_total'],
                            DEFAULT_CURRENCY) : 0).'\', // total - required
    \''.(isset($totals['ot_tax']) ? $this->format_raw($totals['ot_tax'],
                            DEFAULT_CURRENCY) : 0).'\', // tax
    \''.(isset($totals['ot_shipping']) ? $this->format_raw($totals['ot_shipping'],
                            DEFAULT_CURRENCY) : 0).'\', // shipping
    ...
    function keys()
    {
        return array('MODULE_HEADER_TAGS_GOOGLE_ANALYTICS_STATUS', 'MODULE_HEADER_TAGS_GOOGLE_ANALYTICS_ID',
            'MODULE_HEADER_TAGS_GOOGLE_ANALYTICS_EC_TRACKING', 'MODULE_HEADER_TAGS_GOOGLE_ANALYTICS_JS_PLACEMENT',
