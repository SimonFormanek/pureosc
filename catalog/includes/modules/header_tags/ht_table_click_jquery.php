...
class ht_table_click_jquery
{
    ...
    function install()
    {
        ...
        tep_db_query("insert into ".TABLE_CONFIGURATION." (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Pages', 'MODULE_HEADER_TAGS_TABLE_CLICK_JQUERY_PAGES', '".implode(';',
                $this->get_default_pages())."', 'The pages to add the jQuery Scripts to.', '6', '0', 'ht_table_click_jquery_show_pages', 'ht_table_click_jquery_edit_pages(', now())");
    ...
    function keys()
    {
        return array('MODULE_HEADER_TAGS_TABLE_CLICK_JQUERY_STATUS', 'MODULE_HEADER_TAGS_TABLE_CLICK_JQUERY_PAGES',
...
function ht_table_click_jquery_edit_pages($values, $key)
{
    ...
    foreach ($files_array as $file) {
        $output .= tep_draw_checkbox_field('ht_table_click_jquery_file[]',
                $file, in_array($file, $values_array)).'&nbsp;'.tep_output_string($file).'<br />';
    }
