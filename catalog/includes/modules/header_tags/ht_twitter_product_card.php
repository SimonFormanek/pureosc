...
class ht_twitter_product_card
{
        ...
        if (($PHP_SELF == FILENAME_PRODUCT_INFO) && isset($_GET['products_id'])) {
            if ($product_check['total'] > 0) {
                ...
                if (tep_db_num_rows($product_info_query) === 1) {
                    ...
                    $data['image:src'] = tep_href_link(DIR_WS_IMAGES.$products_image,
                        '', 'NONSSL', false, false);
    ...
    function keys()
    {
        return array('MODULE_HEADER_TAGS_TWITTER_PRODUCT_CARD_STATUS', 'MODULE_HEADER_TAGS_TWITTER_PRODUCT_CARD_USER_ID',
            'MODULE_HEADER_TAGS_TWITTER_PRODUCT_CARD_SITE_ID', 'MODULE_HEADER_TAGS_TWITTER_PRODUCT_CARD_SORT_ORDER');
    }
}
