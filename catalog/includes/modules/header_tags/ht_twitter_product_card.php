...
class ht_twitter_product_card {
    ...
    public function __construct() {
        $this->ht_twitter_product_card();
    }

    function ht_twitter_product_card() {
        $this->title = _('Twitter Product Card');
        $this->description = _('Add Twitter Product Card tags to your product information pages.');
            ...
            if ($product_check['total'] > 0) {
                ...
                if (tep_db_num_rows($product_info_query) === 1) {
                    ...
                    if ($product_info['products_date_available'] > date('Y-m-d H:i:s')) {
                        $data['data2'] = _('Pre-Order');
                        $data['label2'] = tep_date_short($product_info['products_date_available']);
                    } elseif ($product_info['products_quantity'] > 0) {
                        $data['data2'] = _('In Stock');
                        $data['label2'] = _('BUY NOW');
                    } else {
                        $data['data2'] = _('Out of Stock');
                        $data['label2'] = _('CONTACT US');
                    }
