...
class ht_product_opengraph {
    ...
    public function __construct() {
        $this->ht_product_opengraph();
    }

    function ht_product_opengraph() {
        $this->title = _('Product Open Graph Meta Tags');
        $this->description = _('Add Open Graph Meta Tags to your product info pages, this is good for Facebook and for Pinterest Rich Pins.');
    ...
    function execute() {
        global $_GET, $oscTemplate, $languages_id, $currencies, $currency, $product_check;

        if (($_SERVER['PHP_SELF'] == FILENAME_PRODUCT_INFO) && isset($_GET['products_id'])) {
            if ($product_check['total'] > 0) {
                ...
                if (tep_db_num_rows($product_info_query) === 1) {
                    ...
                    $data['product:availability'] = ( $product_info['products_quantity'] > 0 ) ? _('stock') : _('oos');
