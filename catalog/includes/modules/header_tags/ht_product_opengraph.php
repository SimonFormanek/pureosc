...
class ht_product_opengraph
{
        ...
        if (($PHP_SELF == FILENAME_PRODUCT_INFO) && isset($_GET['products_id'])) {
            if ($product_check['total'] > 0) {
                ...
                if (tep_db_num_rows($product_info_query) === 1) {
                    ...
                    $product_description = substr(trim(preg_replace('/\s\s+/',
                                ' ',
                    ...
                    $data['og:image'] = tep_href_link(DIR_WS_IMAGES.$products_image,
                        '', 'NONSSL', false, false);
                    ...
                    $data['og:url'] = tep_href_link(FILENAME_PRODUCT_INFO,
                        'products_id='.$product_info['products_id'], 'NONSSL',
                        false);

                    $data['product:availability'] = ( $product_info['products_quantity']
                        > 0 ) ? MODULE_HEADER_TAGS_PRODUCT_OPENGRAPH_TEXT_IN_STOCK
                            : MODULE_HEADER_TAGS_PRODUCT_OPENGRAPH_TEXT_OUT_OF_STOCK;
    ...
    function keys()
    {
        return array('MODULE_HEADER_TAGS_PRODUCT_OPENGRAPH_STATUS', 'MODULE_HEADER_TAGS_PRODUCT_OPENGRAPH_APP_ID',
