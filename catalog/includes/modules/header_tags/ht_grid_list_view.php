...
class ht_grid_list_view
{
    ...
    function install()
    {
        ...
        tep_db_query("insert into ".TABLE_CONFIGURATION." (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Pages', 'MODULE_HEADER_TAGS_GRID_LIST_VIEW_PAGES', '".implode(';',
                $this->get_default_pages())."', 'The pages to add the Grid List JS Scripts to.', '6', '4', 'ht_grid_list_view_show_pages', 'ht_grid_list_view_edit_pages(', now())");
    ...
    function keys()
    {
        return array('MODULE_HEADER_TAGS_GRID_LIST_VIEW_STATUS', 'MODULE_HEADER_TAGS_GRID_LIST_VIEW_PAGES',
            'MODULE_HEADER_TAGS_GRID_LIST_VIEW_SORT_ORDER');
    }
...
function ht_grid_list_view_edit_pages($values, $key)
{
    ...
    foreach ($files_array as $file) {
        $output .= tep_draw_checkbox_field('ht_grid_list_view_file[]', $file,
