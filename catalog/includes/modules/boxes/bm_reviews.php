...
class bm_reviews
{
    ...
    function bm_reviews()
    {
        ...
        if (defined('MODULE_BOXES_REVIEWS_STATUS')) {
            ...
            $this->group = ((MODULE_BOXES_REVIEWS_CONTENT_PLACEMENT == 'Left Column')
    ...
    function execute()
    {
        global $languages_id, $_GET, $currencies, $oscTemplate;
...
// display random review box
            ...
            $rand_review_text = tep_break_string(tep_output_string_protected($rand_review['reviews_text']),
                15, '-<br />');

            $reviews_box_contents .= '<div class="text-center"><a href="'.tep_href_link(FILENAME_PRODUCT_REVIEWS,
                    ...
                    SMALL_IMAGE_HEIGHT).'</a></div><div><a href="'.tep_href_link(FILENAME_PRODUCT_REVIEWS,
                    'products_id='.$random_product['products_id']).'">'.$rand_review_text.'</a>...</div><div class="text-center" title="'.sprintf(MODULE_BOXES_REVIEWS_BOX_TEXT_OF_5_STARS,
                    $random_product['reviews_rating']).'">'.tep_draw_stars($random_product['reviews_rating']).'</div>';
        } elseif (isset($_GET['products_id'])) {
// display 'write a review' box
            $reviews_box_contents .= '<span class="fa fa-thumbs-up"></span> <a href="'.tep_href_link(FILENAME_PRODUCT_REVIEWS_WRITE,
                    'products_id='.$_GET['products_id']).'">'.MODULE_BOXES_REVIEWS_BOX_WRITE_REVIEW.'</a>';
        } else {
// display 'no reviews' box
    ...
    function keys()
    {
        return array('MODULE_BOXES_REVIEWS_STATUS', 'MODULE_BOXES_REVIEWS_CONTENT_PLACEMENT',
