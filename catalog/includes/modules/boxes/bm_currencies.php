...
class bm_currencies
{
    ...
    function bm_currencies()
    {
        ...
        if (defined('MODULE_BOXES_CURRENCIES_STATUS')) {
            ...
            $this->group = ((MODULE_BOXES_CURRENCIES_CONTENT_PLACEMENT == 'Left Column')
        ...
        if (substr(basename($_SERVER['PHP_SELF']), 0, 8) != 'checkout') {
            if (isset($currencies) && is_object($currencies) && (count($currencies->currencies)
                ...
                while (list($key, $value) = each($_GET)) {
                    if (is_string($value) && ($key != 'currency') && ($key != tep_session_name())
                        && ($key != 'x') && ($key != 'y')) {
                        $hidden_get_variables .= tep_draw_hidden_field($key,
                            $value);
                    }
                }
    ...
    function keys()
    {
        return array('MODULE_BOXES_CURRENCIES_STATUS', 'MODULE_BOXES_CURRENCIES_CONTENT_PLACEMENT',
