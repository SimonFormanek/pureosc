...
class bm_authors
{
    ...
    function bm_authors()
    {
        ...
        if (defined('MODULE_BOXES_AUTHORS_STATUS')) {
            ...
            $this->group = ((MODULE_BOXES_AUTHORS_CONTENT_PLACEMENT == 'Left Column')
        ...
        if ($number_of_author_rows = tep_db_num_rows($authors_query)) {
            if ($number_of_author_rows <= MAX_DISPLAY_AUTHORS_IN_A_LIST) {
                ...
                while ($authors = tep_db_fetch_array($authors_query)) {
                    $authors_name = ((strlen($authors['authors_name']) > MAX_DISPLAY_AUTHOR_NAME_LEN)
                            ? substr($authors['authors_name'], 0,
                            MAX_DISPLAY_AUTHOR_NAME_LEN).'..' : $authors['authors_name']);
                    if (isset($_GET['authors_id']) && ($_GET['authors_id'] == $authors['authors_id']))
                            $authors_name = '<strong>'.$authors_name.'</strong>';
                    $content      .= '<li><a href="'.tep_href_link(FILENAME_ARTICLES,
                            'authors_id='.$authors['authors_id']).'">'.$authors_name.'</a></li>';
                }
                $content .= '</ul>';
            } else {
                // Display a drop-down
                ...
                $content = tep_draw_form('authors',
                        tep_href_link(FILENAME_ARTICLES, '', 'NONSSL', false),
                        'get').
                    tep_draw_pull_down_menu('authors_id', $authors_array,
                        (isset($_GET['authors_id']) ? $_GET['authors_id'] : ''),
                        'onchange="this.form.submit();" size="'.MAX_AUTHORS_LIST.'" style="width: 100%"').tep_hide_session_id().
                    '</form>';
            }
        }
    ...
    function keys()
    {
        return array('MODULE_BOXES_AUTHORS_STATUS', 'MODULE_BOXES_AUTHORS_CONTENT_PLACEMENT',
