...
class bm_languages
{
    ...
    function bm_languages()
    {
        ...
        if (defined('MODULE_BOXES_LANGUAGES_STATUS')) {
            ...
            $this->group = ((MODULE_BOXES_LANGUAGES_CONTENT_PLACEMENT == 'Left Column')
    ...
    function execute()
    {
        ...
        if (substr(basename($PHP_SELF), 0, 8) != 'checkout') {
            ...
            if (count($lng->catalog_languages) > 1) {
                ...
                if (USE_WWW == 'true') $wwwstring        = 'www.';
                else $wwwstring        = '';

                $scheme = $_SERVER['REQUEST_SCHEME'].'://';
                reset($lng->catalog_languages);
                while (list($key, $value) = each($lng->catalog_languages)) {
                    if ($key != DEFAULT_LANGUAGE) $key              = $key.'.';
                    else $key              = '';
                    $languages_string .= ' <a href="'.$scheme.$wwwstring.$key.$domain.$_SERVER["REQUEST_URI"].'">'.tep_image(DIR_WS_LANGUAGES.$value['directory'].'/images/'.$value['image'],
                            $value['name'], NULL, NULL, NULL, false).'</a> ';
                }
    ...
    function keys()
    {
        return array('MODULE_BOXES_LANGUAGES_STATUS', 'MODULE_BOXES_LANGUAGES_CONTENT_PLACEMENT',
