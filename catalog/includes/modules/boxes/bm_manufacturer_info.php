...
class bm_manufacturer_info
{
    ...
    function bm_manufacturer_info()
    {
        ...
        if (defined('MODULE_BOXES_MANUFACTURER_INFO_STATUS')) {
            ...
            $this->group = ((MODULE_BOXES_MANUFACTURER_INFO_CONTENT_PLACEMENT == 'Left Column')
    ...
    function execute()
    {
        global $_GET, $languages_id, $oscTemplate;
        ...
        if (isset($_GET['products_id'])) {
            ...
            if (tep_db_num_rows($manufacturer_query)) {
                ...
                if (tep_not_null($manufacturer['manufacturers_image']))
                        $manufacturer_info_string .= '<div>'.tep_image(DIR_WS_IMAGES.$manufacturer['manufacturers_image'],
                            $manufacturer['manufacturers_name']).'</div>';
                if (tep_not_null($manufacturer['manufacturers_url']))
                        $manufacturer_info_string .= '<div class="text-center"><a href="'.tep_href_link(FILENAME_REDIRECT,
                            'action=manufacturer&manufacturers_id='.$manufacturer['manufacturers_id']).'" target="_blank">'.sprintf(MODULE_BOXES_MANUFACTURER_INFO_BOX_HOMEPAGE,
                            $manufacturer['manufacturers_name']).'</a></div>';
    ...
    function keys()
    {
        return array('MODULE_BOXES_MANUFACTURER_INFO_STATUS', 'MODULE_BOXES_MANUFACTURER_INFO_CONTENT_PLACEMENT',
