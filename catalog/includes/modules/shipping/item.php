...
// class constructor
    function item()
    {
        ...
        $this->code        = 'item';
        $this->title       = _('Per Item');
        $this->description = _('Per Item');
        $this->sort_order  = defined('MODULE_SHIPPING_ITEM_SORT_ORDER') ? constant('MODULE_SHIPPING_ITEM_SORT_ORDER')
                : '';
        $this->icon        = '';
        $this->tax_class   = defined('MODULE_SHIPPING_ITEM_TAX_CLASS') ? constant('MODULE_SHIPPING_ITEM_TAX_CLASS')
                : '';
        $this->enabled     = defined('MODULE_SHIPPING_ITEM_STATUS') && constant('MODULE_SHIPPING_ITEM_STATUS')
            == 'True';

        if (($this->enabled === true) && ((int) MODULE_SHIPPING_ITEM_ZONE > 0)) {
            $check_flag  = false;
            $check_query = tep_db_query("select zone_id from ".constant('TABLE_ZONES_TO_GEO_ZONES')." where geo_zone_id = '".MODULE_SHIPPING_ITEM_ZONE."' and zone_country_id = '".$order->delivery['country']['id']."' order by zone_id");
...
// class methods
    ...
    function keys()
    {
        return array('MODULE_SHIPPING_ITEM_STATUS', 'MODULE_SHIPPING_ITEM_COST',
            'MODULE_SHIPPING_ITEM_HANDLING', 'MODULE_SHIPPING_ITEM_TAX_CLASS', 'MODULE_SHIPPING_ITEM_ZONE',
