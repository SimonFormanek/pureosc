...
// rest of the world eof
    ...
    function install()
    {
        ...
        for ($i = 1; $i <= $this->num_zones; $i++) {
            ...
            tep_db_query("insert into ".TABLE_CONFIGURATION." (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone ".$i." Countries (Empty for all others not listed above)', 'MODULE_SHIPPING_ZONES_COUNTRIES_".$i."', '".$default_countries."', 'Comma separated list of two character ISO country codes that are part of Zone ".$i.".', '6', '0', now())");
    ...
    function keys()
    {
        $keys = array('MODULE_SHIPPING_ZONES_STATUS', 'NUM_ZONES', 'MODULE_SHIPPING_ZONES_MODE',
            'MODULE_SHIPPING_ZONES_TAX_CLASS', 'MODULE_SHIPPING_ZONES_SORT_ORDER');
