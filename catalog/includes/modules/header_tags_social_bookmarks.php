...
switch (true) {
    case (basename($_SERVER['PHP_SELF']) === FILENAME_PRODUCT_INFO):
        ...
        $URL   = urlencode(StripSID(tep_href_link(FILENAME_PRODUCT_INFO,
                    'products_id='.$product_info['products_id'], $request_type,
                    false)));
    ...
    case (!tep_not_null($TITLE) && isset($_GET['cPath'])):
        ...
        $NAME           = htmlspecialchars($category['categories_name'],
            ENT_QUOTES);
        $TITLE          = urlencode($category['categories_name']);
        $URL            = urlencode(StripSID(tep_href_link(FILENAME_DEFAULT,
                    'cPath='.$category_id, $request_type, false)));
    ...
    case (defined('FILENAME_ARTICLE_INFO') && basename($_SERVER['PHP_SELF']) === FILENAME_ARTICLE_INFO):
        ...
        $URL   = urlencode(StripSID(tep_href_link(FILENAME_ARTICLE_INFO,
                    'articles_id='.$article_info['articles_id'], $request_type,
                    false)));
        break;
    ...
    case (defined('FILENAME_INFORMATION') && basename($_SERVER['PHP_SELF']) === FILENAME_INFORMATION):
        ...
        $URL   = urlencode(StripSID(tep_href_link(FILENAME_INFORMATION,
                    'info_id='.(int) $_GET['info_id'], $request_type, false)));
        break;
    ...
    case (defined('FILENAME_PAGES') && basename($_SERVER['PHP_SELF']) === FILENAME_PAGES):
        ...
        $URL   = urlencode(StripSID(tep_href_link(FILENAME_PAGES,
                    'pages='.tep_db_prepare_input($_GET['page']), $request_type,
                    false)));
        break;
