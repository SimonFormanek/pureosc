    ...
    function update_credit_account($i)
    {
        ...
        if (preg_match('/^GIFT/', addslashes($order->products[$i]['model']))) {
            ...
            if ($this->credit_tax == 'true')
                    $gv_order_amount = $gv_order_amount * (100 + $order->products[$i]['tax'])
                    / 100;
//        $gv_order_amount += 0.001;
    ...
    function credit_selection()
    {
        ...
        if (tep_db_num_rows($gv_query)) {
            $selection_string .= '<tr>'."\n";
            $selection_string .= '  <td width="10">'.tep_draw_separator('pixel_trans.gif',
                    '10', '1').'</td>';
    ...
    function collect_posts()
    {
        ...
        if ($_POST['gv_redeem_code']) {
            ...
            if (tep_db_num_rows($gv_query) != 0) {
                ...
                if ((tep_db_num_rows($redeem_query) != 0) && ($gv_result['coupon_type']
                    == 'G')) {
                    tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                            'error_message='.urlencode(ERROR_NO_INVALID_REDEEM_GV),
                            'SSL'));
                }
            }
            ...
            if ($gv_result['coupon_type'] == 'G') {
                ...
                tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                        'error_message='.urlencode(ERROR_REDEEMED_AMOUNT.$currencies->format($gv_amount)),
                        'SSL'));
            }
        }
        ...
        switch ($method) {
            ...
            case 'Credit Note':
                ...
                $tax_desc                             = tep_get_tax_description($this->tax_class,
                    $order->delivery['country']['id'],
                    $order->delivery['zone_id']);
                $tod_amount                           = $this->deduction / (100 + $tax_rate)
                    * $tax_rate;
...
//          $order->info['total'] -= $tod_amount;   //// ????? Strider
    ...
    function keys()
    {
        return array('MODULE_ORDER_TOTAL_GV_STATUS', 'MODULE_ORDER_TOTAL_GV_SORT_ORDER',
            'MODULE_ORDER_TOTAL_GV_QUEUE', 'MODULE_ORDER_TOTAL_GV_INC_SHIPPING',
            'MODULE_ORDER_TOTAL_GV_INC_TAX', 'MODULE_ORDER_TOTAL_GV_CALC_TAX', 'MODULE_ORDER_TOTAL_GV_TAX_CLASS',
