...
class sb_google_buzz
{
    ...
    {
        $this->title        = _('Google Buzz');
        $this->public_title = _('Share on Google Buzz');
        $this->description  = _('Share products on Google Buzz.');
    ...
    function getOutput()
    {
        ...
        return '<a href="http://www.google.com/buzz/post?url='.urlencode(tep_href_link(FILENAME_PRODUCT_INFO,
                    'products_id='.$HTTP_GET_VARS['products_id'], 'NONSSL',
                    false)).'" target="_blank"><img src="images/social_bookmarks/'.$this->icon.'" border="0" title="'.tep_output_string_protected($this->public_title).'" alt="'.tep_output_string_protected($this->public_title).'" /></a>';
    }
