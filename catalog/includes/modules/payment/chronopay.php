...
// class constructor
    function chronopay()
    {
        ...
        $this->enabled      = ((MODULE_PAYMENT_CHRONOPAY_STATUS == 'True') ? true
                : false);
...
// class methods
    ...
    function confirmation()
    {
        ...
        if (tep_session_is_registered('cartID')) {
            ...
            if (tep_session_is_registered('cart_ChronoPay_ID')) {
                ...
                if (($curr['currency'] != $order->info['currency']) || ($cartID != substr($cart_ChronoPay_ID,
                        0, strlen($cartID)))) {
                    $check_query = tep_db_query('select orders_id from '.TABLE_ORDERS_STATUS_HISTORY.' where orders_id = "'.(int) $order_id.'" limit 1');
    ...
    function process_button()
    {
        ...
        $total_price = number_format($order->info['total'] * $currencies->get_value('USD'),
            $currencies->currencies['USD']['decimal_places'], '.', '');
        $order_id    = substr($cart_ChronoPay_ID,
            strpos($cart_ChronoPay_ID, '-') + 1);

        $process_button_string = tep_draw_hidden_field('product_id',
            ...
            tep_draw_hidden_field('cb_url',
                urlencode(tep_href_link('ext/modules/payment/chronopay/callback.php',
                        '', 'SSL', true, true, true))).
            tep_draw_hidden_field('cb_type', 'P').
            tep_draw_hidden_field('decline_url',
                urlencode(tep_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL'))).
            ...
            tep_draw_hidden_field('cs3',
                md5(MODULE_PAYMENT_CHRONOPAY_PRODUCT_ID.$order_id.$customer_id.$total_price.MODULE_PAYMENT_CHRONOPAY_MD5_HASH));
    ...
    function before_process()
    {
        ...
        tep_db_query("update ".TABLE_ORDERS." set orders_status = '".(MODULE_PAYMENT_CHRONOPAY_ORDER_STATUS_ID
            > 0 ? (int) MODULE_PAYMENT_CHRONOPAY_ORDER_STATUS_ID : (int) DEFAULT_ORDERS_STATUS_ID)."', last_modified = now() where orders_id = '".(int) $order_id."'");

        $sql_data_array = array('orders_id' => $order_id,
            'orders_status_id' => (MODULE_PAYMENT_CHRONOPAY_ORDER_STATUS_ID > 0 ? (int) MODULE_PAYMENT_CHRONOPAY_ORDER_STATUS_ID
...
// unregister session variables used during checkout
    ...
    function keys()
    {
        return array('MODULE_PAYMENT_CHRONOPAY_STATUS', 'MODULE_PAYMENT_CHRONOPAY_PRODUCT_ID',
            'MODULE_PAYMENT_CHRONOPAY_MD5_HASH', 'MODULE_PAYMENT_CHRONOPAY_ZONE',
            'MODULE_PAYMENT_CHRONOPAY_PREPARE_ORDER_STATUS_ID', 'MODULE_PAYMENT_CHRONOPAY_ORDER_STATUS_ID',
