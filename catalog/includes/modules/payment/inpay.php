...
class inpay
{
    ...
    function update_status()
    {
        ...
        if (($this->enabled === true) && ((int) MODULE_PAYMENT_INPAY_ZONE > 0)) {
            $check_flag  = false;
            $check_query = tep_db_query("select zone_id from ".constant('TABLE_ZONES_TO_GEO_ZONES')." where geo_zone_id = '".MODULE_PAYMENT_INPAY_ZONE."' and zone_country_id = '".$order->billing['country']['id']."' order by zone_id");
    ...
    function selection()
    {
        ...
        if (tep_session_is_registered('cart_inpay_Standard_ID')) {
            $order_id = substr($cart_inpay_Standard_ID,
                strpos($cart_inpay_Standard_ID, '-') + 1);
        ...
        return array('id' => $this->code,
            'module' => $this->public_title, 'fields' => array(array('title' => '',
                    'field' => MODULE_PAYMENT_INPAY_TEXT_PUBLIC_HTML)));
    }
    ...
    function confirmation()
    {
        ...
        if (tep_session_is_registered('cartID')) {
            ...
            if (tep_session_is_registered('cart_inpay_Standard_ID')) {
                ...
                if (($curr['currency'] != $order->info['currency']) || ($cartID != substr($cart_inpay_Standard_ID,
                        0, strlen($cartID)))) {
                    $check_query = tep_db_query('select orders_id from '.TABLE_ORDERS_STATUS_HISTORY.' where orders_id = "'.(int) $order_id.'" limit 1');
    ...
    function process_button()
    {
        ...
        $parameters            = array('cmd' => '_xclick',
            ...
            'custom' => $customer_id,
            'no_note' => '1',
            'notify_url' => tep_href_link('ext/modules/payment/inpay/pb_handler.php',
                '', 'SSL', false, false),
        ...
        if (is_numeric($sendto) && ($sendto > 0)) {
            ...
            $address                     = $order->billing['street_address'].' '.$order->billing['city'].' '.
                tep_get_zone_code($order->billing['country']['id'],
    ...
    function before_process()
    {
        ...
        if (($current_status_id == MODULE_PAYMENT_INPAY_COMP_ORDER_STATUS_ID) || ($current_status_id
            == $delivered_status)) {
            $update_status = false;
        }
        if ($update_status) {
        ...
        for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
            ...
            if ((MODULE_PAYMENT_INPAY_DECREASE_STOCK_ON_CREATION == 'True') && (STOCK_LIMITED
                == 'true')) {
                if (DOWNLOAD_ENABLED == 'true') {
    ...
    function install()
    {
        $created_status_id     = $this->set_order_status('Processing [inpay]',
            true);
        $sum_too_low_status_id = $this->set_order_status('Sum too low [inpay]',
            true);
        $completed_status_id   = $this->set_order_status('Completed [inpay]',
            true);
    ...
    function keys()
    {
        ...
        return array('MODULE_PAYMENT_INPAY_STATUS', 'MODULE_PAYMENT_INPAY_GATEWAY_SERVER',
            'MODULE_PAYMENT_INPAY_MERCHANT_ID', 'MODULE_PAYMENT_INPAY_SECRET_KEY',
            'MODULE_PAYMENT_INPAY_FLOW_LAYOUT', 'MODULE_PAYMENT_INPAY_DECREASE_STOCK_ON_CREATION',
            'MODULE_PAYMENT_INPAY_DEBUG_EMAIL', 'MODULE_PAYMENT_INPAY_ZONE', 'MODULE_PAYMENT_INPAY_SORT_ORDER',
            'MODULE_PAYMENT_INPAY_CREATE_ORDER_STATUS_ID', 'MODULE_PAYMENT_INPAY_SUM_TOO_LOW_ORDER_STATUS_ID',
