...
// class constructor
    function ipayment_pp()
    {
        ...
        $this->enabled      = ((MODULE_PAYMENT_IPAYMENT_PP_STATUS == 'True') ? true
                : false);
...
// class methods
    function update_status()
    {
        ...
        if (($this->enabled === true) && ((int) MODULE_PAYMENT_IPAYMENT_PP_ZONE > 0)) {
            $check_flag  = false;
            $check_query = tep_db_query("select zone_id from ".constant('TABLE_ZONES_TO_GEO_ZONES')." where geo_zone_id = '".MODULE_PAYMENT_IPAYMENT_PP_ZONE."' and zone_country_id = '".$order->billing['country']['id']."' order by zone_id");
    ...
    function process_button()
    {
        ...
        $process_button_string = tep_draw_hidden_field('silent', '1').
            ...
            tep_draw_hidden_field('trxuser_id',
                MODULE_PAYMENT_IPAYMENT_PP_USER_ID).
            tep_draw_hidden_field('trxpassword',
                MODULE_PAYMENT_IPAYMENT_PP_PASSWORD).
            ...
            tep_draw_hidden_field('trx_typ',
                ((MODULE_PAYMENT_IPAYMENT_PP_TRANSACTION_METHOD == 'Capture') ? 'auth'
                    : 'preauth')).
            ...
            tep_draw_hidden_field('hidden_trigger_url',
                tep_href_link('ext/modules/payment/ipayment/callback_pp.php',
                    '', 'SSL', false)).
        ...
        if (tep_not_null(MODULE_PAYMENT_IPAYMENT_PP_SECRET_HASH_PASSWORD)) {
            $process_button_string .= tep_draw_hidden_field('trx_securityhash',
                md5(MODULE_PAYMENT_IPAYMENT_PP_USER_ID.($this->format_raw($order->info['total'])
                    * 100).$currency.MODULE_PAYMENT_IPAYMENT_PP_PASSWORD.MODULE_PAYMENT_IPAYMENT_PP_SECRET_HASH_PASSWORD));
        }
...
// verify ret_param_checksum
            if ($_GET['ret_param_checksum'] != md5(MODULE_PAYMENT_IPAYMENT_PP_USER_ID.($this->format_raw($order->info['total'])
                    * 100).$currency.$_GET['ret_authcode'].$_GET['ret_booknr'].MODULE_PAYMENT_IPAYMENT_PP_SECRET_HASH_PASSWORD)) {
                $pass = false;
            }
...
// verify ret_url_checksum
    ...
    function get_error()
    {
        ...
        $error = array('title' => MODULE_PAYMENT_IPAYMENT_PP_ERROR_HEADING,
            'error' => ((isset($_GET['error'])) ? stripslashes(urldecode($_GET['error']))
                : MODULE_PAYMENT_IPAYMENT_PP_ERROR_MESSAGE));
    ...
    function keys()
    {
        return array('MODULE_PAYMENT_IPAYMENT_PP_STATUS', 'MODULE_PAYMENT_IPAYMENT_PP_ID',
            'MODULE_PAYMENT_IPAYMENT_PP_USER_ID', 'MODULE_PAYMENT_IPAYMENT_PP_PASSWORD',
            'MODULE_PAYMENT_IPAYMENT_PP_TRANSACTION_METHOD', 'MODULE_PAYMENT_IPAYMENT_PP_SECRET_HASH_PASSWORD',
            'MODULE_PAYMENT_IPAYMENT_PP_DEBUG_EMAIL', 'MODULE_PAYMENT_IPAYMENT_PP_ZONE',
            'MODULE_PAYMENT_IPAYMENT_PP_ORDER_STATUS_ID', 'MODULE_PAYMENT_IPAYMENT_PP_SORT_ORDER');
    }
...
// format prices without currency formatting
    ...
    function sendDebugEmail($checksum_match = 0)
    {
        ...
        if (tep_not_null(MODULE_PAYMENT_IPAYMENT_PP_DEBUG_EMAIL)) {
            ...
            tep_mail('', MODULE_PAYMENT_IPAYMENT_PP_DEBUG_EMAIL,
                'iPayment (Prepaid) Transaction', $email_body, STORE_OWNER,
