...
class sage_pay_direct
{
    ...
    function sage_pay_direct()
    {
        ...
        $this->sort_order   = defined('MODULE_PAYMENT_SAGE_PAY_DIRECT_SORT_ORDER')
                ? MODULE_PAYMENT_SAGE_PAY_DIRECT_SORT_ORDER : 0;
        $this->enabled      = defined('MODULE_PAYMENT_SAGE_PAY_DIRECT_STATUS') && (MODULE_PAYMENT_SAGE_PAY_DIRECT_STATUS
            == 'True') ? true : false;
        $this->order_status = defined('MODULE_PAYMENT_SAGE_PAY_DIRECT_ORDER_STATUS_ID')
            && ((int) MODULE_PAYMENT_SAGE_PAY_DIRECT_ORDER_STATUS_ID > 0) ? (int) MODULE_PAYMENT_SAGE_PAY_DIRECT_ORDER_STATUS_ID
                : 0;
    ...
    function update_status()
    {
        ...
        if (($this->enabled === true) && ((int) MODULE_PAYMENT_SAGE_PAY_DIRECT_ZONE
            > 0)) {
            $check_flag  = false;
            $check_query = tep_db_query("select zone_id from ".constant('TABLE_ZONES_TO_GEO_ZONES')." where geo_zone_id = '".MODULE_PAYMENT_SAGE_PAY_DIRECT_ZONE."' and zone_country_id = '".$order->billing['country']['id']."' order by zone_id");
    ...
    function pre_confirmation_check()
    {
        if ($this->templateClassExists()) {
            $GLOBALS['oscTemplate']->addBlock($this->getSubmitCardDetailsJavascript(),
                'header_tags');
        }
    }
    ...
    function confirmation()
    {
        ...
        $months_array = array();
        for ($i = 1; $i < 13; $i++) {
            $months_array[] = array('id' => sprintf('%02d', $i), 'text' => sprintf('%02d',
                    $i));
        }

        $year_valid_to_array = array();
        for ($i = $today['year']; $i < $today['year'] + 10; $i++) {
            $year_valid_to_array[] = array('id' => strftime('%y',
        ...
        for ($i = $today['year'] - 4; $i < $today['year'] + 1; $i++) {
            $year_valid_from_array[] = array('id' => strftime('%y',
                    mktime(0, 0, 0, 1, 1, $i)), 'text' => strftime('%Y',
                    mktime(0, 0, 0, 1, 1, $i)));
        }
            ...
            if (tep_db_num_rows($tokens_query) > 0) {
                ...
                while ($tokens = tep_db_fetch_array($tokens_query)) {
                    $content .= '<tr class="moduleRow" id="sagepay_card_'.(int) $tokens['id'].'">'.
                        ...
                        '  <td valign="top">'.tep_output_string_protected($tokens['number_filtered']).'&nbsp;&nbsp;'.tep_output_string_protected(substr($tokens['expiry_date'],
                                0, 2)).'/'.strftime('%Y',
                            mktime(0, 0, 0, 1, 1,
                                (2000 + substr($tokens['expiry_date'], 2)))).'&nbsp;&nbsp;'.tep_output_string_protected($tokens['card_type']).'</td>'.
                        '</tr>';
                    ...
                    if (MODULE_PAYMENT_SAGE_PAY_DIRECT_VERIFY_WITH_CVC == 'True') {
                        $content .= '<tr class="moduleRowExtra" id="sagepay_card_cvc_'.(int) $tokens['id'].'">'.
                            ...
                            '  <td valign="top">'.MODULE_PAYMENT_SAGE_PAY_DIRECT_CREDIT_CARD_CVC.'&nbsp;'.tep_draw_input_field('cc_cvc_tokens_nh-dns['.(int) $tokens['id'].']',
        ...
        $content .= '<table id="sagepay_table_new_card" border="0" width="100%" cellspacing="0" cellpadding="2">'.
            ...
            '  <td>'.tep_draw_pull_down_menu('cc_type', $card_types, '',
                'id="sagepay_card_type"').'</td>'.
            '</tr>'.
            '<tr>'.
            '  <td width="30%">'.MODULE_PAYMENT_SAGE_PAY_DIRECT_CREDIT_CARD_OWNER.'</td>'.
            '  <td>'.tep_draw_input_field('cc_owner',
                $order->billing['firstname'].' '.$order->billing['lastname'],
                'maxlength="50"').'</td>'.
            '</tr>'.
            '<tr>'.
            '  <td width="30%">'.MODULE_PAYMENT_SAGE_PAY_DIRECT_CREDIT_CARD_NUMBER.'</td>'.
            '  <td>'.tep_draw_input_field('cc_number_nh-dns', '',
                'maxlength="20"').'</td>'.
            '</tr>';

        if ((MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_MAESTRO == 'True') || (MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_AMEX
            == 'True')) {
            $content .= '<tr>'.
                '  <td width="30%">'.MODULE_PAYMENT_SAGE_PAY_DIRECT_CREDIT_CARD_STARTS.'</td>'.
                '  <td>'.tep_draw_pull_down_menu('cc_starts_month',
                    $months_array, '', 'id="sagepay_card_date_start"').'&nbsp;'.tep_draw_pull_down_menu('cc_starts_year',
                    $year_valid_from_array).'&nbsp;'.MODULE_PAYMENT_SAGE_PAY_DIRECT_CREDIT_CARD_STARTS_INFO.'</td>'.
                '</tr>';
        }

        $content .= '<tr>'.
            '  <td width="30%">'.MODULE_PAYMENT_SAGE_PAY_DIRECT_CREDIT_CARD_EXPIRES.'</td>'.
            '  <td>'.tep_draw_pull_down_menu('cc_expires_month', $months_array).'&nbsp;'.tep_draw_pull_down_menu('cc_expires_year',
                $year_valid_to_array).'</td>'.
            '</tr>';
        ...
        if ((MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_MAESTRO == 'True')) {
            $content .= '<tr>'.
                ...
                '  <td>'.tep_draw_input_field('cc_issue_nh-dns', '',
                    'id="sagepay_card_issue" size="3" maxlength="2"').'&nbsp;'.MODULE_PAYMENT_SAGE_PAY_DIRECT_CREDIT_CARD_ISSUE_NUMBER_INFO.'</td>'.
                '</tr>';
        }
        ...
        if (MODULE_PAYMENT_SAGE_PAY_DIRECT_VERIFY_WITH_CVC == 'True') {
            $content .= '<tr>'.
                ...
                '  <td>'.tep_draw_input_field('cc_cvc_nh-dns', '',
                    'size="5" maxlength="4"').'</td>'.
                '</tr>';
        }
    ...
    function before_process()
    {
        ...
        if (isset($_GET['check'])) {
            if (($_GET['check'] == '3D') && isset($_POST['MD']) && tep_not_null($_POST['MD'])
                && isset($_POST['PaRes']) && tep_not_null($_POST['PaRes'])) {
                ...
                $transaction_response = $this->sendTransactionToGateway($gateway_url,
                    $post_string);
            } elseif (($_GET['check'] == 'PAYPAL') && isset($_POST['Status'])) {
                if (($_POST['Status'] == 'PAYPALOK') && isset($_POST['VPSTxId'])
                    && isset($_POST['CustomerEMail']) && isset($_POST['PayerID'])) {
                    ...
                    $transaction_response = $this->sendTransactionToGateway($gateway_url,
                        $post_string);
                } elseif (isset($_POST['StatusDetail']) && ($_POST['StatusDetail']
                    == 'Paypal transaction cancelled by client.')) {
            ...
            if (MODULE_PAYMENT_SAGE_PAY_DIRECT_TOKENS == 'True') {
                if (isset($_POST['sagepay_card']) && is_numeric($_POST['sagepay_card'])
                    && ($_POST['sagepay_card'] > 0)) {
                    ...
                    if (tep_db_num_rows($token_query) == 1) {
                        ...
                        if (isset($_POST['cc_cvc_tokens_nh-dns']) && is_array($_POST['cc_cvc_tokens_nh-dns'])
                            && isset($_POST['cc_cvc_tokens_nh-dns'][$_POST['sagepay_card']])) {
                            $sagepay_token_cvc = substr($_POST['cc_cvc_tokens_nh-dns'][$_POST['sagepay_card']],
                                0, 4);
                        }
                    }
                }
            ...
            if (!isset($sagepay_token)) {
                ...
                if ($cc_type != 'PAYPAL') {
                    $cc_owner   = isset($_POST['cc_owner']) ? substr($_POST['cc_owner'],
                            0, 50) : null;
                    $cc_number  = isset($_POST['cc_number_nh-dns']) ? substr(preg_replace('/[^0-9]/',
                                '', $_POST['cc_number_nh-dns']), 0, 20) : null;
                    $cc_start   = null;
                    $cc_expires = null;
                    $cc_issue   = isset($_POST['cc_issue_nh-dns']) ? substr($_POST['cc_issue_nh-dns'],
                            0, 2) : null;
                    $cc_cvc     = isset($_POST['cc_cvc_nh-dns']) ? substr($_POST['cc_cvc_nh-dns'],
                            0, 4) : null;
                    ...
                    $year_valid_to_array = array();
                    for ($i = $today['year']; $i < $today['year'] + 10; $i++) {
                        $year_valid_to_array[] = strftime('%y',
                            mktime(0, 0, 0, 1, 1, $i));
                    }

                    $year_valid_from_array = array();
                    for ($i = $today['year'] - 4; $i < $today['year'] + 1; $i++) {
                        $year_valid_from_array[] = strftime('%y',
                            mktime(0, 0, 0, 1, 1, $i));
                    }

                    if (!isset($cc_owner) || empty($cc_owner)) {
                        tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                                'payment_error='.$this->code.'&error=cardowner',
                                'SSL'));
                    }
                    ...
                    if ((($cc_type == 'MAESTRO') && (MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_MAESTRO
                        == 'True')) || (($cc_type == 'AMEX') && (MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_AMEX
                        == 'True'))) {
                        if (!isset($_POST['cc_starts_month']) || !in_array($_POST['cc_starts_month'],
                                $months_array)) {
                            tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                                    'payment_error='.$this->code.'&error=cardstart',
                                    'SSL'));
                        }

                        if (!isset($_POST['cc_starts_year']) || !in_array($_POST['cc_starts_year'],
                        ...
                        $cc_start = substr($_POST['cc_starts_month'].$_POST['cc_starts_year'],
                            0, 4);
                    }

                    if (!isset($_POST['cc_expires_month']) || !in_array($_POST['cc_expires_month'],
                            $months_array)) {
                        tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                                'payment_error='.$this->code.'&error=cardexpires',
                                'SSL'));
                    }

                    if (!isset($_POST['cc_expires_year']) || !in_array($_POST['cc_expires_year'],
                            $year_valid_to_array)) {
                        tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                                'payment_error='.$this->code.'&error=cardexpires',
                                'SSL'));
                    ...
                    if (($_POST['cc_expires_year'] == date('y')) && ($_POST['cc_expires_month']
                        < date('m'))) {
                        tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                                'payment_error='.$this->code.'&error=cardexpires',
                                'SSL'));
                    ...
                    $cc_expires = substr($_POST['cc_expires_month'].$_POST['cc_expires_year'],
                        0, 4);
                    ...
                    if ((($cc_type == 'MAESTRO') && (MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_MAESTRO
                        ...
                        if (!isset($cc_issue) || empty($cc_issue)) {
                            tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                                    'payment_error='.$this->code.'&error=cardissue',
                                    'SSL'));
                        }
                    }
                    ...
                    if (MODULE_PAYMENT_SAGE_PAY_DIRECT_VERIFY_WITH_CVC == 'True') {
                        if (!isset($cc_cvc) || empty($cc_cvc)) {
                            tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                                    'payment_error='.$this->code.'&error=cardcvc',
                                    'SSL'));
                        }
                    }
                }
            ...
            $params = array('VPSProtocol' => $this->api_version,
                ...
                'Vendor' => substr(MODULE_PAYMENT_SAGE_PAY_DIRECT_VENDOR_LOGIN_NAME,
                    0, 15),
                'VendorTxCode' => substr(date('YmdHis').'-'.$customer_id.'-'.$cartID,
                    0, 40),
                'Amount' => $this->format_raw($order->info['total']),
                'Currency' => $currency,
                'Description' => substr(STORE_NAME, 0, 100),
                'BillingSurname' => substr($order->billing['lastname'], 0, 20),
                'BillingFirstnames' => substr($order->billing['firstname'], 0,
                    20),
                'BillingAddress1' => substr($order->billing['street_address'],
                    0, 100),
                ...
                'DeliveryFirstnames' => substr($order->delivery['firstname'], 0,
                    20),
                'DeliveryAddress1' => substr($order->delivery['street_address'],
                    0, 100),
                ...
                'CustomerEMail' => substr($order->customer['email_address'], 0,
                    255),
                'Apply3DSecure' => '0',
            ...
            if (isset($sagepay_token)) {
                ...
                if ($cc_type == 'PAYPAL') {
                    $params['PayPalCallbackURL'] = tep_href_link(FILENAME_CHECKOUT_PROCESS,
                        'check=PAYPAL', 'SSL');
                } else {
                    $params['CardHolder']  = $cc_owner;
                    $params['CardNumber']  = $cc_number;
                    $params['ExpiryDate']  = $cc_expires;
                    $params['CreateToken'] = ((MODULE_PAYMENT_SAGE_PAY_DIRECT_TOKENS
                        == 'True') && isset($_POST['cc_save']) && ($_POST['cc_save']
                        == 'true') ? '1' : '0');
            ...
            if (!empty($ip_address) && (ip2long($ip_address) != -1) && (ip2long($ip_address)
                != false)) {
            ...
            if ($params['BillingCountry'] == 'US') {
                $params['BillingState'] = tep_get_zone_code($order->billing['country']['id'],
                    $order->billing['zone_id'], '');
            }

            if ($params['DeliveryCountry'] == 'US') {
                $params['DeliveryState'] = tep_get_zone_code($order->delivery['country']['id'],
            ...
            foreach ($order->products as $product) {
                ...
                $contents[] = str_replace(array(':', "\n", "\r", '&'), '',
                        $product_name).':'.$product['qty'].':'.$this->format_raw($product['final_price']).':'.$this->format_raw(($product['tax']
                        / 100) * $product['final_price']).':'.$this->format_raw((($product['tax']
                        / 100) * $product['final_price']) + $product['final_price']).':'.$this->format_raw(((($product['tax']
                        / 100) * $product['final_price']) + $product['final_price'])
                        * $product['qty']);
            }

            foreach ($order_totals as $ot) {
                $contents[] = str_replace(array(':', "\n", "\r", '&'), '',
                        strip_tags($ot['title'])).':---:---:---:---:'.$this->format_raw($ot['value']);
            }

            $params['Basket'] = substr(sizeof($contents).':'.implode(':',
                    $contents), 0, 7500);
        ...
        if (isset($params['CreateToken']) && ($params['CreateToken'] == '1')) {
            ...
            $sagepay_token_cc_number = str_repeat('X',
                    strlen($params['CardNumber']) - 4).substr($params['CardNumber'],
                    -4);
        ...
        if (($sage_pay_response['Status'] != 'OK') && ($sage_pay_response['Status']
            != 'AUTHENTICATED') && ($sage_pay_response['Status'] != 'REGISTERED')) {
            $this->sendDebugEmail($sage_pay_response);

            $error = $this->getErrorMessageNumber($sage_pay_response['StatusDetail']);

            tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                    'payment_error='.$this->code.(tep_not_null($error) ? '&error='.$error
                            : ''), 'SSL'));
        }
    }
    ...
    function after_process()
    {
        ...
        if (isset($_GET['check']) && ($_GET['check'] == 'PAYPAL') && isset($_POST['Status'])
            && ($_POST['Status'] == 'PAYPALOK') && isset($_POST['VPSTxId']) && isset($sage_pay_response['VPSTxId'])
            && ($_POST['VPSTxId'] == $sage_pay_response['VPSTxId'])) {
    ...
    function getParams()
    {
        if (tep_db_num_rows(tep_db_query("show tables like 'customers_sagepay_tokens'"))
            != 1) {
            $sql = <<<EOD
...
EOD;
        ...
        $params = array('MODULE_PAYMENT_SAGE_PAY_DIRECT_STATUS' => array('title' => 'Enable Sage Pay Direct Module',
            ...
            'MODULE_PAYMENT_SAGE_PAY_DIRECT_TRANSACTION_ORDER_STATUS_ID' => array(
        ...
        if (MODULE_PAYMENT_SAGE_PAY_DIRECT_VERIFY_SSL == 'True') {
            ...
            if (file_exists(DIR_FS_CATALOG.'ext/modules/payment/sage_pay/sagepay.com.crt')) {
                curl_setopt($curl, CURLOPT_CAINFO,
                    DIR_FS_CATALOG.'ext/modules/payment/sage_pay/sagepay.com.crt');
            } elseif (file_exists(DIR_FS_CATALOG.'includes/cacert.pem')) {
                curl_setopt($curl, CURLOPT_CAINFO,
                    DIR_FS_CATALOG.'includes/cacert.pem');
            }
        } else {
...
// format prices without currency formatting
    ...
    function getTestLinkInfo()
    {
        ...
        $test_url = tep_href_link(FILENAME_MODULES,
            'set=payment&module='.$this->code.'&action=install&subaction=conntest');
...
EOD;
    ...
    {
        return class_exists('oscTemplate') && isset($GLOBALS['oscTemplate']) && is_object($GLOBALS['oscTemplate'])
            && (get_class($GLOBALS['oscTemplate']) == 'oscTemplate');
    }
        ...
        if (tep_not_null(MODULE_PAYMENT_SAGE_PAY_DIRECT_DEBUG_EMAIL)) {
            ...
            if (!empty($_POST)) {
                if (isset($_POST['cc_number_nh-dns'])) {
                    $_POST['cc_number_nh-dns'] = 'XXXX'.substr($_POST['cc_number_nh-dns'],
                            -4);
                }
            ...
            if (!empty($email_body)) {
                tep_mail('', MODULE_PAYMENT_SAGE_PAY_DIRECT_DEBUG_EMAIL,
                    'Sage Pay Direct Debug E-Mail', trim($email_body),
