...
class paypal_pro_hs
{
    ...
    function paypal_pro_hs()
    {
        ...
        $this->sort_order   = defined('MODULE_PAYMENT_PAYPAL_PRO_HS_SORT_ORDER')
                ? MODULE_PAYMENT_PAYPAL_PRO_HS_SORT_ORDER : 0;
        $this->enabled      = defined('MODULE_PAYMENT_PAYPAL_PRO_HS_STATUS') && (MODULE_PAYMENT_PAYPAL_PRO_HS_STATUS
            == 'True') ? true : false;
        $this->order_status = defined('MODULE_PAYMENT_PAYPAL_PRO_HS_PREPARE_ORDER_STATUS_ID')
            && ((int) MODULE_PAYMENT_PAYPAL_PRO_HS_PREPARE_ORDER_STATUS_ID > 0) ? (int) MODULE_PAYMENT_PAYPAL_PRO_HS_PREPARE_ORDER_STATUS_ID
                : 0;
        ...
        if ($this->enabled === true) {
            if (!tep_not_null(MODULE_PAYMENT_PAYPAL_PRO_HS_ID) || !tep_not_null(MODULE_PAYMENT_PAYPAL_PRO_HS_API_USERNAME)
                || !tep_not_null(MODULE_PAYMENT_PAYPAL_PRO_HS_API_PASSWORD) || !tep_not_null(MODULE_PAYMENT_PAYPAL_PRO_HS_API_SIGNATURE)) {
    ...
    function update_status()
    {
        ...
        if (($this->enabled == true) && ((int) MODULE_PAYMENT_PAYPAL_PRO_HS_ZONE
            > 0)) {
            $check_flag  = false;
    ...
    function selection()
    {
        ...
        if (tep_session_is_registered('cart_PayPal_Pro_HS_ID')) {
            $order_id = substr($cart_PayPal_Pro_HS_ID,
                strpos($cart_PayPal_Pro_HS_ID, '-') + 1);
    ...
    function confirmation()
    {
        ...
        if (tep_session_is_registered('cartID')) {
            ...
            if (tep_session_is_registered('cart_PayPal_Pro_HS_ID')) {
                ...
                if (($curr['currency'] != $order->info['currency']) || ($cartID != substr($cart_PayPal_Pro_HS_ID,
                        0, strlen($cartID)))) {
                    $check_query = tep_db_query('select orders_id from '.TABLE_ORDERS_STATUS_HISTORY.' where orders_id = "'.(int) $order_id.'" limit 1');
            ...
            $params = array('business' => MODULE_PAYMENT_PAYPAL_PRO_HS_ID,
                ...
                'invoice' => $order_id,
                'custom' => $customer_id,
                'paymentaction' => MODULE_PAYMENT_PAYPAL_PRO_HS_TRANSACTION_METHOD
                == 'Sale' ? 'sale' : 'authorization',
                'return' => tep_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL'),
                'notify_url' => tep_href_link('ext/modules/payment/paypal/pro_hosted_ipn.php',
                    '', 'SSL', false, false),
                'shipping' => $this->format_raw($order->info['shipping_cost']),
                'tax' => $this->format_raw($order->info['tax']),
                'subtotal' => $this->format_raw($order->info['total'] - $order->info['shipping_cost']
                    - $order->info['tax']),
                ...
                'billing_city' => $order->billing['city'],
                'billing_state' => tep_get_zone_code($order->billing['country']['id'],
            ...
            if (is_numeric($sendto) && ($sendto > 0)) {
                ...
                $params['state']            = tep_get_zone_code($order->delivery['country']['id'],
            ...
            if (tep_not_null(MODULE_PAYMENT_PAYPAL_PRO_HS_TEXT_PAYPAL_RETURN_BUTTON)
                && (strlen(MODULE_PAYMENT_PAYPAL_PRO_HS_TEXT_PAYPAL_RETURN_BUTTON)
                <= 60)) {
            ...
            $response = $this->sendTransactionToGateway($this->api_url,
                $params_string);
        ...
        $iframe_url = tep_href_link('ext/modules/payment/paypal/hosted_checkout.php',
            'key='.$pphs_key, 'SSL');
        $form_url   = tep_href_link(FILENAME_CHECKOUT_PAYMENT,
            'payment_error=paypal_pro_hs', 'SSL');
...
EOD;
    ...
    function before_process()
    {
        ...
        if (!is_array($result) || !isset($result['ACK']) || (($result['ACK'] != 'Success')
            && ($result['ACK'] != 'SuccessWithWarning'))) {
            tep_redirect(tep_href_link(FILENAME_SHOPPING_CART,
                    'error_message='.stripslashes($result['L_LONGMESSAGE0'])));
        }
        ...
        if (!isset($result['RECEIVERBUSINESS']) || !in_array($result['RECEIVERBUSINESS'],
                $seller_accounts) || ($result['INVNUM'] != $order_id) || ($result['CUSTOM']
            != $customer_id)) {
            tep_redirect(tep_href_link(FILENAME_SHOPPING_CART));
        }
        ...
        if (!tep_db_num_rows($check_query) || ($order_id != $tx_order_id) || ($customer_id
            != $tx_customer_id)) {
            tep_redirect(tep_href_link(FILENAME_SHOPPING_CART));
        }
        ...
        if ((MODULE_PAYMENT_PAYPAL_PRO_HS_ORDER_STATUS_ID > 0) && ($check['orders_status']
            == MODULE_PAYMENT_PAYPAL_PRO_HS_ORDER_STATUS_ID)) {
    ...
    function getParams()
    {
        ...
        $params = array('MODULE_PAYMENT_PAYPAL_PRO_HS_STATUS' => array('title' => 'Enable PayPal Payments Pro (Hosted Solution)',
            ...
            'MODULE_PAYMENT_PAYPAL_PRO_HS_ORDER_STATUS_ID' => array('title' => 'Set PayPal Acknowledged Order Status',
                ...
                'set_func' => 'tep_cfg_pull_down_order_statuses(',
                'use_func' => 'tep_get_order_status_name'),
            'MODULE_PAYMENT_PAYPAL_PRO_HS_TRANSACTIONS_ORDER_STATUS_ID' => array(
...
EOD;
    ...
    function verifyTransaction($is_ipn = false)
    {
        ...
        $tx_pending_reason = (isset($pphs_result['PENDINGREASON'])) ? $pphs_result['PENDINGREASON']
                : null;
        $tx_reason_code    = (isset($pphs_result['REASONCODE'])) ? $pphs_result['REASONCODE']
                : null;

        if (is_numeric($tx_order_id) && ($tx_order_id > 0) && is_numeric($tx_customer_id)
            && ($tx_customer_id > 0)) {
            ...
            if (tep_db_num_rows($order_query) === 1) {
                ...
                $comment_status = 'Transaction ID: '.$tx_transaction_id.'; '.
                    $tx_payment_status.' ('.ucfirst($tx_payer_status).'; '.$currencies->format($tx_amount,
                        false, $tx_currency).')';

                if ($tx_payment_status == 'Pending') {
                    $comment_status .= '; '.$tx_pending_reason;
                } elseif (($tx_payment_status == 'Reversed') || ($tx_payment_status
                    == 'Refunded')) {
                    $comment_status .= '; '.$tx_reason_code;
                }

                if ($tx_amount != number_format($total['value'] * $order['currency_value'],
                        $currencies->get_decimal_places($order['currency']))) {
                    $comment_status .= '; PayPal transaction value ('.$tx_amount.') does not match order value ('.number_format($total['value']
                            * $order['currency_value'],
                            $currencies->get_decimal_places($order['currency'])).')';
                } elseif ($tx_payment_status == 'Completed') {
                    $new_order_status = (MODULE_PAYMENT_PAYPAL_PRO_HS_ORDER_STATUS_ID
                        > 0 ? MODULE_PAYMENT_PAYPAL_PRO_HS_ORDER_STATUS_ID : $new_order_status);
                }
    ...
    function sendDebugEmail($response = array())
    {
        ...
        if (tep_not_null(MODULE_PAYMENT_PAYPAL_PRO_HS_DEBUG_EMAIL)) {
            ...
            if (!empty($email_body)) {
                tep_mail('', MODULE_PAYMENT_PAYPAL_PRO_HS_DEBUG_EMAIL,
                    'PayPal Payments Pro (Hosted Solution) Debug E-Mail',
