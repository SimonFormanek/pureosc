...
// class constructor
    function moneybookers()
    {
        ...
        $this->enabled      = ((MODULE_PAYMENT_MONEYBOOKERS_STATUS == 'True') ? true
                : false);
        ...
        if (defined('MODULE_PAYMENT_MONEYBOOKERS_IFRAME') && (MODULE_PAYMENT_MONEYBOOKERS_IFRAME
            == 'True')) {
            $this->form_action_url = tep_href_link('ext/modules/payment/moneybookers/checkout.php',
                '', 'SSL');
        } else {
...
// class methods
    function update_status()
    {
        ...
        if (($this->enabled === true) && ((int) MODULE_PAYMENT_MONEYBOOKERS_ZONE > 0)) {
            $check_flag  = false;
            $check_query = tep_db_query("select zone_id from ".constant('TABLE_ZONES_TO_GEO_ZONES')." where geo_zone_id = '".MODULE_PAYMENT_MONEYBOOKERS_ZONE."' and zone_country_id = '".$order->billing['country']['id']."' order by zone_id");
    ...
    function selection()
    {
        ...
        return array('id' => $this->code,
            'module' => $this->public_title.(!empty($this->_payment_method_image)
                ? '<br />'.tep_image('ext/modules/payment/moneybookers/logos/'.$this->_payment_method_image,
                $this->public_title) : ''));
    }
    ...
    function _prepareOrder()
    {
        ...
        if (tep_session_is_registered($this->_mbcartID)) {
            ...
            if (($curr['currency'] != $order->info['currency']) || ($cartID != substr($GLOBALS[$this->_mbcartID],
                    0, strlen($cartID)))) {
                $check_query = tep_db_query('select orders_id from '.TABLE_ORDERS_STATUS_HISTORY.' where orders_id = "'.(int) $order_id.'" limit 1');
    ...
    function confirmation()
    {
        ...
        if (tep_session_is_registered('cartID')) {
            ...
            $parameters = array('pay_to_email' => MODULE_PAYMENT_MONEYBOOKERS_PAY_TO,
                ...
                'transaction_id' => substr($GLOBALS[$this->_mbcartID],
                    strpos($GLOBALS[$this->_mbcartID], '-') + 1),
                'return_url' => tep_href_link(FILENAME_CHECKOUT_PROCESS,
                    'osig='.md5(MODULE_PAYMENT_MONEYBOOKERS_SECRET_WORD.$GLOBALS[$this->_mbcartID]),
                    'SSL'),
                ...
                'cancel_url' => tep_href_link(FILENAME_CHECKOUT_PAYMENT, '',
                    'SSL'),
                'cancel_url_target' => 1,
                'status_url' => tep_href_link('ext/modules/payment/moneybookers/callback.php',
                    '', 'SSL', false, false),
            ...
            $response = $this->sendTransactionToGateway('https://www.moneybookers.com/app/payment.pl',
                $params_string, true);
    ...
    function before_process()
    {
        ...
        if (isset($_GET['transaction_id']) && isset($_GET['msid'])) {
            if ($_GET['transaction_id'] == substr($GLOBALS[$this->_mbcartID],
                    strpos($GLOBALS[$this->_mbcartID], '-') + 1)) {
        ...
        if ($pass === true) {
            ...
            tep_db_query("update ".TABLE_ORDERS." set orders_status = '".(MODULE_PAYMENT_MONEYBOOKERS_ORDER_STATUS_ID
                > 0 ? (int) MODULE_PAYMENT_MONEYBOOKERS_ORDER_STATUS_ID : (int) DEFAULT_ORDERS_STATUS_ID)."', last_modified = now() where orders_id = '".(int) $order_id."'");

            $sql_data_array = array('orders_id' => $order_id,
                'orders_status_id' => (MODULE_PAYMENT_MONEYBOOKERS_ORDER_STATUS_ID
...
// unregister session variables used during checkout
    ...
    function install()
    {
        ...
        if (!isset($_GET['active']) || ($_GET['active'] != 'true')) {
            tep_redirect(tep_href_link('ext/modules/payment/moneybookers/activation.php',
                    'selected_box=modules&set=payment'));
        }
...
// Transactions order status
        ...
        tep_db_query("insert into ".TABLE_CONFIGURATION." (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('E-Mail Address', 'MODULE_PAYMENT_MONEYBOOKERS_PAY_TO', '".(isset($_GET['email'])
                    ? $_GET['email'] : '')."', 'The Moneybookers seller e-mail address to accept payments for', '6', '4', now())");
        tep_db_query("insert into ".TABLE_CONFIGURATION." (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Merchant ID', 'MODULE_PAYMENT_MONEYBOOKERS_MERCHANT_ID', '".(isset($_GET['custid'])
                    ? $_GET['custid'] : '')."', 'The Moneybookers merchant ID assigned to the seller e-mail address', '6', '4', now())");
        ...
        tep_db_query("insert into ".TABLE_CONFIGURATION." (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Store Logo Image', 'MODULE_PAYMENT_MONEYBOOKERS_STORE_IMAGE', '".tep_catalog_href_link('images/'.STORE_LOGO,
                '', 'SSL')."', 'The URL of the store logo image to display on the gateway transaction page. This must be served through HTTPS otherwise it will not be shown.', '6', '4', now())");
    ...
    function keys()
    {
        return array('MODULE_PAYMENT_MONEYBOOKERS_STATUS', 'MODULE_PAYMENT_MONEYBOOKERS_PAY_TO',
            'MODULE_PAYMENT_MONEYBOOKERS_MERCHANT_ID', 'MODULE_PAYMENT_MONEYBOOKERS_SECRET_WORD',
            'MODULE_PAYMENT_MONEYBOOKERS_STORE_IMAGE', 'MODULE_PAYMENT_MONEYBOOKERS_IFRAME',
            'MODULE_PAYMENT_MONEYBOOKERS_DEBUG_EMAIL', 'MODULE_PAYMENT_MONEYBOOKERS_ZONE',
            'MODULE_PAYMENT_MONEYBOOKERS_PREPARE_ORDER_STATUS_ID', 'MODULE_PAYMENT_MONEYBOOKERS_TRANSACTIONS_ORDER_STATUS_ID',
            'MODULE_PAYMENT_MONEYBOOKERS_ORDER_STATUS_ID', 'MODULE_PAYMENT_MONEYBOOKERS_SORT_ORDER',
...
// format prices without currency formatting
    ...
    function sendTransactionToGateway($url, $parameters, $with_headers = false)
    {
        ...
        if (function_exists('curl_init')) {
            ...
            exec(escapeshellarg(MODULE_PAYMENT_MONEYBOOKERS_CURL).' -d '.escapeshellarg($parameters).' "'.$server['scheme'].'://'.$server['host'].$server['path'].(isset($server['query'])
                        ? '?'.$server['query'] : '').'" -P '.$server['port'].' -k',
                $result);
            $result = implode("\n", $result);
        }
