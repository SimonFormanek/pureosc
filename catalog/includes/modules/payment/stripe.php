...
class stripe
{
    ...
    function stripe()
    {
        ...
        $this->sort_order   = defined('MODULE_PAYMENT_STRIPE_SORT_ORDER') ? MODULE_PAYMENT_STRIPE_SORT_ORDER
                : 0;
        $this->enabled      = defined('MODULE_PAYMENT_STRIPE_STATUS') && (MODULE_PAYMENT_STRIPE_STATUS
            == 'True') ? true : false;
        $this->order_status = defined('MODULE_PAYMENT_STRIPE_ORDER_STATUS_ID') && ((int) MODULE_PAYMENT_STRIPE_ORDER_STATUS_ID
            > 0) ? (int) MODULE_PAYMENT_STRIPE_ORDER_STATUS_ID : 0;
    ...
    function update_status()
    {
        ...
        if (($this->enabled === true) && ((int) MODULE_PAYMENT_STRIPE_ZONE > 0)) {
            $check_flag  = false;
            $check_query = tep_db_query("select zone_id from ".constant('TABLE_ZONES_TO_GEO_ZONES')." where geo_zone_id = '".MODULE_PAYMENT_STRIPE_ZONE."' and zone_country_id = '".$order->delivery['country']['id']."' order by zone_id");
    ...
    function pre_confirmation_check()
    {
        ...
        if ($this->templateClassExists()) {
            $oscTemplate->addBlock($this->getSubmitCardDetailsJavascript(),
                'header_tags');
        }
    }
    ...
    function confirmation()
    {
            ...
            if (tep_db_num_rows($tokens_query) > 0) {
                ...
                while ($tokens = tep_db_fetch_array($tokens_query)) {
                    $content .= '<tr class="moduleRow" id="stripe_card_'.(int) $tokens['id'].'">'.
                        ...
                        '  <td valign="top"><strong>'.tep_output_string_protected($tokens['card_type']).'</strong>&nbsp;&nbsp;****'.tep_output_string_protected($tokens['number_filtered']).'&nbsp;&nbsp;'.tep_output_string_protected(substr($tokens['expiry_date'],
                                0, 2).'/'.substr($tokens['expiry_date'], 2)).'</td>'.
                        '</tr>';
                }
        ...
        $address = array('address_line1' => $order->billing['street_address'],
            ...
            'address_state' => tep_get_zone_name($order->billing['country_id'],
    ...
    function before_process()
    {
        ...
        if (MODULE_PAYMENT_STRIPE_TOKENS == 'True') {
            if (isset($_POST['stripe_card']) && is_numeric($_POST['stripe_card'])
                && ($_POST['stripe_card'] > 0)) {
                ...
                if (tep_db_num_rows($token_query) === 1) {
                    ...
                    $stripe_token_array = explode(':|:', $token['stripe_token'],
                        2);
                    ...
                    tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                            'payment_error='.$this->code.'&error=cardstored',
                            'SSL'));
                }
            }
        }
        ...
        if (empty($params) && isset($_POST['stripeToken']) && !empty($_POST['stripeToken'])) {
            if ((MODULE_PAYMENT_STRIPE_TOKENS == 'True') && isset($_POST['cc_save'])
                && ($_POST['cc_save'] == 'true')) {
                ...
                if ($stripe_customer_id === false) {
                    ...
                    $stripe_card_id = $this->addCard($_POST['stripeToken'],
                        $stripe_customer_id);
                }
        ...
        if (!empty($params)) {
            ...
            $params['capture']  = (MODULE_PAYMENT_STRIPE_TRANSACTION_METHOD == 'Capture')
                    ? 'true' : 'false';

            $stripe_result = json_decode($this->sendTransactionToGateway('https://api.stripe.com/v1/charges',
                    $params), true);

            if (is_array($stripe_result) && !empty($stripe_result)) {
                if (isset($stripe_result['object']) && ($stripe_result['object']
                    == 'charge')) {
                    return true;
                }
            }
    ...
    function after_process()
    {
        ...
        if (MODULE_PAYMENT_STRIPE_TOKENS == 'True') {
            ...
            } elseif (isset($_POST['stripe_card']) && is_numeric($_POST['stripe_card'])
                && ($_POST['stripe_card'] > 0)) {
    ...
    function getParams()
    {
        if (tep_db_num_rows(tep_db_query("show tables like 'customers_stripe_tokens'"))
            != 1) {
            $sql = <<<EOD
...
EOD;
    ...
    function sendTransactionToGateway($url, $parameters = null,
                                      $curl_opts = array())
    {
        ...
        if (MODULE_PAYMENT_STRIPE_VERIFY_SSL == 'True') {
            ...
            if (file_exists(DIR_FS_CATALOG.'ext/modules/payment/stripe/stripe.com.crt')) {
                curl_setopt($curl, CURLOPT_CAINFO,
                    DIR_FS_CATALOG.'ext/modules/payment/stripe/stripe.com.crt');
...
EOD;
    ...
    {
        $stripe_result = json_decode($this->sendTransactionToGateway('https://api.stripe.com/v1/charges/oscommerce_connection_test'),
            true);
...
EOD;
    ...
    function sendDebugEmail($response = array())
    {
        ...
        if (tep_not_null(MODULE_PAYMENT_STRIPE_DEBUG_EMAIL)) {
            ...
            if (!empty($email_body)) {
                tep_mail('', MODULE_PAYMENT_STRIPE_DEBUG_EMAIL,
                    'Stripe Debug E-Mail', trim($email_body), STORE_OWNER,
    ...
    function createCustomer($token)
    {
        ...
        $result = json_decode($this->sendTransactionToGateway('https://api.stripe.com/v1/customers',
                $params), true);

        if (is_array($result) && !empty($result) && isset($result['object']) && ($result['object']
            == 'customer')) {
            ...
            $expiry = tep_db_prepare_input(str_pad($result['cards']['data'][0]['exp_month'],
                    2, '0', STR_PAD_LEFT).$result['cards']['data'][0]['exp_year']);
    ...
    function addCard($token, $customer)
    {
        ...
        $result = json_decode($this->sendTransactionToGateway('https://api.stripe.com/v1/customers/'.$customer.'/cards',
                $params), true);
        ...
        if (is_array($result) && !empty($result) && isset($result['object']) && ($result['object']
            ...
            $expiry = tep_db_prepare_input(str_pad($result['exp_month'], 2, '0',
                    STR_PAD_LEFT).$result['exp_year']);
    ...
    function deleteCard($card, $customer, $token_id)
    {
        ...
        $result = $this->sendTransactionToGateway('https://api.stripe.com/v1/customers/'.$customer.'/cards/'.$card,
            null, array(CURLOPT_CUSTOMREQUEST => 'DELETE'));

        if (!is_array($result) || !isset($result['object']) || ($result['object']
            != 'card')) {
            $this->sendDebugEmail($result);
        }
