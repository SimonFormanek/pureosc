...
// class methods
    function update_status()
    {
        ...
        if (($this->enabled === true) && ((int) MODULE_PAYMENT_PSIGATE_ZONE > 0)) {
            $check_flag  = false;
            $check_query = tep_db_query("select zone_id from ".constant('TABLE_ZONES_TO_GEO_ZONES')." where geo_zone_id = '".MODULE_PAYMENT_PSIGATE_ZONE."' and zone_country_id = '".$order->billing['country']['id']."' order by zone_id");
            ...
            $selection = array('id' => $this->code,
                ...
                'fields' => array(array('title' => MODULE_PAYMENT_PSIGATE_TEXT_CREDIT_CARD_OWNER,
                    ...
                    array('title' => MODULE_PAYMENT_PSIGATE_TEXT_CREDIT_CARD_EXPIRES,
                        'field' => tep_draw_pull_down_menu('psigate_cc_expires_month',
                            $expires_month).'&nbsp;'.tep_draw_pull_down_menu('psigate_cc_expires_year',
                            $expires_year))));
        } else {
            $selection = array('id' => $this->code,
                'module' => $this->title);
    ...
    function pre_confirmation_check()
    {
        ...
        if (MODULE_PAYMENT_PSIGATE_INPUT_MODE == 'Local') {
            ...
            $result        = $cc_validation->validate($_POST['psigate_cc_number'],
                $_POST['psigate_cc_expires_month'],
                $_POST['psigate_cc_expires_year']);

            $error = '';
            switch ($result) {
                case -1:
                    $error = sprintf(TEXT_CCVAL_ERROR_UNKNOWN_CARD,
        ...
        if (MODULE_PAYMENT_PSIGATE_INPUT_MODE == 'Local') {
            $confirmation = array('title' => $this->title.': '.$this->cc_card_type,
                'fields' => array(array('title' => MODULE_PAYMENT_PSIGATE_TEXT_CREDIT_CARD_OWNER,
                        'field' => $order->billing['firstname'].' '.$order->billing['lastname']),
                    array('title' => MODULE_PAYMENT_PSIGATE_TEXT_CREDIT_CARD_NUMBER,
                        'field' => substr($this->cc_card_number, 0, 4).str_repeat('X',
                            (strlen($this->cc_card_number) - 8)).substr($this->cc_card_number,
                            -4)),
                    array('title' => MODULE_PAYMENT_PSIGATE_TEXT_CREDIT_CARD_EXPIRES,
                        'field' => strftime('%B, %Y',
                            mktime(0, 0, 0, $_POST['psigate_cc_expires_month'],
                                1, '20'.$_POST['psigate_cc_expires_year'])))));
    ...
    function process_button()
    {
        ...
        $process_button_string = tep_draw_hidden_field('MerchantID',
            ...
            tep_draw_hidden_field('FullTotal',
                number_format($order->info['total'] * $currencies->get_value(MODULE_PAYMENT_PSIGATE_CURRENCY),
            ...
            tep_draw_hidden_field('NoThanksURL',
                tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                    'payment_error='.$this->code, 'NONSSL', true)).
        ...
        if ($order->billing['country']['iso_code_2'] == 'US') {
            ...
            $process_button_string .= tep_draw_hidden_field('Bstate',
                $billing_state['zone_code']);
        } else {
            $process_button_string .= tep_draw_hidden_field('Bstate',
                $order->billing['state']);
        }

        $process_button_string .= tep_draw_hidden_field('Bzip',
    ...
    function keys()
    {
        return array('MODULE_PAYMENT_PSIGATE_STATUS', 'MODULE_PAYMENT_PSIGATE_MERCHANT_ID',
            'MODULE_PAYMENT_PSIGATE_TRANSACTION_MODE', 'MODULE_PAYMENT_PSIGATE_TRANSACTION_TYPE',
            'MODULE_PAYMENT_PSIGATE_INPUT_MODE', 'MODULE_PAYMENT_PSIGATE_CURRENCY',
            'MODULE_PAYMENT_PSIGATE_ZONE', 'MODULE_PAYMENT_PSIGATE_ORDER_STATUS_ID',
