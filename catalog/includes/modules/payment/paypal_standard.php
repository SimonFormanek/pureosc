...
class paypal_standard
{
    ...
    function paypal_standard()
    {
        ...
        $this->sort_order   = defined('MODULE_PAYMENT_PAYPAL_STANDARD_SORT_ORDER')
                ? MODULE_PAYMENT_PAYPAL_STANDARD_SORT_ORDER : 0;
        $this->enabled      = defined('MODULE_PAYMENT_PAYPAL_STANDARD_STATUS') && (MODULE_PAYMENT_PAYPAL_STANDARD_STATUS
            == 'True') ? true : false;
        $this->order_status = defined('MODULE_PAYMENT_PAYPAL_STANDARD_PREPARE_ORDER_STATUS_ID')
            && ((int) MODULE_PAYMENT_PAYPAL_STANDARD_PREPARE_ORDER_STATUS_ID > 0)
                ? (int) MODULE_PAYMENT_PAYPAL_STANDARD_PREPARE_ORDER_STATUS_ID : 0;
    ...
    function update_status()
    {
        ...
        if (($this->enabled == true) && ((int) MODULE_PAYMENT_PAYPAL_STANDARD_ZONE
            > 0)) {
            $check_flag  = false;
    ...
    function selection()
    {
        ...
        if (tep_session_is_registered('cart_PayPal_Standard_ID')) {
            $order_id = substr($cart_PayPal_Standard_ID,
                strpos($cart_PayPal_Standard_ID, '-') + 1);
    ...
    function confirmation()
    {
        ...
        if (tep_session_is_registered('cartID')) {
            ...
            if (tep_session_is_registered('cart_PayPal_Standard_ID')) {
                ...
                if (($curr['currency'] != $order->info['currency']) || ($cartID != substr($cart_PayPal_Standard_ID,
                        0, strlen($cartID)))) {
                    $check_query = tep_db_query('select orders_id from '.TABLE_ORDERS_STATUS_HISTORY.' where orders_id = "'.(int) $order_id.'" limit 1');
...
// remove shipping tax in total tax value
        ...
        $parameters            = array('cmd' => '_cart',
            ...
            'amount_1' => $this->format_raw($order->info['total'] - $order->info['shipping_cost']
                - $total_tax),
            'currency_code' => $currency,
            'invoice' => substr($cart_PayPal_Standard_ID,
                strpos($cart_PayPal_Standard_ID, '-') + 1),
            'custom' => $customer_id,
            'no_note' => '1',
            'notify_url' => tep_href_link('ext/modules/payment/paypal/standard_ipn.php',
                '', 'SSL', false, false),
            'rm' => '2',
            'return' => tep_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL'),
            'cancel_return' => tep_href_link(FILENAME_CHECKOUT_PAYMENT, '',
                'SSL'),
            'bn' => 'OSCOM23_PS',
            'paymentaction' => ((MODULE_PAYMENT_PAYPAL_STANDARD_TRANSACTION_METHOD
            == 'Sale') ? 'sale' : 'authorization'));

        if (defined('MODULE_PAYMENT_PAYPAL_STANDARD_TEXT_PAYPAL_RETURN_BUTTON') && tep_not_null(MODULE_PAYMENT_PAYPAL_STANDARD_TEXT_PAYPAL_RETURN_BUTTON)
            && (strlen(MODULE_PAYMENT_PAYPAL_STANDARD_TEXT_PAYPAL_RETURN_BUTTON)
            <= 60)) {
        ...
        if (is_numeric($sendto) && ($sendto > 0)) {
            ...
            $parameters['state']            = tep_get_zone_code($order->delivery['country']['id'],
                $order->delivery['zone_id'], $order->delivery['state']);
            ...
            $parameters['state']       = tep_get_zone_code($order->billing['country']['id'],
                $order->billing['zone_id'], $order->billing['state']);
        ...
        foreach ($order->products as $product) {
            if (DISPLAY_PRICE_WITH_TAX == 'true') {
                $product_price = $this->format_raw($product['final_price'] + tep_calculate_tax($product['final_price'],
                        $product['tax']));
            } else {
...
// order totals are processed on checkout confirmation but not captured into a variable
                ...
                if ($GLOBALS[$class]->enabled) {
                    for ($i = 0, $n = sizeof($GLOBALS[$class]->output); $i < $n; $i++) {
                        if (tep_not_null($GLOBALS[$class]->output[$i]['title']) && tep_not_null($GLOBALS[$class]->output[$i]['text'])) {
                            if (!in_array($GLOBALS[$class]->code,
                                    array('ot_subtotal', 'ot_shipping', 'ot_tax',
                                    'ot_total'))) {
        ...
        if (($has_negative_price == false) && ($this->format_raw($paypal_item_total)
        ...
        if (MODULE_PAYMENT_PAYPAL_STANDARD_EWP_STATUS == 'True') {
            ...
            $fp = fopen(MODULE_PAYMENT_PAYPAL_STANDARD_EWP_WORKING_DIRECTORY.'/'.$random_string.'data.txt',
                'w');
            fwrite($fp, $data);
            fclose($fp);
            ...
            if (function_exists('openssl_pkcs7_sign') && function_exists('openssl_pkcs7_encrypt')) {
                openssl_pkcs7_sign(MODULE_PAYMENT_PAYPAL_STANDARD_EWP_WORKING_DIRECTORY.'/'.$random_string.'data.txt',
                    MODULE_PAYMENT_PAYPAL_STANDARD_EWP_WORKING_DIRECTORY.'/'.$random_string.'signed.txt',
                    file_get_contents(MODULE_PAYMENT_PAYPAL_STANDARD_EWP_PUBLIC_KEY),
                    file_get_contents(MODULE_PAYMENT_PAYPAL_STANDARD_EWP_PRIVATE_KEY),
                    array('From' => MODULE_PAYMENT_PAYPAL_STANDARD_ID),
                    PKCS7_BINARY);
...
// remove headers from the signature
                ...
                $fp = fopen(MODULE_PAYMENT_PAYPAL_STANDARD_EWP_WORKING_DIRECTORY.'/'.$random_string.'signed.txt',
                    'w');
                fwrite($fp, $signed);
                fclose($fp);
                ...
                openssl_pkcs7_encrypt(MODULE_PAYMENT_PAYPAL_STANDARD_EWP_WORKING_DIRECTORY.'/'.$random_string.'signed.txt',
                    MODULE_PAYMENT_PAYPAL_STANDARD_EWP_WORKING_DIRECTORY.'/'.$random_string.'encrypted.txt',
                    file_get_contents(MODULE_PAYMENT_PAYPAL_STANDARD_EWP_PAYPAL_KEY),
                    array('From' => MODULE_PAYMENT_PAYPAL_STANDARD_ID),
                    PKCS7_BINARY);
...
// remove headers from the encrypted result
                ...
                $fh   = fopen(MODULE_PAYMENT_PAYPAL_STANDARD_EWP_WORKING_DIRECTORY.'/'.$random_string.'encrypted.txt',
                    'rb');
                $data = fread($fh,
                    filesize(MODULE_PAYMENT_PAYPAL_STANDARD_EWP_WORKING_DIRECTORY.'/'.$random_string.'encrypted.txt'));
                fclose($fh);
    ...
    function before_process()
    {
        ...
        if (isset($_POST['receiver_email']) && (($_POST['receiver_email'] == MODULE_PAYMENT_PAYPAL_STANDARD_ID)
            || (defined('MODULE_PAYMENT_PAYPAL_STANDARD_PRIMARY_ID') && tep_not_null(MODULE_PAYMENT_PAYPAL_STANDARD_PRIMARY_ID)
            && ($_POST['receiver_email'] == MODULE_PAYMENT_PAYPAL_STANDARD_PRIMARY_ID)))) {
            ...
            $result = $this->sendTransactionToGateway($this->form_action_url,
                $parameters);
        }
        ...
        if (!tep_db_num_rows($check_query) || ($order_id != $_POST['invoice']) || ($customer_id
            != $_POST['custom'])) {
            tep_redirect(tep_href_link(FILENAME_SHOPPING_CART));
        }
        ...
        if ((MODULE_PAYMENT_PAYPAL_STANDARD_ORDER_STATUS_ID > 0) && ($check['orders_status']
    ...
    function getParams()
    {
        ...
        $params = array('MODULE_PAYMENT_PAYPAL_STANDARD_STATUS' => array('title' => 'Enable PayPal Payments Standard',
            ...
            'MODULE_PAYMENT_PAYPAL_STANDARD_ORDER_STATUS_ID' => array('title' => 'Set PayPal Acknowledged Order Status',
                ...
                'set_func' => 'tep_cfg_pull_down_order_statuses(',
                'use_func' => 'tep_get_order_status_name'),
            'MODULE_PAYMENT_PAYPAL_STANDARD_TRANSACTIONS_ORDER_STATUS_ID' => array(
    ...
    function sendTransactionToGateway($url, $parameters)
    {
        ...
        if (MODULE_PAYMENT_PAYPAL_STANDARD_VERIFY_SSL == 'True') {
            ...
            if (file_exists(DIR_FS_CATALOG.'ext/modules/payment/paypal/paypal.com.crt')) {
                curl_setopt($curl, CURLOPT_CAINFO,
                    DIR_FS_CATALOG.'ext/modules/payment/paypal/paypal.com.crt');
        ...
        if (tep_not_null(MODULE_PAYMENT_PAYPAL_STANDARD_PROXY)) {
            ...
            curl_setopt($curl, CURLOPT_PROXY,
                MODULE_PAYMENT_PAYPAL_STANDARD_PROXY);
        }
...
// format prices without currency formatting
    ...
    function sendDebugEmail($response = '', $ipn = false)
    {
        ...
        if (tep_not_null(MODULE_PAYMENT_PAYPAL_STANDARD_DEBUG_EMAIL)) {
            ...
            if (!empty($email_body)) {
                tep_mail('', MODULE_PAYMENT_PAYPAL_STANDARD_DEBUG_EMAIL,
                    'PayPal Standard Debug E-Mail'.($ipn == true ? ' (IPN)' : ''),
                    trim($email_body), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
            }
        }
    }
...
EOD;
    ...
    function verifyTransaction($is_ipn = false)
    {
        ...
        if (isset($_POST['invoice']) && is_numeric($_POST['invoice']) && ($_POST['invoice']
            > 0) && isset($_POST['custom']) && is_numeric($_POST['custom']) && ($_POST['custom']
            > 0)) {
            ...
            if (tep_db_num_rows($order_query) === 1) {
                ...
                $comment_status = 'Transaction ID: '.$_POST['txn_id'].'; '.
                    $_POST['payment_status'].' ('.ucfirst($_POST['payer_status']).'; '.$currencies->format($_POST['mc_gross'],
                        false, $_POST['mc_currency']).')';

                if ($_POST['payment_status'] == 'Pending') {
                    $comment_status .= '; '.$_POST['pending_reason'];
                } elseif (($_POST['payment_status'] == 'Reversed') || ($_POST['payment_status']
                    == 'Refunded')) {
                    $comment_status .= '; '.$_POST['reason_code'];
                }

                if ($_POST['mc_gross'] != number_format($total['value'] * $order['currency_value'],
                        $currencies->get_decimal_places($order['currency']))) {
                    $comment_status .= '; PayPal transaction value ('.$_POST['mc_gross'].') does not match order value ('.number_format($total['value']
                            * $order['currency_value'],
                            $currencies->get_decimal_places($order['currency'])).')';
                } elseif ($_POST['payment_status'] == 'Completed') {
                    $new_order_status = (MODULE_PAYMENT_PAYPAL_STANDARD_ORDER_STATUS_ID
                        > 0 ? MODULE_PAYMENT_PAYPAL_STANDARD_ORDER_STATUS_ID : $new_order_status);
                }
