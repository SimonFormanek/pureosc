...
class rbsworldpay_hosted
{
    ...
    function rbsworldpay_hosted()
    {
        ...
        $this->sort_order   = defined('MODULE_PAYMENT_RBSWORLDPAY_HOSTED_SORT_ORDER')
                ? MODULE_PAYMENT_RBSWORLDPAY_HOSTED_SORT_ORDER : 0;
        $this->enabled      = defined('MODULE_PAYMENT_RBSWORLDPAY_HOSTED_STATUS')
            && (MODULE_PAYMENT_RBSWORLDPAY_HOSTED_STATUS == 'True') ? true : false;
        $this->order_status = defined('MODULE_PAYMENT_RBSWORLDPAY_HOSTED_PREPARE_ORDER_STATUS_ID')
            && ((int) MODULE_PAYMENT_RBSWORLDPAY_HOSTED_PREPARE_ORDER_STATUS_ID > 0)
    ...
    function update_status()
    {
        ...
        if (($this->enabled === true) && ((int) MODULE_PAYMENT_RBSWORLDPAY_HOSTED_ZONE
            > 0)) {
            $check_flag  = false;
            $check_query = tep_db_query("select zone_id from ".constant('TABLE_ZONES_TO_GEO_ZONES')." where geo_zone_id = '".MODULE_PAYMENT_RBSWORLDPAY_HOSTED_ZONE."' and zone_country_id = '".$order->billing['country']['id']."' order by zone_id");
    ...
    function selection()
    {
        ...
        if (tep_session_is_registered('cart_RBS_Worldpay_Hosted_ID')) {
            $order_id = substr($cart_RBS_Worldpay_Hosted_ID,
                strpos($cart_RBS_Worldpay_Hosted_ID, '-') + 1);
    ...
    function confirmation()
    {
        ...
        if (tep_session_is_registered('cart_RBS_Worldpay_Hosted_ID')) {
            ...
            if (($curr['currency'] != $order->info['currency']) || ($cartID != substr($cart_RBS_Worldpay_Hosted_ID,
                    0, strlen($cartID)))) {
                $check_query = tep_db_query('select orders_id from '.TABLE_ORDERS_STATUS_HISTORY.' where orders_id = "'.(int) $order_id.'" limit 1');
    ...
    function process_button()
    {
        ...
        $process_button_string = tep_draw_hidden_field('instId',
            ...
            tep_draw_hidden_field('signature',
                md5(MODULE_PAYMENT_RBSWORLDPAY_HOSTED_MD5_PASSWORD.':'.$this->format_raw($order->info['total']).':'.$currency.':'.$order_id)).
            tep_draw_hidden_field('MC_callback',
                tep_href_link('ext/modules/payment/rbsworldpay/hosted_callback.php',
                    '', 'SSL', false)).
            ...
            tep_draw_hidden_field('M_hash',
                md5(tep_session_id().$customer_id.$order_id.$language.number_format($order->info['total'],
                        2).MODULE_PAYMENT_RBSWORLDPAY_HOSTED_MD5_PASSWORD));
    ...
    function before_process()
    {
        ...
        if (!isset($_GET['hash']) || ($_GET['hash'] != md5(tep_session_id().$customer_id.$order_id.$language.number_format($order->info['total'],
                    2).MODULE_PAYMENT_RBSWORLDPAY_HOSTED_MD5_PASSWORD))) {
            $this->sendDebugEmail();
        ...
        $order_status_id = (MODULE_PAYMENT_RBSWORLDPAY_HOSTED_ORDER_STATUS_ID > 0
                ? (int) MODULE_PAYMENT_RBSWORLDPAY_HOSTED_ORDER_STATUS_ID : (int) DEFAULT_ORDERS_STATUS_ID);
        ...
        if ($order['orders_status'] == MODULE_PAYMENT_RBSWORLDPAY_HOSTED_PREPARE_ORDER_STATUS_ID) {
            ...
            if (tep_db_num_rows($order_status_query)) {
                ...
                $sql_data_array = array('customer_notified' => (SEND_EMAILS == 'true')
                        ? '1' : '0',
                    'comments' => $order->info['comments']);

                tep_db_perform(TABLE_ORDERS_STATUS_HISTORY, $sql_data_array,
                    'update',
                    "orders_status_history_id = '".(int) $order_status['orders_status_history_id']."'");
            }
        }
...
// unregister session variables used during checkout
    ...
    function getParams()
    {
        ...
        $params = array('MODULE_PAYMENT_RBSWORLDPAY_HOSTED_STATUS' => array('title' => 'Enable WorldPay Hosted Payment Pages',
            ...
            'MODULE_PAYMENT_RBSWORLDPAY_HOSTED_PREPARE_ORDER_STATUS_ID' => array(
            ...
            'MODULE_PAYMENT_RBSWORLDPAY_HOSTED_ORDER_STATUS_ID' => array('title' => 'Set Order Status',
                ...
                'set_func' => 'tep_cfg_pull_down_order_statuses(',
                'use_func' => 'tep_get_order_status_name'),
            'MODULE_PAYMENT_RBSWORLDPAY_HOSTED_TRANSACTIONS_ORDER_STATUS_ID' => array(
...
// format prices without currency formatting
    ...
    function sendDebugEmail($response = array())
    {
        ...
        if (tep_not_null(MODULE_PAYMENT_RBSWORLDPAY_HOSTED_DEBUG_EMAIL)) {
            ...
            if (!empty($email_body)) {
                tep_mail('', MODULE_PAYMENT_RBSWORLDPAY_HOSTED_DEBUG_EMAIL,
                    'WorldPay Hosted Debug E-Mail', trim($email_body),
                    STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
            }
        }
    }
