...
class paypal_pro_dp
{
    ...
    function paypal_pro_dp()
    {
        ...
        $this->sort_order   = defined('MODULE_PAYMENT_PAYPAL_PRO_DP_SORT_ORDER')
                ? MODULE_PAYMENT_PAYPAL_PRO_DP_SORT_ORDER : 0;
        $this->enabled      = defined('MODULE_PAYMENT_PAYPAL_PRO_DP_STATUS') && (MODULE_PAYMENT_PAYPAL_PRO_DP_STATUS
            == 'True') ? true : false;
        $this->order_status = defined('MODULE_PAYMENT_PAYPAL_PRO_DP_ORDER_STATUS_ID')
            && ((int) MODULE_PAYMENT_PAYPAL_PRO_DP_ORDER_STATUS_ID > 0) ? (int) MODULE_PAYMENT_PAYPAL_PRO_DP_ORDER_STATUS_ID
                : 0;

        if (!defined('MODULE_PAYMENT_INSTALLED') || !tep_not_null(MODULE_PAYMENT_INSTALLED)
            || !in_array('paypal_express.php',
                explode(';', MODULE_PAYMENT_INSTALLED)) || !defined('MODULE_PAYMENT_PAYPAL_EXPRESS_STATUS')
            || (MODULE_PAYMENT_PAYPAL_EXPRESS_STATUS != 'True')) {
        ...
        if ($this->enabled === true) {
            if (!tep_not_null(MODULE_PAYMENT_PAYPAL_PRO_DP_API_USERNAME) || !tep_not_null(MODULE_PAYMENT_PAYPAL_PRO_DP_API_PASSWORD)
                || !tep_not_null(MODULE_PAYMENT_PAYPAL_PRO_DP_API_SIGNATURE)) {
    ...
    function update_status()
    {
        ...
        if (($this->enabled == true) && ((int) MODULE_PAYMENT_PAYPAL_PRO_DP_ZONE
            > 0)) {
            $check_flag  = false;
    ...
    function confirmation()
    {
        ...
        for ($i = $today['year']; $i < $today['year'] + 10; $i++) {
            $year_expires_array[] = array('id' => strftime('%Y',
                    mktime(0, 0, 0, 1, 1, $i)), 'text' => strftime('%Y',
                    mktime(0, 0, 0, 1, 1, $i)));
        }
        ...
        $content = '<table id="paypal_table_new_card" border="0" width="100%" cellspacing="0" cellpadding="2">'.
            ...
            '  <td>'.tep_draw_pull_down_menu('cc_type', $types_array, '',
                'id="paypal_card_type"').'</td>'.
            '</tr>'.
            '<tr>'.
            '  <td width="30%">'.MODULE_PAYMENT_PAYPAL_PRO_DP_CARD_OWNER.'</td>'.
            '  <td>'.tep_draw_input_field('cc_owner',
                $order->billing['firstname'].' '.$order->billing['lastname']).'</td>'.
            '</tr>'.
            '<tr>'.
            '  <td width="30%">'.MODULE_PAYMENT_PAYPAL_PRO_DP_CARD_NUMBER.'</td>'.
            '  <td>'.tep_draw_input_field('cc_number_nh-dns', '',
                'id="paypal_card_num"').'</td>'.
            '</tr>';

        if ((MODULE_PAYMENT_PAYPAL_PRO_DP_CARDTYPE_MAESTRO == 'True') || (MODULE_PAYMENT_PAYPAL_PRO_DP_CARDTYPE_AMEX
            == 'True')) {
            $content .= '<tr>'.
                '  <td width="30%">'.MODULE_PAYMENT_PAYPAL_PRO_DP_CARD_VALID_FROM.'</td>'.
                '  <td>'.tep_draw_pull_down_menu('cc_starts_month',
                    $months_array, '', 'id="paypal_card_date_start"').'&nbsp;'.tep_draw_pull_down_menu('cc_starts_year',
                    $year_valid_from_array).'&nbsp;'.MODULE_PAYMENT_PAYPAL_PRO_DP_CARD_VALID_FROM_INFO.'</td>'.
                '</tr>';
        }
        ...
        if (MODULE_PAYMENT_PAYPAL_PRO_DP_CARDTYPE_MAESTRO == 'True') {
            $content .= '<tr>'.
                ...
                '  <td>'.tep_draw_input_field('cc_issue_nh-dns', '',
                    'id="paypal_card_issue" size="3" maxlength="2"').'&nbsp;'.MODULE_PAYMENT_PAYPAL_PRO_DP_CARD_ISSUE_NUMBER_INFO.'</td>'.
                '</tr>';
        }
    ...
    function before_process()
    {
        ...
        if (isset($_POST['cc_owner']) && !empty($_POST['cc_owner']) && isset($_POST['cc_type'])
            && $this->isCardAccepted($_POST['cc_type']) && isset($_POST['cc_number_nh-dns'])
            && !empty($_POST['cc_number_nh-dns'])) {
            ...
            $params = array('USER' => MODULE_PAYMENT_PAYPAL_PRO_DP_API_USERNAME,
                ...
                'PAYMENTACTION' => ((MODULE_PAYMENT_PAYPAL_PRO_DP_TRANSACTION_METHOD
                ...
                'FIRSTNAME' => substr($_POST['cc_owner'], 0,
                    strpos($_POST['cc_owner'], ' ')),
                'LASTNAME' => substr($_POST['cc_owner'],
                    strpos($_POST['cc_owner'], ' ') + 1),
                'STREET' => $order->billing['street_address'],
                'CITY' => $order->billing['city'],
                'STATE' => tep_get_zone_code($order->billing['country']['id'],
            ...
            if (is_numeric($sendto) && ($sendto > 0)) {
                ...
                $params['SHIPTOSTATE']       = tep_get_zone_code($order->delivery['country']['id'],
            ...
            foreach ($order_totals as $ot) {
                if (!in_array($ot['code'],
                        array('ot_subtotal', 'ot_shipping', 'ot_tax', 'ot_total'))) {
            ...
            if ($this->format_raw($item_params['ITEMAMT'] + $item_params['TAXAMT']
                    + $item_params['SHIPPINGAMT']) == $params['AMT']) {
            ...
            $response       = $this->sendTransactionToGateway($api_url,
                $post_string);
            $response_array = array();
            parse_str($response, $response_array);

            if (($response_array['ACK'] != 'Success') && ($response_array['ACK']
                != 'SuccessWithWarning')) {
                $this->sendDebugEmail($response_array);

                tep_redirect(tep_href_link(FILENAME_SHOPPING_CART,
                        'error_message='.stripslashes($response_array['L_LONGMESSAGE0']),
                        'SSL'));
            }
        } else {
            tep_redirect(tep_href_link(FILENAME_CHECKOUT_CONFIRMATION,
                    'error_message='.MODULE_PAYMENT_PAYPAL_PRO_DP_ERROR_ALL_FIELDS_REQUIRED,
                    'SSL'));
        }
    }
    ...
    function getParams()
    {
        ...
        $params = array('MODULE_PAYMENT_PAYPAL_PRO_DP_STATUS' => array('title' => 'Enable PayPal Payments Pro (Direct Payment)',
            ...
            'MODULE_PAYMENT_PAYPAL_PRO_DP_TRANSACTIONS_ORDER_STATUS_ID' => array(
...
// format prices without currency formatting
    ...
    {
        return (isset($this->cc_types[$card]) && defined('MODULE_PAYMENT_PAYPAL_PRO_DP_CARDTYPE_'.$card)
            && (constant('MODULE_PAYMENT_PAYPAL_PRO_DP_CARDTYPE_'.$card) == 'True'));
    }
...
EOD;
    ...
    function sendDebugEmail($response = array())
    {
        ...
        if (tep_not_null(MODULE_PAYMENT_PAYPAL_PRO_DP_DEBUG_EMAIL)) {
            ...
            if (!empty($email_body)) {
                tep_mail('', MODULE_PAYMENT_PAYPAL_PRO_DP_DEBUG_EMAIL,
                    'PayPal Payments Pro (Direct Payment) Debug E-Mail',
