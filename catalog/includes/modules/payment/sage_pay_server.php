...
class sage_pay_server
{
    ...
    function sage_pay_server()
    {
        ...
        $this->sort_order   = defined('MODULE_PAYMENT_SAGE_PAY_SERVER_SORT_ORDER')
                ? MODULE_PAYMENT_SAGE_PAY_SERVER_SORT_ORDER : 0;
        $this->enabled      = defined('MODULE_PAYMENT_SAGE_PAY_SERVER_STATUS') && (MODULE_PAYMENT_SAGE_PAY_SERVER_STATUS
            == 'True') ? true : false;
        $this->order_status = defined('MODULE_PAYMENT_SAGE_PAY_SERVER_ORDER_STATUS_ID')
            && ((int) MODULE_PAYMENT_SAGE_PAY_SERVER_ORDER_STATUS_ID > 0) ? (int) MODULE_PAYMENT_SAGE_PAY_SERVER_ORDER_STATUS_ID
                : 0;
    ...
    function update_status()
    {
        ...
        if (($this->enabled === true) && ((int) MODULE_PAYMENT_SAGE_PAY_SERVER_ZONE
            > 0)) {
            $check_flag  = false;
            $check_query = tep_db_query("select zone_id from ".constant('TABLE_ZONES_TO_GEO_ZONES')." where geo_zone_id = '".MODULE_PAYMENT_SAGE_PAY_SERVER_ZONE."' and zone_country_id = '".$order->billing['country']['id']."' order by zone_id");
    ...
    function before_process()
    {
        ...
        if (isset($_GET['check']) && ($_GET['check'] == 'PROCESS')) {
            if (isset($_GET['skcode']) && tep_session_is_registered('sagepay_server_skey_code')
                && ($_GET['skcode'] == $sagepay_server_skey_code)) {
            ...
            $params = array('VPSProtocol' => $this->api_version,
                ...
                'Vendor' => substr(MODULE_PAYMENT_SAGE_PAY_SERVER_VENDOR_LOGIN_NAME,
                    0, 15),
                ...
                'NotificationURL' => $this->formatURL(tep_href_link('ext/modules/payment/sage_pay/server.php',
                        'check=SERVER&skcode='.$sagepay_server_skey_code, 'SSL',
                        false)),
...
// don't show an error message for user cancelled/aborted transactions
    ...
    function getParams()
    {
        if (tep_db_num_rows(tep_db_query("show tables like 'sagepay_server_securitykeys'"))
            != 1) {
            $sql = <<<EOD
...
EOD;
        ...
        $params = array('MODULE_PAYMENT_SAGE_PAY_SERVER_STATUS' => array('title' => 'Enable Sage Pay Server Module',
            ...
            'MODULE_PAYMENT_SAGE_PAY_SERVER_TRANSACTION_ORDER_STATUS_ID' => array(
...
EOD;
    ...
    function sendDebugEmail($response = array())
    {
        ...
        if (tep_not_null(MODULE_PAYMENT_SAGE_PAY_SERVER_DEBUG_EMAIL)) {
            ...
            if (!empty($email_body)) {
                tep_mail('', MODULE_PAYMENT_SAGE_PAY_SERVER_DEBUG_EMAIL,
                    'Sage Pay Server Debug E-Mail', trim($email_body),
