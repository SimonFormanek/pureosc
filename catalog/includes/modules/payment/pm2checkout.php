...
// class methods
    function update_status()
    {
        ...
        if (($this->enabled === true) && ((int) MODULE_PAYMENT_2CHECKOUT_ZONE > 0)) {
            $check_flag  = false;
            $check_query = tep_db_query("select zone_id from ".constant('TABLE_ZONES_TO_GEO_ZONES')." where geo_zone_id = '".MODULE_PAYMENT_2CHECKOUT_ZONE."' and zone_country_id = '".$order->billing['country']['id']."' order by zone_id");
    ...
    function selection()
    {
        return array('id' => $this->code,
            'module' => $this->public_title.(strlen(MODULE_PAYMENT_2CHECKOUT_TEXT_PUBLIC_DESCRIPTION)
    ...
    function process_button()
    {
        ...
        $process_button_string = tep_draw_hidden_field('sid',
            ...
            tep_draw_hidden_field('cart_order_id',
                date('YmdHis').'-'.$customer_id.'-'.$cartID).
            ...
            tep_draw_hidden_field('street_address',
            ...
            tep_draw_hidden_field('ship_street_address',
        ...
        for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
            $process_button_string .= tep_draw_hidden_field('c_prod_'.($i + 1),
                    (int) $order->products[$i]['id'].','.(int) $order->products[$i]['qty']).
                tep_draw_hidden_field('c_name_'.($i + 1),
                    $order->products[$i]['name']).
                tep_draw_hidden_field('c_description_'.($i + 1),
                    $order->products[$i]['name']).
                tep_draw_hidden_field('c_price_'.($i + 1),
                    $this->format_raw(tep_add_tax($order->products[$i]['final_price'],
        ...
        $process_button_string .= tep_draw_hidden_field('return_url',
        ...
        $process_button_string .= tep_draw_hidden_field('cart_brand_name',
                'oscommerce').
    ...
    function before_process()
    {
        ...
        if (($_POST['credit_card_processed'] != 'Y') && ($_POST['credit_card_processed']
            != 'K')) {
            tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                    'payment_error='.$this->code, 'SSL', true, false));
        }
    }
...
// The KEY value returned from the gateway is intentionally broken for Test transactions so it is only checked in Production mode
        if (tep_not_null(MODULE_PAYMENT_2CHECKOUT_SECRET_WORD) && (MODULE_PAYMENT_2CHECKOUT_TESTMODE
            == 'Production')) {
            if (strtoupper(md5(MODULE_PAYMENT_2CHECKOUT_SECRET_WORD.MODULE_PAYMENT_2CHECKOUT_LOGIN.$_POST['order_number'].$this->order_format($order->info['total'],
                            MODULE_PAYMENT_2CHECKOUT_CURRENCY))) != strtoupper($_POST['key'])) {
                $sql_data_array = array('orders_id' => (int) $insert_id,
                    'orders_status_id' => (int) $order->info['order_status'],
                    'date_added' => 'now()',
                    'customer_notified' => '0',
    ...
    function keys()
    {
        return array('MODULE_PAYMENT_2CHECKOUT_STATUS', 'MODULE_PAYMENT_2CHECKOUT_LOGIN',
            'MODULE_PAYMENT_2CHECKOUT_TESTMODE', 'MODULE_PAYMENT_2CHECKOUT_SECRET_WORD',
            'MODULE_PAYMENT_2CHECKOUT_ROUTINE', 'MODULE_PAYMENT_2CHECKOUT_CURRENCY',
