...
// class constructor
    function paypoint_secpay()
    {
        ...
        $this->enabled      = ((MODULE_PAYMENT_PAYPOINT_SECPAY_STATUS == 'True')
                ? true : false);
...
// class methods
    function update_status()
    {
        ...
        if (($this->enabled === true) && ((int) MODULE_PAYMENT_PAYPOINT_SECPAY_ZONE
            > 0)) {
            $check_flag  = false;
            $check_query = tep_db_query("select zone_id from ".constant('TABLE_ZONES_TO_GEO_ZONES')." where geo_zone_id = '".MODULE_PAYMENT_PAYPOINT_SECPAY_ZONE."' and zone_country_id = '".$order->billing['country']['id']."' order by zone_id");
...
// Calculate the digest to send to SECPAY
        ...
        $digest_string = STORE_NAME.date('Ymdhis').number_format($order->info['total']
...
// Incase this gets 'fixed' at the SECPay end do a search and replace on the trans_id too
        ...
        $process_button_string = tep_draw_hidden_field('merchant',
            ...
            tep_draw_hidden_field('ship_addr_1',
            ...
            tep_draw_hidden_field('callback',
                tep_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL', false).';'.tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                    'payment_error='.$this->code, 'SSL', false)).
            tep_draw_hidden_field(tep_session_name(), tep_session_id()).
            tep_draw_hidden_field('options',
                'test_status='.$test_status.',dups=false,cb_flds='.tep_session_name()).
    ...
    function before_process()
    {
        ...
        if (($_GET['valid'] == 'true') && ($_GET['code'] == 'A') && !empty($_GET['auth_code'])
            && empty($_GET['resp_code']) && !empty($_GET[tep_session_name()])) {
            $DIGEST_PASSWORD = MODULE_PAYMENT_PAYPOINT_SECPAY_READERS_DIGEST;
            list($REQUEST_URI, $CHECK_SUM) = split('hash=',
                $HTTP_SERVER_VARS['REQUEST_URI']);

            if ($_GET['hash'] != md5($REQUEST_URI.$DIGEST_PASSWORD)) {
                tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                        tep_session_name().'='.$_GET[tep_session_name()].'&payment_error='.$this->code."&detail=hash",
                        'SSL', false, false));
            }
        } else {
            tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                    tep_session_name().'='.$_GET[tep_session_name()].'&payment_error='.$this->code,
                    'SSL', false, false));
        }
    }
    ...
    function keys()
    {
        return array('MODULE_PAYMENT_PAYPOINT_SECPAY_STATUS', 'MODULE_PAYMENT_PAYPOINT_SECPAY_MERCHANT_ID',
            'MODULE_PAYMENT_PAYPOINT_SECPAY_REMOTE', 'MODULE_PAYMENT_PAYPOINT_SECPAY_READERS_DIGEST',
            'MODULE_PAYMENT_PAYPOINT_SECPAY_CURRENCY', 'MODULE_PAYMENT_PAYPOINT_SECPAY_TEST_STATUS',
            'MODULE_PAYMENT_PAYPOINT_SECPAY_ZONE', 'MODULE_PAYMENT_PAYPOINT_SECPAY_ORDER_STATUS_ID',
