...
// class constructor
    function ipayment_elv()
    {
        ...
        $this->enabled      = ((MODULE_PAYMENT_IPAYMENT_ELV_STATUS == 'True') ? true
                : false);
...
// class methods
    function update_status()
    {
        ...
        if (($this->enabled === true) && ((int) MODULE_PAYMENT_IPAYMENT_ELV_ZONE > 0)) {
            $check_flag  = false;
            $check_query = tep_db_query("select zone_id from ".constant('TABLE_ZONES_TO_GEO_ZONES')." where geo_zone_id = '".MODULE_PAYMENT_IPAYMENT_ELV_ZONE."' and zone_country_id = '".$order->billing['country']['id']."' order by zone_id");
    ...
    function process_button()
    {
        ...
        $process_button_string = tep_draw_hidden_field('silent', '1').
            ...
            tep_draw_hidden_field('trxuser_id',
                MODULE_PAYMENT_IPAYMENT_ELV_USER_ID).
            tep_draw_hidden_field('trxpassword',
                MODULE_PAYMENT_IPAYMENT_ELV_PASSWORD).
            ...
            tep_draw_hidden_field('trx_typ',
                ((MODULE_PAYMENT_IPAYMENT_ELV_TRANSACTION_METHOD == 'Capture') ? 'auth'
                    : 'preauth')).
            ...
            tep_draw_hidden_field('hidden_trigger_url',
                tep_href_link('ext/modules/payment/ipayment/callback_elv.php',
                    '', 'SSL', false)).
        ...
        if (tep_not_null(MODULE_PAYMENT_IPAYMENT_ELV_SECRET_HASH_PASSWORD)) {
            $process_button_string .= tep_draw_hidden_field('trx_securityhash',
                md5(MODULE_PAYMENT_IPAYMENT_ELV_USER_ID.($this->format_raw($order->info['total'])
                    * 100).$currency.MODULE_PAYMENT_IPAYMENT_ELV_PASSWORD.MODULE_PAYMENT_IPAYMENT_ELV_SECRET_HASH_PASSWORD));
        }
...
// verify ret_param_checksum
            if ($_GET['ret_param_checksum'] != md5(MODULE_PAYMENT_IPAYMENT_ELV_USER_ID.($this->format_raw($order->info['total'])
                    * 100).$currency.$_GET['ret_authcode'].$_GET['ret_booknr'].MODULE_PAYMENT_IPAYMENT_ELV_SECRET_HASH_PASSWORD)) {
                $pass = false;
            }
...
// verify ret_url_checksum
    ...
    function get_error()
    {
        ...
        $error = array('title' => MODULE_PAYMENT_IPAYMENT_ELV_ERROR_HEADING,
            'error' => ((isset($_GET['error'])) ? stripslashes(urldecode($_GET['error']))
                : MODULE_PAYMENT_IPAYMENT_ELV_ERROR_MESSAGE));
    ...
    function keys()
    {
        return array('MODULE_PAYMENT_IPAYMENT_ELV_STATUS', 'MODULE_PAYMENT_IPAYMENT_ELV_ID',
            'MODULE_PAYMENT_IPAYMENT_ELV_USER_ID', 'MODULE_PAYMENT_IPAYMENT_ELV_PASSWORD',
            'MODULE_PAYMENT_IPAYMENT_ELV_TRANSACTION_METHOD', 'MODULE_PAYMENT_IPAYMENT_ELV_SECRET_HASH_PASSWORD',
            'MODULE_PAYMENT_IPAYMENT_ELV_DEBUG_EMAIL', 'MODULE_PAYMENT_IPAYMENT_ELV_ZONE',
            'MODULE_PAYMENT_IPAYMENT_ELV_ORDER_STATUS_ID', 'MODULE_PAYMENT_IPAYMENT_ELV_SORT_ORDER');
    }
...
// format prices without currency formatting
    ...
    function sendDebugEmail($checksum_match = 0)
    {
        ...
        if (tep_not_null(MODULE_PAYMENT_IPAYMENT_ELV_DEBUG_EMAIL)) {
            ...
            tep_mail('', MODULE_PAYMENT_IPAYMENT_ELV_DEBUG_EMAIL,
                'iPayment (ELV) Transaction', $email_body, STORE_OWNER,
