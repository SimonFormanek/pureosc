...
class authorizenet_cc_aim
{
    ...
    function authorizenet_cc_aim()
    {
        ...
        $this->sort_order   = defined('MODULE_PAYMENT_AUTHORIZENET_CC_AIM_SORT_ORDER')
                ? MODULE_PAYMENT_AUTHORIZENET_CC_AIM_SORT_ORDER : 0;
        $this->enabled      = defined('MODULE_PAYMENT_AUTHORIZENET_CC_AIM_STATUS')
            && (MODULE_PAYMENT_AUTHORIZENET_CC_AIM_STATUS == 'True') ? true : false;
        $this->order_status = defined('MODULE_PAYMENT_AUTHORIZENET_CC_AIM_ORDER_STATUS_ID')
            && ((int) MODULE_PAYMENT_AUTHORIZENET_CC_AIM_ORDER_STATUS_ID > 0) ? (int) MODULE_PAYMENT_AUTHORIZENET_CC_AIM_ORDER_STATUS_ID
                : 0;

        if (defined('MODULE_PAYMENT_AUTHORIZENET_CC_AIM_STATUS')) {
            if ((MODULE_PAYMENT_AUTHORIZENET_CC_AIM_TRANSACTION_SERVER == 'Test')
                || (MODULE_PAYMENT_AUTHORIZENET_CC_AIM_TRANSACTION_MODE == 'Test')) {
                $this->title        .= ' [Test]';
    ...
    function confirmation()
    {
        ...
        for ($i = 1; $i < 13; $i++) {
            $expires_month[] = array('id' => sprintf('%02d', $i), 'text' => sprintf('%02d',
                    $i));
        }
        ...
        $confirmation = array('fields' => array(array('title' => MODULE_PAYMENT_AUTHORIZENET_CC_AIM_CREDIT_CARD_OWNER_FIRSTNAME,
                    'field' => tep_draw_input_field('cc_owner_firstname',
                        $order->billing['firstname'])),
                array('title' => MODULE_PAYMENT_AUTHORIZENET_CC_AIM_CREDIT_CARD_OWNER_LASTNAME,
                    'field' => tep_draw_input_field('cc_owner_lastname',
                ...
                array('title' => MODULE_PAYMENT_AUTHORIZENET_CC_AIM_CREDIT_CARD_EXPIRES,
                    'field' => tep_draw_pull_down_menu('cc_expires_month',
                        $expires_month).'&nbsp;'.tep_draw_pull_down_menu('cc_expires_year',
                        $expires_year)),
                array('title' => MODULE_PAYMENT_AUTHORIZENET_CC_AIM_CREDIT_CARD_CCV,
                    'field' => tep_draw_input_field('cc_ccv_nh-dns', '',
                        'size="5" maxlength="4"'))));
    ...
    function before_process()
    {
        ...
        $params = array('x_login' => substr(MODULE_PAYMENT_AUTHORIZENET_CC_AIM_LOGIN_ID,
                0, 20),
            'x_tran_key' => substr(MODULE_PAYMENT_AUTHORIZENET_CC_AIM_TRANSACTION_KEY,
                0, 16),
            'x_version' => $this->api_version,
            'x_type' => ((MODULE_PAYMENT_AUTHORIZENET_CC_AIM_TRANSACTION_METHOD == 'Capture')
                ? 'AUTH_CAPTURE' : 'AUTH_ONLY'),
            'x_method' => 'CC',
            'x_amount' => substr($this->format_raw($order->info['total']), 0, 15),
            'x_currency_code' => substr($currency, 0, 3),
            'x_card_num' => substr(preg_replace('/[^0-9]/', '',
                    $_POST['cc_number_nh-dns']), 0, 22),
        ...
        for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
            $post_string .= '&x_line_item='.urlencode($i + 1).'<|>'.urlencode(substr($order->products[$i]['name'],
                        0, 31)).'<|>'.urlencode(substr($order->products[$i]['name'],
                        0, 255)).'<|>'.urlencode($order->products[$i]['qty']).'<|>'.urlencode($this->format_raw($order->products[$i]['final_price'])).'<|>'.urlencode($order->products[$i]['tax']
                    > 0 ? 'YES' : 'NO');
        }
        ...
        if (($response['x_response_code'] == '1') || ($response['x_response_code']
            == '4')) {
            if ((tep_not_null(MODULE_PAYMENT_AUTHORIZENET_CC_AIM_MD5_HASH) && (strtoupper($response['x_MD5_Hash'])
                != strtoupper(md5(MODULE_PAYMENT_AUTHORIZENET_CC_AIM_MD5_HASH.MODULE_PAYMENT_AUTHORIZENET_CC_AIM_LOGIN_ID.$response['x_trans_id'].$this->format_raw($order->info['total'])))))
                || ($response['x_amount'] != $this->format_raw($order->info['total']))) {
    ...
    function getParams()
    {
        ...
        $params = array('MODULE_PAYMENT_AUTHORIZENET_CC_AIM_STATUS' => array('title' => 'Enable Authorize.net Advanced Integration Method',
            ...
            'MODULE_PAYMENT_AUTHORIZENET_CC_AIM_REVIEW_ORDER_STATUS_ID' => array(
                'title' => 'Review Order Status',
                'desc' => 'Set the status of orders flagged as being under review to this value',
                'value' => '0',
                'use_func' => 'tep_get_order_status_name',
                'set_func' => 'tep_cfg_pull_down_order_statuses('),
            'MODULE_PAYMENT_AUTHORIZENET_CC_AIM_TRANSACTION_ORDER_STATUS_ID' => array(
    ...
    function sendTransactionToGateway($url, $parameters)
    {
        ...
        if (MODULE_PAYMENT_AUTHORIZENET_CC_AIM_VERIFY_SSL == 'True') {
            ...
            if (file_exists(DIR_FS_CATALOG.'ext/modules/payment/authorizenet/authorize.net.crt')) {
                curl_setopt($curl, CURLOPT_CAINFO,
                    DIR_FS_CATALOG.'ext/modules/payment/authorizenet/authorize.net.crt');
        ...
        if (tep_not_null(MODULE_PAYMENT_AUTHORIZENET_CC_AIM_PROXY)) {
            ...
            curl_setopt($curl, CURLOPT_PROXY,
                MODULE_PAYMENT_AUTHORIZENET_CC_AIM_PROXY);
        }
...
// format prices without currency formatting
    ...
    function sendDebugEmail($response = array())
    {
        ...
        if (tep_not_null(MODULE_PAYMENT_AUTHORIZENET_CC_AIM_DEBUG_EMAIL)) {
            ...
            if (!empty($email_body)) {
                tep_mail('', MODULE_PAYMENT_AUTHORIZENET_CC_AIM_DEBUG_EMAIL,
                    'Authorize.net AIM Debug E-Mail', trim($email_body),
