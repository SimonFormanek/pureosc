...
class authorizenet_cc_sim
{
    ...
    function authorizenet_cc_sim()
    {
        ...
        $this->sort_order   = defined('MODULE_PAYMENT_AUTHORIZENET_CC_SIM_SORT_ORDER')
                ? MODULE_PAYMENT_AUTHORIZENET_CC_SIM_SORT_ORDER : 0;
        $this->enabled      = defined('MODULE_PAYMENT_AUTHORIZENET_CC_SIM_STATUS')
            && (MODULE_PAYMENT_AUTHORIZENET_CC_SIM_STATUS == 'True') ? true : false;
        $this->order_status = defined('MODULE_PAYMENT_AUTHORIZENET_CC_SIM_ORDER_STATUS_ID')
            && ((int) MODULE_PAYMENT_AUTHORIZENET_CC_SIM_ORDER_STATUS_ID > 0) ? (int) MODULE_PAYMENT_AUTHORIZENET_CC_SIM_ORDER_STATUS_ID
                : 0;

        if (defined('MODULE_PAYMENT_AUTHORIZENET_CC_SIM_STATUS')) {
            if ((MODULE_PAYMENT_AUTHORIZENET_CC_SIM_TRANSACTION_SERVER == 'Test')
                || (MODULE_PAYMENT_AUTHORIZENET_CC_SIM_TRANSACTION_MODE == 'Test')) {
                $this->title        .= ' [Test]';
    ...
    function update_status()
    {
        ...
        if (($this->enabled === true) && ((int) MODULE_PAYMENT_AUTHORIZENET_CC_SIM_ZONE
            > 0)) {
            $check_flag  = false;
            $check_query = tep_db_query("select zone_id from ".cfg('TABLE_ZONES_TO_GEO_ZONES')." where geo_zone_id = '".MODULE_PAYMENT_AUTHORIZENET_CC_SIM_ZONE."' and zone_country_id = '".$order->billing['country']['id']."' order by zone_id");
    ...
    function process_button()
    {
        ...
        $params = array('x_login' => substr(MODULE_PAYMENT_AUTHORIZENET_CC_SIM_LOGIN_ID,
                0, 20),
            'x_version' => $this->api_version,
            'x_show_form' => 'PAYMENT_FORM',
            'x_delim_data' => 'FALSE',
            'x_relay_response' => 'TRUE',
            'x_relay_url' => tep_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL',
                false),
            ...
            'x_phone' => substr(preg_replace('/[^0-9]/', '',
                    $order->customer['telephone']), 0, 25),
            ...
            'x_method' => 'CC',
            'x_type' => MODULE_PAYMENT_AUTHORIZENET_CC_SIM_TRANSACTION_METHOD == 'Capture'
                ? 'AUTH_CAPTURE' : 'AUTH_ONLY',
            'x_freight' => $this->format_raw($order->info['shipping_cost']),
            'x_fp_sequence' => $sequence,
            'x_fp_timestamp' => $tstamp,
            'x_fp_hash' => $this->_hmac(MODULE_PAYMENT_AUTHORIZENET_CC_SIM_TRANSACTION_KEY,
                MODULE_PAYMENT_AUTHORIZENET_CC_SIM_LOGIN_ID.'^'.$sequence.'^'.$tstamp.'^'.$this->format_raw($order->info['total']).'^'.$currency),
        ...
        if (is_numeric($sendto) && ($sendto > 0)) {
            $params['x_ship_to_first_name'] = substr($order->delivery['firstname'],
                0, 50);
            $params['x_ship_to_last_name']  = substr($order->delivery['lastname'],
                0, 50);
            $params['x_ship_to_company']    = substr($order->delivery['company'],
                0, 50);
            $params['x_ship_to_address']    = substr($order->delivery['street_address'],
                0, 60);
            $params['x_ship_to_city']       = substr($order->delivery['city'],
                0, 40);
            $params['x_ship_to_state']      = substr($order->delivery['state'],
                0, 40);
            $params['x_ship_to_zip']        = substr($order->delivery['postcode'],
                0, 20);
            $params['x_ship_to_country']    = substr($order->delivery['country']['title'],
                0, 60);
        }
        ...
        for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
            $process_button_string .= tep_draw_hidden_field('x_line_item',
                ($i + 1).'<|>'.substr($order->products[$i]['name'], 0, 31).'<|><|>'.$order->products[$i]['qty'].'<|>'.$this->format_raw($order->products[$i]['final_price']).'<|>'.($order->products[$i]['tax']
                > 0 ? 'YES' : 'NO'));
        }
    ...
    function before_process()
    {
        ...
        foreach ($check_array as $check) {
            if (!isset($_POST[$check]) || !is_string($_POST[$check]) || (strlen($_POST[$check])
                < 1)) {
                $error = 'general';
                break;
            }
        ...
        if ($error === false) {
            if (($_POST['x_response_code'] == '1') || ($_POST['x_response_code']
                == '4')) {
                if (tep_not_null(MODULE_PAYMENT_AUTHORIZENET_CC_SIM_MD5_HASH) && (!isset($_POST['x_MD5_Hash'])
                    || (strtoupper($_POST['x_MD5_Hash']) != strtoupper(md5(MODULE_PAYMENT_AUTHORIZENET_CC_SIM_MD5_HASH.MODULE_PAYMENT_AUTHORIZENET_CC_SIM_LOGIN_ID.$_POST['x_trans_id'].$this->format_raw($order->info['total'])))))) {
                    $error = 'verification';
                ...
                if (($error === false) && ($_POST['x_response_code'] == '4')) {
                    if (MODULE_PAYMENT_AUTHORIZENET_CC_SIM_REVIEW_ORDER_STATUS_ID
                        > 0) {
        ...
        if ($error !== false) {
            ...
            tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                    'payment_error='.$this->code.'&error='.$error, 'SSL'));
        }
    ...
    function after_process()
    {
        ...
        if (isset($_POST['x_avs_code']) && is_string($_POST['x_avs_code']) && !empty($_POST['x_avs_code'])) {
            if (defined('MODULE_PAYMENT_AUTHORIZENET_CC_SIM_TEXT_AVS_'.$_POST['x_avs_code'])) {
                $avs_response = cfg('MODULE_PAYMENT_AUTHORIZENET_CC_SIM_TEXT_AVS_'.$_POST['x_avs_code']).' ('.$_POST['x_avs_code'].')';
            } else {
        ...
        if (isset($_POST['x_cvv2_resp_code']) && is_string($_POST['x_cvv2_resp_code'])
            ...
            if (defined('MODULE_PAYMENT_AUTHORIZENET_CC_SIM_TEXT_CVV2_'.$_POST['x_cvv2_resp_code'])) {
                $cvv2_response = cfg('MODULE_PAYMENT_AUTHORIZENET_CC_SIM_TEXT_CVV2_'.$_POST['x_cvv2_resp_code']).' ('.$_POST['x_cvv2_resp_code'].')';
            } else {
        ...
        if (isset($_POST['x_cavv_response']) && is_string($_POST['x_cavv_response'])
            ...
            if (defined('MODULE_PAYMENT_AUTHORIZENET_CC_SIM_TEXT_CAVV_'.$_POST['x_cavv_response'])) {
                $cavv_response = cfg('MODULE_PAYMENT_AUTHORIZENET_CC_SIM_TEXT_CAVV_'.$_POST['x_cavv_response']).' ('.$_POST['x_cavv_response'].')';
            } else {
...
EOD;
    ...
    function getParams()
    {
        ...
        $params = array('MODULE_PAYMENT_AUTHORIZENET_CC_SIM_STATUS' => array('title' => 'Enable Authorize.net Server Integration Method',
            ...
            'MODULE_PAYMENT_AUTHORIZENET_CC_SIM_REVIEW_ORDER_STATUS_ID' => array(
                'title' => 'Review Order Status',
                'desc' => 'Set the status of orders flagged as being under review to this value',
                'value' => '0',
                'use_func' => 'tep_get_order_status_name',
                'set_func' => 'tep_cfg_pull_down_order_statuses('),
            'MODULE_PAYMENT_AUTHORIZENET_CC_SIM_TRANSACTION_ORDER_STATUS_ID' => array(
...
// format prices without currency formatting
    ...
    function sendDebugEmail($response = array())
    {
        ...
        if (tep_not_null(MODULE_PAYMENT_AUTHORIZENET_CC_SIM_DEBUG_EMAIL)) {
            ...
            if (!empty($email_body)) {
                tep_mail('', MODULE_PAYMENT_AUTHORIZENET_CC_SIM_DEBUG_EMAIL,
                    'Authorize.net SIM Debug E-Mail', trim($email_body),
