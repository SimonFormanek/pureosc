...
// class constructor
    function moneybookers_sft()
    {
        ...
        $this->enabled      = ((MODULE_PAYMENT_MONEYBOOKERS_SFT_STATUS == 'True')
                ? true : false);
...
// class methods
    function update_status()
    {
        ...
        if (!defined('MODULE_PAYMENT_MONEYBOOKERS_STATUS')) {
            $this->enabled = false;
        } elseif (($this->enabled === true) && ((int) MODULE_PAYMENT_MONEYBOOKERS_SFT_ZONE
            > 0)) {
            $check_flag  = false;
            $check_query = tep_db_query("select zone_id from ".constant('TABLE_ZONES_TO_GEO_ZONES')." where geo_zone_id = '".MODULE_PAYMENT_MONEYBOOKERS_SFT_ZONE."' and zone_country_id = '".$order->billing['country']['id']."' order by zone_id");
    ...
    function keys()
    {
        return array('MODULE_PAYMENT_MONEYBOOKERS_SFT_STATUS', 'MODULE_PAYMENT_MONEYBOOKERS_SFT_ZONE',
            'MODULE_PAYMENT_MONEYBOOKERS_SFT_PREPARE_ORDER_STATUS_ID', 'MODULE_PAYMENT_MONEYBOOKERS_SFT_TRANSACTIONS_ORDER_STATUS_ID',
            'MODULE_PAYMENT_MONEYBOOKERS_SFT_ORDER_STATUS_ID', 'MODULE_PAYMENT_MONEYBOOKERS_SFT_SORT_ORDER');
    }
}
