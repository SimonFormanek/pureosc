...
/**
 * GPWebPay payment with FlexiBee support module 
 * @author Vitex
 */
class phgpwebpay {
    ...
    /**
     * GPWebPay with FlexiBee support
     * 
     * @global order $order
     */
    public function __construct() {
        global $order;
        $this->signature = 'phgpwebpay|phgpwebpay|0.1|0.1';
        $this->title = _('PureOSC\'s GP WebPay');
        $this->icon = (defined('DIR_WS_CATALOG_IMAGES') ? cfg('DIR_WS_CATALOG_IMAGES') : cfg('DIR_WS_IMAGES')) . 'gpwebpay.png';
        $this->public_title = _('Card Payment');
        $this->description = _('PureHTML provided GP WebPay Payment with optional FlexiBee support');
        $this->sort_order = cfg('MODULE_PAYMENT_GPWEBPAY_SORT_ORDER');
        $this->enabled = cfg('MODULE_PAYMENT_GPWEBPAY_STATUS') == 'True';
    ...
    /**
     * Obtain 3dSecure server URI
     * 
     * @return string
     */
    public static function getServerURL() {
        return cfg('MODULE_PAYMENT_GPWEBPAY_GATEWAY_SERVER') == 'Production' ? 'https://3dsecure.gpwebpay.com/pgw/order.do' : 'https://test.3dsecure.gpwebpay.com/pgw/order.do';
    }
...
//                }
        ...
        if (tep_session_is_registered('cartID')) {
            ...
            if ($insert_order === true) {
                ...
                $sql_data_array = array('customers_id' => $customer_id,
                    ...
                    'customers_vat_number' => intval($order->customer['vat_number']),
    ...
    function process_button() {
        ...
        $signer = new \AdamStipak\Webpay\Signer(
                cfg('MODULE_PAYMENT_GPWEBPAY_SECRET_KEY'),
                cfg('MODULE_PAYMENT_GPWEBPAY_SECRET_KEY_PASSWORD'),
                cfg('MODULE_PAYMENT_GPWEBPAY_PUBLIC_KEY')      // Path of public key.
        );

        $api = new \AdamStipak\Webpay\Api(
                cfg('MODULE_PAYMENT_GPWEBPAY_MERCHANT_ID'), // Merchant number.
        ...
        $successUrl = tep_href_link(cfg('FILENAME_CHECKOUT_PROCESS'),
                'payment=done', 'SSL');
        ...
        try {
            ...
            if (!array_key_exists('js', $_GET) || ($_GET['js'] != 'active')) {
                \PureOSC\CustomerLog::singleton()->logPaymentEvent($parameters, $customer_id);
            }

            foreach ($parameters as $key => $value) {
                $process_button_string .= tep_draw_hidden_field($key, $value);
            }
            $process_button_string .= new \Ease\Html\ScriptTag('function orderConfirmed() { 
                $.ajax({
  type: "POST",
  async: false,
...
//            $process_button_string          .= '<a class="btn btn-danger" href="checkout_success.php?'.http_build_query($fakeResponseData).'"> ByPass '.$e->getMessage().' </a>';
    ...
    function before_process() {
        ...
        if ($update_status) {
           ...
           \PureOSC\CustomerLog::singleton()->logPaymentEvent($_REQUEST, $customer_id, $_REQUEST['RESULTTEXT'], 'ext:orders:'. $order_id);
...
//                case 14:
     ...
     */
    function install() {
        $created_status_id = $this->set_order_status('Processing [' . $this->code . ']',
                true);
        $sum_too_low_status_id = $this->set_order_status('Sum too low [' . $this->code . ']',
                true);
        $completed_status_id = $this->set_order_status('Completed [' . $this->code . ']',
                true);
        ...
        tep_db_query("insert into " . cfg('TABLE_CONFIGURATION') . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('" . _('Your secret key') . "', 'MODULE_PAYMENT_GPWEBPAY_SECRET_KEY', '', '" . _('Your secret key (supplied by gpwebpay)') . "', '6', '" . $sort_order++ . "', now())");
        ...
        tep_db_query("insert into " . cfg('TABLE_CONFIGURATION') . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Your public key', 'MODULE_PAYMENT_GPWEBPAY_PUBLIC_KEY', '', '" . _('Your public key (supplied by gpwebpay)') . "', '6', '" . $sort_order++ . "', now())");
...
//        tep_db_query("insert into ".cfg('TABLE_CONFIGURATION')." (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('"._('Signing Check error message')."', 'MODULE_PAYMENT_GPWEBPAY_SIGNING_STATUS', 'False', '', '6', '".$sort_order++."', 'gpwebpay::getSigningErrorMessage' , ','gpwebpay::getSigningErrorMessage('");
    ...
    function remove() {
        tep_db_query("delete from " . cfg('TABLE_CONFIGURATION') . " where configuration_key in ('" . implode("', '",
                        $this->keys()) . "')");
        tep_db_query("delete from " . cfg('TABLE_ORDERS_STATUS') . " where orders_status_name like '%[" . $this->code . "]%'");
    }
     ...
     * @return string
     */
    function calcGpwebpayMd5Key(array $order) {
        $sk = cfg('MODULE_PAYMENT_GPWEBPAY_SECRET_KEY');
