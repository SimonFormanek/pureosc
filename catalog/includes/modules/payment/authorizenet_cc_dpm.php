...
class authorizenet_cc_dpm
{
    ...
    function authorizenet_cc_dpm()
    {
        ...
        $this->sort_order   = defined('MODULE_PAYMENT_AUTHORIZENET_CC_DPM_SORT_ORDER')
                ? MODULE_PAYMENT_AUTHORIZENET_CC_DPM_SORT_ORDER : 0;
        $this->enabled      = defined('MODULE_PAYMENT_AUTHORIZENET_CC_DPM_STATUS')
            && (MODULE_PAYMENT_AUTHORIZENET_CC_DPM_STATUS == 'True') ? true : false;
        $this->order_status = defined('MODULE_PAYMENT_AUTHORIZENET_CC_DPM_ORDER_STATUS_ID')
            && ((int) MODULE_PAYMENT_AUTHORIZENET_CC_DPM_ORDER_STATUS_ID > 0) ? (int) MODULE_PAYMENT_AUTHORIZENET_CC_DPM_ORDER_STATUS_ID
                : 0;

        if (defined('MODULE_PAYMENT_AUTHORIZENET_CC_DPM_STATUS')) {
            if ((MODULE_PAYMENT_AUTHORIZENET_CC_DPM_TRANSACTION_SERVER == 'Test')
                || (MODULE_PAYMENT_AUTHORIZENET_CC_DPM_TRANSACTION_MODE == 'Test')) {
                $this->title        .= ' [Test]';
    ...
    function confirmation()
    {
        ...
        for ($i = 1; $i < 13; $i++) {
            $expiry_field .= '<option value="'.sprintf('%02d', $i).'">'.sprintf('%02d',
                    $i).'</option>';
        }
        ...
        for ($i = $today['year']; $i < $today['year'] + 10; $i++) {
            $expiry_field .= '<option value="'.strftime('%y',
                    mktime(0, 0, 0, 1, 1, $i)).'">'.strftime('%Y',
                    mktime(0, 0, 0, 1, 1, $i)).'</option>';
        }
...
EOD;
        ...
        $confirmation = array('fields' => array(array('title' => MODULE_PAYMENT_AUTHORIZENET_CC_DPM_CREDIT_CARD_OWNER_FIRSTNAME,
                    'field' => tep_draw_input_field('x_first_name',
                        $order->billing['firstname'])),
                array('title' => MODULE_PAYMENT_AUTHORIZENET_CC_DPM_CREDIT_CARD_OWNER_LASTNAME,
                    'field' => tep_draw_input_field('x_last_name',
                ...
                array('title' => MODULE_PAYMENT_AUTHORIZENET_CC_DPM_CREDIT_CARD_CCV,
                    'field' => tep_draw_input_field('x_card_code', '',
                        'size="5" maxlength="4"'))));
    ...
    function process_button()
    {
        ...
        $params = array('x_login' => substr(MODULE_PAYMENT_AUTHORIZENET_CC_DPM_LOGIN_ID,
                0, 20),
            ...
            'x_method' => 'CC',
            'x_type' => MODULE_PAYMENT_AUTHORIZENET_CC_DPM_TRANSACTION_METHOD == 'Capture'
                ? 'AUTH_CAPTURE' : 'AUTH_ONLY',
            'x_freight' => $this->format_raw($order->info['shipping_cost']),
            'x_fp_sequence' => $sequence,
            'x_fp_timestamp' => $tstamp,
            'x_fp_hash' => $this->_hmac(MODULE_PAYMENT_AUTHORIZENET_CC_DPM_TRANSACTION_KEY,
                MODULE_PAYMENT_AUTHORIZENET_CC_DPM_LOGIN_ID.'^'.$sequence.'^'.$tstamp.'^'.$this->format_raw($order->info['total']).'^'.$currency),
    ...
    {
        global $_POST, $order, $authorizenet_cc_dpm_error;
        ...
        if ($error === false) {
            if (($_POST['x_response_code'] == '1') || ($_POST['x_response_code']
                == '4')) {
                if (tep_not_null(MODULE_PAYMENT_AUTHORIZENET_CC_DPM_MD5_HASH) && (!isset($_POST['x_MD5_Hash'])
                    || (strtoupper($_POST['x_MD5_Hash']) != strtoupper(md5(MODULE_PAYMENT_AUTHORIZENET_CC_DPM_MD5_HASH.MODULE_PAYMENT_AUTHORIZENET_CC_DPM_LOGIN_ID.$_POST['x_trans_id'].$this->format_raw($order->info['total'])))))) {
                    $error = 'verification';
                ...
                if (($error === false) && ($_POST['x_response_code'] == '4')) {
                    if (MODULE_PAYMENT_AUTHORIZENET_CC_DPM_REVIEW_ORDER_STATUS_ID
                        > 0) {
...
EOD;
    ...
    function get_error()
    {
        global $_GET, $authorizenet_cc_dpm_error;
    ...
    function getParams()
    {
        ...
        $params = array('MODULE_PAYMENT_AUTHORIZENET_CC_DPM_STATUS' => array('title' => 'Enable Authorize.net Direct Post Method',
            ...
            'MODULE_PAYMENT_AUTHORIZENET_CC_DPM_REVIEW_ORDER_STATUS_ID' => array(
                'title' => 'Review Order Status',
                'desc' => 'Set the status of orders flagged as being under review to this value',
                'value' => '0',
                'use_func' => 'tep_get_order_status_name',
                'set_func' => 'tep_cfg_pull_down_order_statuses('),
            'MODULE_PAYMENT_AUTHORIZENET_CC_DPM_TRANSACTION_ORDER_STATUS_ID' => array(
...
// format prices without currency formatting
    ...
    function sendDebugEmail($response = array())
    {
        ...
        if (tep_not_null(MODULE_PAYMENT_AUTHORIZENET_CC_DPM_DEBUG_EMAIL)) {
            ...
            if (!empty($email_body)) {
                tep_mail('', MODULE_PAYMENT_AUTHORIZENET_CC_DPM_DEBUG_EMAIL,
                    'Authorize.net DPM Debug E-Mail', trim($email_body),
