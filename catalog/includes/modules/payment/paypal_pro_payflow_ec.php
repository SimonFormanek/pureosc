...
class paypal_pro_payflow_ec
{
    ...
    function paypal_pro_payflow_ec()
    {
        ...
        $this->sort_order   = defined('MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_EC_SORT_ORDER')
                ? MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_EC_SORT_ORDER : 0;
        $this->enabled      = defined('MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_EC_STATUS')
            && (MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_EC_STATUS == 'True') ? true : false;
        $this->order_status = defined('MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_EC_ORDER_STATUS_ID')
            && ((int) MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_EC_ORDER_STATUS_ID > 0) ? (int) MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_EC_ORDER_STATUS_ID
                : 0;

        if (!defined('MODULE_PAYMENT_INSTALLED') || !tep_not_null(MODULE_PAYMENT_INSTALLED)
            || !in_array('paypal_pro_payflow_dp.php',
                explode(';', MODULE_PAYMENT_INSTALLED)) || !defined('MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_DP_STATUS')
            || (MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_DP_STATUS != 'True')) {
    ...
    function update_status()
    {
        ...
        if (($this->enabled == true) && ((int) MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_EC_ZONE
            > 0)) {
            $check_flag  = false;
    ...
    function checkout_initialization_method()
    {
        ...
        $string = '<a href="'.tep_href_link('ext/modules/payment/paypal/express_payflow.php',
                '', 'SSL').'"><img src="'.MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_EC_BUTTON.'" border="0" alt="" title="'.$button_title.'" /></a>';
    ...
    function pre_confirmation_check()
    {
        ...
        if ($response_array['RESULT'] != '0') {
            tep_redirect(tep_href_link(FILENAME_SHOPPING_CART,
                    'error_message='.urlencode($response_array['OSCOM_ERROR_MESSAGE']),
                    'SSL'));
        } elseif (!tep_session_is_registered('ppeuk_secret') || ($response_array['CUSTOM']
            != $ppeuk_secret)) {
        ...
        $messageStack->add('checkout_confirmation',
            '<span id="PayPalNotice">'.MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_EC_NOTICE_CHECKOUT_CONFIRMATION.'</span><script>$("#PayPalNotice").parent().css({backgroundColor: "#fcf8e3", border: "1px #faedd0 solid", color: "#a67d57", padding: "5px" });</script>',
            'paypal');
    ...
    function before_process()
    {
        ...
        if ($response_array['RESULT'] == '0') {
            if (!tep_session_is_registered('ppeuk_secret') || ($response_array['CUSTOM']
                != $ppeuk_secret)) {
                tep_redirect(tep_href_link(FILENAME_SHOPPING_CART, '', 'SSL'));
    ...
    function getParams()
    {
        ...
        $params = array('MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_EC_STATUS' => array('title' => 'Enable PayPal Express Checkout (Payflow Edition)',
            ...
            'MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_EC_TRANSACTIONS_ORDER_STATUS_ID' => array(
...
// format prices without currency formatting
    ...
    function setExpressCheckout($parameters)
    {
        ...
        $params = array('USER' => (tep_not_null(MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_EC_USERNAME)
                ? MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_EC_USERNAME : MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_EC_VENDOR),
            ...
            'TENDER' => 'P',
            'TRXTYPE' => ((MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_EC_TRANSACTION_METHOD
            == 'Sale') ? 'S' : 'A'),
            'ACTION' => 'S',
            'RETURNURL' => tep_href_link('ext/modules/payment/paypal/express_payflow.php',
                'osC_Action=retrieve', 'SSL'),
    ...
    function sendDebugEmail($response = array())
    {
        ...
        if (tep_not_null(MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_EC_DEBUG_EMAIL)) {
            ...
            if (!empty($email_body)) {
                tep_mail('', MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_EC_DEBUG_EMAIL,
                    'PayPal Express Checkout (Payflow Edition) Debug E-Mail',
