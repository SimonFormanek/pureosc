...
class paypal_pro_payflow_dp
{
    ...
    function paypal_pro_payflow_dp()
    {
        ...
        $this->sort_order   = defined('MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_DP_SORT_ORDER')
                ? MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_DP_SORT_ORDER : 0;
        $this->enabled      = defined('MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_DP_STATUS')
            && (MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_DP_STATUS == 'True') ? true : false;
        $this->order_status = defined('MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_DP_ORDER_STATUS_ID')
            && ((int) MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_DP_ORDER_STATUS_ID > 0) ? (int) MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_DP_ORDER_STATUS_ID
                : 0;

        if (!defined('MODULE_PAYMENT_INSTALLED') || !tep_not_null(MODULE_PAYMENT_INSTALLED)
            || !in_array('paypal_pro_payflow_ec.php',
                explode(';', MODULE_PAYMENT_INSTALLED)) || !defined('MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_EC_STATUS')
            || (MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_EC_STATUS != 'True')) {
    ...
    function update_status()
    {
        ...
        if (($this->enabled == true) && ((int) MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_DP_ZONE
            > 0)) {
            $check_flag  = false;
    ...
    function confirmation()
    {
        ...
        $content = '<table id="paypal_table_new_card" border="0" width="100%" cellspacing="0" cellpadding="2">'.
            ...
            '  <td>'.tep_draw_input_field('cc_owner_firstname',
                $order->billing['firstname']).'</td>'.
            '</tr>'.
            '<tr>'.
            '  <td width="30%">'.MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_DP_CARD_OWNER_LASTNAME.'</td>'.
            '  <td>'.tep_draw_input_field('cc_owner_lastname',
                $order->billing['lastname']).'</td>'.
            '</tr>'.
            '<tr>'.
    ...
    function before_process()
    {
        ...
        if (isset($_POST['cc_owner_firstname']) && !empty($_POST['cc_owner_firstname'])
            && isset($_POST['cc_owner_lastname']) && !empty($_POST['cc_owner_lastname'])
            && isset($_POST['cc_number_nh-dns']) && !empty($_POST['cc_number_nh-dns'])) {
            ...
            $params = array('USER' => (tep_not_null(MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_DP_USERNAME)
                ...
                'TENDER' => 'C',
                'TRXTYPE' => ((MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_DP_TRANSACTION_METHOD
                == 'Sale') ? 'S' : 'A'),
                ...
                'BILLTOSTATE' => tep_get_zone_code($order->billing['country']['id'],
            ...
            if ($this->format_raw($item_params['ITEMAMT'] + $item_params['TAXAMT']
                    + $item_params['FREIGHTAMT']) == $params['AMT']) {
            ...
            tep_redirect(tep_href_link(FILENAME_CHECKOUT_CONFIRMATION,
                    'error_message='.MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_DP_ERROR_ALL_FIELDS_REQUIRED,
                    'SSL'));
        }
    }
    ...
    function getParams()
    {
        ...
        $params = array('MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_DP_STATUS' => array('title' => 'Enable PayPal Payments Pro (Payflow Edition)',
            ...
            'MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_DP_TRANSACTIONS_ORDER_STATUS_ID' => array(
    ...
    function sendTransactionToGateway($url, $parameters)
    {
        ...
        $request_id = (isset($order) && is_object($order)) ? md5($cartID.tep_session_id().$this->format_raw($order->info['total']))
                : 'oscom_conn_test';
...
EOD;
    ...
    function sendDebugEmail($response = array())
    {
        ...
        if (tep_not_null(MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_DP_DEBUG_EMAIL)) {
            ...
            if (!empty($email_body)) {
                tep_mail('', MODULE_PAYMENT_PAYPAL_PRO_PAYFLOW_DP_DEBUG_EMAIL,
                    'PayPal Payments Pro (Payflow Edition) Debug E-Mail',
