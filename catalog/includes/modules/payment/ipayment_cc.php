...
// class constructor
    function ipayment_cc()
    {
        ...
        $this->title        = __('MODULE_PAYMENT_IPAYMENT_CC_TEXT_TITLE',_('Ipayment'));
        $this->public_title = __('MODULE_PAYMENT_IPAYMENT_CC_TEXT_PUBLIC_TITLE',_('Ipayment'));
        $this->description  = __('MODULE_PAYMENT_IPAYMENT_CC_TEXT_DESCRIPTION',_('Ipayment description'));
        $this->sort_order   = cfg('MODULE_PAYMENT_IPAYMENT_CC_SORT_ORDER');
        $this->enabled      = cfg('MODULE_PAYMENT_IPAYMENT_CC_STATUS') == 'True';

        if (cfg('MODULE_PAYMENT_IPAYMENT_CC_ORDER_STATUS_ID')) {
            $this->order_status = cfg('MODULE_PAYMENT_IPAYMENT_CC_ORDER_STATUS_ID');
        }
        ...
        $this->form_action_url = 'https://ipayment.de/merchant/'. cfg('MODULE_PAYMENT_IPAYMENT_CC_ID').'/processor/2.0/';
    }
...
// class methods
    ...
    function confirmation()
    {
        ...
        for ($i = 1; $i < 13; $i++) {
            $expires_month[] = array('id' => sprintf('%02d', $i), 'text' => strftime('%B',
        ...
        $confirmation = array('fields' => array(array('title' => MODULE_PAYMENT_IPAYMENT_CC_TEXT_CREDIT_CARD_OWNER,
                    'field' => tep_draw_input_field('addr_name',
                ...
                array('title' => MODULE_PAYMENT_IPAYMENT_CC_TEXT_CREDIT_CARD_EXPIRES,
                    'field' => tep_draw_pull_down_menu('cc_expdate_month',
                        $expires_month).'&nbsp;'.tep_draw_pull_down_menu('cc_expdate_year',
                        $expires_year)),
                array('title' => MODULE_PAYMENT_IPAYMENT_CC_TEXT_CREDIT_CARD_CHECKNUMBER,
                    'field' => tep_draw_input_field('cc_checkcode', '',
                        'size="5" maxlength="4"'))));
    ...
    function process_button()
    {
        ...
        if (is_numeric($order->billing['zone_id']) && ($order->billing['zone_id']
            > 0)) {
        ...
        $process_button_string = tep_draw_hidden_field('silent', '1').
            ...
            tep_draw_hidden_field('trxuser_id',
                MODULE_PAYMENT_IPAYMENT_CC_USER_ID).
            tep_draw_hidden_field('trxpassword',
                MODULE_PAYMENT_IPAYMENT_CC_PASSWORD).
            ...
            tep_draw_hidden_field('trx_amount',
                $this->format_raw($order->info['total']) * 100).
            tep_draw_hidden_field('trx_typ',
                ((MODULE_PAYMENT_IPAYMENT_CC_TRANSACTION_METHOD == 'Capture') ? 'auth'
                    : 'preauth')).
            tep_draw_hidden_field('addr_email',
                $order->customer['email_address']).
            tep_draw_hidden_field('addr_street',
                $order->billing['street_address']).
            tep_draw_hidden_field('addr_city', $order->billing['city']).
            tep_draw_hidden_field('addr_zip', $order->billing['postcode']).
            tep_draw_hidden_field('addr_country',
            ...
            tep_draw_hidden_field('silent_error_url',
                tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                    'payment_error='.$this->code, 'SSL', true)).
            tep_draw_hidden_field('hidden_trigger_url',
                tep_href_link('ext/modules/payment/ipayment/callback_cc.php',
                    '', 'SSL', false)).
        ...
        if (tep_not_null(MODULE_PAYMENT_IPAYMENT_CC_SECRET_HASH_PASSWORD)) {
            $process_button_string .= tep_draw_hidden_field('trx_securityhash',
                md5(MODULE_PAYMENT_IPAYMENT_CC_USER_ID.($this->format_raw($order->info['total'])
                    * 100).$currency.MODULE_PAYMENT_IPAYMENT_CC_PASSWORD.MODULE_PAYMENT_IPAYMENT_CC_SECRET_HASH_PASSWORD));
        }
    ...
    {
        global $_GET, $HTTP_SERVER_VARS, $order, $currency;

        if ($_GET['ret_errorcode'] != '0') {
            tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                    'payment_error='.$this->code.'&error='.tep_output_string_protected($_GET['ret_errormsg'])));
        }
...
// verify ret_param_checksum
            if ($_GET['ret_param_checksum'] != md5(MODULE_PAYMENT_IPAYMENT_CC_USER_ID.($this->format_raw($order->info['total'])
                    * 100).$currency.$_GET['ret_authcode'].$_GET['ret_booknr'].MODULE_PAYMENT_IPAYMENT_CC_SECRET_HASH_PASSWORD)) {
                $pass = false;
            }
...
// verify ret_url_checksum
    ...
    function get_error()
    {
        ...
        $error = array('title' => MODULE_PAYMENT_IPAYMENT_CC_ERROR_HEADING,
            'error' => ((isset($_GET['error'])) ? stripslashes(urldecode($_GET['error']))
                : MODULE_PAYMENT_IPAYMENT_CC_ERROR_MESSAGE));
    ...
    function keys()
    {
        return array('MODULE_PAYMENT_IPAYMENT_CC_STATUS', 'MODULE_PAYMENT_IPAYMENT_CC_ID',
            'MODULE_PAYMENT_IPAYMENT_CC_USER_ID', 'MODULE_PAYMENT_IPAYMENT_CC_PASSWORD',
            'MODULE_PAYMENT_IPAYMENT_CC_TRANSACTION_METHOD', 'MODULE_PAYMENT_IPAYMENT_CC_SECRET_HASH_PASSWORD',
            'MODULE_PAYMENT_IPAYMENT_CC_DEBUG_EMAIL', 'MODULE_PAYMENT_IPAYMENT_CC_ZONE',
            'MODULE_PAYMENT_IPAYMENT_CC_ORDER_STATUS_ID', 'MODULE_PAYMENT_IPAYMENT_CC_SORT_ORDER');
    }
...
// format prices without currency formatting
    ...
    function sendDebugEmail($checksum_match = 0)
    {
        ...
        if (tep_not_null(MODULE_PAYMENT_IPAYMENT_CC_DEBUG_EMAIL)) {
            ...
            tep_mail('', MODULE_PAYMENT_IPAYMENT_CC_DEBUG_EMAIL,
                'iPayment (Credit Card) Transaction', $email_body, STORE_OWNER,
