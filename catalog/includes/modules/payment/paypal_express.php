...
class paypal_express
{
    ...
    function paypal_express()
    {
        ...
        $this->sort_order   = defined('MODULE_PAYMENT_PAYPAL_EXPRESS_SORT_ORDER')
                ? MODULE_PAYMENT_PAYPAL_EXPRESS_SORT_ORDER : 0;
        $this->enabled      = defined('MODULE_PAYMENT_PAYPAL_EXPRESS_STATUS') && (MODULE_PAYMENT_PAYPAL_EXPRESS_STATUS
            == 'True') ? true : false;
        $this->order_status = defined('MODULE_PAYMENT_PAYPAL_EXPRESS_ORDER_STATUS_ID')
            && ((int) MODULE_PAYMENT_PAYPAL_EXPRESS_ORDER_STATUS_ID > 0) ? (int) MODULE_PAYMENT_PAYPAL_EXPRESS_ORDER_STATUS_ID
                : 0;
...
// When changing the shipping address due to no shipping rates being available, head straight to the checkout confirmation page
    ...
    function update_status()
    {
        ...
        if (($this->enabled === true) && ((int) MODULE_PAYMENT_PAYPAL_EXPRESS_ZONE
            > 0)) {
            $check_flag  = false;
            $check_query = tep_db_query("select zone_id from ".constant('TABLE_ZONES_TO_GEO_ZONES')." where geo_zone_id = '".MODULE_PAYMENT_PAYPAL_EXPRESS_ZONE."' and zone_country_id = '".$order->delivery['country']['id']."' order by zone_id");
    ...
    function checkout_initialization_method()
    {
        ...
        $string = '<a href="'.tep_href_link('ext/modules/payment/paypal/express.php',
                '', 'SSL').'"><img src="'.$image_button.'" border="0" alt="" title="'.$button_title.'" /></a>';
    ...
    function pre_confirmation_check()
    {
        ...
        if (($response_array['ACK'] != 'Success') && ($response_array['ACK'] != 'SuccessWithWarning')) {
            tep_redirect(tep_href_link(FILENAME_SHOPPING_CART,
                    ...
                    'SSL'));
        } elseif (!tep_session_is_registered('ppe_secret') || ($response_array['PAYMENTREQUEST_0_CUSTOM']
            != $ppe_secret)) {
            tep_redirect(tep_href_link(FILENAME_SHOPPING_CART, '', 'SSL'));
        }

        if (tep_session_is_registered('ppe_order_total_check')) {
            $messageStack->add('checkout_confirmation',
                '<span id="PayPalNotice">'.MODULE_PAYMENT_PAYPAL_EXPRESS_NOTICE_CHECKOUT_CONFIRMATION.'</span><script>$("#PayPalNotice").parent().css({backgroundColor: "#fcf8e3", border: "1px #faedd0 solid", color: "#a67d57", padding: "5px" });</script>',
                'paypal');
        }
    ...
    function confirmation()
    {
        ...
        if (empty($comments)) {
            $confirmation = array('fields' => array(array('title' => MODULE_PAYMENT_PAYPAL_EXPRESS_TEXT_COMMENTS,
                        'field' => tep_draw_textarea_field('ppecomments',
                            'soft', '60', '5', $comments))));
        }
    ...
    function before_process()
    {
        ...
        if (($response_array['ACK'] == 'Success') || ($response_array['ACK'] == 'SuccessWithWarning')) {
            if (!tep_session_is_registered('ppe_secret') || ($response_array['PAYMENTREQUEST_0_CUSTOM']
                != $ppe_secret)) {
                tep_redirect(tep_href_link(FILENAME_SHOPPING_CART, '', 'SSL'));
            } elseif (($response_array['PAYMENTREQUEST_0_AMT'] != $this->format_raw($order->info['total']))
                && !tep_session_is_registered('ppe_order_total_check')) {
        ...
        if (is_numeric($sendto) && ($sendto > 0)) {
            ...
            $params['PAYMENTREQUEST_0_SHIPTOSTATE']       = tep_get_zone_code($order->delivery['country']['id'],
    ...
    function getParams()
    {
        ...
        $params = array('MODULE_PAYMENT_PAYPAL_EXPRESS_STATUS' => array('title' => 'Enable PayPal Express Checkout',
            ...
            'MODULE_PAYMENT_PAYPAL_EXPRESS_TRANSACTIONS_ORDER_STATUS_ID' => array(
    ...
    function sendTransactionToGateway($url, $parameters)
    {
        ...
        if (tep_not_null(MODULE_PAYMENT_PAYPAL_EXPRESS_PROXY)) {
            ...
            curl_setopt($curl, CURLOPT_PROXY,
                MODULE_PAYMENT_PAYPAL_EXPRESS_PROXY);
        }
...
// format prices without currency formatting
    ...
    function setExpressCheckout($parameters)
    {
        ...
        $params = array('VERSION' => $this->api_version,
            ...
            'PAYMENTREQUEST_0_PAYMENTACTION' => ((MODULE_PAYMENT_PAYPAL_EXPRESS_TRANSACTION_METHOD
            == 'Sale') || (!tep_not_null(MODULE_PAYMENT_PAYPAL_EXPRESS_API_USERNAME))
                ? 'Sale' : 'Authorization'),
            'RETURNURL' => tep_href_link('ext/modules/payment/paypal/express.php',
                'osC_Action=retrieve', 'SSL', true, false),
            'CANCELURL' => tep_href_link('ext/modules/payment/paypal/express.php',
                'osC_Action=cancel', 'SSL', true, false),
            'BRANDNAME' => STORE_NAME,
            'SOLUTIONTYPE' => (MODULE_PAYMENT_PAYPAL_EXPRESS_ACCOUNT_OPTIONAL == 'True')
                ? 'Sole' : 'Mark');
    ...
    function sendDebugEmail($response = array())
    {
        ...
        if (tep_not_null(MODULE_PAYMENT_PAYPAL_EXPRESS_DEBUG_EMAIL)) {
            ...
            if (!empty($email_body)) {
                tep_mail('', MODULE_PAYMENT_PAYPAL_EXPRESS_DEBUG_EMAIL,
                    'PayPal Express Checkout Debug E-Mail', trim($email_body),
