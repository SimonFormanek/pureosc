...
class braintree_cc
{
    ...
    function braintree_cc()
    {
        ...
        $this->sort_order   = defined('MODULE_PAYMENT_BRAINTREE_CC_SORT_ORDER') ? MODULE_PAYMENT_BRAINTREE_CC_SORT_ORDER
                : 0;
        $this->enabled      = defined('MODULE_PAYMENT_BRAINTREE_CC_STATUS') && (MODULE_PAYMENT_BRAINTREE_CC_STATUS
            == 'True') ? true : false;
        $this->order_status = defined('MODULE_PAYMENT_BRAINTREE_CC_ORDER_STATUS_ID')
            && ((int) MODULE_PAYMENT_BRAINTREE_CC_ORDER_STATUS_ID > 0) ? (int) MODULE_PAYMENT_BRAINTREE_CC_ORDER_STATUS_ID
                : 0;
        ...
        if (version_compare(PHP_VERSION, '5.2.1', '<')) {
            $braintree_error = sprintf(MODULE_PAYMENT_BRAINTREE_CC_ERROR_ADMIN_PHP,
                '5.2.1');
        }

        if (!isset($braintree_error)) {
            $requiredExtensions = array('xmlwriter', 'SimpleXML', 'openssl', 'dom',
                'hash', 'curl');
            ...
            if (!empty($exts)) {
                $braintree_error = sprintf(MODULE_PAYMENT_BRAINTREE_CC_ERROR_ADMIN_PHP_EXTENSIONS,
                    implode('<br />', $exts));
            }
        }

        if (!isset($braintree_error) && defined('MODULE_PAYMENT_BRAINTREE_CC_STATUS')) {
            if (!tep_not_null(MODULE_PAYMENT_BRAINTREE_CC_MERCHANT_ID) || !tep_not_null(MODULE_PAYMENT_BRAINTREE_CC_PUBLIC_KEY)
                || !tep_not_null(MODULE_PAYMENT_BRAINTREE_CC_PRIVATE_KEY) || !tep_not_null(MODULE_PAYMENT_BRAINTREE_CC_CLIENT_KEY)) {
        ...
        if (!isset($braintree_error) && defined('MODULE_PAYMENT_BRAINTREE_CC_STATUS')) {
            ...
            if ($ma_error === true) {
                $braintree_error = sprintf(MODULE_PAYMENT_BRAINTREE_CC_ERROR_ADMIN_MERCHANT_ACCOUNTS,
                    DEFAULT_CURRENCY);
            }
        }
    ...
    function confirmation()
    {
        ...
        for ($i = $today['year']; $i < $today['year'] + 10; $i++) {
            $years_array[] = array('id' => tep_output_string(strftime('%Y',
                        mktime(0, 0, 0, 1, 1, $i))),
                'text' => tep_output_string_protected(strftime('%Y',
                        mktime(0, 0, 0, 1, 1, $i))));
        }
        ...
        if (!$this->isValidCurrency($currency)) {
            $content .= sprintf(MODULE_PAYMENT_BRAINTREE_CC_CURRENCY_CHARGE,
                $currencies->format($order->info['total'], true,
            ...
            if (tep_db_num_rows($tokens_query) > 0) {
                ...
                while ($tokens = tep_db_fetch_array($tokens_query)) {
                    $content .= '<tr class="moduleRow" id="braintree_card_'.(int) $tokens['id'].'">'.
                        ...
                        '  <td valign="top">'.MODULE_PAYMENT_BRAINTREE_CC_CREDITCARD_LAST_4.'&nbsp;'.tep_output_string_protected($tokens['number_filtered']).'&nbsp;&nbsp;'.tep_output_string_protected(substr($tokens['expiry_date'],
                                0, 2).'/'.substr($tokens['expiry_date'], 2)).'&nbsp;&nbsp;'.tep_output_string_protected($tokens['card_type']).'</td>'.
                        '</tr>';
        ...
        $content .= '<table id="braintree_table_new_card" border="0" width="100%" cellspacing="0" cellpadding="2">'.
            ...
            '  <td>'.tep_draw_pull_down_menu('month', $months_array).' / '.tep_draw_pull_down_menu('year',
                $years_array).'</td>'.
            '</tr>';
    ...
    function before_process()
    {
        ...
        if (MODULE_PAYMENT_BRAINTREE_CC_TOKENS == 'True') {
            if (isset($_POST['braintree_card']) && is_numeric($_POST['braintree_card'])
                && ($_POST['braintree_card'] > 0)) {
                ...
                if (tep_db_num_rows($token_query) == 1) {
                    ...
                    if (MODULE_PAYMENT_BRAINTREE_CC_VERIFY_WITH_CVV == 'True') {
                        ...
                        if (isset($_POST['token_cvv']) && is_array($_POST['token_cvv'])
                            && isset($_POST['token_cvv'][$_POST['braintree_card']])) {
                        ...
                        if (!isset($braintree_token_cvv) || empty($braintree_token_cvv)) {
                            tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                                    'payment_error='.$this->code.'&error=cardcvv',
                                    'SSL'));
                        }
                    }
                }
        ...
        if (!isset($braintree_token)) {
            ...
            if (!isset($cc_owner) || empty($cc_owner)) {
                tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                        'payment_error='.$this->code.'&error=cardowner', 'SSL'));
            }

            if (!isset($cc_number) || empty($cc_number)) {
                tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                        'payment_error='.$this->code.'&error=cardnumber', 'SSL'));
            }

            if (!isset($cc_expires_month) || !in_array($cc_expires_month,
                    $months_array)) {
                tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                        'payment_error='.$this->code.'&error=cardexpires', 'SSL'));
            }

            if (!isset($cc_expires_year) || !in_array($cc_expires_year,
                    $years_array)) {
                tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                        'payment_error='.$this->code.'&error=cardexpires', 'SSL'));
            }
            ...
            if (MODULE_PAYMENT_BRAINTREE_CC_VERIFY_WITH_CVV == 'True') {
                if (!isset($cc_cvv) || empty($cc_cvv)) {
                    tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                            'payment_error='.$this->code.'&error=cardcvv', 'SSL'));
                }
            }
        }
        ...
        Braintree_Configuration::environment(MODULE_PAYMENT_BRAINTREE_CC_TRANSACTION_SERVER
        ...
        $data = array('amount' => $this->format_raw($order->info['total'],
                $currency),
            ...
            'billing' => array('firstName' => $order->billing['firstname'],
                ...
                'region' => tep_get_zone_name($order->billing['country_id'],
        ...
        if ($order->content_type != 'virtual') {
            $data['shipping'] = array('firstName' => $order->delivery['firstname'],
                ...
                'region' => tep_get_zone_name($order->delivery['country_id'],
        ...
        if (!isset($braintree_token)) {
            ...
            if ((MODULE_PAYMENT_BRAINTREE_CC_TOKENS == 'True') && isset($_POST['cc_save'])
                && ($_POST['cc_save'] == 'true')) {
    ...
    function after_process()
    {
        ...
        if ((MODULE_PAYMENT_BRAINTREE_CC_TOKENS == 'True') && isset($_POST['cc_save'])
            && ($_POST['cc_save'] == 'true') && !isset($braintree_token) && isset($braintree_result->transaction->creditCard['token'])) {
    ...
    function getParams()
    {
        if (tep_db_num_rows(tep_db_query("show tables like 'customers_braintree_tokens'"))
            != 1) {
            $sql = <<<EOD
...
{
    return tep_draw_textarea_field('configuration['.$name.']', '', '50', '12',
        $value);
}
...
function tep_cfg_braintree_cc_set_merchant_accounts($value, $key)
{
    ...
    foreach ($c_array as $c) {
        ...
        $result .= '&nbsp;'.tep_draw_input_field('braintree_ma['.$c.']',
                (isset($data[$c]) ? $data[$c] : '')).'<br />';
    }
