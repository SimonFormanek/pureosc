...
class sage_pay_form
{
    ...
    function sage_pay_form()
    {
        ...
        $this->sort_order   = defined('MODULE_PAYMENT_SAGE_PAY_FORM_SORT_ORDER')
                ? MODULE_PAYMENT_SAGE_PAY_FORM_SORT_ORDER : 0;
        $this->enabled      = defined('MODULE_PAYMENT_SAGE_PAY_FORM_STATUS') && (MODULE_PAYMENT_SAGE_PAY_FORM_STATUS
            == 'True') ? true : false;
        $this->order_status = defined('MODULE_PAYMENT_SAGE_PAY_FORM_ORDER_STATUS_ID')
            && ((int) MODULE_PAYMENT_SAGE_PAY_FORM_ORDER_STATUS_ID > 0) ? (int) MODULE_PAYMENT_SAGE_PAY_FORM_ORDER_STATUS_ID
                : 0;
    ...
    function update_status()
    {
        ...
        if (($this->enabled === true) && ((int) MODULE_PAYMENT_SAGE_PAY_FORM_ZONE
            > 0)) {
            $check_flag  = false;
            $check_query = tep_db_query("select zone_id from ".constant('TABLE_ZONES_TO_GEO_ZONES')." where geo_zone_id = '".MODULE_PAYMENT_SAGE_PAY_FORM_ZONE."' and zone_country_id = '".$order->billing['country']['id']."' order by zone_id");
    ...
    function process_button()
    {
        ...
        $params = array('VPSProtocol' => $this->api_version,
            'Vendor' => substr(MODULE_PAYMENT_SAGE_PAY_FORM_VENDOR_LOGIN_NAME,
                0, 15));
        ...
        $crypt = array('ReferrerID' => 'C74D7B82-E9EB-4FBD-93DB-76F0F551C802',
            ...
            'FailureURL' => tep_href_link(FILENAME_CHECKOUT_PAYMENT,
                'payment_error='.$this->code, 'SSL'),
            'CustomerName' => substr($order->billing['firstname'].' '.$order->billing['lastname'],
                0, 100),
            ...
            'BillingAddress1' => substr($order->billing['street_address'], 0,
                100),
        ...
        if ($crypt['BillingCountry'] == 'US') {
            $crypt['BillingState'] = tep_get_zone_code($order->billing['country']['id'],
                $order->billing['zone_id'], '');
        }

        $crypt['BillingPhone']       = substr($order->customer['telephone'], 0,
            20);
        $crypt['DeliverySurname']    = substr($order->delivery['lastname'], 0,
            20);
        $crypt['DeliveryFirstnames'] = substr($order->delivery['firstname'], 0,
            20);
        $crypt['DeliveryAddress1']   = substr($order->delivery['street_address'],
            0, 100);
        $crypt['DeliveryCity']       = substr($order->delivery['city'], 0, 40);
        $crypt['DeliveryPostCode']   = substr($order->delivery['postcode'], 0,
            10);
        $crypt['DeliveryCountry']    = $order->delivery['country']['iso_code_2'];

        if ($crypt['DeliveryCountry'] == 'US') {
            $crypt['DeliveryState'] = tep_get_zone_code($order->delivery['country']['id'],
                $order->delivery['zone_id'], '');
        }

        if (tep_not_null(MODULE_PAYMENT_SAGE_PAY_FORM_VENDOR_EMAIL)) {
            $crypt['VendorEMail'] = substr(MODULE_PAYMENT_SAGE_PAY_FORM_VENDOR_EMAIL,
                0, 255);
        }
        ...
        if (tep_not_null(MODULE_PAYMENT_SAGE_PAY_FORM_CUSTOMER_EMAIL_MESSAGE)) {
            $crypt['eMailMessage'] = substr(MODULE_PAYMENT_SAGE_PAY_FORM_CUSTOMER_EMAIL_MESSAGE,
                0, 7500);
        }
        ...
        $crypt['Basket']        = substr(sizeof($contents).':'.implode(':',
                $contents), 0, 7500);
...
// don't show an error message for user cancelled/aborted transactions
    ...
    function getParams()
    {
        ...
        $params = array('MODULE_PAYMENT_SAGE_PAY_FORM_STATUS' => array('title' => 'Enable Sage Pay Form Module',
            ...
            'MODULE_PAYMENT_SAGE_PAY_FORM_TRANSACTION_ORDER_STATUS_ID' => array(
...
// encrypt
        return '@'.strtoupper(bin2hex(mcrypt_encrypt(MCRYPT_RIJNDAEL_128,
                        MODULE_PAYMENT_SAGE_PAY_FORM_ENCRYPTION_PASSWORD,
                        $string, MCRYPT_MODE_CBC,
                        MODULE_PAYMENT_SAGE_PAY_FORM_ENCRYPTION_PASSWORD)));
    }
    ...
    function decryptParams($string)
    {
        ...
        return mcrypt_decrypt(MCRYPT_RIJNDAEL_128,
            MODULE_PAYMENT_SAGE_PAY_FORM_ENCRYPTION_PASSWORD, $string,
            MCRYPT_MODE_CBC, MODULE_PAYMENT_SAGE_PAY_FORM_ENCRYPTION_PASSWORD);
    }
    ...
    function sendDebugEmail($response = array())
    {
        ...
        if (tep_not_null(MODULE_PAYMENT_SAGE_PAY_FORM_DEBUG_EMAIL)) {
            ...
            if (!empty($email_body)) {
                tep_mail('', MODULE_PAYMENT_SAGE_PAY_FORM_DEBUG_EMAIL,
                    'Sage Pay Form Debug E-Mail', trim($email_body),
...
{
    return tep_draw_textarea_field('configuration['.$key.']', 'soft', 60, 5,
        $value);
}
?>
