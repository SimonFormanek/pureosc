...
class sofortueberweisung_direct
{
    ...
    {
        $this->sofortueberweisung_direct();
    }
...
// class constructor
    function sofortueberweisung_direct()
    {
        ...
        $this->enabled      = ((MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_STATUS == 'True')
                ? true : false);

        if ((int) MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_PREPARE_ORDER_STATUS_ID
            > 0) {
...
// class methods
    function update_status()
    {
        ...
        if (($this->enabled === true) && ((int) MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_ZONE
            > 0)) {
            $check_flag  = false;
            $check_query = tep_db_query("select zone_id from ".constant('TABLE_ZONES_TO_GEO_ZONES')." where geo_zone_id = '".MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_ZONE."' and zone_country_id = '".$order->billing['country']['id']."' order by zone_id");
    ...
    function selection()
    {
        ...
        if (tep_session_is_registered('cart_Sofortueberweisung_Direct_ID')) {
            $order_id = substr($cart_Sofortueberweisung_Direct_ID,
                strpos($cart_Sofortueberweisung_Direct_ID, '-') + 1);
    ...
    function confirmation()
    {
        ...
        if (tep_session_is_registered('cart_Sofortueberweisung_Direct_ID')) {
            ...
            if (($curr['currency'] != $order->info['currency']) || ($cartID != substr($cart_Sofortueberweisung_Direct_ID,
                    0, strlen($cartID)))) {
                $check_query = tep_db_query('select orders_id from '.TABLE_ORDERS_STATUS_HISTORY.' where orders_id = "'.(int) $order_id.'" limit 1');
    ...
    function process_button()
    {
        ...
        $parameter['betrag']  = number_format($order->info['total'] * $currencies->get_value('EUR'),
            2, '.', '');  // Beziffert den Zahlungsbetrag, der an Sie �bermittelt werden soll
        $vzweck1              = str_replace('{{orderid}}', $order_id,
        ...
        $vzweck1 = str_replace('{{order_date}}', strftime(DATE_FORMAT_SHORT),
            $vzweck1);
        $vzweck2 = str_replace('{{order_date}}', strftime(DATE_FORMAT_SHORT),
            $vzweck2);

        $vzweck1 = str_replace('{{customer_id}}', $customer_id, $vzweck1);
        $vzweck2 = str_replace('{{customer_id}}', $customer_id, $vzweck2);

        $vzweck1 = str_replace('{{customer_name}}',
            $order->customer['firstname'].' '.$order->customer['lastname'],
            $vzweck1);
        $vzweck2 = str_replace('{{customer_name}}',
            $order->customer['firstname'].' '.$order->customer['lastname'],
            $vzweck2);

        $vzweck1 = str_replace('{{customer_company}}',
            $order->customer['company'], $vzweck1);
        $vzweck2 = str_replace('{{customer_company}}',
            $order->customer['company'], $vzweck2);

        $vzweck1 = str_replace('{{customer_email}}',
            $order->customer['email_address'], $vzweck1);
        $vzweck2 = str_replace('{{customer_email}}',
            $order->customer['email_address'], $vzweck2);
    ...
    function before_process()
    {
        ...
        $order_total_integer = number_format($order->info['total'] * $currencies->get_value('EUR'),
                2, '.', '') * 100;
        ...
        if (tep_db_num_rows($check_query)) {
            ...
            if ($check['orders_status'] == MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_PREPARE_ORDER_STATUS_ID) {
                ...
                if (($md5var4 == $HTTP_GET_VARS['sovar4']) && ((int) $HTTP_GET_VARS['betrag_integer']
                    == (int) $order_total_integer)) {
                    $sql_data_array['comments'] = 'Zahlung durch Sofort�berweisung Weiter-Button/Weiterleitung best�tigt!';
                } else {
                    $sql_data_array['comments'] = MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_TEXT_CHECK_ERROR.'\n'.($HTTP_GET_VARS['betrag_integer']
                        / 100).'!='.($order_total_integer / 100);
                }

                if (MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_STORE_TRANSACTION_DETAILS
                    == 'True') {
                    $sql_data_array['comments'] = (!empty($sql_data_array['comments'])
        ...
        tep_db_query("update ".TABLE_ORDERS." set orders_status = '".(MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_ORDER_STATUS_ID
            > 0 ? (int) MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_ORDER_STATUS_ID
                    : (int) DEFAULT_ORDERS_STATUS_ID)."', last_modified = now() where orders_id = '".(int) $order_id."'");

        $sql_data_array = array('orders_id' => $order_id,
            'orders_status_id' => (MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_ORDER_STATUS_ID
            > 0 ? (int) MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_ORDER_STATUS_ID
...
// unregister session variables used during checkout
    ...
    function install()
    {
        ...
        $kdnr           = (isset($HTTP_GET_VARS['kdnr']) && !empty($HTTP_GET_VARS['kdnr']))
                ? tep_db_prepare_input($HTTP_GET_VARS['kdnr']) : '10000';
        $projekt        = (isset($HTTP_GET_VARS['projekt']) && !empty($HTTP_GET_VARS['projekt']))
                ? tep_db_prepare_input($HTTP_GET_VARS['projekt']) : '500000';
        $input_passwort = (isset($HTTP_GET_VARS['input_passwort']) && !empty($HTTP_GET_VARS['input_passwort']))
                ? tep_db_prepare_input($HTTP_GET_VARS['input_passwort']) : '';
        $bna_passwort   = (isset($HTTP_GET_VARS['bna_passwort']) && !empty($HTTP_GET_VARS['bna_passwort']))
                ? tep_db_prepare_input($HTTP_GET_VARS['bna_passwort']) : '';
        $cnt_passwort   = (isset($HTTP_GET_VARS['cnt_passwort']) && !empty($HTTP_GET_VARS['cnt_passwort']))
                ? tep_db_prepare_input($HTTP_GET_VARS['cnt_passwort']) : '';
    ...
    function keys()
    {
        return array('MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_STATUS', 'MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_KDNR',
            'MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_PROJEKT', 'MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_INPUT_PASSWORT',
            'MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_BNA_PASSWORT', 'MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_CNT_PASSWORT',
            'MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_STORE_TRANSACTION_DETAILS',
            'MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_ZONE', 'MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_PREPARE_ORDER_STATUS_ID',
            'MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_ORDER_STATUS_ID', 'MODULE_PAYMENT_SOFORTUEBERWEISUNG_DIRECT_SORT_ORDER');
    }
}
