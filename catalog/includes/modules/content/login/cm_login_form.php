...
class cm_login_form {
  ...
  function execute() {
    global $_GET, $_POST, $sessiontoken, $login_customer_id, $login_customer_password, $messageStack, $oscTemplate;
...
// Check if email exists
      $customer_query = tep_db_query("select customers_id, customers_password from " . TABLE_CUSTOMERS_REAL . " where customers_email_address = '" . tep_db_input($email_address) . "' limit 1");
...
//add crypted password to session
          $login_customer_password = \PureOSC\SslCrypto::encrypt_session_password($source);
