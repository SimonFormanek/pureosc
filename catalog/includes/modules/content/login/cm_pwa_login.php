...
class cm_pwa_login
{
    ...
    function isEnabled()
    {
        ...
        if (!tep_session_is_registered('customer_id') && ($cart->count_contents()
            > 0) && (MODULE_CONTENT_PWA_LOGIN_STATUS == 'True')) {
            return $this->enabled;
        }
    }
    ...
    function keys()
    {
        return array('MODULE_CONTENT_PWA_LOGIN_STATUS', 'MODULE_CONTENT_PWA_LOGIN_CONTENT_WIDTH',
            'MODULE_CONTENT_PWA_LOGIN_SORT_ORDER', 'GUEST_CHECKOUT_TELEPHONE');
    }
}
