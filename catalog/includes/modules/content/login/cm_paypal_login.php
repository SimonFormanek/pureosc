...
class cm_paypal_login
{
    ...
    function cm_paypal_login()
    {
        ...
        if (defined('FILENAME_MODULES') && ($PHP_SELF == 'modules_content.php') && isset($_GET['action'])
...
// check if e-mail address exists in database and login or create customer account
                    if (!tep_session_is_registered('customer_id')) {
                        ...
                        if (tep_db_num_rows($check_query)) {
                            ...
                            if ($this->hasAttribute('phone') && isset($response['phone_number'])
                                && tep_not_null($response['phone_number'])) {
...
// check if paypal shipping address exists in the address book
                    ...
                    $return_url = tep_href_link(FILENAME_LOGIN,
                        'action=paypal_login_process', 'SSL');
                }
            }
        }

        echo '<script>window.opener.location.href="'.str_replace('&amp;', '&',
            $return_url).'";window.close();</script>';
...
// Register PayPal Express Checkout as the default payment method
        if (!tep_session_is_registered('payment') || ($payment != 'paypal_express')) {
            if (defined('MODULE_PAYMENT_INSTALLED') && tep_not_null(MODULE_PAYMENT_INSTALLED)) {
                if (in_array('paypal_express.php',
                        explode(';', MODULE_PAYMENT_INSTALLED))) {
    ...
    function getToken($params)
    {
        ...
        $parameters = array('client_id' => MODULE_CONTENT_PAYPAL_LOGIN_CLIENT_ID,
            ...
            'redirect_uri' => str_replace('&amp;', '&',
                tep_href_link(FILENAME_LOGIN, 'action=paypal_login', 'SSL')));
        ...
        $result = $this->sendRequest('https://'.$api_server.'/v1/identity/openidconnect/tokenservice',
            $post_string);
    ...
    function getTestLinkInfo()
    {
        ...
        $test_url = tep_href_link('modules_content.php',
            'module='.$this->code.'&action=install&subaction=conntest');
...
EOD;
        ...
        $info = '<p><img src="images/icon_info.gif" border="0">&nbsp;<a href="javascript:openShowUrlsDialog();" style="text-decoration: underline; font-weight: bold;">'.MODULE_CONTENT_PAYPAL_LOGIN_DIALOG_URLS_LINK_TITLE.'</a></p>'.
            ...
            '  <p><strong>'.MODULE_CONTENT_PAYPAL_LOGIN_DIALOG_URLS_RETURN_TEXT.'</strong><br /><br />'.htmlspecialchars(str_replace('&amp;',
                    '&',
                    tep_catalog_href_link('login.php', 'action=paypal_login',
                        'SSL'))).'</p>'.
            '  <p><strong>'.MODULE_CONTENT_PAYPAL_LOGIN_DIALOG_URLS_PRIVACY_TEXT.'</strong><br /><br />'.htmlspecialchars(str_replace('&amp;',
                    '&', tep_catalog_href_link('privacy.php', '', 'SSL'))).'</p>'.
            '  <p><strong>'.MODULE_CONTENT_PAYPAL_LOGIN_DIALOG_URLS_TERMS_TEXT.'</strong><br /><br />'.htmlspecialchars(str_replace('&amp;',
                    '&', tep_catalog_href_link('conditions.php', '', 'SSL'))).'</p>'.
            '</div>'.
            $js;
    ...
    function hasAttribute($attribute)
    {
        return in_array($attribute,
            explode(';', MODULE_CONTENT_PAYPAL_LOGIN_ATTRIBUTES));
    }
    ...
    foreach (cm_paypal_login_get_attributes() as $group => $attributes) {
        ...
        foreach ($attributes as $attribute => $scope) {
            if (in_array($attribute, $required_array)) {
                $output .= tep_draw_radio_field('cm_paypal_login_attributes_tmp_'.$attribute,
                        $attribute, true).'&nbsp;';
            } else {
                $output .= tep_draw_checkbox_field('cm_paypal_login_attributes[]',
                        $attribute, in_array($attribute, $values_array)).'&nbsp;';
            }
