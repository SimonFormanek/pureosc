...
class cm_pi_reviews
{
    ...
    function execute()
    {
        ...
        if (tep_db_num_rows($review_query) > 0) {
            while ($review = tep_db_fetch_array($review_query)) {
                ...
                $review_data .= '  <footer>'.sprintf(MODULE_CONTENT_PRODUCT_INFO_REVIEWS_TEXT_RATED,
                        tep_draw_stars($review['reviews_rating']), $review_name,
                        $review_name).'</footer>';
    ...
    function keys()
    {
        return array('MODULE_CONTENT_PRODUCT_INFO_REVIEWS_STATUS', 'MODULE_CONTENT_PRODUCT_INFO_REVIEWS_CONTENT_WIDTH',
            'MODULE_CONTENT_PRODUCT_INFO_REVIEWS_CONTENT_LIMIT', 'MODULE_CONTENT_PRODUCT_INFO_REVIEWS_SORT_ORDER');
    }
}
