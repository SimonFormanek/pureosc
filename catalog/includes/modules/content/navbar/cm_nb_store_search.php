...
class cm_nb_store_search
{
    ...
    function __construct()
    {
        ...
        if (defined('MODULE_NAVIGATION_BAR_STORE_SEARCH_STATUS')) {
            ...
            $this->side       = ((MODULE_NAVIGATION_BAR_STORE_SEARCH_PLACEMENT === 'left')
                    ? 'left' : 'right');
        }
    }
    ...
    public function getOutput()
    {
        ...
        $search_box = tep_navbar_store_search('btn-info',
    ...
    public function install()
    {
        ...
        tep_db_query("insert into ".TABLE_CONFIGURATION." (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Pages', 'MODULE_NAVIGATION_BAR_STORE_SEARCH_PAGES', '".implode(';',
                $this->get_default_pages())."', 'The pages to add the Store Search\'s results.', '6', '0', 'cm_nb_store_search_show_pages', 'cm_nb_store_search_pages(', now())");
...
function cm_nb_store_search_pages($values, $key)
{
    ...
    foreach ($files_array as $file) {
        $output .= tep_draw_checkbox_field('cm_nb_store_search_file[]', $file,
