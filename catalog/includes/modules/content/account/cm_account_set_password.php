...
class cm_account_set_password
{
    ...
    function execute()
    {
        ...
        if (tep_session_is_registered('customer_id')) {
            ...
            if (empty($check['customers_password'])) {
                ...
                $before_eight = array_slice($oscTemplate->_data['account']['account']['links'],
                    0, $counter, true);
                $after_eight  = array_slice($oscTemplate->_data['account']['account']['links'],
                    $counter + 1, null, true);

                $oscTemplate->_data['account']['account']['links'] = $before_eight;

                if (MODULE_CONTENT_ACCOUNT_SET_PASSWORD_ALLOW_PASSWORD == 'True') {
                    $oscTemplate->_data['account']['account']['links'] += array(
                        'set_password' => array('title' => MODULE_CONTENT_ACCOUNT_SET_PASSWORD_SET_PASSWORD_LINK_TITLE,
    ...
    function keys()
    {
        return array('MODULE_CONTENT_ACCOUNT_SET_PASSWORD_STATUS', 'MODULE_CONTENT_ACCOUNT_SET_PASSWORD_ALLOW_PASSWORD',
