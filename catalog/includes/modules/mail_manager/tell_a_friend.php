...
//define values for placeholder variables
    $product_link  = tep_href_link(FILENAME_PRODUCT_INFO,
        'products_id='.$_GET['products_id'], 'NONSSL', false);
    $product_image = tep_image($image_urlfix.DIR_WS_IMAGES.$product_info['products_image_med'],
        $product_info['products_name'], '', '', '');
    $product_name  = $product_info['products_name'];


//define placeholders
    $placeholders        = array('$storeurl', '$storename', '$storeemail', '$toname',
        '$fromname', '$product', '$link', '$image', '$message');
    $values              = array(HTTP_SERVER, STORE_NAME, STORE_OWNER_EMAIL_ADDRESS,
        $to_name, $from_name, $product_name, $product_link, $product_image, $message);
...
//send email      
    tep_mm_sendmail($to_name, $to_email_address, $from_name,
        $from_email_address, $output_subject, $output_content_html,
        $output_content_txt);
