...
if (tep_session_is_registered('customer_is_guest')) {
    ...
    $invoice_url = EMAIL_TEXT_INVOICE_URL.' '.tep_href_link(FILENAME_ACCOUNT_HISTORY_INFO,
...
//define placeholders
$placeholders        = array('$storeurl', '$storename', '$storeemail', '$separator',
    '$orderno', '$orderdate', '$invoiceurl', '$productsorderedhead', '$productsordered',
    '$deliveryaddresshead', '$deliveryaddress', '$billingaddresshead', '$billingaddress',
...
//send email      
tep_mm_sendmail($order->customer['firstname'].' '.$order->customer['lastname'],
    $order->customer['email_address'], STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS,
    $output_subject, $output_content_html, strip_tags($output_content_txt));
?>