...
//define values for placeholder variables
    ...
    $status_newhtml = sprintf(EMAIL_HTML_STATUS_UPDATE,
        $orders_status_array[$status]);
    $status_newtxt  = sprintf(EMAIL_TEXT_STATUS_UPDATE,
        $orders_status_array[$status]);
    $comments       = $notify_comments;
    $invoice_url    = EMAIL_TEXT_INVOICE_URL.' '.tep_catalog_href_link(FILENAME_CATALOG_ACCOUNT_HISTORY_INFO,
            'order_id='.$oID, 'SSL');

//define placeholders
    $placeholders        = array('$storeurl', '$storename', '$storeemail', '$customername',
        '$customeremail', '$emailsubject', '$orderno', '$orderdate', '$statusnewhtml',
...
//send email      
    tep_mm_sendmail($check_status['customers_name'],
        $check_status['customers_email_address'], STORE_OWNER,
...
//if mail manager status update email 'inactive', or value=0, process normally via oscommerce
    ...
    tep_mail($check_status['customers_name'],
        $check_status['customers_email_address'], EMAIL_TEXT_SUBJECT, $email,
