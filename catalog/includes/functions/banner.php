    ...
    if (tep_db_num_rows($banners_query)) {
        while ($banners = tep_db_fetch_array($banners_query)) {
            if (tep_not_null($banners['expires_date'])) {
                ...
                if (($banners['expires_impressions'] > 0) && ($banners['banners_shown']
...
function tep_display_banner($action, $identifier)
{
    ...
    if (tep_not_null($banner['banners_html_text'])) {
        ...
        $banner_string = '<a href="'.tep_href_link(FILENAME_REDIRECT,
                'action=banner&goto='.$banner['banners_id']).'">'.tep_image(DIR_WS_IMAGES.$banner['banners_image'],
