    ...
    if (is_array($_GET) && (sizeof($_GET) > 0)) {
        ...
        while (list($key, $value) = each($_GET)) {
            if (is_string($value) && (strlen($value) > 0) && ($key != tep_session_name())
                && ($key != 'error') && (!in_array($key, $exclude_array)) && ($key
                != 'x') && ($key != 'y')) {
...
function tep_get_path($current_category_id = '')
{
    ...
    if (tep_not_null($current_category_id)) {
        ...
        if ($cp_size == 0) {
            ...
            $cPath_new           = '';
            $last_category_query = tep_db_query("select parent_id from ".TABLE_CATEGORIES." where categories_id = '".(int) $cPath_array[($cp_size
                - 1)]."'");
...
function tep_round($number, $precision)
{
    if (strpos($number, '.') && (strlen(substr($number, strpos($number, '.') + 1))
        ...
        if (substr($number, -1) >= 5) {
            if ($precision > 1) {
                $number = substr($number, 0, -1) + ('0.'.str_repeat(0,
                        $precision - 1).'1');
            } elseif ($precision == 1) {
...
function tep_get_tax_rate($class_id, $country_id = -1, $zone_id = -1)
{
    ...
    if (!isset($tax_rates[$class_id][$country_id][$zone_id]['rate'])) {
        ...
        if (tep_db_num_rows($tax_query)) {
            ...
            $tax_rates[$class_id][$country_id][$zone_id]['rate'] = ($tax_multiplier
                - 1.0) * 100;
        } else {
...
function tep_address_label($customers_id, $address_id = 1, $html = false,
                           $boln = '', $eoln = "\n")
{
    if (is_array($address_id) && !empty($address_id)) {
        return tep_address_format($address_id['address_format_id'], $address_id,
            $html, $boln, $eoln);
    }
...
function tep_get_categories($categories_array = '', $parent_id = '0',
                            $indent = '')
{
    global $languages_id;
    ...
    while ($categories       = tep_db_fetch_array($categories_query)) {
        ...
        if ($categories['categories_id'] != $parent_id) {
            $categories_array = tep_get_categories($categories_array,
                $categories['categories_id'], $indent.'&nbsp;&nbsp;');
        }
    }
...
function tep_checkdate($date_to_check, $format_string, &$date_array)
{
    ...
    $separators = array('-', ' ', '/', '.');
    $month_abbr = array('jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep',
        'oct', 'nov', 'dec');
    ...
    if ($date_separator_idx != -1) {
        $format_string_array = explode($separators[$date_separator_idx],
            $format_string);
        if (sizeof($format_string_array) != 3) {
            return false;
        }

        $date_to_check_array = explode($separators[$date_separator_idx],
            $date_to_check);
        ...
        for ($i = 0; $i < $size; $i++) {
            ...
            if (($format_string_array[$i] == 'yyyy') || ($format_string_array[$i]
                == 'aaaa')) $year  = $date_to_check_array[$i];
        }
    } else {
    ...
    if ($sortby) {
        $sort_prefix = '<a href="'.tep_href_link($_SERVER['PHP_SELF'],
                ...
                == $colnum.'d' || substr($sortby, 0, 1) != $colnum ? TEXT_ASCENDINGLY
                    : TEXT_DESCENDINGLY).TEXT_BY.$heading).'" class="productListing-heading">';
        $sort_suffix = (substr($sortby, 0, 1) == $colnum ? (substr($sortby, 1, 1)
            == 'a' ? '+' : '-') : '').'</a>';
    }
...
function tep_mail($to_name, $to_email_address, $email_subject, $email_text,
                  $from_email_name, $from_email_address, $attachments = array())
{
    ...
    if (count($attachments) > 0)
            ...
            $message->add_attachment($aV, $aK, 'base64',
    ...
    $message->send($to_name, $to_email_address, $from_email_name,
        $from_email_address, iconv("UTF-8", "ASCII//TRANSLIT", $email_subject));
}
...
function tep_array_to_string($array, $exclude = '', $equals = '=',
                             $separator = '&')
{
...
function tep_setcookie($name, $value = '', $expire = 0, $path = '/',
                       $domain = '', $secure = 0)
{
    setcookie($name, $value, $expire, $path,
        (tep_not_null($domain) ? $domain : ''), $secure);
}
