...
function tep_header_tag_page($file)
{
    ...
    if ($pageTags['append_default_description'] && tep_not_null($tmpTags['def_desc']))
            $sortOrder['description'][$pageTags['sortorder_description']] = $tmpTags['def_desc'];
    ...
    $pageName   = substr($path_parts['basename'], 0,
            strpos($path_parts['basename'], '.')).' ';
    $pageName   = ucwords(preg_replace("/[^A-Za-z0-9]/", " ", $pageName));

    if (!tep_not_null($header_tags_array['title']))
            $header_tags_array['title']       = $pageName.(tep_not_null($tmpTags['def_title'])
                ? HEADER_TAGS_SEPARATOR_DESCRIPTION.' '.$tmpTags['def_title'] : '');
    ...
    if (!tep_not_null($header_tags_array['description']))
            $header_tags_array['description'] = $pageName.(tep_not_null($tmpTags['def_desc'])
                ? HEADER_TAGS_SEPARATOR_DESCRIPTION.' '.$tmpTags['def_desc'] : '');
    if (!tep_not_null($header_tags_array['keywords']))
            $header_tags_array['keywords']    = $pageName.(tep_not_null($tmpTags['def_keywords'])
                ? HEADER_TAGS_SEPARATOR_KEYWORD.' '.$tmpTags['def_keywords'] : '');
    if (!tep_not_null($header_tags_array['logo']))
            $header_tags_array['logo']        = $pageName.(tep_not_null($tmpTags['def_logo_text'])
                ? HEADER_TAGS_SEPARATOR_DESCRIPTION.' '.$tmpTags['def_logo_text']
                : '');
...
{
    if (count($sortOrder) == 0) return;

    $sortOrder = MultiKeySort($sortOrder);

    if (isset($sortOrder['title']))
            $header_tags_array['title']       = ltrim(tep_db_prepare_input(implode(' '.HEADER_TAGS_SEPARATOR_DESCRIPTION.' ',
                    str_replace('"', '&quot;', $sortOrder['title']))),
            ' '.HEADER_TAGS_SEPARATOR_DESCRIPTION);
    if (isset($sortOrder['description']))
            $header_tags_array['desc']        = ltrim(tep_db_prepare_input(implode(' '.HEADER_TAGS_SEPARATOR_DESCRIPTION.' ',
                    str_replace('"', '&quot;', $sortOrder['description']))),
            ' '.HEADER_TAGS_SEPARATOR_DESCRIPTION);
    if (isset($sortOrder['keywords']))
            $header_tags_array['keywords']    = ltrim(tep_db_prepare_input(implode(' '.HEADER_TAGS_SEPARATOR_KEYWORD.' ',
                    $sortOrder['keywords'])),
            ' '.HEADER_TAGS_SEPARATOR_KEYWORD);
    if (isset($sortOrder['logo']))
            $header_tags_array['logo_text']   = ltrim(tep_db_prepare_input(implode(' '.HEADER_TAGS_SEPARATOR_DESCRIPTION.' ',
                    $sortOrder['logo'])),
            ' '.HEADER_TAGS_SEPARATOR_DESCRIPTION);
    if (isset($sortOrder['logo_1']))
            $header_tags_array['logo_text_1'] = ltrim(tep_db_prepare_input(implode(' '.HEADER_TAGS_SEPARATOR_DESCRIPTION.' ',
                    $sortOrder['logo_1'])),
            ' '.HEADER_TAGS_SEPARATOR_DESCRIPTION);
    if (isset($sortOrder['logo_2']))
            $header_tags_array['logo_text_2'] = ltrim(tep_db_prepare_input(implode(' '.HEADER_TAGS_SEPARATOR_DESCRIPTION.' ',
                    $sortOrder['logo_2'])),
            ' '.HEADER_TAGS_SEPARATOR_DESCRIPTION);
    if (isset($sortOrder['logo_3']))
            $header_tags_array['logo_text_3'] = ltrim(tep_db_prepare_input(implode(' '.HEADER_TAGS_SEPARATOR_DESCRIPTION.' ',
                    $sortOrder['logo_3'])),
            ' '.HEADER_TAGS_SEPARATOR_DESCRIPTION);
    if (isset($sortOrder['logo_4']))
            $header_tags_array['logo_text_4'] = ltrim(tep_db_prepare_input(implode(' '.HEADER_TAGS_SEPARATOR_DESCRIPTION.' ',
                    $sortOrder['logo_4'])),
            ' '.HEADER_TAGS_SEPARATOR_DESCRIPTION);
}

function GetCategoryAndManufacturer($sortOrder, $pageTags, $defaultTags,
                                    $catStr, $manStr, $product = false)
{
    ...
    if ($category_depth == 'nested' || $category_depth == 'products')
            $type = 'cat';
    else if (isset($_GET['manufacturers_id'])) $type = 'man';

    $product = ($product && ($pageTags['append_category'] || $pageTags['append_manufacturer']
        || $defaultTags['default_logo_append_category']) ? true : false);

    if (($type == 'cat' || $type == 'top') && ($pageTags['append_category'] || $defaultTags['default_logo_append_group']
        || $defaultTags['default_logo_append_category'] || $product)) {
        if ($category_depth == 'nested' || $category_depth == 'products' || $product) {
            $the_category_query = tep_db_query($catStr);
            $parentStr          = '';

            if (HEADER_TAGS_ADD_CATEGORY_PARENTS == 'Duplicate Categories' && $product
                && tep_db_num_rows($the_category_query) > 1) { //selected product is in multiple categories
                $ctr        = 0;
                ...
                if (HEADER_TAGS_ADD_CATEGORY_PARENTS == 'Full Category Path')
                        $parentStr = GetCategoryParentString($current_category_id,
                        $languages_id);
            }

            if (tep_not_null($the_category['htc_title_tag'])) {
                $catTitle                                            = tep_not_null($parentStr)
                        ? ($parentStr.strip_tags($the_category['htc_title_tag']))
                        : strip_tags($the_category['htc_title_tag']);
                $sortOrder['title'][$pageTags['sortorder_category']] = tep_not_null($sortOrder['title'][$pageTags['sortorder_category']])
                        ? $sortOrder['title'][$pageTags['sortorder_category']].' '.HEADER_TAGS_SEPARATOR_DESCRIPTION.' '.$catTitle
                        : $catTitle;
                $sortOrder['logo'][$pageTags['sortorder_category']]  = tep_not_null($sortOrder['logo'][$pageTags['sortorder_category']])
                        ? $sortOrder['logo'][$pageTags['sortorder_category']].' '.HEADER_TAGS_SEPARATOR_DESCRIPTION.' '.$catTitle
                        : $catTitle;
            }
            if (tep_not_null($the_category['htc_desc_tag'])) {
                $catDesc                                                   = tep_not_null($parentStr)
                        ? ($parentStr.strip_tags($the_category['htc_desc_tag']))
                        : strip_tags($the_category['htc_desc_tag']);
                $sortOrder['description'][$pageTags['sortorder_category']] = tep_not_null($sortOrder['description'][$pageTags['sortorder_category']])
                        ? $sortOrder['description'][$pageTags['sortorder_category']].' '.HEADER_TAGS_SEPARATOR_DESCRIPTION.' '.$catDesc
                        : $catDesc;
            }
            if (tep_not_null($the_category['htc_keywords_tag'])) {
                $catKeywords                                            = tep_not_null($parentStr)
                        ? (str_replace(HEADER_TAGS_SEPARATOR_DESCRIPTION,
                        HEADER_TAGS_SEPARATOR_KEYWORD, $parentStr).strip_tags($the_category['htc_keywords_tag']))
                        : strip_tags($the_category['htc_keywords_tag']);
                $sortOrder['keywords'][$pageTags['sortorder_category']] = tep_not_null($sortOrder['keywords'][$pageTags['sortorder_category']])
                        ? $sortOrder['keywords'][$pageTags['sortorder_category']].' '.HEADER_TAGS_SEPARATOR_KEYWORD.' '.$catkeywords
                        : $catKeywords;
            }
        }
    }

    if (tep_not_null($manStr) && (($type == 'top' || $type == 'man') && ($pageTags['append_manufacturer']
        || $defaultTags['default_logo_append_group'] || $defaultTags['default_logo_append_manufacturer']))) {
        ...
        if (tep_not_null($the_manufacturer['htc_title_tag'])) {
            $sortOrder['title'][$pageTags['sortorder_manufacturer']] = tep_not_null($sortOrder['title'][$pageTags['sortorder_manufacturer']])
                    ? $sortOrder['title'][$pageTags['sortorder_manufacturer']].' '.HEADER_TAGS_SEPARATOR_DESCRIPTION.' '.strip_tags($the_manufacturer['htc_title_tag'])
                    : strip_tags($the_manufacturer['htc_title_tag']);
            $sortOrder['logo'][$pageTags['sortorder_manufacturer']]  = tep_not_null($sortOrder['logo'][$pageTags['sortorder_manufacturer']])
                    ? $sortOrder['title'][$pageTags['sortorder_manufacturer']].' '.HEADER_TAGS_SEPARATOR_DESCRIPTION.' '.strip_tags($the_manufacturer['htc_title_tag'])
                    : strip_tags($the_manufacturer['htc_title_tag']);
        }
        if (tep_not_null($the_manufacturer['htc_desc_tag'])) {
            $sortOrder['description'][$pageTags['sortorder_manufacturer']] = tep_not_null($sortOrder['description'][$pageTags['sortorder_manufacturer']])
                    ? $sortOrder['title'][$pageTags['sortorder_manufacturer']].' '.HEADER_TAGS_SEPARATOR_DESCRIPTION.' '.strip_tags($the_manufacturer['htc_desc_tag'])
                    : strip_tags($the_manufacturer['htc_desc_tag']);
        ...
        if (tep_not_null($the_manufacturer['htc_keywords_tag'])) {
            $sortOrder['keywords'][$pageTags['sortorder_manufacturer']] = tep_not_null($sortOrder['keywords'][$pageTags['sortorder_manufacturer']])
                    ? $sortOrder['title'][$pageTags['sortorder_manufacturer']].' '.HEADER_TAGS_SEPARATOR_KEYWORD.' '.strip_tags($the_manufacturer['htc_keywords_tag'])
                    : strip_tags($the_manufacturer['htc_keywords_tag']);
        }
    }
    return $sortOrder;
}
...
function GetCanonicalURL()
{
    ...
    if ($cnt == 1 && basename($_SERVER['SCRIPT_FILENAME']) === FILENAME_DEFAULT) { //home page
        ...
        if (tep_not_null($cPath) || (isset($_GET['manufacturers_id']) && $category_depth
            == 'top')) {
            $args = tep_get_all_get_params(array('action', 'currency', tep_session_name(),
                'cPath', 'manufacturers_id', 'sort', 'page', 'language'));
            return StripSID(tep_href_link(FILENAME_DEFAULT, $args,
                    $request_type, false));
        }
    ...
    $args = tep_get_all_get_params(array('action', 'currency', tep_session_name(),
        'cPath', 'manufacturers_id', 'sort', 'page', 'language'));
    return StripSID(tep_href_link(basename($_SERVER['SCRIPT_FILENAME']), $args,
            $request_type, false));
}
...
function GetHTSTagCloudLink($keyword, $languages_id)
{
    ...
    if (tep_db_num_rows($hts_tags_query) > 0) {
        ...
        return tep_href_link(FILENAME_PRODUCT_INFO,
    ...
    if (tep_db_num_rows($hts_tags_query) > 0) {
        ...
        return tep_href_link(FILENAME_DEFAULT,
            'manufacturers_id='.$id['manufacturers_id']);
    }
        ...
        foreach ($kwords as $word) {
            ...
            if (HEADER_TAGS_KEYWORD_HIGHLIGHTER == 'Highlight Individual Words') {
                ...
                foreach ($parts as $oneword) {
                    $description = str_ireplace($oneword,
                        '<strong>'.$oneword.'</strong>', $description); //the actual replacement
                }
            } else {
                $description = str_ireplace($word,
                    '<strong>'.$word.'</strong>', $description); //the actual replacement
            }

            $description = str_replace('<strong><strong>', '<strong>',
                $description);          //the following is a lazy way to
            $description = str_replace('</strong></strong>', '</strong>',
                $description);       //clear duplicate tags. It would be  
            $description = str_replace('<b><strong>', '<strong>', $description);               //better coded with preg_replace
            $description = str_replace('</strong></b>', '</strong>',
                $description);
        }
    }
...
{
    global $language;
    return ( (HEADER_TAGS_ENABLE_CACHE == 'None') ? false : ReadCacheFromDB($header_tags_array,
            'header_tags_'.$filename.'_'.$language.'_cache_id_'.$id) );
}
