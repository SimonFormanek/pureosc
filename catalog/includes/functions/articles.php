...
function tep_get_topic_path($current_topic_id = '')
{
    ...
    if (tep_not_null($current_topic_id)) {
        ...
        if ($cp_size == 0) {
            ...
            $tPath_new        = '';
            $last_topic_query = tep_db_query("select parent_id from ".TABLE_TOPICS." where topics_id = '".(int) $tPath_array[($cp_size
                - 1)]."'");
...
function tep_count_articles_in_topic($topic_id, $include_inactive = false)
{
    ...
    if (tep_db_num_rows($child_topics_query)) {
        while ($child_topics = tep_db_fetch_array($child_topics_query)) {
            $articles_count += tep_count_articles_in_topic($child_topics['topics_id'],
                $include_inactive);
        }
    }
...
function tep_get_topics($topics_array = '', $parent_id = '0', $indent = '')
{
    ...
    while ($topics       = tep_db_fetch_array($topics_query)) {
        ...
        if ($topics['topics_id'] != $parent_id) {
            $topics_array = tep_get_topics($topics_array, $topics['topics_id'],
                $indent);
        }
    }
...
function tep_get_all_blog_articles_array()
{
    global $languages_id;
    $articlesArray  = array();
    $articles_query = tep_db_query("select a.articles_id, ad.articles_name from   ".TABLE_ARTICLES." a left join ".TABLE_ARTICLES_TO_TOPICS." a2t on a.articles_id = a2t.articles_id left join ".TABLE_TOPICS_DESCRIPTION." td on a2t.topics_id = td.topics_id left join ".TABLE_AUTHORS." au on a.authors_id = au.authors_id left join ".TABLE_ARTICLES_DESCRIPTION." ad on a.articles_id = ad.articles_id  where (a.articles_date_available IS NULL or to_days(a.articles_date_available) <= to_days(now()))  and a.articles_status = '1' and a.articles_is_blog = 1 and ad.language_id = '".(int) $languages_id."' and td.language_id = '".(int) $languages_id."' order by a.articles_sort_order, ad.articles_name, a.articles_date_added desc");
    while ($articles       = tep_db_fetch_array($articles_query)) {
        $articlesArray[] = array('id' => $articles['articles_id'], 'text' => $articles['articles_name']);
    }
    return $articlesArray;
...
function GetArticleLinsByTopic($topic)
{
    ...
    $articles_query = tep_db_query("SELECT a.articles_id, ad.articles_name, a.articles_is_blog, td.topics_name from ".
        ...
        where  a.articles_status = '1' and 
               ...
               ad.language_id = '".(int) $languages_id."' and 
               td.language_id = '".(int) $languages_id."'");

    $aLinks = '';
    while ($db     = tep_db_fetch_array($articles_query)) {
        $locn   = ($db['articles_is_blog'] ? 'article_blog.php' : 'article_info.php');
        $aLinks .= '<li><a href="'.tep_href_link($locn,
                'articles_id='.$db['articles_id']).'">'.$db['articles_name'].'</a></li>';
    }
