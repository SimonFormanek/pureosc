...
while ($articles = tep_db_fetch_array($articles_query)) {
  ...
  if (isset($articles['articles_alias'])) {
    ...
    elseif ($this->on_empty_custom_alias_auto_create_alias)
      $articles_alias = $this->create_alias($articles['articles_name'], $create_alias_options);
    else
...
if ($this->use_default_language_aliases) {
  ...
  foreach ($this->articles as $key => $value) {
    ...
    if (isset($this->catalog_languages_rev[DEFAULT_LANGUAGE]) && $this->catalog_languages_rev[DEFAULT_LANGUAGE] == $key)
      continue;
...
while ($topics = tep_db_fetch_array($topics_query)) {
  ...
  if (isset($topics['topics_alias'])) {
    ...
    elseif ($this->on_empty_custom_alias_auto_create_alias)
      $topics_alias = $this->create_alias($topics['topics_name'], $create_alias_options);
    else
  ...
  while ($loop && ( ( $option = each($children[$parent]) ) || ( $parent > $root_id ) )) {
    if ($option === false) {
      ...
      if ($this->full_path_aliases)
        $path_alias = count($stack) <= 0 ? $option['value']['alias'] : implode('/', $stack_alias);
      else
        $path_alias = implode('/', $stack_alias);
      ...
      if (isset($this->articles[$clang][$option['value']['id']])) {
        ...
        if ($this->full_path_aliases)
          foreach ($this->articles[$clang][$option['value']['id']] as $key => $value)
            $this->articles[$clang][$option['value']['id']][$key] = $value == '' ? '' : $path_alias . '/' . $value;
        else
          foreach ($this->articles[$clang][$option['value']['id']] as $key => $value)
            $this->articles[$clang][$option['value']['id']][$key] = $value == '' ? '' : $lang_alias . $value;
      }
      ...
      if ($this->full_path_aliases)
        $path_alias = count($stack) <= 0 ? $option['value']['alias'] : implode('/', $stack_alias) . '/' . $option['value']['alias'];
      else
        $path_alias = $option['value']['alias'];
      ...
      if (count($stack) <= 0)
        ...
        $this->topics[$clang][$rt . implode('_', $stack) . '_' . $option['value']['id']] = $path_alias;
