...
while ($articles       = tep_db_fetch_array($articles_query)) {
    ...
    if (isset($articles['articles_alias'])) {
        ...
        if (tep_not_null($articles['articles_alias']))
                $articles_alias = $articles['articles_alias'];
        elseif ($this->on_empty_custom_alias_auto_create_alias)
                $articles_alias = $this->create_alias($articles['articles_name'],
                $create_alias_options);
        else
                $articles_alias = ''; //by making it empty tep_href_link will use the osc default urls those with get variables
    } else
            $articles_alias = $this->create_alias($articles['articles_name'],
            $create_alias_options);

    $this->articles[$articles['language_id']][$articles['topics_id']][$articles['articles_id']]
        = $articles_alias;
}
...
if ($this->use_default_language_aliases) {
    ...
    foreach ($this->articles as $key => $value) {
        ...
        if (isset($this->catalog_languages_rev[DEFAULT_LANGUAGE]) && $this->catalog_languages_rev[DEFAULT_LANGUAGE]
            == $key) continue;
...
while ($topics       = tep_db_fetch_array($topics_query)) {
    ...
    if (isset($topics['topics_alias'])) {
        ...
        if (tep_not_null($topics['topics_alias']))
                $topics_alias = $topics['topics_alias'];
        elseif ($this->on_empty_custom_alias_auto_create_alias)
                $topics_alias = $this->create_alias($topics['topics_name'],
                $create_alias_options);
        else
                $topics_alias = ''; //by making it empty tep_href_link will use the osc default urls those with get variables
    } else
            $topics_alias = $this->create_alias($topics['topics_name'],
            $create_alias_options);

    $items[$topics['language_id']][$topics['topics_id']] = array('name' => $topics['topics_name'],
        'alias' => $topics_alias, 'parent_id' => $topics['parent_id'], 'id' => $topics['topics_id']);
}
    ...
    foreach ($items as $key => $value) {
        ...
        if ($this->on_empty_custom_alias_auto_create_alias) {
            ...
            foreach ($value as $k => $v)
                $items[$key][$k]['alias'] = $items[$this->catalog_languages_rev[DEFAULT_LANGUAGE]][$k]['alias'];
        }
    }
    //$items_copy = null;
}
...
foreach ($clangs as $clang) {
    ...
    if ($citems <= 0) ;
    elseif ($citems == 1)
            $children[]                     = $items[$clang]; //in case we have one category item without subtopics, rare but possible
    else
            foreach ($items[$clang] as $item)
    ...
    while ($loop && ( ( $option = each($children[$parent]) ) || ( $parent > $root_id ) )) {
        if ($option === false) {
            ...
            if ($this->full_path_aliases)
                    $path_alias = count($stack) <= 0 ? $option['value']['alias']
                        : implode('/', $stack_alias);
            ...
            if ($option['value']['alias'] == '') $path_alias = '';
                ...
                if (isset($this->catalog_languages[$clang]) && $this->catalog_languages[$clang]
                    == DEFAULT_LANGUAGE) {
            ...
            if (count($stack) <= 0)
                    $this->topics[$clang][$rt.$option['value']['id']] = $path_alias;
            else
                    $this->topics[$clang][$rt.implode('_', $stack)]   = $path_alias;

            if (isset($this->articles[$clang][$option['value']['id']])) {

                if ($this->full_path_aliases)
                        foreach ($this->articles[$clang][$option['value']['id']] as $key => $value)
                        $this->articles[$clang][$option['value']['id']][$key] = $value
                            == '' ? '' : $path_alias.'/'.$value;
                else
                        foreach ($this->articles[$clang][$option['value']['id']] as $key => $value)
                        $this->articles[$clang][$option['value']['id']][$key] = $value
            ...
            if ($this->full_path_aliases)
                    $path_alias = count($stack) <= 0 ? $option['value']['alias']
                        : implode('/', $stack_alias).'/'.$option['value']['alias'];
            ...
            if (count($stack) <= 0)
                    ...
                    $this->topics[$clang][$rt.implode('_', $stack).'_'.$option['value']['id']]
                    = $path_alias;
