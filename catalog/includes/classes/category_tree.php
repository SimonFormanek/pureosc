...
class category_tree
{
    ...
    public function __construct()
    {
        ...
        if (isset($_category_tree_data)) {
            ...
            $categories_query = tep_db_query("select c.categories_id, c.parent_id, c.categories_image, cd.categories_name from ".TABLE_CATEGORIES." c, ".TABLE_CATEGORIES_DESCRIPTION." cd where c.categories_id = cd.categories_id and cd.language_id = '".(int) $languages_id."' AND sort_order > 0 order by c.parent_id, c.sort_order, cd.categories_name");
                ...
                if (isset($this->_data[$category_id]) && (($this->max_level == '0')
                    ...
                    if ($this->follow_cpath === true) {
                        if (in_array($category_id, $this->cpath_array)) {
                            $result .= $this->_buildBranch($category_id,
                                $level + 1);
                        }
                    } else {
        ...
        if (isset($this->_data[$parent_id])) {
            foreach ($this->_data[$parent_id] as $category_id => $category) {
                ...
                $result[] = array('id' => $category_link,
                    'title' => str_repeat($this->spacer_string,
                        $this->spacer_multiplier * $level).$category['name']);

                if (isset($this->_data[$category_id]) && (($this->max_level == '0')
                    || ($this->max_level > $level + 1))) {
                    if ($this->follow_cpath === true) {
                        if (in_array($category_id, $this->cpath_array)) {
                            $result = $this->buildBranchArray($category_id,
                                $level + 1, $result);
                        }
                    } else {
                        $result = $this->buildBranchArray($category_id,
    ...
    {
        return $this->buildBranchArray((empty($parent_id) ? $this->root_category_id
                    : $parent_id));
    }
    ...
    function setParentGroupString($parent_group_start_string,
    ...
    function setCategoryPath($cpath, $cpath_start_string = '',
                             $cpath_end_string = '')
    {
