...
class actionRecorder
{
    ...
    function actionRecorder($module, $user_id = null, $user_name = null)
    {
        ...
        if (defined('MODULE_ACTION_RECORDER_INSTALLED') && tep_not_null(MODULE_ACTION_RECORDER_INSTALLED)) {
            if (tep_not_null($module) && in_array($module.'.'.substr($_SERVER['PHP_SELF'],
                    ...
                    explode(';', MODULE_ACTION_RECORDER_INSTALLED))) {
                if (!class_exists($module)) {
                    if (file_exists(DIR_WS_MODULES.'action_recorder/'.$module.'.'.substr($_SERVER['PHP_SELF'],
                                (strrpos($_SERVER['PHP_SELF'], '.') + 1)))) {
                        include(DIR_WS_LANGUAGES.$language.'/modules/action_recorder/'.$module.'.'.substr($_SERVER['PHP_SELF'],
                                (strrpos($_SERVER['PHP_SELF'], '.') + 1)));
                        include(DIR_WS_MODULES.'action_recorder/'.$module.'.'.substr($_SERVER['PHP_SELF'],
                                (strrpos($_SERVER['PHP_SELF'], '.') + 1)));
                    } else {
                        return false;
                    }
    ...
    function canPerform()
    {
        if (tep_not_null($this->_module)) {
            return $GLOBALS[$this->_module]->canPerform($this->_user_id,
                    $this->_user_name);
        }
    ...
    {
        if (tep_not_null($this->_module)) {
            tep_db_query("insert into ".TABLE_ACTION_RECORDER." (module, user_id, user_name, identifier, success, date_added) values ('".tep_db_input($this->_module)."', '".(int) $this->_user_id."', '".tep_db_input($this->_user_name)."', '".tep_db_input($this->getIdentifier())."', '".($success
                === true ? 1 : 0)."', now())");
        }
    }
