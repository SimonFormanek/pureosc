        ...
        if (is_numeric($products_id) && is_numeric($qty) && ($attributes_pass_check
            ...
            if (($check_product !== false) && ($check_product['products_status']
                ...
                if ($this->in_cart($products_id_string)) {
                    $this->update_quantity($products_id_string, $qty,
                        $attributes);
                } else {
...
// insert into database
                    ...
                    if (is_array($attributes)) {
                        reset($attributes);
                        while (list($option, $value) = each($attributes)) {
                            $this->contents[$products_id_string]['attributes'][$option]
                                = $value;
// insert into database
                            if (tep_session_is_registered('customer_id'))
...
// products price
            ...
            if ($product       = tep_db_fetch_array($product_query)) {
                ...
                $this->total_virtual  += tep_add_tax($products_price,
                        $products_tax) * $qty * $no_count;
                $this->weight_virtual += ($qty * $products_weight) * $no_count;
                $this->total          += tep_add_tax($products_price,
                        $products_tax) * $qty;
                /*                 * * EOF alterations for CCGV ** */
                ...
                while (list($option, $value) = each($this->contents[$products_id]['attributes'])) {
                    ...
                    if ($attribute_price['price_prefix'] == '+') {
                        /*                         * * Altered for CCGV **			
                          ...
                          $this->total -= $currencies->calculate_price($attribute_price['options_values_price'], $products_tax, $qty);
                         */
                        $this->total += tep_add_tax($attribute_price['options_values_price'],
                            $products_tax, $qty);
                    } else {
                        $this->total -= tep_add_tax($attribute_price['options_values_price'],
                            $products_tax, $qty);
        ...
        while (list($products_id, ) = each($this->contents)) {
            ...
            if ($products       = tep_db_fetch_array($products_query)) {
                ...
                $products_array[] = array('id' => $products_id,
                    ...
                    'attributes' => (isset($this->contents[$products_id]['attributes'])
    ...
    {  // get total number of items in cart disregard gift vouchers
        ...
        if (is_array($this->contents)) {
            ...
            while (list($products_id, ) = each($this->contents)) {
                ...
                if (defined('NO_COUNT_ZERO_WEIGHT') && cfg('NO_COUNT_ZERO_WEIGHT') == 1) {
