...
# end function
  ...
  function strip($string) {
    ...
    $pattern = $this->attributes['SEO_REMOVE_ALL_SPEC_CHARS'] == 'true' ? "([^[:alnum:]])" : "/[^a-z0-9- ]/i";
...
# end function is_cached()
  ...
  function check_redirect() {
    ...
    $this->uri_parsed = $this->not_null($this->path_info) ? parse_url(basename($_SERVER['SCRIPT_NAME']) . '?' . $this->parse_path($this->path_info)) : parse_url(basename($_SERVER['REQUEST_URI']));
