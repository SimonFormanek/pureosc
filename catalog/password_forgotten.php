...
if (isset($_GET['action']) && ($_GET['action'] == 'process') && isset($_POST['formid'])
    ...
    if (tep_db_num_rows($check_customer_query)) {
        ...
        $actionRecorder = new actionRecorder('ar_reset_password',
            $check_customer['customers_id'], $email_address);
        ...
        if ($actionRecorder->canPerform()) {
            ...
            $reset_key_url = tep_href_link(FILENAME_PASSWORD_RESET,
                'account='.urlencode($email_address).'&key='.$reset_key, 'SSL',
                false);
            ...
            $messageStack->add('password_forgotten',
                sprintf(ERROR_ACTION_RECORDER,
                    (defined('MODULE_ACTION_RECORDER_RESET_PASSWORD_MINUTES') ? (int) MODULE_ACTION_RECORDER_RESET_PASSWORD_MINUTES
                            : 5)));
        }
    } else {
