<?php
define('SERVER_INSTANCE','shop'); // admin|shop
//NEW1:define('HTTPS_SERVER', 'http://pureosc.redmine.local'); //no thirth domain = use default language; en.domain.com, cs.domain.com
//NEW1:define('HTTP_SERVER', HTTPS_SERVER); 
define('RSYNC_LOCAL_DEST_PATH','/home/osc/WWW/'); //trailing slash! '/' DEST can be source...
define('RSYNC_REMOTE_DEST_DIR','/home/osc/WWW/');
