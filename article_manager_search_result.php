  ...
  <div class="contentText">
    ...
    if (tep_db_num_rows($articles_query)) {
        ...
        while ($articles = tep_db_fetch_array($articles_query))  {
           ...
           echo '<div class="smallText"><a href="' . tep_href_link(FILENAME_ARTICLE_INFO, 'articles_id='.$articles['articles_id'])  . '"><b>' . $articles['articles_name'] . '</b></a></div>';
           echo '<div class="smallText">' . (strlen($cleanedDescription) > MAX_ARTICLE_ABSTRACT_LENGTH ? substr($cleanedDescription, 0, MAX_ARTICLE_ABSTRACT_LENGTH) . '<a href="' . tep_href_link(FILENAME_ARTICLE_INFO, 'articles_id='.$articles['articles_id'])  . '">' . TEXT_SEARCH_SEE_MORE . '</a>' : $cleanedDescription ) . '</div>';
           echo '</div>';
        }
        echo '</div>';
    } else {
