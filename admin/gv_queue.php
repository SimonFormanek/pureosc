...
if ($_GET['action'] == 'confirmrelease' && isset($_GET['gid'])) {
    ...
    if ($gv_result['release_flag'] == 'N') {
        ...
        if ($gv_resulta = tep_db_fetch_array($gv_query)) {
            ...
            $message         .= sprintf(TEXT_REDEEM_COUPON_MESSAGE_AMOUNT,
    ...
    <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
                <tr>
                    <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                            ...
                            while ($gv_list      = tep_db_fetch_array($gv_query)) {
                                ...
                                if ((is_object($gInfo)) && ($gv_list['unique_id']
                                    == $gInfo->unique_id)) {
                                    echo '              <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\''.tep_href_link(FILENAME_GV_QUEUE,
                                        tep_get_all_get_params(array('gid', 'action')).'gid='.$gInfo->unique_id.'&action=edit').'\'">'."\n";
                                } else {
                                    echo '              <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\''.tep_href_link('gv_queue.php',
                                        tep_get_all_get_params(array('gid', 'action')).'gid='.$gv_list['unique_id']).'\'">'."\n";
                                }
                                ?>
                    ...
                    && ($gv_list['unique_id'] == $gInfo->unique_id)) {
                    echo tep_image(DIR_WS_IMAGES.'icon_arrow_right.gif');
                } else {
                    echo '<a href="'.tep_href_link(FILENAME_GV_QUEUE,
                        'page='.$_GET['page'].'&gid='.$gv_list['unique_id']).'">'.tep_image(DIR_WS_IMAGES.'icon_info.gif',
                        IMAGE_ICON_INFO).'</a>';
                } ?>&nbsp;</td>
                    </tr>
            <?php
        ...
        switch ($_GET['action']) {
            case 'release':
                ...
                $contents[] = array('align' => 'center', 'text' => '<a href="'.tep_href_link(FILENAME_GV_QUEUE,
                        'action=confirmrelease&gid='.$gInfo->unique_id, 'NONSSL').'">'.tep_draw_button('Confirm').'</a> <a href="'.tep_href_link(FILENAME_GV_QUEUE,
                        'action=cancel&gid='.$gInfo->unique_id, 'NONSSL').'">'.tep_draw_button(IMAGE_CANCEL,
                        'cancel').'</a>');
                break;
            default:
                $heading[]  = array('text' => '['.$gInfo->unique_id.'] '.tep_datetime_short($gInfo->date_created).' '.$currencies->format($gInfo->amount));
                $contents[] = array('align' => 'center', 'text' => '<a href="'.tep_href_link(FILENAME_GV_QUEUE,
                        'action=release&gid='.$gInfo->unique_id, 'NONSSL').'">'.tep_draw_button('Release').'</a>'.'&nbsp;&nbsp;<a href="'.tep_href_link(FILENAME_GV_QUEUE,
                        'action=delete&gid='.$gInfo->unique_id, 'NONSSL').'">'.tep_draw_button('Delete',
                        'trash', null, 'primary').'</a>');
                break;
        }
