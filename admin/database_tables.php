...
while ($charsets       = tep_db_fetch_array($charsets_query)) {
    $mysql_charsets[] = array('id' => $charsets['Charset'], 'text' => sprintf(ACTION_UTF8_CONVERSION_FROM,
            $charsets['Charset']));
}
    ...
    default:
        ...
        while ($sql       = tep_db_fetch_array($sql_query)) {
            $table_data[] = array(tep_output_string_protected($sql['Name']),
                ...
                round(($sql['Data_length'] + $sql['Index_length']) / 1024 / 1024,
                    2).'M',
...
<?php
if (isset($action)) {
    echo '<div style="float: right;">'.tep_draw_button(IMAGE_BACK,
        'triangle-1-w', tep_href_link('database_tables.php')).'</div>';
}
?>
