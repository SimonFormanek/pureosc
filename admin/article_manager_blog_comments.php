...
if (tep_not_null($action)) {
    ...
    switch ($action) {
        ...
        case 'save':
                    ...
                    for ($c = 0; $c < count($_POST['comments']); ++$c) {
                        ...
                        if (count($_POST['comments'][$language_id])) {
                            ...
                            if ($action == 'insert') {
                                ...
                                tep_db_perform(TABLE_ARTICLES_BLOG,
                                    $sql_data_array);
                            } elseif ($action == 'save') {
                                tep_db_perform(TABLE_ARTICLES_BLOG,
                                    $sql_data_array, 'update',
                                    "unique_id = '".(int) $_POST['unique_id'][$language_id][$c]."' and articles_id = '".(int) $articles_id."' and language_id = '".(int) $language_id."'");
                            }
                        }
                    }
                }
            ...
            tep_redirect(tep_href_link(FILENAME_ARTICLES_BLOG_COMMENTS,
                    (isset($_GET['page']) ? 'page='.$_GET['page'].'&' : '').'comID='.$articles_id));
            break;
        case 'deleteconfirm':
...
if ($action == 'new') {
    ...
    <tr>
        ...
        while ($comments = tep_db_fetch_array($comments_query)) {
            ...
            if ((!isset($_GET['comID']) || (isset($_GET['comID']) && ($_GET['comID']
                == $comments['articles_id']))) && !isset($comInfo) && (substr($action,
                    0, 3) != 'new')) {
            ...
            if (isset($comInfo) && is_object($comInfo) && ($comments['articles_id']
                == $comInfo->articles_id)) {
                echo '              <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\''.tep_href_link(FILENAME_ARTICLES_BLOG_COMMENTS,
                    'page='.$_GET['page'].'&comID='.$comments['articles__id'].'&action=edit').'\'">'."\n";
            } else {
                echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\''.tep_href_link(FILENAME_ARTICLES_BLOG_COMMENTS,
                    'page='.$_GET['page'].'&comID='.$comments['articles_id']).'\'">'."\n";
            }
            ?>
    ...
    switch ($action) {
        case 'delete':
            ...
            $contents   = array('form' => tep_draw_form('articles',
                    FILENAME_ARTICLES_BLOG_COMMENTS,
                    'page='.$_GET['page'].'&comID='.$comInfo->articles_id.'&action=deleteconfirm'));
            ...
            $contents[] = array('align' => 'center', 'text' => '<br>'.tep_image_submit('button_delete.gif',
                    IMAGE_DELETE).' <a href="'.tep_href_link(FILENAME_ARTICLES_BLOG_COMMENTS,
                    'page='.$_GET['page'].'&comID='.$comInfo->articles_id).'">'.tep_image_button('button_cancel.gif',
                    IMAGE_CANCEL).'</a>');
            break;
        default:
            if (isset($comInfo) && is_object($comInfo)) {
                ...
                $contents[] = array('align' => 'center', 'text' => '<a href="'.tep_href_link(FILENAME_ARTICLES_BLOG_COMMENTS,
                        'page='.$_GET['page'].'&comID='.$comInfo->articles_id.'&action=edit').'">'.tep_image_button('button_edit.gif',
