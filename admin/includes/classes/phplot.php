...
//////////////////////////////////////////////////////
    ...
    function PHPlot($which_width = 600, $which_height = 400,
                    $which_output_file = "", $which_input_file = "")
    {
    ...
    function SetFileFormat($which_file_format)
    {
        ...
        if (($asked == 'jpg') || ($asked == 'png') || ($asked == 'gif') || ($asked
            == 'wbmp')) {
    ...
    function DrawBackground()
    {
        ...
        if ($this->background_done == 0) { //Don't draw it twice if drawing two plots on one image
            ImageFilledRectangle($this->img, 0, 0, $this->image_width,
                $this->image_height, $this->ndx_bg_color);
    ...
    function DrawImageBorder()
    {
        switch ($this->image_border_type) {
            case "raised":
                ImageLine($this->img, 0, 0, $this->image_width - 1, 0,
                    $this->ndx_i_light);
                ImageLine($this->img, 1, 1, $this->image_width - 2, 1,
                    $this->ndx_i_light);
                ImageLine($this->img, 0, 0, 0, $this->image_height - 1,
                    $this->ndx_i_light);
                ImageLine($this->img, 1, 1, 1, $this->image_height - 2,
                    $this->ndx_i_light);
                ImageLine($this->img, $this->image_width - 1, 0,
                    $this->image_width - 1, $this->image_height - 1,
                    $this->ndx_i_dark);
                ImageLine($this->img, 0, $this->image_height - 1,
                    $this->image_width - 1, $this->image_height - 1,
                    $this->ndx_i_dark);
                ImageLine($this->img, $this->image_width - 2, 1,
                    $this->image_width - 2, $this->image_height - 2,
                    $this->ndx_i_dark);
                ImageLine($this->img, 1, $this->image_height - 2,
                    $this->image_width - 2, $this->image_height - 2,
                    $this->ndx_i_dark);
                break;
            case "plain":
                ImageLine($this->img, 0, 0, $this->image_width, 0,
                    $this->ndx_i_dark);
                ImageLine($this->img, $this->image_width - 1, 0,
                    $this->image_width - 1, $this->image_height,
                    $this->ndx_i_dark);
                ImageLine($this->img, $this->image_width - 1,
                    $this->image_height - 1, 0, $this->image_height - 1,
                    $this->ndx_i_dark);
                ImageLine($this->img, 0, 0, 0, $this->image_height,
                    $this->ndx_i_dark);
                break;
            default:
                break;
    ...
    function SetDrawDataLabels($which_ddl)
    ...
    function DrawXLabel()
    {
        if ($this->use_ttf == 1) {
            ...
            $this->DrawText($this->x_label_ttffont, $this->x_label_angle, $xpos,
                $ypos, $this->ndx_label_color, $this->x_label_ttffont_size,
            ...
            $this->DrawText($this->small_font, $this->x_label_angle, $xpos,
                $ypos, $this->ndx_label_color, "", $this->x_label_txt, 'center');
        }
        return true;
    }
    ...
    {
        if ($this->use_ttf == 1) {
            $size = $this->TTFBBoxSize($this->y_label_ttffont_size, 90,
                $this->y_label_ttffont, $this->y_label_txt);
            $xpos = 8 + $size[0];
            $ypos = ($size[1]) / 2 + $this->ytr(($this->plot_max_y + $this->plot_min_y)
                    / 2.0);
            $this->DrawText($this->y_label_ttffont, 90, $xpos, $ypos,
                $this->ndx_label_color, $this->y_label_ttffont_size,
                $this->y_label_txt);
        } else {
            $xpos = 8;
            ...
            $this->DrawText($this->small_font, 90, $xpos, $ypos,
                $this->ndx_label_color, $this->y_label_ttffont_size,
                $this->y_label_txt);
        }
        return true;
    ...
    function DrawText($which_font, $which_angle, $which_xpos, $which_ypos,
                      $which_color, $which_size, $which_text,
                      $which_halign = 'left', $which_valign = '')
    {

        if ($this->use_ttf == 1) {
            $size = $this->TTFBBoxSize($which_size, $which_angle, $which_font,
                $which_text);
            ...
            ImageTTFText($this->img, $which_size, $which_angle, $which_xpos,
                $which_ypos, $which_color, $which_font, $which_text);
        } else {
            if ($which_valign == 'top') {
            ...
            if ($which_angle == 90) {  //Vertical Code Submitted by Marlin Viss
                for ($i = 0; $i < count($str); $i++) {
                    ImageStringUp($this->img, $which_font,
                        ($i * $height + $which_xpos), $which_ypos, $str[$i],
                        $which_color);
                }
            } else {
                for ($i = 0; $i < count($str); $i++) {
                    if ($which_halign == 'center') {
                        ...
                        ImageString($this->img, (int) $which_font, $xpos,
                            ($i * $height + $which_ypos), $str[$i], $which_color);
                    } else {
                        ImageString($this->img, (int) $which_font, $which_xpos,
                            ($i * $height + $which_ypos), $str[$i], $which_color);
                    }
                }
            }
    ...
    function DrawTitle()
    {
        if ($this->use_ttf == 1) {
            ...
            $this->DrawText($this->title_ttffont, $this->title_angle, $xpos,
                $ypos, $this->ndx_title_color, $this->title_ttffont_size,
...
// function FindDataLimits
    ...
    function SetMargins()
    {
        ...
        if ($this->use_ttf == 1) {
            $title_size = $this->TTFBBoxSize($this->title_ttffont_size,
                $this->title_angle, $this->title_ttffont, 'X'); //An array
            if ($nbLines == 1) {
            ...
            $title_size = array(ImageFontWidth($this->title_font) * strlen($this->title_txt),
...
//exit;
    ...
    function SetMarginsPixels($which_lm, $which_rm, $which_tm, $which_bm)
    {
        ...
        $this->SetNewPlotAreaPixels($which_lm, $which_tm,
            ($this->image_width - $which_rm), ($this->image_height - $which_bm));
        return true;
    }
...
//function SetPlotAreaWorld
    ...
    function DrawError($error_message)
    {
        ...
        if ($this->use_ttf == 1) {
            ImageRectangle($this->img, 0, 0, $this->image_width,
                $this->image_height, ImageColorAllocate($this->img, 255, 255,
                    255));
            ImageTTFText($this->img, $this->small_ttffont_size, 0, $xpos, $ypos,
                ImageColorAllocate($this->img, 0, 0, 0), $this->axis_ttffont,
                $error_message);
        } else {
            ImageRectangle($this->img, 0, 0, $this->image_width,
                $this->image_height, ImageColorAllocate($this->img, 255, 255,
                    255));
            ImageString($this->img, $this->small_font, 1, $ypos, $error_message,
    ...
    function SetXLabelHeight()
    {
        ...
        if ($this->use_ttf == 1) {
            ...
            $size = $this->TTFBBoxSize($this->x_label_ttffont_size, 0,
                $this->axis_ttffont, $this->x_label_txt);
            $tmp  = $size[1];
            ...
            $size = $this->TTFBBoxSize($this->axis_ttffont_size,
                $this->x_datalabel_angle, $this->axis_ttffont, $string);

            $this->x_label_height = 2 * $tmp + $size[1] + 4;
        } else {
            //For Non-TTF fonts we can have only angles 0 or 90
            if ($this->x_datalabel_angle == 90) {
                $this->x_label_height = $this->x_datalabel_maxlength * ImageFontWidth($this->small_font)
                    / 1.5;
            } else {
...
//function SetXLabelHeight
    ...
    function SetYLabelWidth()
    {
        ...
        if ($this->use_ttf == 1) {
            $size = $this->TTFBBoxSize($this->axis_ttffont_size, 0,
                $this->axis_ttffont, $ylab);
        } else {
...
//exit;
    ...
    function SetEqualXCoord()
    {
        ...
        $space                  = ($this->plot_area[2] - $this->plot_area[0]) / ($this->number_x_points
            * 2) * $this->group_frac_width;
        $group_width            = $space * 2;
...
//////////////COLORS
    function SetRGBArray($which_color_array)
    {
        if (is_array($which_color_array)) {
            ...
            $this->rgb_array = array("white" => array(255, 255, 255), "black" => array(
                    0, 0, 0));
            exit;
        }
    ...
    function SetRgbColor($color_asked)
    {
        ...
        if (count($color_asked) == 3) { //already array of 3 rgb
            ...
            if (substr($color_asked, 0, 1) == "#") {  //asking in #FFFFFF format. 
                $ret_val = array(hexdec(substr($color_asked, 1, 2)), hexdec(substr($color_asked,
                            3, 2)), hexdec(substr($color, 5, 2)));
            } else {
    ...
    function SetDataColors($which_data, $which_border)
    {
        ...
        if (!$which_data) {
            $which_data   = array(array(0, 255, 0), array(0, 0, 248), 'yellow', array(
                    255, 0, 0), 'orange');
...
//function SetErrorBarColors
        ...
        switch ($this->plot_border_type) {
            case "left" :
                ImageLine($this->img, $this->plot_area[0],
                    $this->ytr($this->plot_min_y), $this->plot_area[0],
                    $this->ytr($this->plot_max_y), $this->ndx_grid_color);
                break;
            case "none":
                //Draw No Border
                break;
            default:
                ImageRectangle($this->img, $this->plot_area[0],
                    $this->ytr($this->plot_min_y), $this->plot_area[2],
                    $this->ytr($this->plot_max_y), $this->ndx_grid_color);
                break;
        }
        $this->DrawYAxis();
    ...
    function SetHorizTickIncrement($which_ti)
    {
        ...
        if ($which_ti) {
            ...
            $this->horiz_tick_increment = ($this->plot_max_x - $this->plot_min_x )
                / 10;
        }
    ...
    function SetVertTickIncrement($which_ti)
    {
        ...
        if ($which_ti) {
            ...
            $this->vert_tick_increment = ($this->plot_max_y - $this->plot_min_y )
                / 10;
        }
    ...
    function DrawYAxis()
    {
        ...
        ImageLine($this->img, $yaxis_x, $this->plot_area[1], $yaxis_x,
            $this->plot_area[3], $this->ndx_grid_color);
...
//function DrawYAxis
    ...
    function DrawXAxis()
    {
        ...
        ImageLine($this->img, $this->plot_area[0] + 1,
            $this->ytr($this->x_axis_position), $this->xtr($this->plot_max_x) - 1,
            $this->ytr($this->x_axis_position), $this->ndx_tick_color);
    ...
    function DrawHorizontalTicks()
    {
        ...
        //Left Bottom
        ImageLine($this->img, $this->plot_area[0],
            $this->plot_area[3] + $this->tick_length, $this->plot_area[0],
            $this->plot_area[3], $this->ndx_tick_color);
        ...
        switch ($this->x_grid_label_type) {
            case "title":
                ...
                break;
            case "data":
                $xlab = number_format($this->plot_min_x, $this->x_precision,
                        ".", ",")."$this->si_units";
                break;
            case "none":
                $xlab = '';
        ...
        if ($this->x_datalabel_angle == 90) {
            ...
            $ypos = ( $this->small_font_width * strlen($xlab) + $this->plot_area[3]
                + $this->small_font_height);
            ImageStringUp($this->img, $this->small_font, $xpos, $ypos, $xlab,
                $this->ndx_text_color);
        } else {
            $xpos = $this->plot_area[0] - $this->small_font_width * strlen($xlab)
                / 2;
            $ypos = $this->plot_area[3] + $this->small_font_height;
            ImageString($this->img, $this->small_font, $xpos, $ypos, $xlab,
                $this->ndx_text_color);
        }
        ...
        while ($x_tmp <= $this->plot_max_x) {
            ...
            ImageLine($this->img, $x_pixels,
                $this->plot_area[3] + $this->tick_length, $x_pixels,
                $this->plot_area[3], $this->ndx_tick_color);
            //Top Tick
            ...
            if ($this->draw_x_grid == 1) {
                ImageLine($this->img, $x_pixels, $this->plot_area[1], $x_pixels,
                    $this->plot_area[3], $this->ndx_light_grid_color);
            }
            ...
            if ($this->x_datalabel_angle == 90) {  //Vertical Code Submitted by Marlin Viss
                ImageStringUp($this->img, $this->small_font,
                    ...
                    ( $this->small_font_width * strlen($xlab) + $this->plot_area[3]
                    + $this->small_font_height), $xlab, $this->ndx_text_color);
            } else {
                ImageString($this->img, $this->small_font,
                    ( $x_pixels - $this->small_font_width * strlen($xlab) / 2),
                    ( $this->small_font_height + $this->plot_area[3]), $xlab,
                    $this->ndx_text_color);
            }
...
//function FormatYTickLabel
    ...
    {  //ylab in world coord.
        ...
        if (($this->vert_tick_position == "plotleft") || ($this->vert_tick_position
            == "both")) {
            ImageLine($this->img, (-$this->tick_length + $yaxis_x), $y_pixels,
                $yaxis_x, $y_pixels, $this->ndx_tick_color);
        }

        //Ticks to the Right of the Plot Area
        if (($this->vert_tick_position == "plotright") || ($this->vert_tick_position
            == "both")) {
            ImageLine($this->img, ($this->plot_area[2] + $this->tick_length),
                $y_pixels, $this->plot_area[2], $y_pixels, $this->ndx_tick_color);
        }

        //Ticks on the Y Axis 
        if (($this->vert_tick_position == "yaxis")) {
            ImageLine($this->img, ($yaxis_x - $this->tick_length), $y_pixels,
                $yaxis_x, $y_pixels, $this->ndx_tick_color);
        }
...
//ajo working
        ...
        ImageString($this->img, $this->small_font,
            ...
            ( -($this->small_font_height / 2.0) + $y_pixels), $which_ylab,
            $this->ndx_text_color);
    }
    ...
    function DrawVerticalTicks()
    {
        ...
        if ($this->skip_top_tick != 1) { //If tick increment doesn't hit the top 
            ...
            ImageLine($this->img, $this->plot_area[0] + 1,
                $this->ytr($this->plot_max_y), $this->plot_area[2] - 1,
                $this->ytr($this->plot_max_y), $this->ndx_light_grid_color);
        }
        ...
        if ($this->skip_bottom_tick != 1) {
            ...
            ImageLine($this->img, $this->xtr($this->plot_min_x) + 1,
                $this->ytr($this->plot_min_y), $this->xtr($this->plot_max_x),
                $this->ytr($this->plot_min_y), $this->ndx_light_grid_color);
        }
...
// function DrawVerticalTicks
    ...
    function SetTranslation()
    {
        if ($this->xscale_type == "log") {
            $this->xscale = ($this->plot_area_width) / (log10($this->plot_max_x)
                - log10($this->plot_min_x));
        } else {
            $this->xscale = ($this->plot_area_width) / ($this->plot_max_x - $this->plot_min_x);
        }
        if ($this->yscale_type == "log") {
            $this->yscale = ($this->plot_area_height) / (log10($this->plot_max_y)
                - log10($this->plot_min_y));
        } else {
...
// function SetTranslation
    ...
    function DrawDataLabel($lab, $x_world, $y_world)
    {
        ...
        if ($this->use_ttf) {
//ajjjo
            $lab_size = $this->TTFBBoxSize($this->axis_ttffont_size,
                $this->x_datalabel_angle, $this->axis_ttffont, $lab); //An array
            $y        = $this->ytr($y_world) - $lab_size[1];  //in pixels
            $x        = $this->xtr($x_world) - $lab_size[0] / 2;
            ImageTTFText($this->img, $this->axis_ttffont_size,
                $this->x_datalabel_angle, $x, $y, $this->ndx_text_color,
                $this->axis_ttffont, $lab);
        } else {
            ...
            if ($this->x_datalabel_angle == 90) {
                ...
                ImageStringUp($this->img, $this->small_font, $x, $y, $lab,
                    $this->ndx_text_color);
            } else {
                $y = $this->ytr($y_world) - $this->small_font_height; //in pixels
                $x = $this->xtr($x_world) - ($this->small_font_width * StrLen($lab))
                    / 2;
                ImageString($this->img, $this->small_font, $x, $y, $lab,
                    $this->ndx_text_color);
            }
        }
    }
    ...
    function DrawPieChart()
    {
        ...
        ImageArc($this->img, $xpos, $ypos, $diameter, $diameter, 0, 360,
            $this->ndx_grid_color);
        ...
        while (list(, $val) = each($sumarr)) {
            ...
            $label_txt   = number_format(($val / $total * 100),
                    $this->y_precision, ".", ",")."%";
            $val         = 360 * ($val / $total);
...
//Need this again for FillToBorder
            ...
            ImageLine($this->img, $xpos, $ypos, $out_x, $out_y,
                $this->ndx_grid_color);
            //ImageLine($this->img, $xpos, $ypos, $label_x, $label_y, $this->ndx_grid_color);
            ImageFillToBorder($this->img, $mid_x, $mid_y, $this->ndx_grid_color,
                $slicecol);

            if ($this->use_ttf) {
                ImageTTFText($this->img, $this->axis_ttffont_size, 0, $label_x,
                    $label_y, $this->ndx_grid_color, $this->axis_ttffont,
                    $label_txt);
            } else {
                ImageString($this->img, $this->small_font, $label_x, $label_y,
                    $label_txt, $this->ndx_grid_color);
            }
...
//echo "start = $start_lines<br />";
                    if ($start_lines == 1) {
                        for ($width = 0; $width < $this->line_width; $width++) {
                            ImageLine($this->img, $x_now_pixels,
                                $y_now_pixels + $width, $lastx[$i],
                                $lasty[$i] + $width, $barcol);
                        }
                    }
                    ...
                    $this->DrawYErrorBar($x_now, $y_now, $val,
                        $this->error_bar_shape, $error_barcol);
                } elseif ($key % 3 == 1) {
                    $this->DrawYErrorBar($x_now, $y_now, -$val,
                        $this->error_bar_shape, $error_barcol);
                }
        ...
        while (list(, $row) = each($this->data_values)) {
            ...
            while (list($key, $val) = each($row)) {
                if ($key == 0) {
                    ...
                    $this->DrawYErrorBar($xpos, $ypos, $val,
                        $this->error_bar_shape, $error_barcol);
                } elseif ($key % 3 == 1) {
                    $mine = $val;
                    $this->DrawYErrorBar($xpos, $ypos, -$val,
        ...
        while (list(, $row) = each($this->data_values)) {
            ...
            while (list($k, $v) = each($row)) {
                if ($k == 0) {
                    ...
                    ImageLine($this->img, $xpos, $y1, $xpos, $this->ytr($v),
                        $barcol);
                    $color_index++;
                }
            }
...
//function DrawThinBarLines
    ...
    function DrawYErrorBar($x_world, $y_world, $error_height, $error_bar_type,
                           $color)
    {
        $x1 = $this->xtr($x_world);
        $y1 = $this->ytr($y_world);
        ...
        switch ($error_bar_type) {
            case "line":
                break;
            case "tee":
                ImageLine($this->img, $x1 - $this->error_bar_size, $y2,
                    $x1 + $this->error_bar_size, $y2, $color);
                break;
            default:
                ImageLine($this->img, $x1 - $this->error_bar_size, $y2,
                    $x1 + $this->error_bar_size, $y2, $color);
    ...
    function DrawDot($x_world, $y_world, $dot_type, $color)
    {
        ...
        switch ($dot_type) {
            case "halfline":
                ImageFilledRectangle($this->img, $x1, $this->ytr($y_world),
                    $this->xtr($x_world), $this->ytr($y_world), $color);
                break;
            case "line":
                ImageFilledRectangle($this->img, $x1, $this->ytr($y_world), $x2,
                    $this->ytr($y_world), $color);
                break;
            case "rect":
                ImageFilledRectangle($this->img, $x1, $y1, $x2, $y2, $color);
                break;
            case "circle":
                ImageArc($this->img, $x1 + $half_point, $y1 + $half_point,
                    $this->point_size, $this->point_size, 0, 360, $color);
                break;
            case "dot":
                ImageArc($this->img, $x1 + $half_point, $y1 + $half_point,
                    $this->point_size, $this->point_size, 0, 360, $color);
                ImageFillToBorder($this->img, $x1 + $half_point,
                    $y1 + $half_point, $color, $color);
                break;
            case "diamond":
                        ...
                        if ($start_lines == 1) {
                            for ($width = 0; $width < $this->line_width; $width++) {
                                if ($this->line_style[$i] == "dashed") {
                                    $this->DrawDashedLine($x_now,
                                        $y_now + $width, $lastx[$i],
                                        $lasty[$i] + $width, 4, 4, $barcol);
                                } else {
                                    ImageLine($this->img, $x_now,
                                        $y_now + $width, $lastx[$i],
                                        $lasty[$i] + $width, $barcol);
                                }
                            }
...
//function DrawLineSeries
    ...
    function DrawDashedLine($x1pix, $y1pix, $x2pix, $y2pix, $dash_length,
                            $dash_space, $color)
    {
        ...
        $line_length = ceil(sqrt(pow(($x2pix - $x1pix), 2) + pow(($y2pix - $y1pix),
                    2)));
                    ...
                    if ((strval($v) != "")) {   //Allow for missing Y data 
                        if ($this->shading > 0) {
                            for ($i = 0; $i < ($this->shading); $i++) {
                                ...
                                ImageFilledRectangle($this->img, $x1 + $i,
                                    $y1 - $i, $x2 + $i, $y2 - $i,
                                    $this->ndx_i_light);
                            }
                        }
                        ...
                        ImageFilledRectangle($this->img, $x1, $y1, $x2, $y2,
                            $barcol);
                        ImageRectangle($this->img, $x1, $y1, $x2, $y2,
                            $bordercol);
                        if ($this->draw_data_labels == '1') {  //ajo
                            ...
                            $this->DrawText($this->x_label_ttffont,
                                $this->x_label_angle,
                                $x1 + $this->record_bar_width / 2, $y1,
                                $this->ndx_label_color,
                                $this->x_label_ttffont_size, $v, 'center', 'top');
                        }
                    }
...
//function DrawBars
    ...
    function DrawLegend($which_x1, $which_y1, $which_boxtype)
    {
        ...
        if ((!$which_x1) || (!$which_y1)) {
            $box_start_x = $this->plot_area[2] - $this->small_font_width * ($max_legend_length
                + 4);
        ...
        //Lower Right
        $box_end_y = $box_start_y + $this->small_font_height * (count($this->legend)
            + 1) + 2 * $vert_margin;
        //$box_end_x = $this->plot_area[2] - 5;
        $box_end_x = $box_start_x + $this->small_font_width * ($max_legend_length
            + 4) - 5;


        // Draw box for legend
        ImageFilledRectangle($this->img, $box_start_x, $box_start_y, $box_end_x,
            $box_end_y, $this->ndx_bg_color);
        ImageRectangle($this->img, $box_start_x, $box_start_y, $box_end_x,
        ...
        while (list(, $leg) = each($this->legend)) {
            $y_pos = $box_start_y + $this->small_font_height * ($i) * ($line_spacing)
                + $vert_margin;

            ImageString($this->img, $this->small_font,
                $box_start_x + $this->small_font_width * ( $max_legend_length - strlen($leg)
                + 1 ), $y_pos, $leg, $this->ndx_text_color);

            if ($color_index >= count($this->ndx_data_color)) $color_index = 0;
            // Draw a box in the data color
            ImageFilledRectangle($this->img,
                $box_end_x - $this->small_font_width * 2, $y_pos + 1,
                $box_end_x - $this->small_font_width, $y_pos + $dot_height,
                $this->ndx_data_color[$color_index]);

            ImageRectangle($this->img, $box_end_x - $this->small_font_width * 2,
                $y_pos + 1, $box_end_x - $this->small_font_width,
                $y_pos + $dot_height, $this->ndx_text_color);
            $i++;
            $color_index++;
        }
    }
//function DrawLegend
    ...
    function DrawGraph()
    {
        ...
        if (!is_array($this->data_values)) {
            ...
            if (!$this->data_color) {
                $this->SetDataColors(array('blue', 'green', 'yellow', 'red', 'orange',
                    'blue'), array('black'));
            }
