...
class splitPageResults
{
                              ...
                              &$sql_query, &$query_num_rows)
    {
        ...
        $reviews_count_query = tep_db_query("select count(*) as total ".substr($sql_query,
                $pos_from, ($pos_to - $pos_from)));
...
// calculate number of pages needing links
        ...
        if ($num_pages > 1) {
            ...
            $display_links .= sprintf(TEXT_RESULT_PAGE,
                tep_draw_pull_down_menu($page_name, $pages_array,
                    $current_page_number, 'onchange="this.form.submit();"'),
                $num_pages);
            ...
            if ($parameters != '') {
                ...
                while (list(, $pair) = each($pairs)) {
                    ...
                    $display_links .= tep_draw_hidden_field(rawurldecode($key),
                        rawurldecode($value));
                }
            }
    ...
    function display_count($query_numrows, $max_rows_per_page,
                           $current_page_number, $text_output)
    {
