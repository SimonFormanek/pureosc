...
if (!isset($_SESSION['admin']['AAS']['ajaxToken']) || empty($_SESSION['admin']['AAS']['ajaxToken']))
        $_SESSION['admin']['AAS']['ajaxToken'] = md5(uniqid(mt_rand(), true)); //generateToken(); //md5(uniqid(mt_rand(),true)); //session_id();
define('AAS_AJAX', $_SESSION['admin']['AAS']['ajaxToken']);

//load language
if (file_exists('ext/aas/languages/'.$language.'.php'))
        require 'ext/aas/languages/'.$language.'.php';
elseif (file_exists('ext/aas/languages/english.php'))
        require 'ext/aas/languages/english.php';
else {
    ?>
...
<?php if ($modules_count > 0) {
    foreach ($modules as $key => $module) { ?>
        <?php if (tep_not_null($module['css']['style'])) {
            foreach ($module['css']['style'] as $ext_css) {
                if (tep_not_null($ext_css)) { ?><link href="ext/aas_modules/<?php echo $key; ?>/css/<?php echo $ext_css; ?>" rel="stylesheet" /><?php }
            }
        } ?>

        <?php if (tep_not_null($module['css']['print'])) {
            foreach ($module['css']['print'] as $ext_css) {
                if (tep_not_null($ext_css)) { ?><link href="ext/aas_modules/<?php echo $key; ?>/css/<?php echo $ext_css; ?>" rel="stylesheet" media="print" /><?php }
            }
        } ?>
            <?php }
        } ?>
    ...
    <body>
        ...
        <div id="panel" style="<?php echo isset($search) && $search != '' ? '' : 'display:none;'; ?>">
            ...
            echo tep_draw_input_field('search', '',
                'id="lfora" class="lfora0" placeholder="'.AAS_TEXT_PLACEHOLDER_SEARCH.'" style="display:inline-block;vertical-align:top;" required="required"');
            ?>
<?php if (isset($_GET['search'])) { ?><input class="search_clear_button lfor" onclick="clearSearchQuery('<?php echo isset($_SESSION['admin']['AAS']['preSearchUrl'])
            ? $_SESSION['admin']['AAS']['preSearchUrl'] : tep_href_link(FILENAME_AAS,
            ''); ?>')" type="button" value="x" /><?php } ?>
                <?php echo tep_draw_pull_down_menu('searchOnField',
                    $searchOnFieldArray, '',
                    ' style="display:inline-block;vertical-align:top;" onchange="this.form.submit();"'); ?>

                <select style="display:inline-block;vertical-align:top;" name="orderBy" onchange="submit();">
            <?php foreach ($orderByArray as $key => $value)
                echo '<option '.(isset($_GET['orderBy']) && $_GET['orderBy'] == $key
            ...
            <?php foreach ($ascDescArray as $key => $value)
                echo '<option '.(isset($_GET['ascDesc']) && $_GET['ascDesc'] == $key
                        ? 'selected="selected"' : '' ).' value="'.$key.'">'.$value.'</option>'; ?>
                </select>
                </form>
            </div>
        ...
        <div id="koumpakia-wrapper">
            ...
            <?php if (!isset($aasAac['default']['print'][$_SESSION['admin']['id']])) {
                if ($categories_entries > 0 || $entries > 0) {
                    ...
                    <ul class="dropdown">
                    <?php if ($categories_entries > 0 && $entries > 0) { ?><li><a onclick="printHtmlElement(1, this)" href="#"><?php echo AAS_TEXT_PRINT_CATEGORIES_AND_PRODUCTS_TABLE; ?></a><?php if ($entries
                > 0) { ?><label><input type="checkbox" name="printHtmlElement1" ><?php echo AAS_TEXT_PRINT_INCLUDE_PAGINATION; ?></label><?php } ?></li><?php } ?>
            ...
            <?php if (!isset($aasAac['default']['all_edit'][$_SESSION['admin']['id']])
                && ($categories_entries > 0 || $entries > 0)) { ?>
...
<?php
$massColumnsEdit = false;
if (!isset($aasAac['default']['mass_columns_edit'][$_SESSION['admin']['id']])) {
    ...
    foreach ($fieldsArray as $key => $value) {
        ...
        if ($value['visible'] && $value['massEdit']) {
            ...
            if (!isset($aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']])) {
                $massColumnsEdit = true;
                break;
            } elseif (isset($aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']])
                && $aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']]
                == '0') {
                $massColumnsEdit = true;
                break;
            }
...
<?php if ($defaults['enableDonations']) { ?><div id="donationbutton" class="koumpakia  buttonakia" data-title="<?php echo AAS_BUTTON_TOOLTIP_DONATION; ?>" ><img src="ext/aas/images/glyphicons_226_euro.png" alt="" style="padding-top:2px;"></div><?php } ?>
            ...
            <?php if ($defaults['displayGoToDefaultAdministrationPanel']) { ?><div class="koumpakia buttonakia" data-title="<?php echo AAS_HEADING_TITLE_BACK_TO_DEFAULT_ADMIN; ?>" ><a href="<?php echo tep_href_link(FILENAME_DEFAULT,
                ''); ?>" ><img src="ext/aas/images/glyphicons_020_home.png" alt="" style="width:24px;"></a></div><?php } ?>

            <?php if ($defaults['displayLogoffButton']) { ?><div id="logoffbutton" class="koumpakia buttonakia" data-title="<?php echo AAS_HEADING_TITLE_LOGOFF; ?>" ><a href="<?php echo tep_href_link(FILENAME_LOGIN,
                    'action=logoff'); ?>" ><img src="ext/aas/images/glyphicons_026_road.png" alt="" style="height:23px;"></a></div><?php } ?>

            <?php if ($defaults['displayGoBackButton']) {
                if (!empty($cPath_back)) {
                    ?>
                    <div style="padding:8px 5px;" class="koumpakia buttonakia" data-title="<?php echo AAS_HEADING_TITLE_BACK_TO_PARENT; ?>" ><a href="<?php echo tep_href_link(FILENAME_AAS,
                'cPath='.$parent_id); ?>"><img src="ext/aas/images/glyphicons_221_unshare.png" alt=""></a></div>
    <?php }
}
?>
            ...
            <div id="nav">
                ...
                <a href="<?php echo tep_href_link(FILENAME_AAS, 'cPath=0'); ?>"><?php echo AAS_TEXT_TOP; ?></a>&nbsp;<span class="raquo" data-catid="0">&raquo;</span>
                        ...
                        <a href="<?php echo tep_href_link(FILENAME_AAS, 'cPath='.$key); ?>"><?php echo $pathy; ?></a>
                                ...
                                if (++$pathakia_cnt < $pathakia_count || $categories_entries
                                    > 0) {
                                    ?><span class="raquo" data-catid="<?php echo $key; ?>">&raquo;</span>&nbsp;<?php }
                }
                            ?>
            </div>
                        ...
                        <div class="pagination-form-wrapper">
                            <form name="itemsPerRowSelection-form" action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post">
                                ...
                                <select class="entriesPerPage" name="entriesPerPage" onchange="submit();">
                                    <?php foreach ($perPageArray as $ppa)
                                        echo '<option '.($ppa == $entriesPerPage
                                                ? 'selected="selected"' : '').' value="'.$ppa.'">'.$ppa.'</option>'; ?>
                            ...
                            <?php } ?>
                            <?php if ($defaults['inTableSearchPosition']['visible']
                                && $defaults['inTableSearchPosition']['y'] == 'top') { ?><div class="inTableSearch-wrapper inTableSearch-wrapper-<?php echo isset($defaults['inTableSearchPosition']['x'])
                                    ? $defaults['inTableSearchPosition']['x'] : 'right'; ?>"><input type="text" id="search" placeholder="<?php echo AAS_TEXT_IN_TABLE_SEARCH; ?>" class="inTableSearch inTableSearch-top" /></div><?php } ?>
                    ...
                    <thead>
                        <tr>
                            ...
                            foreach ($fieldsArray as $key => $value) {
                                ...
                                if ($value['visible']) {
                                    ...
                                    if ($key == 'products_price_gross' && isset($currency_symbols[0]))
                                            echo'<th data-clmn="'.$key.'" class="draggable"><div class="draghandle">&nbsp;</div>'.$currency_symbols[0]['symbol_left'].' '.$value['theadText'].' '.$currency_symbols[0]['symbol_right'].'</th>'.PHP_EOL;
                                    else
                                            echo'<th data-clmn="'.$key.'" class="draggable"><div class="draghandle">&nbsp;</div>'.$value['theadText'].'</th>'.PHP_EOL;
                                }
                            }
                            ?>
                        </tr>
                    </thead>
                            <?php if ($displayCategories && $categories_entries
                                > 0) { ?>
                        <tbody class="categories_tbody">
                                ...
                                while ($categories = tep_db_fetch_array($categories_query)) {
                                    ...
                                    $bcp = $bool_count_products ? tep_products_in_category_count($categories['categories_id'],
                                            true).'<span class="cat_products_count" id="catid_'.$categories['categories_id'].'">'.AAS_HEADING_PRODUCTS_COUNT.'</span>'
                                            : '';
                                ...
                                <tr class="folder gradeX" id="cat_<?php echo $categories['categories_id']; ?>">
                                    ...
                                    <td class="nojedit centerAlign previewPage" style="padding:0px;"><?php echo '<a href="'.tep_href_link(FILENAME_AAS,
                                'cPath='.$categories['categories_id']).'">'.tep_image(DIR_WS_ICONS.'folder.gif',
                                ICON_FOLDER).'</a>'; ?>

                                        <a target="_blank" class="view_category_class" title="<?php echo AAS_VIEW_CATEGORYS_PAGE; ?>" href="<?php echo tep_aas_link('front',
                                'index.php',
                                'cPath='.$categories['categories_id'].'&&language='.$languages_selected['code']); ?>"><img src="ext/aas/images/glyphicons_152_new_window_op.png" alt=""></a>
                                    </td>
                                    <?php if (isset($aasAac['fields_disable_action']['categories_name'][$_SESSION['admin']['id']])
                                        && $aasAac['fields_disable_action']['categories_name'][$_SESSION['admin']['id']]) { ?>
                                    ...
                                    <?php } else { ?>
                                        <td  data-column="categories_name" class="<?php echo $bool_count_products
                            || $bool_count_subcategories ? 'nojedit' : 'cursor-pointer'; ?> centerAlign"><?php echo $categories['categories_name'].$anwkatwteleia.$bcp.$comma.$bcs; ?></td>
                                    <?php
                                    }
                                            ...
                                            switch ($key) {
                                                ...
                                                case 'date_added':
                                                    ...
                                                    if (isset($categories[$key])) {
                                                        $dtdateAdded = substr($categories[$key],
                                                            0, -9);
                                                ...
                                                case'products_image':
                                                    ...
                                                    if (isset($aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']])
                                                        ...
                                                        echo'<td style="'.$_table_td_rules[$key]['style'].'" class="'.($value['cellEditable']
                                                                ? '' : 'nojedit').' '.$_table_td_rules[$key]['class'].'" >'.tep_info_image($categories['categories_image'],
                                                            $categories['categories_name'],
                                                            SMALL_IMAGE_WIDTH,
                                                            SMALL_IMAGE_HEIGHT).'</td>';
                                                    } else
                                                            echo'<td style="'.$_table_td_rules[$key]['style'].'" class="'.($value['cellEditable']
                                                                ? '' : 'nojedit').' '.$_table_td_rules[$key]['class'].'" ><a data-poc="category" data-categoryname="'.$categories['categories_name'].'" class="product_image_link" href="#">'.tep_info_image($categories['categories_image'],
                                                            $categories['categories_name'],
                                                            SMALL_IMAGE_WIDTH,
                                                            SMALL_IMAGE_HEIGHT).'</a></td>';
...
//store for attributes cloning select list
                        $products_names_id_array[$products['products_id']] = $fieldsArray['products_model']['visible']
                                ? $products['products_name'].' [ '.$products['products_model'].' ]'
                                : $products['products_name'];

                        if ($fieldsArray['products_status']['visible'] && $products['products_status']
                            == '0') $status_unavailable_class = ' unavailable';
                        else $status_unavailable_class = '';

                        if (isset($ptc_array[$products['products_id']]))
                                $isLinked = ' tr_linked';
                        else $isLinked = '';
                        ?>
                            <tr class="product <?php echo ($counter & 1) && $defaults['colorEachTableRowDifferently']
                            ? 'even' : 'odd';
                    echo $status_unavailable_class.' '.$isLinked; ?>" id="pid_<?php echo (int) $products['products_id']; ?>" data-category="<?php echo $products['categories_id']; ?>">
                        ...
                        <?php if ($ascDesc === 'DESC') echo $totalRows - ( ($counter++)
                            + (($currentPage - 1) * $entriesPerPage));
                        else echo ( ++$counter) + (($currentPage - 1) * $entriesPerPage); ?>    
                                    <a target="_blank" class="view_product_class" title="<?php echo AAS_VIEW_PRODUCTS_PAGE; ?>" href="<?php echo tep_aas_link('front',
                        'product_info.php',
                        'products_id='.$products['products_id'].'&&language='.$languages_selected['code']); ?>"><img src="ext/aas/images/glyphicons_152_new_window_op.png" alt=""></a>
                                </td>
                        <?php
                        if (isset($aasAac['fields_disable_action']['products_name'][$_SESSION['admin']['id']])
                            && $aasAac['fields_disable_action']['products_name'][$_SESSION['admin']['id']]) {
                        ...
                        foreach ($fieldsArray as $key => $value) {
                            ...
                            if ($value['visible']) {
                                ...
                                if (!isset($_table_td_rules[$key]))
                                        $_table_td_rules[$key] = array('style' => '',
                                        'class' => '');
                                ...
                                switch ($key) {
                                    ...
                                    case'products_date_available':
                                        ...
                                        if (isset($aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']])
                                            ...
                                            echo'<td style="'.$_table_td_rules[$key]['style'].'" class="nojedit '.$_table_td_rules[$key]['class'].'" data-column="'.$key.'" >'.(tep_not_null($products[$key])
                                                    ? substr($products[$key], 0,
                                                    -9) : '').'</td>';
                                        } else {

                                            echo'<td style="'.$_table_td_rules[$key]['style'].'" class="'.($value['cellEditable']
                                                    ? '' : 'nojedit').' '.$_table_td_rules[$key]['class'].'" data-column="'.$key.'" ><input type="text" style="text-align:center;cursor:pointer;" id="datepicker_'.(int) $products['products_id'].'" class="datepicker lfor" value="'.substr($products[$key],
                                                0, -9).'" /><a id="product-available-to-null_'.$products['products_id'].'" class="product-available-to-null" title="'.AAS_TITLE_TEXT_NOT_AVAILABLE_DATE.'" style="'.(tep_not_null($products[$key])
                                                    ? '' : 'visibility:hidden;').'" href="#"><img style="opacity:0.3;height:20px" src="ext/aas/images/remove_white_no_round_1.png" alt="Never Expire"></a></td>';
                                        }

                                        break;
                                    case 'products_description':

                                        $hasDescription = tep_not_null($products['products_description'])
                                                ? true : false;

                                        echo'<td style="'.$_table_td_rules[$key]['style'].'" class="'.($value['cellEditable']
                                                ? '' : 'nojedit').' '.$_table_td_rules[$key]['class'].' '.($hasDescription
                                                ? 'hasDescription_true' : 'hasDescription_false').'" data-column="'.$key.'" >';
                                        ?>
                                        <img data-productname="<?php echo $products['products_name']; ?>" id="trigger_<?php echo (int) $products['products_id']; ?>" title="<?php echo AAS_VIEW_EDIT_DESCRIPTION; ?>" class="<?php echo isset($aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']])
                                        && $aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']]
                                                ? '' : 'descriptionbuttonImg'; ?>" src="<?php echo DIR_WS_CATALOG_LANGUAGES.$languages_selected['directory'].'/images/'.$languages_selected['image']; ?>" alt="<?php echo $languages_selected['name']; ?>" >&nbsp;
                                        <?php
                                        if ($hasDescription)
                                                echo tep_image('ext/aas/images/icn_alert_success.png');
                                        else
                                                echo tep_image('ext/aas/images/icn_alert_error.png');
                                        echo '</td>';
                                    ...
                                    case 'products_ordered':
                                        ...
                                        echo'<td style="'.$_table_td_rules[$key]['style'].'" class="'.($value['cellEditable']
                                                ? '' : 'nojedit').' '.$_table_td_rules[$key]['class'].'"  data-column="'.$key.'" >'.$products[$key].'</td>';
                                    ...
                                    case 'products_sort_order':
                                        ...
                                        echo'<td style="'.$_table_td_rules[$key]['style'].'" class="'.($value['cellEditable']
                                                ? '' : 'nojedit').' '.$_table_td_rules[$key]['class'].'"  data-column="'.$key.'" >'.(tep_not_null($products[$key])
                                                ? $products[$key] : '0').'</td>';
                                        break;
                                    case'products_status':
                                        ...
                                        if (isset($aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']])
                                            && $aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']])
                                                echo '<td style="'.$_table_td_rules[$key]['style'].'" class="nojedit '.$_table_td_rules[$key]['class'].'"><img src="ext/aas/images/'.$ficon.'" alt="'.$statusTitle.'" title="'.$statusTitle.'"></td>';
                                        else
                                                echo '<td style="'.$_table_td_rules[$key]['style'].'" class="'.($value['cellEditable']
                                                    ? '' : 'nojedit').' '.$_table_td_rules[$key]['class'].'"  id="products_status_'.(int) $products['products_id'].'" ><a class="radiostockajax" id="status_'.$products['products_id'].'" href="#" ><img src="ext/aas/images/'.$ficon.'" alt="'.$statusTitle.'" title="'.$statusTitle.'"></a></td>';
                                    ...
                                    case'attributes':
                                        ...
                                        if (isset($aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']])
                                            ...
                                            if (!isset($products_attributes[$products['products_id']])) {
                                                ...
                                                echo'<td style="'.$_table_td_rules[$key]['style'].'" class="'.($value['cellEditable']
                                                        ? '' : 'nojedit').' '.$_table_td_rules[$key]['class'].' attributesCellLink_zero" >';
                                                echo '<span id="attributes_trigger_'.(int) $products['products_id'].'">0</span>';
                                            } else {

                                                echo'<td style="'.$_table_td_rules[$key]['style'].'" class="'.($value['cellEditable']
                                                        ? '' : 'nojedit').' '.$_table_td_rules[$key]['class'].' attributesCellLink_non_zero" >';
                                            ...
                                            if (!isset($products_attributes[$products['products_id']])) {
                                                ...
                                                echo'<td style="'.$_table_td_rules[$key]['style'].'" class="'.($value['cellEditable']
                                                        ? '' : 'nojedit').' '.$_table_td_rules[$key]['class'].' attributesCell attributesCellLink_zero" >';
                                                ...
                                                echo'<td style="'.$_table_td_rules[$key]['style'].'" class="'.($value['cellEditable']
                                                        ? '' : 'nojedit').' '.$_table_td_rules[$key]['class'].' attributesCell  attributesCellLink_non_zero" >';
                                    ...
                                    case'tax_class_title':
                                        ...
                                        if (isset($aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']])
                                            ...
                                            <?php foreach ($tax_class_array as $tca) {
                                                if ($tca['id'] == $products['products_tax_class_id']) echo $tca['title'].' '.$tca['tax_rate'].'%';
                                                break;
                                            } ?>
                                            </td>
                                        <?php } else { ?>
                                            <td style="<?php echo $_table_td_rules[$key]['style']; ?>" class="<?php echo $value['cellEditable']
                                ? '' : 'nojedit'; ?>" >
                                                <select class="selectMenus" id="tcts_<?php echo (int) $products['products_id']; ?>" name="tax_class_title_select_<?php echo (int) $products['products_id']; ?>" onchange="selectMenuChange(this, 'products_tax_class_id')" style=""><option value="0" <?php echo $products['products_tax_class_id']
                        == '0' ? 'selected="selected"' : ''; ?>><?php echo AAS_NONE; ?></option><?php foreach ($tax_class_array as $tca)
                            echo '<option value="'.$tca['id'].'_'.$tca['tax_rate'].'" '.($tca['id']
                            == $products['products_tax_class_id'] ? 'selected="selected"'
                                    : '').' >'.$tca['title'].' '.$tca['tax_rate'].'%</option>'; ?>
                                                </select></td>
                                        <?php
                                        }
                                    ...
                                    case'manufacturers_name':
                                        ...
                                        if (isset($aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']])
                                            ...
                                            foreach ($manufacturers_array as $man) {
                                                if ($man['id'] == $products['manufacturers_id'])
                                                        echo $man['name'];
                                            }
                                            echo '</td>';
                                        } else {
                                            ...
                                            <td style="<?php echo $_table_td_rules[$key]['style']; ?>" class="<?php echo $value['cellEditable']
                                ? '' : 'nojedit'; ?>" >
                                                <select class="selectMenus" id="manu_<?php echo (int) $products['products_id']; ?>" name="manufacturers_select_<?php echo (int) $products['products_id']; ?>" onchange="selectMenuChange(this, 'manufacturers_id')" ><option value="0" <?php echo $products['manufacturers_id']
                        == '0' ? 'selected="selected"' : ''; ?>><?php echo AAS_NONE; ?></option><?php foreach ($manufacturers_array as $man)
                            echo '<option value="'.$man['id'].'" '.($man['id'] == $products['manufacturers_id']
                                    ? 'selected="selected"' : '').' >'.$man['name'].'</option>'; ?>
                                        ...
                                        case'products_image':
                                            ...
                                            if (isset($aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']])
                                                ...
                                                echo'<td style="'.$_table_td_rules[$key]['style'].'" class="'.($value['cellEditable']
                                                        ? '' : 'nojedit').' '.$_table_td_rules[$key]['class'].'" >'.tep_info_image($products[$key],
                                                    $products['products_name'],
                                                    SMALL_IMAGE_WIDTH,
                                                    SMALL_IMAGE_HEIGHT).'</td>'.PHP_EOL;
                                            } else
                                                    echo'<td style="'.$_table_td_rules[$key]['style'].'" class="'.($value['cellEditable']
                                                        ? '' : 'nojedit').' '.$_table_td_rules[$key]['class'].'" ><a data-poc="product" data-productname="'.$products['products_name'].'" class="product_image_link" href="#">'.tep_info_image($products[$key],
                                                    $products['products_name'],
                                                    SMALL_IMAGE_WIDTH,
                                                    SMALL_IMAGE_HEIGHT).'</a></td>'.PHP_EOL;
                                        ...
                                        case'products_price_gross':
                                            ...
                                            echo'<td data-price-net="'.$products['products_price'].'" data-tax-rate="'.$taxRate.'" data-column="'.$key.'" id="'.$key.'_'.$products['products_id'].'" style="'.$_table_td_rules[$key]['style'].' '.$cursorDefault.'" class="'.$noJedit.' '.$_table_td_rules[$key]['class'].'" >'.tep_get_price_with_tax($products['products_price'],
                                                $taxRate, $currency_symbols[0]).'</td>';

                                            break;

                                        case'products_order_status':

                                            echo'<td id="'.$key.'_'.$products['products_id'].'" style="'.$_table_td_rules[$key]['style'].'" class="'.($value['cellEditable']
                                                    ? '' : 'nojedit').' '.$_table_td_rules[$key]['class'].'" >';
                                            $numItems = count($orders_status_array);
                                            foreach ($orders_status_array as $key_osa => $osa)
                                                echo $osa.': '.(isset($products_quantity_by_orders[$products['products_id']][$key_osa])
                                                        ? $products_quantity_by_orders[$products['products_id']][$key_osa]
                                                        : '0').($key_osa < $numItems
                                                        ? ' / ' : '');
                                            echo '</td>';
                                        ...
                                        case'special':
                                            ...
                                            if (isset($aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']])
                                                ...
                                                if (isset($specials_products[$products['products_id']])) {
                                                    ...
                                                    if ($specials_products[$products['products_id']]['status']
                                                        == '0')
                                                            $td_bg = 'unavailable';
                                                }

                                                echo'<td data-column="'.$key.'" id="'.$key.'_'.$products['products_id'].'" style="'.$_table_td_rules[$key]['style'].'" class="'.($value['cellEditable']
                                                        ? '' : 'nojedit').' '.$_table_td_rules[$key]['class'].' '.$td_bg.'" >';
                                                if (isset($specials_products[$products['products_id']])) {
                                                ...
                                                if ($fieldsArray['products_price_gross']['visible']) {
                                                    $taxRate = tep_get_tax_rate($products['products_tax_class_id']);
                                                    echo '<span class="specialPriceGross">'.$currencies->format(tep_get_price_with_tax($specials_products[$products['products_id']]['new_price'],
                                                            $taxRate,
                                                            $currency_symbols[0])).' (Gross)</span>';
                                                }
                                                ?>
                                                ...
                                                ,&nbsp;<?php echo AAS_SPECIALS_TEXT_EXPIRES_AT; ?><input type="text" style="text-align:center" class="lfor specials_expires_at" id="specials_expires_at_<?php echo $specials_products[$products['products_id']]['specials_id']; ?>" value="<?php echo substr($specials_products[$products['products_id']]['expires_date'],
                                0, -9); ?>" />

                                                <a id="specials-unexpire_<?php echo $specials_products[$products['products_id']]['specials_id']; ?>" class="specials-unexpire" title="<?php echo AAS_SPECIALS_TEXT_NEVER_EXPIRE; ?>" style="<?php echo tep_not_null($specials_products[$products['products_id']]['expires_date'])
                                    ? '' : 'visibility:hidden;'; ?>" href="#"><img style="opacity:0.3;height:20px" src="ext/aas/images/remove_white_no_round_1.png" alt="never expire"></a>
                                    ...
                                    case'products_linked':
                                        ...
                                        if (isset($aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']])
                                            ...
                                            ?>
                                            <td id="products_linked_<?php echo $products['products_id']; ?>" style="<?php echo $_table_td_rules[$key]['style']; ?> text-align:left" class="<?php echo $value['cellEditable']
                                                    ? '' : 'nojedit'; ?>" >
                                                    <?php
                                                    $caid = tep_not_null($categoryId)
                                                            ? (int) $categoryId : 0;

                                                    //if(isset($protoca_array[$products['products_id']][1]['cid']) && (int)$protoca_array[$products['products_id']][1]['cid']==$caid){
                                                    if (!isset($ptc_array[$products['products_id']])) {

                                                        if (count($protoca_array[$products['products_id']])
                                                            > 1) {

                                                            echo AAS_TEXT_LINKED_WITH;
                                                            ?><br /><br />
                                                                <?php
                                                                foreach ($protoca_array[$products['products_id']] as $kpropo => $propro) {
                                                                    ...
                                                                    > 0 ? implode('&nbsp;&raquo;&nbsp;',
                                                                            $tjohn)
                                                                            : AAS_TEXT_TOP;
                                                                    echo '&nbsp;&raquo;&nbsp;<a href="'.tep_href_link(FILENAME_AAS,
                                                                        'cPath='.$propro['cid']).'">'.$propro['cname'].'</a>&nbsp;<button data-cid="'.$propro['cid'].'" data-pid="'.$products['products_id'].'" class="applyButton removeLinkedProductFromParent" style="font-size:10px;margin:2px 0">Unlink</button></div>';
                                                                }
                                                            } else
                                                                    echo AAS_TEXT_NOT_LINKED;
                                                        } else {
                                                                ...
                                                                foreach ($protoca_array[$products['products_id']] as $kpropo => $propro) {
                                                                    ...
                                                                    if ($propro['linked']
                                                                        ...
                                                                        > 0 ? implode('&nbsp;&raquo;&nbsp;',
                                                                                ...
                                                                                : AAS_TEXT_TOP;
                                                                        echo '&nbsp;&raquo;&nbsp;<a href="'.tep_href_link(FILENAME_AAS,
                                                                            'cPath='.$propro['cid']).'">'.$propro['cname'].'</a><br><br>';
                                                                    } else {
                                                                        ...
                                                                        > 0 ? implode('&nbsp;&raquo;&nbsp;',
                                                                                ...
                                                                                : AAS_TEXT_TOP;
                                                                        echo '&nbsp;&raquo;&nbsp;<a href="'.tep_href_link(FILENAME_AAS,
                                                                            'cPath='.$propro['cid']).'">'.$propro['cname'].'</a><br>';
                                                                    }
                                                                }
                                                            }
                                                        }
                ...
                case 'sort_order':
                    ...
                    if (isset($aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']])
                        ...
                        echo'<td class="nojedit products_sort_order '.$_table_td_rules[$key]['class'].'" data-column="products_'.$key.'" >'.(isset($products['products_sort_order'])
                                ? $products['products_sort_order'] : '---').'</td>';
                    } else {

                        echo'<td style="'.(isset($products['products_sort_order'])
                        && (!isset($search) || $search == "" ) ? $_table_td_rules[$key]['style']
                                : '').'" class="'.($value['cellEditable'] && isset($products['products_sort_order'])
                                ? '' : 'nojedit').' products_sort_order '.$_table_td_rules[$key]['class'].'" data-column="products_'.$key.'" >'.(isset($products['products_sort_order'])
                                ? $products['products_sort_order'] : '---').'</td>';
                    }
                    ...
                    if (isset($products['products_'.$key])) {
                        ...
                        if (isset($aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']])
                            && $aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']])
                                ...
                                echo '&nbsp;<input type="hidden" name="edit_date_added_input" value="'.$dtdateAdded.'" class="edit_date_added_input"><img class="edit_date_added" src="ext/aas/images/glyphicons_030_pencil.png" alt="edit">';
                ...
                default:
                    ...
                    echo'<td data-massedit="'.$value['massEdit'].'" data-column="'.$key.'" id="'.$key.'_'.$products['products_id'].'" style="'.$_table_td_rules[$key]['style'].' '.$cursorDefault.'" class="'.$noJedit.' '.$_table_td_rules[$key]['class'].'" >'.(isset($products[$key])
                            ? $products[$key] : '').'</td>';
            }
        }
    }
...
<?php if ($defaults['inTableSearchPosition']['visible'] && $defaults['inTableSearchPosition']['y']
    == 'bottom') { ?><div class="inTableSearch-wrapper inTableSearch-wrapper-<?php echo isset($defaults['inTableSearchPosition']['x'])
            ? $defaults['inTableSearchPosition']['x'] : 'right'; ?>"><input type="text" id="search" placeholder="In Table Search" class="inTableSearch inTableSearch-top" /></div><?php } ?>
    ...
    <?php if ($entries > 0) { ?>
                        <div class="pagination-form-wrapper">
                            <form name="itemsPerRowSelection-form" action="<?php echo htmlspecialchars($_SERVER['REQUEST_URI']); ?>" method="post">
                                ...
                                <select class="entriesPerPage" name="entriesPerPage" onchange="submit();">
                <?php foreach ($perPageArray as $ppa)
                    echo '<option '.($ppa == $entriesPerPage ? 'selected="selected"'
                            : '').' value="'.$ppa.'">'.$ppa.'</option>'; ?>
                                </select><?php if ($entries > 0) { ?><span style="padding-top:10px;"><?php echo AAS_PAGINATION_DISPLAYING; ?><strong><?php echo $entries; ?></strong><?php echo AAS_PAGINATION_FROM; ?><strong><?php echo $totalRows; ?></strong><?php echo AAS_PAGINATION_PRODUCTS; ?></span><?php } ?>
                            </form>
                        </div>
                    <?php } ?>
...
<?php } ?>
            <?php if ($defaults['displayCountProducts'] && $categories_entries > 0) { ?>
                <table class="displayCountProducts" id="tbl-count">
                    <tr>
                        ...
                        <?php if ($bool_count_products) $checked1 = 'checked="checked"';
                        else $checked1 = ''; ?>
    ...
    <?php if ($bool_count_subcategories) $checked2 = 'checked="checked"';
    else $checked2 = ''; ?>
            ...
            <?php if ($defaults['displayDatetime-info']) { ?>
                <div id="datetime-information">
                    ...
                    <script>
                            ...
                            timerUp('<?php echo $time; ?>', '<?php echo $time; ?>', function (tm) {
                                document.getElementById('toolbox-time-stated').innerHTML = tm;
                            });
                    </script>
                </div>
            <?php } ?>
            <?php if ($defaults['displayBottomInformation']) { ?>
                <div id="copywrite"><a target="_blank" href="http://www.alternative-administration-system.com"><?php echo AAS_TITLE; ?></a> <?php echo AAS_TEXT_VERSION;
            echo AAS_VERSION; ?><br><br> <?php echo AAS_TEXT_CREATED_BY; ?><a target="_blank" href="http://www.johnbarounis.com/">John Barounis</a></div>
<?php } ?>
...
<?php if ($defaults['enableToolBox'] && count($products_names_id_array) > 0) { ?>
    ...
    <?php if (isset($aasAac['fields_disable_action'][$key][$_SESSION['admin']['id']])
        && $aasAac['fields_disable_action']['attributes'][$_SESSION['admin']['id']]) { ?>
                        <div class="clear"></div>
    ...
    <?php } else { ?>
        ...
        <?php foreach ($products_names_id_array as $key => $value)
...
<?php if (sizeof($languages_array) > 1 && $defaults['displayLanguageSelection']) { ?>
    ...
    <?php echo tep_draw_form('adminlanguage1', FILENAME_AAS,
        'cPath='.$cPath_back, 'get').tep_draw_pull_down_menu('language',
        $languages_array, $languages_selected['code'],
        'onchange="this.form.submit();"').tep_draw_hidden_field('cPath',
        $cPath_back).'</form>'; ?>
...
<?php if ($defaults['displayByStatusSelection']) { ?>
                <div id="radio" class="floatBottomBar-item">
                    <form action="<?php echo htmlspecialchars($_SERVER['REQUEST_URI']); ?>" method="post">
                        <select name="radio" onchange="submit();" <?php echo $show_products_by_status
    == 0 ? 'class="inactiveClassSelect"' : ''; ?>>
                            <option value="1" <?php echo $show_products_by_status == 1
            ? 'selected="selected"' : ''; ?> ><?php echo AAS_ACTIVE_PRODUCTS; ?></option>
                            <option value="0" <?php echo $show_products_by_status == 0
            ? 'selected="selected"' : ''; ?>><?php echo AAS_INACTIVE_PRODUCTS; ?></option>
                            <option value="2" <?php echo $show_products_by_status == 2
            ? 'selected="selected"' : ''; ?> ><?php echo AAS_ALL_PRODUCTS; ?></option>
                        </select>
                    </form>
                </div>
<?php } ?>
...
<?php if ($defaults['displayOrderBySelection'] && !isset($_GET['search'])) { ?>
    ...
    <?php foreach ($orderByArray as $key => $value)
        echo '<option '.($orderBy == $key ? 'selected="selected"' : '' ).' value="'.$key.'">'.$value.'</option>'; ?>
                        </select>
                    </form>
                </div>
<?php } ?>
...
<?php if ($defaults['displayAscDescSelection'] && !isset($_GET['search'])) { ?>
    ...
    <?php foreach ($ascDescArray as $key => $value)
        echo '<option '.($ascDesc == $key ? 'selected="selected"' : '' ).' value="'.$key.'">'.$value.'</option>'; ?>
                        </select>
                    </form>
                </div>
<?php } ?>
...
<?php if ($defaults['displayCategoriesSelection']) { ?>
                <div class="floatBottomBar-item">
                    <form action="<?php echo htmlspecialchars($_SERVER['REQUEST_URI']); ?>" method="post">
                        <select name="displayCategories" onchange="submit();" <?php echo $displayCategories
    == 0 ? 'class="inactiveClassSelect"' : ''; ?>>
                            <option value="1" <?php echo $displayCategories == 1 ? 'selected="selected"'
            : ''; ?> ><?php echo $displayCategories == 1 ? AAS_CATEGORIES_VISIBLE
            : AAS_DISPLAY_CATEGORIES; ?></option>
                            <option value="0" <?php echo $displayCategories == 0 ? 'selected="selected"'
            : ''; ?>><?php echo $displayCategories == 0 ? AAS_CATEGORIES_HIDDEN : AAS_HIDE_CATEGORIES; ?></option>
                        </select>
                    </form>
                </div>
<?php } ?>
...
if ($defaults['enableAttributesManager'])
        include 'ext/aas/plugins/attributes_manager/index.php';
if ($defaults['enableOnlineUsers'])
        include 'ext/aas/plugins/online_users/index.php';

if ($modules_count > 0) {
    foreach ($modules as $key => $module)
        include 'ext/aas_modules/'.$key.'/index.php';
}
?>
