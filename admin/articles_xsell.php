                    ...
                    <table width="100%" border="0" cellpadding="0"  cellspacing="0">
                        <tr><td align=left>
                                ...
                                function general_db_conct($query_1)
                                {
                                    ...
                                    return array($a_to_pass, $b_to_pass, $c_to_pass,
                                        $d_to_pass, $e_to_pass, $f_to_pass, $g_to_pass,
                                        $h_to_pass, $i_to_pass, $j_to_pass, $k_to_pass,
                                        $l_to_pass, $m_to_pass, $n_to_pass, $o_to_pass);
                                }
//end of function
                                ...
                                if (!isset($_GET['add_related_article_ID'])) {
                                    ...
                                    list ($articles_id, $articles_name, $articles_description, $articles_url)
                                    ...
                                    <table border="0" cellspacing="1" cellpadding="3" bgcolor="#CCCCCC">
                                        ...
                                        for ($i = 0; $i < $num_of_articles; ++$i) {
                                            ...
                                            echo '<td class="dataTableContent"  valign="top">&nbsp;<a href="'.tep_href_link(FILENAME_ARTICLES_XSELL,
                                                'add_related_article_ID='.$articles_id[$i],
                                                'NONSSL').'">Add/Remove</a></td>';

                                            if (count($Related_items) > 1) {
                                                echo '<td class="dataTableContent" valign="top">&nbsp;<a href="'.tep_href_link(FILENAME_ARTICLES_XSELL,
                                                    'sort=1&add_related_article_ID='.$articles_id[$i],
                                                    'NONSSL').'">'.TEXT_SORT.'</a>&nbsp;</td>';
                                            } else {
                                ...
                                if ($_POST && !isset($_GET['sort'])) {
                                    if ($_POST['run_update']) {
                                        ...
                                        $ids = (count($_POST['xsell_id']) ? ' and '.$ids
                                                : ''); //clear if list not present
...
//    if ($_POST[xsell_id])
                        ...
                        if (isset($_GET['add_related_article_ID']) && !$_POST && !isset($_GET['sort'])) {
                            echo tep_draw_form('goto', "articles_xsell.php", '',
                                'get').tep_hide_session_id();
                            echo '<input type="hidden" name="add_related_article_ID" value="'.(int) $_GET['add_related_article_ID'].'" />';
                            echo SELECT_CATEGORY."&nbsp;:".tep_draw_pull_down_menu('cPath',
                            ...
                            if (isset($_GET['cPath'])) {
                                ...
                                ?>" method="post">
                                        ...
                                        list ($products_id, $products_name, $products_description, $products_url )
                                            = general_db_conct($query);
                                        ...
                                        list ($ID_PR, $products_id_pr, $xsell_id_pr)
                                            = general_db_conct($query);
