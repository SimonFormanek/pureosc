...
$(function () {
    ...
    if (config.defaults.enableTableStickyHeaders) {
        ...
        $(window).scroll(function (e) {
            ...
            if ($(this).scrollTop() >= tbl_top)
                $('#tbl_thead_helper').show();
            else
                $('#tbl_thead_helper').hide();
    ...
    if ($('#topPanelToggle').is(':visible'))
        toolBox.css({height: ctoolBoxHeight}).show();
    /*
    ...
    $('#panelToggle').on('click touchend', function () {
        ...
        if (selectedFields.width() <= 0) {
            ...
            selectedFields.animate({width: 0});
    ...
    if (topPanelToggleState)
        $('#leftSidePanel-toggle').animate({top: 100}, 400);
    ...
    $('table#tbl tbody td').not('.nojedit').editable(config.url.ajax, {
        ...
        "callback": function (sValue, y) {
            ...
            if (clmn == 'products_price' || clmn == 'products_price_gross') {
                ...
                if (fields.special)
                    updateSpecialsCell(this.parentNode.getAttribute('id').substr(4));
    ...
    $('.checkboxMassActions').click(function () {
        ...
        if ($('.checkboxMassActions').is(':checked'))
            $('#deletebutton').show();
        else
            $('#deletebutton').hide();
        if ($('.checkboxMassActions').is(':checked'))
    ...
    $('#exportbutton').on('click touchend', function () {
        aas.dialog.open('dialog-export');
    });
    $('#importbutton').on('click touchend', function () {
        $('#overlayWindow-import').show();
    });
    $('#donationbutton').on('click touchend', function () {
        $('#donations').fadeIn('fast', function () {
    ...
    overlayWindow.on('click touchend', '.overlay_language_img', function () {
        ...
        if (config.language_id == languages_id) {
            ...
            if ($('#overlayWindow #overlay-other-languages #overlay-fieldset-' + languages_id).length <= 0)
                editInOtherLanguage(languages_id, languages_alias);
            else
    ...
    $('#desc_preview_changes').on('click touchend', function () {
        ...
        if ($("#iframias").contents().find('#' + previewId).length) {
            ...
            aas.dialog.open('dialog-error-unique-id-wrapper-not-found', '', {wrapperUID: $('#previewid').val()});
    ...
    $(document).on('click touchend', '.descriptionbutton,.descriptionbuttonImg', function () {
        ...
        if (previous_id) {
            $desc_previous.data('previous', previous_id);
            $desc_previous.show();
        } else
            $desc_previous.hide();
    ...
    $('#dialog-attributes').on('click', '.attributes_checkbox_selector', function () {
        ...
        if (thisen.is(":checked")) {
            ...
            if ($('.attributes_checkbox_selector').filter(':not(:checked)').length === 0)
                $('.attributes_checkbox_all_selector').prop('checked', true);
            ...
            if ($('.attributes_checkbox_selector').filter(':checked').length === 0) {
                $('.attributes_checkbox_all_selector').prop('checked', false);
                $('#attributes-delete-selected').hide();
            }
    ...
    $('#dialog-attributes').on('click', '.attributes_checkbox_all_selector', function () {
        ...
        if ($(this).prop('checked')) {
            $('.attributes_checkbox_selector').prop('checked', true).closest('tr').addClass('tr_selected');
            $('#attributes-delete-selected').show();
        } else {
            $('.attributes_checkbox_selector').prop('checked', false).closest('tr').removeClass('tr_selected');
            $('#attributes-delete-selected').hide();
        }
    ...
    $('#masseditbutton').on('click touchend', function () {
        tooltip.hide();
        if (!aas.dialog.isOpen('dialog-massedit'))
            aas.dialog.open('dialog-massedit');
    });

    $('#massColumnsEditbutton').on('click touchend', function () {
        tooltip.hide();
        if (!aas.dialog.isOpen('dialog-massColumnsEdit'))
            aas.dialog.open('dialog-massColumnsEdit');
    });
        ...
        aas.ajax.do({
            ...
            if (msg == '0') {
                ...
                if (msg != '')
                    ...
                    nav_tooltip_data.html(translate.AAS_TEXT_NO_CATEGORIES_FOUND);
        ...
        uploadFinished: function (i, file, response, time) {
            ...
            if (response == 'aasSessionTimeout') {
                ...
                if (response == '1') {
                    ...
                    aas.dialog.open('dialog-error', translate.AAS_DIALOG_UPLOAD_MODULE_ERROR + '<br /><br />' + response);
            ...
            if (msg) {
                ...
                for (var i in msg) {
                    ...
                    for (var j in msg[i]['developers']) {
                        ...
                        if (msg[i]['developers'][j]['website'] != '')
                            devs.push('<a target="_blank" href="' + msg[i]['developers'][j]['website'] + '">' + msg[i]['developers'][j]['fullname'] + '</a>');
                        else
                            devs.push(msg['modules']['available'][i]['developers'][j]['fullname']);
                    ...
                    if (modules_installed[msg[i]['code']]) {
                        ...
                        if (modules_installed[msg[i]['code']] == msg[i]['version'])
                            statusHTML += '<span style="color:green">' + translate.AAS_UPLOAD_MODULE_STATUS_INSTALLED_AND_UPDATED + '</span>';
                        if (modules_installed[msg[i]['code']] < msg[i]['version'])
                            statusHTML += '<span style="color:red">' + translate.AAS_UPLOAD_MODULE_STATUS_INSTALLED_BUT_OUTDATED + '</span><br>' + translate.AAS_UPLOAD_MODULE_TEXT_CLICK + '<a target="_blank" href="' + msg[i]['url'] + '">' + translate.AAS_UPLOAD_MODULE_TEXT_HERE + '</a>' + translate.AAS_UPLOAD_MODULE_TEXT_TO_GET_NEWEST_VERSION;
                        if (modules_installed[msg[i]['code']] > msg[i]['version'])
                            statusHTML += translate.AAS_UPLOAD_MODULE_TEXT_WRONG_VERSION_INSTALLED + '<br>' + translate.AAS_UPLOAD_MODULE_TEXT_CLICK + '<a target="_blank" href="' + msg[i]['url'] + '">' + translate.AAS_UPLOAD_MODULE_TEXT_HERE + '</a>' + translate.AAS_UPLOAD_MODULE_TEXT_TO_GET_NEWEST_VERSION;

                    } else
                        statusHTML += '<span style="color:navy">' + translate.AAS_UPLOAD_MODULE_STATUS_NOT_INSTALLED + '</span><br>' + translate.AAS_UPLOAD_MODULE_TEXT_CLICK + '<a target="_blank" href="' + msg[i]['url'] + '">' + translate.AAS_UPLOAD_MODULE_TEXT_HERE + '</a>' + translate.AAS_UPLOAD_MODULE_TEXT_TO_INSTALL_IT;
    ...
    $("#dialog-aac .tabContainer ul.tabContainerUl li a").on('click touchend', function () {
        ...
        aas.ajax.do({data: {item: 'getAac', type: type}, url: config.url.actions}, function (msg) {
            ...
            if (msg == '0') {
                ...
                if (type == 'admins_columns_display')
                    $('#aas_dcd-tab2').html(msg);
                else
                    $('#' + activeTab + ' table tbody').html(msg);
...
//DROPDOWN APPLY BUTTON
        ...
        if (column == 'products_price' && sel_list == '1') {
            ...
            for (var i in obj_ds) {
                ...
                switch (option) {
                    ...
                    case '=':
                        timi = value.toFixed(4);
                        break;
                    case '-%':
                        timi = (obj_ds[i] - (obj_ds[i] * value / 100)).toFixed(4);
                        break;
                    case '+%':
                        timi = (obj_ds[i] + (obj_ds[i] * value / 100)).toFixed(4);
                        break;
                    case '-':
                        timi = (obj_ds[i] - value).toFixed(4);
                        break;
                    case '+':
                        timi = (obj_ds[i] + value).toFixed(4);
                        break;
            ...
            for (var i in obj_ds) {
                ...
                switch (option) {
                    ...
                    case '=':
                        ...
                        break;
                    case '-%':
                        timi = (obj_ds[i]['price'] - (obj_ds[i]['price'] * value / 100)).toFixed(4);
                        break;
                    case '+%':
                        timi = (obj_ds[i]['price'] + (obj_ds[i]['price'] * value / 100)).toFixed(4);
                        break;
                    case '-':
                        timi = (obj_ds[i]['price'] - value).toFixed(4);
                        break;
                    case '+':
                        timi = (obj_ds[i]['price'] + value).toFixed(4);
                        break;
                ...
                switch (column) {
                    ...
                    case'products_weight':
                        ...
                        for (var i = 0, n = pids.length; i < n; i++) {
                            ...
                            if (reloadTempList === false) {
                                ...
                                if ($('#temp_list_elem_' + pids[i]).length)
                                    reloadTempList = true;
                            ...
                            if (option == '=')
                                ...
                                if (option == '+') {
                                    ...
                                    if (td.data('massedit') == 'int')
                                        td.html(parseInt(td_txt) + parseInt(value));
                                    if (td.data('massedit') == 'decimal')
                                        td.html(parseFloat(td_txt) + parseFloat(value));
                                ...
                                if (option == '-') {
                                    ...
                                    if (td.data('massedit') == 'int')
                                        td.text(parseInt(td_txt) - parseInt(value));
                                    if (td.data('massedit') == 'decimal')
                                        td.text(parseFloat(td_txt) - parseFloat(value));
                            ...
                            if (fields.last_modified)
                                updateLastModifiedCell(pids[i]);

                        }

                        if (reloadTempList)
                            reloadTempProductsList();
                    ...
                    case'products_date_added':
                        ...
                        for (i = 0, n = pids.length; i < n; i++) {
                            ...
                            if (jtd.data('agostatus') == 0)
                                jtd.find('span').html(value);
                            else
                                jtd.find('span').html(aas.format.seconds(((Math.round(new Date().getTime() / 1000)) - timestamp) * 1000) + ' ' + translate.AAS_TEXT_AGO);
                            ...
                            for (i = 0, n = pids.length; i < n; i++) {
                                ...
                                switch (option) {
                                    ...
                                    case '=':
                                        ...
                                        break;
                                    case '-%':
                                        timi = (cptext - (cptext * val / 100)).toFixed(4);
                                        break;
                                    case '+%':
                                        timi = (cptext + (cptext * val / 100)).toFixed(4);
                                        break;
                                    case '-':
                                        timi = (cptext - val).toFixed(4);
                                        break;
                                    case '+':
                                        timi = (cptext + val).toFixed(4);
                                        break;
                                ...
                                if (fields.products_price_gross) {
                                    ...
                                    $ppg = $('#products_price_gross_' + pids[i]);
                                    if ($ppg.length <= 0)
                                        continue;//this is not need actually because it will elem will always present
    ...
    $('body').on('click touchend', '.view_product_class,.view_category_class', function () {
        ...
        if (csrc == href && !$('#previewPageVelakiWrapper').is(':hidden')) {
            hidePreviewPagePanel();
            return false;
        }
    ...
    $('#tbl .edit_date_added').prev().datepicker({dateFormat: 'yy-mm-dd', showOn: "button",
        ...
        onSelect: function (dateText, inst) {
            ...
            aas.ajax.do({data: 'value=' + dateText + '&id=' + trId + '&table=' + cls[0] + '&column=date_added', url: config.url.ajax}, function (msg) {
                ...
                if (jtd.data('agostatus') == 0)
                    jtd.find('span').html(resp[0]);
                else
                    jtd.find('span').html(aas.format.seconds(((Math.round(new Date().getTime() / 1000)) - timestamp) * 1000) + ' ' + translate.AAS_TEXT_AGO);

                //update last modified cell if column is visible
                if (fields.last_modified)
                    updateLastModifiedCell(trId);
