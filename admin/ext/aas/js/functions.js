        ...
        do: function (obj, onDone, onFail, onAlways) {
            ...
            return $.ajax(ajax_data).done(function (msg) {
                ...
                if (typeof (onFail) == 'function')
                    ...
                    if ($('#dialog-processing').dialog('isOpen'))
            ...
            if (id) {
                ...
                if (elem.length && !elem.dialog('isOpen')) {
                    ...
                    if (!is_empty(data_obj))
                        elem.data('data', data_obj);
                    if (text != '')
                        elem.find('.message').html(text);

                    if (id == 'dialog-success') {

                        if (config.displaySuccessAlertMessages)
                            elem.dialog('open');
                        else
                            return false;
                        ...
                        if (config.displayErrorAlertMessages)
                            elem.dialog('open');
                        else
                            return false;
                    ...
                    alert(translate.cannot_open_dialog);
        ...
        close: function (id, clearText) {
            ...
            if (id) {
                ...
                if (elem.length) {
                    ...
                    alert(translate.cannot_close_dialog);
    ...
    format: {
        ...
        seconds: function (secs, start, end) {
            ...
            for (var i in this.units) {
                ...
                if (secs < this.units[i][0] && i != 'sec')
                    continue;
                num = parseInt(secs / this.units[i][0]);
                secs = secs % (this.units[i][0]);

                if (num == 1)
                    strs.push(num + ' ' + this.units[i][1]);
                else
                    strs.push(num + ' ' + this.units[i][2]);
            ...
            switch (mcl) {
                ...
                case'1':
                    ...
                    if (post_action == '2') {
                        ...
                        if (moved.length > 0) {
                            ...
                            for (var i = 0, n = moved.length; i < n; i++)
                                $('table#tbl tbody.products_tbody tr#pid_' + moved[i]).remove();
                ...
                case'2':
                    ...
                    if (post_action == '2') {
                        ...
                        if (copied.length > 0) {
                            ...
                            for (var i = 0, n = copied.length; i < n; i++)
                                $('table#tbl tbody.products_tbody tr#pid_' + copied[i]).removeClass('tr_selected').find('.checkboxMassActions').prop('checked', false);
                            aas.dialog.close('dialog-processing');
                ...
                case'3':
                    ...
                    if (post_action == '2') {
                        ...
                        if (msg == 1 || msg == '1')
                            window.location.reload(); //aas.dialog.open('dialog-success',translate.AAS_TEXT_SELECTED_PRODUCTS_LINKED_SUCCESSFULLY);
                        else if (msg == '0' || msg == '0')
                            aas.dialog.open('dialog-error', translate.AAS_TEXT_SELECTED_PRODUCTS_LINKED_FAILED);
                        else if (msg == '3' || msg == '3')
                            aas.dialog.open('dialog-error', translate.AAS_TEXT_SELECTED_PRODUCTS_LINKED_ABORTED_NO_PRODUCTS_TO_LINK);
                        else if (msg == '4' || msg == '4')
                            aas.dialog.open('dialog-error', translate.AAS_TEXT_SELECTED_PRODUCTS_LINKED_FAILED_NOCOLUMN_LINKED);
                        else
                            ;
...
function toolBoxAttributesAction() {
    ...
    switch (action_selected) {
        ...
        case'1':
            arr = aas.get.checked_products();
            break;
        case'2':
            arr = aas.get.checked_products('checkedTempProducts');
            break;
        case'3':
        ...
        aas.ajax.do({data: 'pids=' + arr + '&item=' + action + '&tas=' + tas + '&cid=' + config.categoryId, url: config.url.actions}, function (msg) {
            ...
            if (tas[0] == 1) {//copy
                ...
                if (resp[0] == '1') {
                    ...
                    aas.dialog.open('dialog-error', translate.selected_product_has_not_attributes_to_copy_from);
                else if (resp[0] == '3')
                    aas.dialog.open('dialog-error', translate.not_found_products_to_copy_attributes);
                else
                    aas.dialog.open('dialog-error', translate.attributes_have_not_been_copied);
                ...
                if (resp[0] == '1') {
                    ...
                    aas.dialog.open('dialog-error', translate.AAS_TEXT_NO_PRODUCTS_FOUND_TO_DELETE_THEIR_ATTRIBUTES);
                else
                    aas.dialog.open('dialog-error', translate.attributes_have_not_been_deleted);
...
function tas(val) {
    ...
    if (val.value == '2') {
        ...
        if ($('#toolbox-attributes-select-6').val() == '3')
            $('#toolbox-attributes-select-4,#toolbox-attributes-select-7').show();
        else
            $('#toolbox-attributes-select-4,#toolbox-attributes-select-7').hide();
        ...
        if ($('#toolbox-attributes-select-3').val() == '3')
            $('#toolbox-attributes-select-4,#toolbox-attributes-select-7').show();
        else
            $('#toolbox-attributes-select-4,#toolbox-attributes-select-7').hide();
    ...
    aas.ajax.do({data: 'item=reloadTempProductsList', url: config.url.actions}, function (msg) {
        ...
        if (pids.length > 0) {
            ...
            for (i = 0, n = pids.length; i < n; i++)
                tempar.push('#temp_list_elem_' + pids[i]);
...
function removeProductFromTempList(elem) {
    ...
    aas.ajax.do({data: 'pid=' + pid + '&item=removeFromTempProductsList', url: config.url.actions}, function (msg) {
        ...
        if (msg == '0')
            $('#leftSidePanel-inwrapper').find('.temp-list-select-action').remove();
        aas.dialog.close('dialog-processing');
    ...
    aas.ajax.do({data: 'product_id=' + pid + '&item=fetchOtherLanguage&lid=' + languages_id, url: config.url.actions}, function (msg) {
        ...
        if (msg == '0') {
            ...
            if (loaded_languages_ids) {
                ...
                aas.localStorage.save('AAS:loaded_languages_ids', languages_id);

            if (config.disabled_column_actions.attributes)
                $('.edit_products_attributes').hide();
...
function changeGlobalEditor(id) {
    ...
    if (config.productsDescriptionEditor == 'ckeditor')
        config.productsDescriptionEditor = 'tinymce';
    else if (config.productsDescriptionEditor == 'tinymce')
        config.productsDescriptionEditor = 'textarea';
    else
        config.productsDescriptionEditor = 'ckeditor';
...
function changeEditor(id) {
    ...
    if (pdeditor == 'ckeditor')
        pdeditor = 'tinymce';
    else if (pdeditor == 'tinymce')
        pdeditor = 'textarea';
    else
        pdeditor = 'ckeditor';
            ...
            tinymce.init({
                ...
                setup: function (ed) {
                    ...
                    ed.on('change', function (e) {
                        ...
                        if (ed.id.substring(0, 18) === 'description_editor')
                            liveEditorChangesPreview(ed.getContent(), ed.id);

                    }).on('init', function (args) {

                        if (editorHtmlContent != '') {
                            tinyMCE.get(id).setContent(editorHtmlContent);
                            editorHtmlContent = '';
                        }
...
function submitMainDescriptionChanges() {
    ...
    aas.ajax.do({data: 'value=' + description_data + '&id=' + $('#overlay_pid').val() + '&lid=' + languages_id + '&column=products_description', url: config.url.ajax}, function (msg) {
        ...
        if (msg == 'ok')
            aas.dialog.open('dialog-success', translate.AAS_DIALOGS_PHP_SUCCESS_SUBMITED_SUCCESSFULLY);
        else
            aas.dialog.open('dialog-error', msg);
...
function edify(val) {
    ...
    if (val) {
        ...
        $('table#tbl tbody tr td').not('.nojedit').each(function (index) {
            ...
            var el = $(this);
            if (!column_excludes[el.data('column')])
                el.html('<input class="lfor nojedit" type="text" value="' + el.text() + '">');
...
function edify_proccess() {
    ...
    $('table#tbl tbody.products_tbody tr').each(function (index) {
        ...
        $(this).children('td').not('.nojedit').each(function (i) {
            ...
            if (!column_excludes[column])
                pid_rows_data.push({column: column, value: $(this).children('.lfor').val()});
...
clearSearchQuery = function (queryString) {
    window.location.href = queryString;
}

jumpToPage = function (queryString, val) {
    window.location.href = queryString + val.value;
}
...
function massEditOptionPrices() {
    ...
    for (var i in obj_ds) {
        ...
        switch (sd) {
            ...
            case '=':
                timi = amount.toFixed(4);
                break;
            case '-%':
                timi = (obj_ds[i] - (obj_ds[i] * amount / 100)).toFixed(4);
                break;
            case '+%':
                timi = (obj_ds[i] + (obj_ds[i] * amount / 100)).toFixed(4);
                break;
            case '-':
                timi = (obj_ds[i] - amount).toFixed(4);
                break;
            case '+':
                timi = (obj_ds[i] + amount).toFixed(4);
                break;
    ...
    aas.ajax.do({
        ...
        if (msg == '1') {
            ...
            for (var i in obj_ds)
                $('#value_price_' + i).val(obj_ds[i]);
            aas.dialog.open('dialog-success', translate.AAS_DIALOG_TEXT_SUCCESSFULLY_ALTERED_OPTION_PRICES);

        } else
            aas.dialog.open('dialog-error', translate.AAS_DIALOG_TEXT_COULD_NOT_ALTER_OPTION_PRICES);
...
function deleteProducts(data) {
    ...
    aas.ajax.do({data: {item: 'deleteProducts', pids: productsIds}, url: config.url.actions}, function (msg) {
        ...
        if (msg == '1') {
            ...
            aas.dialog.open('dialog-error', translate.dialog_delete_products_something_went_wrong);
...
function afterConfirm(data) {
    ...
    if (dt.length == 1) {
        ...
        if (data.ed && dt[0] == 'logoff')
            window.location = data.ed;
...
function selectMenuChange(vall, column) {
    ...
    aas.ajax.do({data: 'value=' + val + '&id=' + pid + '&column=' + column, url: config.url.ajax}, function (msg) {
        ...
        if (msg.substr(0, 2) == '1_')
            aas.dialog.open('dialog-success', msg.substr(2));
        else
            aas.dialog.open('dialog-error', msg);
...
function settings(valaoritis, action) {
    ...
    aas.ajax.do({data: 'value=' + valaoritis + '&column=set_session&action=' + action, url: config.url.ajax}, function (msg) {
        ...
        if (msg == '1') {
            ...
            aas.dialog.open('dialog-error', translate.dialog_session_timeout_something_went_wrong);
...
function ats_option_name_change(jthis) {
    ...
    if (!ovs.length) {
        ovs = elem.parent().next().children('.attributes_selectMenus_values_new');
        newAtt = true;
    }
...
function reset_columns_order() {
    ...
    aas.ajax.do({data: 'item=resetColumnsOrder', url: config.url.actions}, function (msg) {
        ...
        if (msg == '1')
            aas.dialog.open('dialog-success', translate.AAS_SETTINGS_RESET_COLUMNS_ORDER_SUCCESS);
        else
            aas.dialog.open('dialog-error', translate.AAS_DIALOG_TEXT_SOMETHING_WENT_WRONG_TRY_AGAIN);
...
function nav_links_construct(catid, nav_cache) {
    ...
    if (nav_cache[catid]) {
        ...
        for (var i in nav_cache[catid]) {
            ...
            if (nav_cache[catid][i]['cid'] == config.categoryId)
                htmla += '<li><span>' + nav_cache[catid][i]['cname'] + '<span></li>';
            else
                htmla += '<li><a href="' + config.url.aas_link + '?cPath=' + nav_cache[catid][i]['cid'] + '">' + nav_cache[catid][i]['cname'] + '<a></li>';
...
function removeLinkedProduct(dt) {
    ...
    aas.ajax.do({
        ...
        if (msg == '1') {
            ...
            aas.dialog.open('dialog-error', translate.AAS_DIALOG_TEXT_LINKED_PRODUCT_NOT_REMOVED);
...
function displayDropDown(jthis) {
    ...
    if (jthis.next('.dropdown')) {
        ...
        if (jthis.next('.dropdown').is(':visible'))
            jthis.next('.dropdown').hide();
        else
            jthis.next('.dropdown').css({left: jthis.offset().left, top: jthis.offset().top + jthis.outerHeight() - $(window).scrollTop()}).show();
...
function printHtmlElement(flag, val) {
    ...
    if (fields.last_modified || fields.date_added)
        $("#print_frame").contents().find('.edit_date_added').remove();


    setTimeout(function () {
        if (frameWindow.focus)
            frameWindow.focus(); // focus is needed for IE
        frameWindow.print();
    }, 1);
...
function updateProductsAttributesCell(value, pid) {
    ...
    if (value <= 0)
        $('table#tbl tbody.products_tbody tr#pid_' + pid + ' td.attributesCell').removeClass('attributesCellLink_non_zero').addClass('attributesCellLink_zero');
    else
        $('table#tbl tbody.products_tbody tr#pid_' + pid + ' td.attributesCell').removeClass('attributesCellLink_zero').addClass('attributesCellLink_non_zero');
...
//TODO make timestamp from server not browser so to prevent time differnce
    ...
    if (cells.length > 1)
        var agostatus = cells.eq(0).data('agostatus');
    ...
    if (agostatus == 0)
        cell.find('span').html(dtv.year + '-' + dtv.month + '-' + dtv.day + ' ' + dtv.hours + ':' + dtv.minutes + ':' + dtv.seconds);
    else
        cell.find('span').html(translate.AAS_TEXT_A_MOMENT_AGO);
...
function colorizeAttributesRows() {
    ...
    $('#attributes-table tbody tr').each(function () {
        ...
        if (!attsRowsBackgroundColorVariations[onv])
            attsRowsBackgroundColorVariations[onv] = randomColor(242);
