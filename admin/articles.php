...
?>
          <tr>
            ...
            <td class="smallText"><?php echo tep_draw_input_field('topics_name[' . $languages[$i]['id'] . ']', (($topics_name[$languages[$i]['id']]) ? stripslashes($topics_name[$languages[$i]['id']]) : tep_get_topic_name($tInfo->topics_id, $languages[$i]['id']))) . '&nbsp;' . $languages[$i]['name']; ?></td>
          </tr>
<?php
    }
?>
...
?>
          <tr>
            ...
            <td class="smallText"><?php echo tep_draw_input_field('topics_heading_title[' . $languages[$i]['id'] . ']', (($topics_name[$languages[$i]['id']]) ? stripslashes($topics_name[$languages[$i]['id']]) : tep_get_topic_heading_title($tInfo->topics_id, $languages[$i]['id']))) . '&nbsp;' . $languages[$i]['name']; ?></td>
          </tr>
<?php
    }
?>
...
?>
          <tr>
            <td class="smallText" valign="top"><?php if ($i == 0) echo TEXT_EDIT_TOPICS_DESCRIPTION; ?></td>
                ...
                <?php
                  ...
                  else
                  {
                    if (ARTICLE_ENABLE_HTML_EDITOR == 'FCKEditor') {
                        ...
                        echo tep_draw_textarea_field('topics_description[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (($topics_description[$languages[$i]['id']]) ? stripslashes($topics_description[$languages[$i]['id']]) : tep_get_topic_description($tInfo->topics_id, $languages[$i]['id'])), 'id = "topics_description[' . $languages[$i]['id'] . ']" class="ckeditor"');
                    } else {
                        echo tep_draw_textarea_field('topics_description[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (($topics_description[$languages[$i]['id']]) ? stripslashes($topics_description[$languages[$i]['id']]) : tep_get_topic_description($tInfo->topics_id, $languages[$i]['id'])));
                    }
                  }
...
?>
          ...
          <tr>
            ...
            <td class="smallText"><?php echo tep_draw_input_field('sort_order', $tInfo->sort_order, 'size="2"'); ?></td>
          </tr>
          <tr>
            <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
...
?></td>
          ...
          <tr>
            ...
            <td class="smallText"><?php echo tep_draw_radio_field('articles_is_blog', '0', $blog_out_status) . '&nbsp;' . TEXT_ARTICLE_BLOG_NO . '&nbsp;' . tep_draw_radio_field('articles_is_blog', '1', $blog_in_status) . '&nbsp;' . TEXT_ARTICLE_BLOG_YES; ?></td>
          </tr>
          <tr>
            <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
          ...
          <tr>
            <td class="smallText"><?php echo TEXT_EDIT_SORT_ORDER; ?></td>
            <td class="smallText"><?php echo tep_draw_input_field('sort_order', $aInfo->articles_sort_order, 'size="2"'); ?></td>
          </tr>
          <tr>
            <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
          ...
          <tr>
            ...
            <td class="smallText"><?php echo tep_draw_pull_down_menu('authors_id', $authors_array, $aInfo->authors_id); ?></td>
          </tr>
          <tr>
            <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
...
?>
          <tr>
            ...
            <td class="smallText"><?php echo tep_draw_input_field('articles_name[' . $languages[$i]['id'] . ']', (isset($articles_name[$languages[$i]['id']]) ? $articles_name[$languages[$i]['id']] : tep_get_articles_name($aInfo->articles_id, $languages[$i]['id'])), 'size="35"') . '&nbsp;' . $languages[$i]['name']; ?></td>
          </tr>
<?php
    }
?>
...
?>
          <tr>
            ...
            <td class="smallText"><?php echo (isset($_GET['aID']) ? tep_draw_input_field('articles_image_name', tep_get_articles_image($aInfo->articles_id, $languages[$i]['id']), 'disabled size="20"') . '&nbsp;&nbsp;' : '') . tep_draw_file_field('articles_image_' . $languages[$i]['id']) . '&nbsp;' . $languages[$i]['name']; ?></td>
          </tr>
<?php
    }
?>
...
?>
          <tr bgcolor="#ffffcc">
            <td class="smallText" valign="top"><?php if ($i == 0) echo sprintf(TEXT_ARTICLES_HEAD_DESC_TAG, MAX_ARTICLE_ABSTRACT_LENGTH); ?></td>
            <td><table border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="smallText">
                <?php
                  if (ARTICLE_ENABLE_HTML_EDITOR == 'No Editor')
                    echo tep_draw_textarea_field('articles_head_desc_tag[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (($articles_head_desc_tag[$languages[$i]['id']]) ? stripslashes($articles_head_desc_tag[$languages[$i]['id']]) : tep_get_articles_head_desc_tag($aInfo->articles_id, $languages[$i]['id'])));
                  else
                  {
                    if (ARTICLE_ENABLE_HTML_EDITOR == 'FCKEditor') {
                        echo tep_draw_fckeditor('articles_head_desc_tag[' . $languages[$i]['id'] . ']','700','300',(isset($articles_head_desc_tag[$languages[$i]['id']]) ? stripslashes($articles_head_desc_tag[$languages[$i]['id']]) : tep_get_articles_head_desc_tag($aInfo->articles_id, $languages[$i]['id'])));
                    } else if (ARTICLE_ENABLE_HTML_EDITOR == 'CKEditor') {
                        echo tep_draw_textarea_field('articles_head_desc_tag[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (($articles_head_desc_tag[$languages[$i]['id']]) ? stripslashes($articles_head_desc_tag[$languages[$i]['id']]) : tep_get_articles_head_desc_tag($aInfo->articles_id, $languages[$i]['id'])), 'id = "articles_head_desc_tag[' . $languages[$i]['id'] . ']" class="ckeditor"');
                    } else {
                        echo tep_draw_textarea_field('articles_head_desc_tag[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (($articles_head_desc_tag[$languages[$i]['id']]) ? stripslashes($articles_head_desc_tag[$languages[$i]['id']]) : tep_get_articles_head_desc_tag($aInfo->articles_id, $languages[$i]['id'])));
                    }
                  }
...
?>
          <tr>
            <td class="smallText" valign="top"><?php if ($i == 0) echo TEXT_ARTICLES_DESCRIPTION; ?></td>
            <td><table border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="smallText">
                <?php
                  if (ARTICLE_ENABLE_HTML_EDITOR == 'No Editor')
                    echo tep_draw_textarea_field('articles_description[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (($articles_description[$languages[$i]['id']]) ? stripslashes($articles_description[$languages[$i]['id']]) : tep_get_articles_description($aInfo->articles_id, $languages[$i]['id'])));
                  else
                  {
                    if (ARTICLE_ENABLE_HTML_EDITOR == 'FCKEditor') {
                        echo tep_draw_fckeditor('articles_description[' . $languages[$i]['id'] . ']','700','300',(isset($articles_description[$languages[$i]['id']]) ? stripslashes($articles_description[$languages[$i]['id']]) : tep_get_articles_description($aInfo->articles_id, $languages[$i]['id'])));
                    } else if (ARTICLE_ENABLE_HTML_EDITOR == 'CKEditor') {
                        echo tep_draw_textarea_field('articles_description[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (($articles_description[$languages[$i]['id']]) ? stripslashes($articles_description[$languages[$i]['id']]) : tep_get_articles_description($aInfo->articles_id, $languages[$i]['id'])), 'id = "articles_description[' . $languages[$i]['id'] . ']" class="ckeditor"');
                    } else {
                        echo tep_draw_textarea_field('articles_description[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (($articles_description[$languages[$i]['id']]) ? stripslashes($articles_description[$languages[$i]['id']]) : tep_get_articles_description($aInfo->articles_id, $languages[$i]['id'])));
                    }
                  }
...
?>
          <tr>
            ...
            <td class="smallText"><?php echo tep_draw_input_field('articles_url[' . $languages[$i]['id'] . ']', (isset($articles_url[$languages[$i]['id']]) ? $articles_url[$languages[$i]['id']] : tep_get_articles_url($aInfo->articles_id, $languages[$i]['id'])), 'size="35"') . '&nbsp;' . $languages[$i]['name']; ?></td>
          </tr>
<?php
    }
?>
...
<?php
    ...
    if (tep_not_null($_POST)) {
      ...
      $imgArray = array();
      for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
          $imgID = sprintf("articles_image_%s", $languages[$i]['id']);
          ...
          if ($art_image->parse() && $art_image->save()) {
              $imgArray[$languages[$i]['id']] = tep_db_input($art_image->filename);
          }
      }
...
?>
      ...
      <tr>
        <td class="smallText"><?php echo sprintf(TEXT_ARTICLE_MORE_INFORMATION, $aInfo->articles_url); ?></td>
      </tr>
<?php
      }
?>
...
/* Re-Post all POST'ed variables */
      ...
      for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
        ...
        echo tep_draw_hidden_field('articles_image_'.$languages[$i]['id'], htmlspecialchars(stripslashes($imgArray[$languages[$i]['id']])));
       ...
       <tr>
        ...
        <td><table border="0" width="100%">
         ...
         <?php } else if (tep_not_null($versionStatus)) {
           ...
           echo tep_draw_form('version_check', FILENAME_ARTICLES, '', 'post') . tep_draw_hidden_field('action', 'getversion');
         ?>
               <tr>
             <td style="float:right"><table border="0" cellpadding="0"><tr>
...
<?php
      if ($articles['articles_is_blog'] == '1') {
        echo tep_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_ICON_STATUS_GREEN, 10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_ARTICLES, 'action=setflagblog&flagblog=0&aID=' . $articles['articles_id'] . '&tPath=' . $tPath) . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_red_light.gif', IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
      } else {
        echo '<a href="' . tep_href_link(FILENAME_ARTICLES, 'action=setflagblog&flagblog=1&aID=' . $articles['articles_id'] . '&tPath=' . $tPath) . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_green_light.gif', IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image(DIR_WS_IMAGES . 'icon_status_red.gif', IMAGE_ICON_STATUS_RED, 10, 10);
      }
