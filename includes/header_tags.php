...
switch (true) {
  ...
  case (basename($_SERVER['SCRIPT_FILENAME']) === FILENAME_DEFAULT):
    ...
    if (! ReadCacheHeaderTags($header_tags_array, basename($_SERVER['SCRIPT_FILENAME']),  $id)) { 
         ...
         if (isset($cPath) && tep_not_null($cPath)) {
             ...
             $canonical_url = tep_href_link(FILENAME_DEFAULT, 'cPath=' . $path, $request_type, false);
         } elseif (isset($_GET['manufacturers_id']) && tep_not_null($_GET['manufacturers_id'])) {
              $canonical_url = tep_href_link(FILENAME_DEFAULT, 'manufacturers_id=' . (int)$_GET['manufacturers_id'], $request_type, false);
         }
  ...
  case (basename($_SERVER['SCRIPT_FILENAME']) === FILENAME_PRODUCT_REVIEWS_WRITE):
    ...
    switch (true) {
     case (basename($_SERVER['SCRIPT_FILENAME']) === FILENAME_PRODUCT_INFO):          $filename = FILENAME_PRODUCT_INFO;          break;
     case (basename($_SERVER['SCRIPT_FILENAME']) === FILENAME_PRODUCT_REVIEWS):       $filename = FILENAME_PRODUCT_REVIEWS;       break;
     case (basename($_SERVER['SCRIPT_FILENAME']) === FILENAME_PRODUCT_REVIEWS_INFO):  $filename = FILENAME_PRODUCT_REVIEWS_INFO;  break;
     case (basename($_SERVER['SCRIPT_FILENAME']) === FILENAME_PRODUCT_REVIEWS_WRITE): $filename = FILENAME_PRODUCT_REVIEWS_WRITE; break;
    ...
    { 
       ...
       // Canonical URL add-on
       if ($_GET['products_id'] != '') {
          $canonical_url = tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . (int)$_GET['products_id'], $request_type, false);
       }
       WriteCacheHeaderTags($header_tags_array, basename($_SERVER['SCRIPT_FILENAME']), $id);
    }
...
// products_new.php
  case (basename($_SERVER['SCRIPT_FILENAME']) === FILENAME_PRODUCTS_NEW):
    if (! ReadCacheHeaderTags($header_tags_array, basename($_SERVER['SCRIPT_FILENAME']), '')) {
      ...
      $canonical_url = tep_href_link(FILENAME_PRODUCTS_NEW, tep_get_all_get_params(array()), $request_type, false);
