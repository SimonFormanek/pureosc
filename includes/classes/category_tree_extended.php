...
*/
  ...
  class category_tree_extended extends category_tree {
     ...
     */
    function set_root_group_string($root_group_start_string, $root_group_end_string) {
     ...
     */
    function set_child_with_subcats_string($child_with_subcats_start_string, $child_with_subcats_end_string) {
    ...
    protected function build_branch_extended($parent_id, $level = 0) {
      ...
      $result .= (($level === 0) && ($this->parent_group_apply_to_root === false)) ?  $this->root_group_start_string : null;
      ...
      if ( isset($this->_data[$parent_id]) ) {
        foreach ( $this->_data[$parent_id] as $category_id => $category ) {
          ...
          if ( isset($this->_data[$category_id]) && (($this->max_level == '0') || ($this->max_level > $level+1)) ) {
            if ( $this->follow_cpath === true ) {
              if ( in_array($category_id, $this->cpath_array) ) {
                $result .= $this->build_branch_extended($category_id, $level+1);
              }
            } else {
              $result .= $this->build_branch_extended($category_id, $level+1);
            }
          ...
          if( tep_has_category_subcategories( $category_id ) === true && in_array($category_id, $this->cpath_array ) ) {
      ...
      $result .= (($level === 0) && ($this->parent_group_apply_to_root === false)) ?  $this->root_group_end_string : null;
