...
<?php
    ...
    while ($xsell = tep_db_fetch_array($xsell_query)) {
      ...
      if (tep_not_null($xsell['articles_image']) && file_exists($path)) {
         echo '<a href="' . tep_href_link('product_info.php', 'products_id=' . $xsell['products_id']) . '">' . tep_image($path, addslashes($xsell['articles_name']), SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a><br>' ;
      }   
      echo '<a href="' . tep_href_link(FILENAME_ARTICLE_INFO, 'articles_id=' . $xsell['articles_id']) . '">' . addslashes($xsell['articles_name']) . '</a></div>';
    }
    echo '</div>'; 
?>
<!-- xsell_articles_eof //-->
