...
function __construct() {
    ...
    public function getOutput() {
      ...
      $search_box = tep_navbar_store_search('btn-info', (MODULE_NAVIGATION_BAR_STORE_SEARCH_FUNCTIONS == 'Descriptions'));
	...
	// width of product image xs
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Pages', 'MODULE_NAVIGATION_BAR_STORE_SEARCH_PAGES', '" . implode(';', $this->get_default_pages()) . "', 'The pages to add the Store Search\'s results.', '6', '0', 'cm_nb_store_search_show_pages', 'cm_nb_store_search_pages(', now())");
...
$keys[] = 'MODULE_NAVIGATION_BAR_STORE_SEARCH_IMAGE_WIDTH_LG';
  ...
  function cm_nb_store_search_show_pages($text) {
  ...
  function cm_nb_store_search_pages($values, $key) {
    ...
    foreach ($files_array as $file) {
      $output .= tep_draw_checkbox_field('cm_nb_store_search_file[]', $file, in_array($file, $values_array)) . '&nbsp;' . tep_output_string($file) . '<br />';
    }
    ...
    $output .= '<script>
                function htrn_update_cfg_value() {
                  ...
                  if ($(\'input[name="cm_nb_store_search_file[]"]\').length > 0) {
                    $(\'input[name="cm_nb_store_search_file[]"]:checked\').each(function() {
                ...
                $(function() {
                  ...
                  if ($(\'input[name="cm_nb_store_search_file[]"]\').length > 0) {
                    $(\'input[name="cm_nb_store_search_file[]"]\').change(function() {
                      htrn_update_cfg_value();
                    });
                  }
                });
