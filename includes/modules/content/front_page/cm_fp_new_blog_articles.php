  ...
  class cm_fp_new_blog_articles {
    ...
    function __construct() {
      ...
      if ( defined('MODULE_CONTENT_FRONT_PAGE_NEW_BLOG_ARTICLES_STATUS') ) {
        ...
        $this->enabled = (MODULE_CONTENT_FRONT_PAGE_NEW_BLOG_ARTICLES_STATUS == 'True');
      }
    }
    ...
    public function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable New Blog Articles Module', 'MODULE_CONTENT_FRONT_PAGE_NEW_BLOG_ARTICLES_STATUS', 'True', 'Should the new products block be shown on the front page?', '6', '1', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_CONTENT_FRONT_PAGE_NEW_BLOG_ARTICLES_SORT_ORDER', '50', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Max Products', 'MODULE_CONTENT_FRONT_PAGE_NEW_BLOG_ARTICLES_MAX_DISPLAY_NEW_PRODUCTS', '9', 'Maximum number of new products to show on the front page.', '6', '2', now())");
    }
    ...
    public function keys() {
      ...
      $keys[] = 'MODULE_CONTENT_FRONT_PAGE_NEW_BLOG_ARTICLES_MAX_DISPLAY_NEW_PRODUCTS';
      return $keys;
    }
    ...
    private function get_data() {
     ...
     where (a.articles_date_available IS NULL or to_days(a.articles_date_available) <= to_days(now()))  and a.articles_status = '1' and a.articles_is_blog = 1 and ad.language_id = '" . (int)$languages_id . "' and td.language_id = '" . (int)$languages_id . "' order by a.articles_date_added desc, ad.articles_name";
     $listing_sql = $articles_all_query_raw;
		if (tep_db_num_rows($listing_sql)){
        return $new_articles_blog_content;
		} else {
      return false;
    }
