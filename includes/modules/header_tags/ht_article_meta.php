...
*/
  ...
  class ht_article_meta {
      ...
      if ( (basename($PHP_SELF) == FILENAME_ARTICLE_INFO) || (basename($PHP_SELF) == FILENAME_ARTICLE_BLOG) ) {
        if (isset($HTTP_GET_VARS['articles_id'])) {
          if ($article_check['total'] > 0) {
            $meta_info_query = tep_db_query("select ad.articles_head_desc_tag from " . TABLE_ARTICLES . " a, " . TABLE_ARTICLES_DESCRIPTION . " ad where  a.articles_id = '" . (int)$HTTP_GET_VARS['articles_id'] . "' and ad.articles_id = a.articles_id and ad.language_id = '" . (int)$languages_id . "'");
            $meta_info = tep_db_fetch_array($meta_info_query);
            
            if (tep_not_null($meta_info['articles_head_desc_tag'])) {
              $oscTemplate->addBlock('<meta name="description" content="' . tep_output_string(strip_tags($meta_info['articles_head_desc_tag'])) . '" />', $this->group);
            }
    ...
    function keys() {
      return array('MODULE_HEADER_TAGS_ARTICLE_META_STATUS', 'MODULE_HEADER_TAGS_ARTICLE_META_KEYWORDS_STATUS', 'MODULE_HEADER_TAGS_ARTICLE_META_SORT_ORDER');
    }
  }
?>
